...
/** API tavanı: checkout DTO'su tek ürün adedini 20 ile sınırlar. */
const CART_MAX_QTY = 20;

/**
 * Bir sepet satırının çıkabileceği azami adet: yakalanan stok ile sipariş-başı
 * limitin küçüğü, API tavanı (20) ile sınırlı. Bilgi yoksa 20'ye düşer
      ...
      updateQuantity: (itemId, quantity) => {
        ...
        const items = get().items.map((i) =>
          i.id === itemId
            ? {
                ...i,
                quantity: Math.min(quantity, maxAllowedQty(i)),
                addedAt: Date.now(),
              }
            : i,
        );
