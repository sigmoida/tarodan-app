...
describe('J52 · Stok-yok ekranı', () => {
  ...
  it('J52.1 stok-yok hero başlık ve gövde gösterilir', async () => {
    getOne.mockResolvedValue({
      data: { data: { id: 'prod-1', title: 'Deri Ceket', status: 'sold', quantity: 0 } },
    });
    getSimilar.mockResolvedValue({ data: { data: [] } });
    renderWithProviders(<ProductUnavailableScreen />);

    expect(await screen.findByTestId('unavailable-hero-title')).toHaveTextContent('Bu ürün artık stokta yok');
  ...
  it('J52.3 tekrar stokta → kutlama başlığı + "Ürünü Gör" butonu push eder', async () => {
    getOne.mockResolvedValue({
      data: { data: { id: 'prod-1', title: 'Deri Ceket', status: 'active', quantity: 3 } },
    });
    getSimilar.mockResolvedValue({ data: { data: [] } });
    renderWithProviders(<ProductUnavailableScreen />);

    expect(await screen.findByTestId('unavailable-hero-title')).toHaveTextContent('İyi haber: ürün tekrar satışta!');
  ...
  it('J52.4 benzer ürün yoksa boş metin gösterilir', async () => {
    ...
    expect(await screen.findByText('Bu kategoride başka aktif ürün bulunamadı.')).toBeOnTheScreen();
  });
  ...
  it('J52.5 benzer ürünler listelenir', async () => {
    ...
    getSimilar.mockResolvedValue({
      data: { data: [{ id: 's1', title: 'Benzer Ceket', price: 200, images: [] }] },
    });
    renderWithProviders(<ProductUnavailableScreen />);
