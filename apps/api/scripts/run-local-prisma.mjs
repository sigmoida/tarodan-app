...
const commands = {
  ...
  media: ["exec", "ts-node", "prisma/seed-media.ts"],
};

if (!commands[mode]) {
  console.error("Usage: node scripts/run-local-prisma.mjs <seed|reset|media>");
  process.exit(2);
}
...
if (
  mode === "media" &&
  ["1", "true"].includes((process.env.SEED_SKIP_IMAGES || "").toLowerCase())
) {
  console.log("SEED_SKIP_IMAGES is set; local seed media sync skipped.");
  process.exit(0);
}
