...
// Bull Redis kökü + salt-okunur kuyruk handle'ları.
// API artık HTTP + producer'dır; consumer'lar (event worker + scheduled processor)
// worker process'ine taşındı (WorkerAppModule). Bkz. worker-app.module.ts.
import { BullRootModule } from "./workers/bull-root.module";
import { QueueMonitorModule } from "./workers/queue-monitor.module";
...
@Module({
  imports: [
    ...
    // Bull Redis bağlantı kökü (connection). Consumer'lar worker process'inde.
    BullRootModule,
    // TÜM kuyrukların SALT-OKUNUR handle'ı: Bull Board + API'nin producer kuyrukları.
    QueueMonitorModule,
