...
export class SearchSyncListener {
  ...
  async handleCarModelChanged(payload: CarModelEvent): Promise<void> {
    ...
    this.logger.debug(`carModel.changed → reindexed ${products.length} products for carModel ${carModelId}`);
  }
  ...
  async handleCarModelDeleted(payload: CarModelEvent): Promise<void> {
    ...
    this.logger.debug(`carModel.deleted → reindexed ${productIds.length} products (carModelId set to null)`);
  }
}
