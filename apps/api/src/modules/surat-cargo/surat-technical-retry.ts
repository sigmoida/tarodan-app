 ...
 */
export async function withSuratTechnicalRetries(
  maxAttempts: number,
  baseDelayMs: number,
  fn: () => Promise<SuratShipmentResult>,
): Promise<SuratShipmentResult> {
  let last: SuratShipmentResult | undefined;
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    const delay =
      baseDelayMs * 2 ** attempt + Math.floor(Math.random() * 120);
