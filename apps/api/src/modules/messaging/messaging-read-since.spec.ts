...
describe("MessagingService getThreadMessages read+since", () => {
      ...
      findMany: jest.fn().mockResolvedValue([
        {
          ...
          createdAt: new Date(),
          readAt: null,
          sender: { id: "u2", displayName: "V" },
          receiver: { id: "u1", displayName: "A" },
        },
      ]),
  ...
  it("applies since filter to the message query", async () => {
    ...
    await service.getThreadMessages("t1", "u1", {
      page: 1,
      pageSize: 50,
      since,
    } as any);
  ...
  it("emits message:read for newly read message ids", async () => {
    ...
    expect(realtime.emitMessageRead).toHaveBeenCalledWith(
      "t1",
      "u1",
      expect.arrayContaining(["m2"]),
    );
  });
});
