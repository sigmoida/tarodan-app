...
export class ElogoService {
  ...
  isEnabled(): boolean {
    return (
      this.configService
        .get<string>("ELOGO_ENABLED", "false")
        ?.trim()
        .toLowerCase() === "true"
    );
  }
  ...
  private get callOptions(): ElogoSoapCallOptions {
    return {
      timeoutMs:
        Number(
          this.configService.get<string>("ELOGO_SOAP_TIMEOUT_MS", "30000"),
        ) || 30000,
    };
  }
  ...
  private async withSession<T>(
    ...
    try {
      ...
      if (looksAuth) {
        this.logger.warn(
          "eLogo oturum geçersiz olabilir, yeniden login deneniyor",
        );
        this.session = null;
  ...
  async resolveDocumentType(
    ...
    try {
      ...
      this.logger.warn(
        `checkUser başarısız (${identifier}), EARCHIVE varsayıldı: ${err.message}`,
      );
      return "EARCHIVE";
    }
  }
  ...
  async checkUser(
    identifier: string,
  ): Promise<import("./helpers/elogo.types").ElogoUserCheckResult | null> {
    const [result] = await this.withSession((sid) =>
      this.client.checkUser([identifier], sid, this.callOptions),
    );
    return result ?? null;
  ...
  async sendDocument(
    ...
    return this.withSession((sid) =>
      this.client.sendDocument(params, sid, this.callOptions),
    );
  }
  ...
  async cancelEArchiveInvoice(
    uuid: string,
    elementId?: string,
  ): Promise<ElogoSendResult> {
    return this.withSession((sid) =>
      this.client.cancelEArchive(uuid, elementId, sid, this.callOptions),
    );
  }
  ...
  async getEArchiveInvoicePdf(uuid: string): Promise<Buffer | null> {
    return this.withSession((sid) =>
      this.client.getEArchiveInvoicePdf(uuid, sid, this.callOptions),
    );
  }
  ...
  refundStrategy(
    ...
    now: Date = new Date(),
  ): "CANCEL" | "RETURN_INVOICE" {
    const issued =
      typeof invoiceIssueDate === "string"
        ? new Date(invoiceIssueDate)
        : invoiceIssueDate;
