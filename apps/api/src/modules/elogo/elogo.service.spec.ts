...
import type { ElogoSession, ElogoUserCheckResult } from "./helpers/elogo.types";
...
describe("ElogoService", () => {
  ...
  ): Promise<ElogoService> {
    login = jest.fn(async (): Promise<ElogoSession> => ({
      sessionId: "G;sess-1",
      acquiredAt: 0,
    }));
  ...
  it("isEnabled() ELOGO_ENABLED değerine bağlı", async () => {
    ...
    expect((await buildService({ ELOGO_ENABLED: "true" })).isEnabled()).toBe(
      true,
    );
    expect((await buildService({ ELOGO_ENABLED: "TRUE" })).isEnabled()).toBe(
      true,
  ...
  it("cancelEArchiveInvoice client.cancelEArchive çağırır", async () => {
    ...
    expect(cancelEArchive).toHaveBeenCalledWith(
      "u1",
      "TRD2026000000001",
      expect.any(String),
      expect.anything(),
  ...
  it("refundStrategy: ≤8 gün CANCEL, >8 gün RETURN_INVOICE", async () => {
    ...
    expect(
      service.refundStrategy(issued, new Date("2026-06-05T00:00:00Z")),
    ).toBe("CANCEL"); // 4 gün
    expect(
      service.refundStrategy(issued, new Date("2026-06-09T00:00:00Z")),
    ).toBe("CANCEL"); // 8 gün (sınır)
    expect(
      service.refundStrategy(issued, new Date("2026-06-12T00:00:00Z")),
    ).toBe("RETURN_INVOICE"); // 11 gün
  });
});
...
describe("StubElogoSoapClient", () => {
  ...
  it("checkUser ELOGO_STUB_EINVOICE_VKNS listesine göre karar verir", async () => {
    ...
    const res = await client.checkUser(
      ["1234567890", "11111111111"],
      "s",
      opts,
    );
