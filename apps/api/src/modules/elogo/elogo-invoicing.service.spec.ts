...
function fakeConfig(values: Record<string, string> = {}): ConfigService {
  ...
  return { get: (k: string, d?: string) => (k in base ? base[k] : d) } as unknown as ConfigService;
}
...
function makePrisma(seed: any = {}) {
  ...
  const store = {
    ...
    elogoInvoice: {
      findUnique: jest.fn(async ({ where }: any) => {
        ...
        if (where.type_sourceId)
          return (
            invoices.find(
              (i) => i.type === where.type_sourceId.type && i.sourceId === where.type_sourceId.sourceId,
            ) ?? null
          );
        return null;
      }),
      findMany: jest.fn(async ({ where }: any) => {
        return invoices.filter((i) => {
          ...
          if (where.attemptCount?.lt != null && !(i.attemptCount < where.attemptCount.lt)) return false;
          return true;
        });
      }),
      ...
      update: jest.fn(async ({ where, data }: any) => {
        ...
        for (const [k, v] of Object.entries<any>(data)) {
          rec[k] = v && typeof v === 'object' && 'increment' in v ? (rec[k] ?? 0) + v.increment : v;
        }
        return rec;
      }),
    },
    ...
    order: { findUnique: jest.fn(async ({ where }: any) => seed.orders?.[where.id] ?? null) },
    commissionLedger: { findUnique: jest.fn(async ({ where }: any) => seed.ledgers?.[where.orderId] ?? null) },
    membershipPayment: {
      findUnique: jest.fn(async ({ where }: any) => seed.memberships?.[where.id] ?? null),
      findFirst: jest.fn(
        async ({ where }: any) =>
          Object.values<any>(seed.memberships ?? {}).find((m) => m.providerPaymentId === where.providerPaymentId) ??
          null,
      ),
    },
    productBoost: {
    ...
    tradeCashPayment: { findUnique: jest.fn(async ({ where }: any) => seed.tradeCash?.[where.id] ?? null) },
    payment: { findFirst: jest.fn(async ({ where }: any) => seed.payments?.[where.orderId] ?? null) },
    address: { findFirst: jest.fn(async ({ where }: any) => seed.addresses?.[where.userId] ?? null) },
    user: { findUnique: jest.fn(async ({ where }: any) => seed.users?.[where.id] ?? null) },
...
function makeElogo(over: Partial<Record<string, any>> = {}): ElogoService {
  return {
    ...
    sendDocument: jest.fn(async () => ({ success: true, code: 1, refId: 999, documentUuid: 'u' })),
...
describe('ElogoInvoicingService', () => {
  it('komisyon: satıcıya e-Arşiv keser, KDV-dahil 120 → matrah 100, gönderim şekli marker', async () => {
    const prisma = makePrisma({
      ...
      ledgers: { o1: { sellerCommission: 120 } },
  ...
  it('alıcı adresi UBL PostalAddress\'e yazılır (Country-only şema hatası önlenir)', async () => {
    const prisma = makePrisma({
      ...
      ledgers: { o1: { sellerCommission: 120 } },
      users: { s1: { displayName: 'Satıcı X', taxId: null } },
      addresses: { s1: { city: 'Bursa', district: 'Nilüfer', address: 'Koleksiyon Sok. No:3' } },
    });
    const elogo = makeElogo();
    const svc = new ElogoInvoicingService(prisma, elogo, fakeConfig());
    await svc.issueCommissionInvoice('o1');
    ...
    expect(xml).toContain('<cbc:CitySubdivisionName>Nilüfer</cbc:CitySubdivisionName>');
  });
  ...
  it('idempotency: zaten "sent" kayıt varsa tekrar göndermez', async () => {
    ...
    prisma.invoices.push({ id: 'pre', type: 'commission', sourceId: 'o1', status: 'sent' });
    const elogo = makeElogo();
    const svc = new ElogoInvoicingService(prisma, elogo, fakeConfig());
  ...
  it('e-Fatura: alıcı GİB mükellefiyse EINVOICE + alias, gönderim şekli YOK', async () => {
    const prisma = makePrisma({
      orders: { o1: { sellerId: 's1' } },
      ledgers: { o1: { sellerCommission: 240 } },
      users: { s1: { displayName: 'Kurumsal A.Ş.', taxId: '1234567890', companyName: 'Kurumsal A.Ş.' } },
    });
    const elogo = makeElogo({
      checkUser: jest.fn(async () => ({ identifier: '1234567890', isEInvoiceUser: true, eInvoicePkAlias: 'urn:mail:pk' })),
    });
    const svc = new ElogoInvoicingService(prisma, elogo, fakeConfig());
    ...
    expect(params.ublXml).toContain('<cbc:ProfileID>TEMELFATURA</cbc:ProfileID>');
  ...
  it('takas iadesi: takas komisyon faturasını iptal eder', async () => {
    const prisma = makePrisma();
    prisma.invoices.push({
      id: 't1', type: 'trade_commission', sourceId: 'tcp1', documentType: 'EARCHIVE',
      status: 'sent', ettn: 'te1', invoiceNumber: 'TRD2026000000009', elogoRefId: '9', issuedAt: new Date(),
    });
    const elogo = makeElogo({ refundStrategy: jest.fn(() => 'CANCEL') });
    const svc = new ElogoInvoicingService(prisma, elogo, fakeConfig());
  ...
  it('iade: boost faturasını da iptal eder (orderId → boost)', async () => {
    ...
    prisma.invoices.push({
      id: 'i1', type: 'boost', sourceId: 'b1', documentType: 'EARCHIVE',
      status: 'sent', ettn: 'e1', invoiceNumber: 'TRD2026000000001', elogoRefId: '1', issuedAt: new Date(),
    });
    const elogo = makeElogo({ refundStrategy: jest.fn(() => 'CANCEL') });
    const svc = new ElogoInvoicingService(prisma, elogo, fakeConfig());
  ...
  it('iade >8 gün: komisyon + hizmet bedeli için AYRI iade faturası (çakışma yok)', async () => {
    const prisma = makePrisma({ users: { r1: { displayName: 'X', taxId: null } } });
    ...
    for (const [id, type, ettn, num, net] of [
      ...
      prisma.invoices.push({
        id, type, sourceId: 'o1', documentType: 'EARCHIVE', status: 'sent', ettn, invoiceNumber: num,
        issuedAt: old, recipientUserId: 'r1', recipientVknTckn: '11111111111', recipientName: 'X', netAmount: net, vatRate: 20,
      });
    }
    const elogo = makeElogo({ refundStrategy: jest.fn(() => 'RETURN_INVOICE') });
    const svc = new ElogoInvoicingService(prisma, elogo, fakeConfig());
  ...
  it('platform satışı: komisyon ATLANIR, alıcıya platform_sale e-Arşivi kesilir', async () => {
    const prisma = makePrisma({
      ...
      ledgers: { o1: { sellerCommission: 14.51, buyerFee: 0 } },
      users: { plat: { displayName: 'Tarodan Official Store', sellerType: 'platform' }, b1: { displayName: 'Alıcı', taxId: null } },
    });
    const elogo = makeElogo();
    const svc = new ElogoInvoicingService(prisma, elogo, fakeConfig());
