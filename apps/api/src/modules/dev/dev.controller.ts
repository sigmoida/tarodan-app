...
export class DevController {
  ...
  async cancelExpiredPayments() { assertTestEnv(); return this.payment.cancelExpiredPayments(); }

  @Post('run/release-expired-reservations')
  async releaseExpiredReservations() { assertTestEnv(); return this.payment.releaseExpiredOrderReservations(); }

  @Post('run/release-holds-due')
  async releaseHoldsDue() { assertTestEnv(); return this.payment.releaseHoldsDue(); }

  @Post('run/reconcile-paytr')
  async reconcilePaytr() { assertTestEnv(); return this.payment.reconcilePendingPaytrPayments(); }

  @Post('run/sweep-out-of-stock')
  async sweepOutOfStock() { assertTestEnv(); return this.productLock.sweepOutOfStockProducts(); }

  @Post('run/expire-offers')
  async expireOffers() { assertTestEnv(); return this.offerScheduler.handleExpiredOffers(); }

  @Post('run/cancel-expired-trades')
  async cancelExpiredTrades() { assertTestEnv(); return this.trade.autoCancelExpiredTrades(); }

  @Post('run/check-expired-memberships')
  async checkExpiredMemberships() { assertTestEnv(); return this.membership.checkExpiredMemberships(); }

  @Post('run/process-auto-renewals')
  async processAutoRenewals() { assertTestEnv(); return this.membershipScheduler.processAutoRenewals(); }

  // ───────── Zaman yolculuğu: tarih backdate ─────────
  /** body: { model, where, data } — data içindeki ISO tarih string'leri Date'e çevrilir. */
  @Post('backdate')
  async backdate(@Body() body: { model: string; where: Record<string, any>; data: Record<string, any> }) {
    assertTestEnv();
    const m = (this.prisma as any)[body.model];
    if (!m?.updateMany) throw new NotFoundException(`Bilinmeyen model: ${body.model}`);
    const data: Record<string, any> = {};
    for (const [k, v] of Object.entries(body.data ?? {})) {
      data[k] = typeof v === 'string' && /^\d{4}-\d{2}-\d{2}T?/.test(v) ? new Date(v) : v;
    }
  ...
  // ───────── DB-seviyesi assertion için generic okuma ─────────
  @Post('find')
  async find(@Body() body: { model: string; where: Record<string, any>; select?: Record<string, any>; orderBy?: any }) {
    assertTestEnv();
    const m = (this.prisma as any)[body.model];
    if (!m?.findFirst) throw new NotFoundException(`Bilinmeyen model: ${body.model}`);
    return m.findFirst({ where: body.where, select: body.select, orderBy: body.orderBy });
  }
  ...
  async count(@Body() body: { model: string; where?: Record<string, any> }) {
    assertTestEnv();
    const m = (this.prisma as any)[body.model];
    if (!m?.count) throw new NotFoundException(`Bilinmeyen model: ${body.model}`);
  ...
  async resetState() {
    assertTestEnv();
    const VOLATILE = [
      'orders', 'payments', 'payment_holds', 'payout_transfers', 'refund_requests', 'invoices',
      'offers',
      'trades', 'trade_items', 'trade_shipments', 'trade_shipment_events', 'trade_cash_payments', 'trade_disputes', 'trade_messages',
      'message_threads', 'messages',
      'carts', 'cart_items',
      'wishlists', 'wishlist_items',
      'support_tickets', 'ticket_messages',
      'notification_logs', 'scheduled_notifications',
      'membership_payments', 'user_memberships',
      'product_ratings', 'ratings', 'product_likes', 'product_boosts',
      'seller_bank_accounts', 'user_follows', 'two_factor_secrets',
    ...
    if (existing.length) {
      ...
      await this.prisma.$executeRawUnsafe(`TRUNCATE TABLE ${cols} RESTART IDENTITY CASCADE`);
    }
    ...
    let snapErr = '';
    try {
      await this.prisma.$executeRawUnsafe(`DELETE FROM products WHERE id NOT IN (SELECT id FROM _seed_products)`);
      await this.prisma.$executeRawUnsafe(`DELETE FROM users WHERE id NOT IN (SELECT id FROM _seed_users)`);
      await this.prisma.$executeRawUnsafe(`UPDATE products SET quantity = 100000, status = 'active' WHERE id IN (SELECT id FROM _seed_products)`);
    } catch (e) {
      snapshot = false;
      snapErr = (e as Error).message.replace(/[\r\n\t ]+/g, ' ').slice(0, 110);
      await this.prisma.$executeRawUnsafe(`UPDATE products SET quantity = 100000 WHERE status = 'active'`).catch(() => {});
    }
    ...
    let memErr = '';
    try {
      await this.prisma.$executeRawUnsafe(`INSERT INTO user_memberships SELECT * FROM _seed_memberships ON CONFLICT (user_id) DO NOTHING`);
    } catch (e) { memErr = (e as Error).message.replace(/[\r\n\t ]+/g, ' ').slice(0, 110); }
    ...
    } catch (e) { ahmetTier = 'ERR:' + (e as Error).message.slice(0, 40); }
    ...
    } catch (e) { dbInfo = 'ERR:' + (e as Error).message.slice(0, 40); }

    let offersAfter = -1;
    try { offersAfter = await this.prisma.offer.count(); } catch {}

    return { ok: true, truncated: existing.length, snapshot, ahmetTier, snapErr, memErr, dbInfo, offersAfter };
  }
}
