...
function makeDeps() {
  ...
  const users: Record<string, any> = {
    u1: { id: "u1", phone: null, isPhoneVerified: false },
  };
  const prisma: any = {
    user: {
    ...
    phoneVerificationToken: {
      findFirst: jest.fn(async ({ where }: any) => {
        ...
        if (where?.usedAt === null)
          filtered = filtered.filter((t) => !t.usedAt);
  ...
  const netgsm: any = {
    formatTurkishNumber: (p: string) =>
      p.startsWith("+") ? p : `+90${p.replace(/\D/g, "")}`,
...
describe("PhoneVerificationService", () => {
  ...
  it("geçersiz numara BadRequestException fırlatır (M5)", async () => {
    ...
    const svc = new PhoneVerificationService(prisma, netgsm);
    await expect(svc.sendCode("u1", "abc")).rejects.toThrow(
      BadRequestException,
    );
    await expect(svc.sendCode("u1", "+++")).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("yanlış kodda hata fırlatır", async () => {
    ...
    const svc = new PhoneVerificationService(prisma, netgsm);
    await svc.sendCode("u1", "+905551234567");
    await expect(svc.verify("u1", "000000")).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it("SMS başarısız olursa token silinir ve hata fırlatır", async () => {
    const { prisma, netgsm, tokenStore } = makeDeps();
    netgsm.sendOtp.mockResolvedValue({
      success: false,
      error: "NetGSM hatası",
    });
    const svc = new PhoneVerificationService(prisma, netgsm);
