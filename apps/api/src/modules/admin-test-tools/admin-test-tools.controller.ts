...
export class AdminTestToolsController {
  ...
  @Post('run-cron')
  @ApiOperation({ summary: 'Bir cron’u manuel tetikle' })
  async runCron(@CurrentUser('id') adminId: string, @Body() body: { key: string }) {
    const res = await this.service.runCron(body?.key);
    await this.writeAudit(adminId, 'test_tools_run_cron', 'Cron', body?.key, null, res.result);
    return res;
  }
  ...
  async adjust(
    ...
    @Body() body: { type: TestToolType; id: string; action: AdjustAction; value?: number },
  ) {
    const res = await this.service.adjust(body.type, body.id, body.action, body.value ?? 0);
