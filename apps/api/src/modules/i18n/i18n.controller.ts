...
export class I18nController {
  ...
  @Get("translations")
  @ApiOperation({
    summary: "Get the shared catalog (or a namespace slice) for a language",
  })
  @ApiQuery({ name: "lang", required: false, enum: ["tr", "en"] })
  @ApiQuery({
    name: "ns",
    required: false,
    description: 'Namespace slice, e.g. "auth" or "server"',
  })
  @ApiResponse({
    status: 200,
    description: "Catalog (or namespace) for the specified language",
  })
  getTranslations(
    ...
    const language =
      lang || this.i18nService.parseAcceptLanguage(acceptLanguage);
    return {
      language,
      namespace: ns ?? null,
