...
describe('I18nService (#223) — shared catalog + ICU', () => {
  ...
  it('interpolates ICU params', () => {
    expect(svc.translate('product.searchResultsFor', 'tr', { query: 'lastik' })).toBe(
      '"lastik" araması',
    );
  });

  it('degrades to the raw key when a key is missing (no throw)', () => {
    expect(svc.translate('__does.not.exist__', 'tr')).toBe('__does.not.exist__');
  });
  ...
  it('serves a namespace slice of the catalog', () => {
    const common = svc.getAllTranslations('en', 'common') as Record<string, unknown>;
  ...
  describe('resolveRequestLocale precedence', () => {
    ...
    it('uses Accept-Language when no user preference', () => {
      expect(resolveRequestLocale({ headers: { 'accept-language': 'en;q=1' } })).toBe('en');
    });
