...
export class ProductSchedulerService implements OnModuleInit {
  ...
  async runUpdatePopularityScores(log: (msg: string) => void = () => {}) {
    ...
    try {
      ...
      log(`${updatedCount} product scores updated`);
      return {
        summary: `${updatedCount} product scores updated`,
  ...
  async runExpireBoosts(log: (msg: string) => void = () => {}) {
    try {
      ...
      log(
        `${expiredProducts.length} product boosts downgraded · ${expiringBoosts.length} boosts ended`,
      );
      return {
        summary: `${expiredProducts.length} boosts downgraded · ${expiringBoosts.length} ended`,
        stats: {
  ...
  async runExpireOldListings(log: (msg: string) => void = () => {}) {
    ...
    try {
      ...
      log(
        `${result.count} old listings deactivated (>${this.LISTING_EXPIRY_DAYS} days)`,
      );
      return {
        summary: `${result.count} old listings deactivated`,
  ...
  async runSendExpirationWarnings(log: (msg: string) => void = () => {}) {
    ...
    try {
      ...
      if (expiringListings.length === 0) {
        ...
        log("No listings expiring soon");
        return {
          summary: "0 expiring listings",
      ...
      log(`${expiringListings.length} listings expiring within 7 days`);
      ...
      const sellerCount = new Set(expiringListings.map((l) => l.seller.id))
        .size;
      log(
        `Expiry warnings sent to ${sellerCount} sellers for ${expiringListings.length} listings`,
      );
      return {
        summary: `${sellerCount} sellers · ${expiringListings.length} listings`,
