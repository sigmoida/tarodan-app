...
export class TradeLifecycleService {
  ...
  async createTrade(
    ...
    const initiatorAddressId = await this.tradeShipment.resolveTradeShippingAddressId(
      initiatorId,
  ...
  async acceptTrade(
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      receiverAddressId = await this.tradeShipment.resolveTradeShippingAddressId(
        userId,
        dto.shippingAddressId,
        { required: true, db: tx },
      );
  ...
  async counterTrade(
    ...
    const newInitiatorAddressId = await this.tradeShipment.resolveTradeShippingAddressId(
      originalReceiverId,
      undefined,
      { required: false },
    );
    ...
    await this.tradeCommon.invalidateProductCaches([...oldProductIds, ...newProductIds]);
  ...
  async resolveDispute(
    ...
    return this.tradeQuery.getTradeById(
      tradeId,
