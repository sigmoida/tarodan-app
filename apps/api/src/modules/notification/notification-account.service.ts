...
export class NotificationAccountService {
  ...
  async sendPasswordResetEmail(userId: string, resetToken: string) {
    ...
    if (dbTemplate?.bodyHtml) {
      html = this.dispatch.substituteTemplateVariables(dbTemplate.bodyHtml, templateData);
      subject = dbTemplate.subject
        ? this.dispatch.substituteTemplateVariables(dbTemplate.subject, templateData)
    ...
    await this.dispatch.logNotification(userId, 'email', 'password_reset', 'Şifre Sıfırlama', '', result.success);
  ...
  async sendGuestCheckoutVerificationCode(email: string, code: string, ttlSeconds: number) {
    await this.dispatch.sendTemplateEmailToAddress(email, 'guest-checkout-otp', {
      code,
