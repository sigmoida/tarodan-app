...
describe('Notification preference gating (wiring)', () => {
  describe('NotificationService', () => {
    ...
    beforeEach(async () => {
      ...
      const module: TestingModule = await Test.createTestingModule({
        providers: [
          ...
          { provide: ExpoPushProvider, useValue: { sendToUser, registerToken: jest.fn(), isConfigured: () => false } },
    ...
    it('createInAppNotification: messageAlerts=false → NEW_MESSAGE için zil+emit+push HEPSİ atlanır', async () => {
      ...
      const result = await service.createInAppNotification('user-1', NotificationType.NEW_MESSAGE, {
        threadId: 't-1',
        senderName: 'Ayşe',
    ...
    it('createInAppNotification: pushNotifications=false → zil+emit kalır ama push GİTMEZ', async () => {
      ...
      const result = await service.createInAppNotification('user-1', NotificationType.ORDER_SHIPPED, {
        orderId: 'o-1',
    ...
    it('createInAppNotification: varsayılan (hepsi açık) → zil + push gider', async () => {
      ...
      await service.createInAppNotification('user-1', NotificationType.NEW_MESSAGE, {
        threadId: 't-1',
        senderName: 'Ayşe',
        messagePreview: 'Merhaba',
      });
  ...
  describe('PushWorker.handleSendNotification', () => {
    ...
    const mockPrisma = {
      user: {
        findUnique: jest.fn(() => Promise.resolve({ notificationSettings: storedSettings })),
      },
      pushToken: {
        findMany: jest.fn().mockResolvedValue([{ token: 'ExponentPushToken[xxxxxxxx]' }]),
    ...
    const makeJob = (type: string) =>
      ({
        id: 'job-1',
        data: { userId: 'user-1', title: 'Başlık', body: 'Gövde', data: { type } },
      }) as any;
    ...
    it('messageAlerts=false → in-app KAYDEDİLMEZ, Expo fetch çağrılmaz, pushSent=false', async () => {
      ...
      expect(res).toMatchObject({ pushSent: false, reason: 'Suppressed by user preference' });
    });
