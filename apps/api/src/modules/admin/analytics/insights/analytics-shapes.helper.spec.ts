...
describe("toSeries", () => {
  ...
  it("aralıkta olmayan kovayı sessizce düşürür", () => {
    const series = toSeries("gmv", [{ bucket: "2026-05-30", value: 9 }], buckets);
...
describe("toBreakdown", () => {
  ...
  it("sayıya göre istendiğinde payı sayıdan hesaplar", () => {
    expect(toBreakdown(rows, "count").map((row) => [row.key, row.share])).toEqual(
      [
        ["b", 90],
        ["a", 10],
      ],
...
describe("toFunnel", () => {
  it("ilk adıma göre dönüşümü ve bir önceki adımdan kaybı verir", () => {
    expect(
      ...
      { key: "created", count: 100, conversionFromFirst: 100, dropOffFromPrevious: 0 },
      { key: "accepted", count: 40, conversionFromFirst: 40, dropOffFromPrevious: 60 },
      { key: "completed", count: 30, conversionFromFirst: 30, dropOffFromPrevious: 25 },
    ]);
  });
