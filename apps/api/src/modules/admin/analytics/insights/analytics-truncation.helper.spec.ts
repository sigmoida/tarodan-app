...
describe("stampTruncations", () => {
  ...
  it("tam olarak damga anında başlayan pencerede uyarmaz", () => {
    expect(
      stampTruncations(windowFrom(ANALYTICS_STAMP_SINCE.orderCancelledAt), [
        "orderCancelledAt",
      ]),
    ).toEqual([]);
  });
  ...
  it("damga istenmediğinde boş döner", () => {
    expect(
      stampTruncations(windowFrom("2020-01-01T00:00:00.000Z"), []),
    ).toEqual([]);
  });
  ...
  it("her damganın bir başlangıç anı vardır", () => {
    for (const stamp of ANALYTICS_STAMP_KEYS) {
      expect(Number.isNaN(Date.parse(ANALYTICS_STAMP_SINCE[stamp]))).toBe(
        false,
      );
    }
