...
export const ALERT_DEFINITIONS: Record<QueryableAlertKey, AlertDefinition> = {
  ...
  stuckShippedOrders: {
    severity: "critical",
    threshold: (config) => ({ value: shippedStaleAlertDays(config), unit: "days" }),
    query: (prisma, now, config) =>
      prisma.order.count({
        where: {
          status: OrderStatus.shipped,
          shipment: {
            is: { shippedAt: { lt: daysAgo(now, shippedStaleAlertDays(config)) } },
          },
        },
      }),
    read: count,
    ...
    query: (prisma, now, config) =>
      prisma.outboxEvent.count({
        where: {
          ...
          updatedAt: { lt: new Date(now.getTime() - outboxStaleProcessingMs(config)) },
        },
      }),
    read: count,
  },
    ...
    query: (prisma, now, config) =>
      prisma.carrierCancellationTask.count({
        where: {
          ...
          requestedAt: {
            lt: new Date(
              now.getTime() - carrierCancellationAlertHours(config) * 60 * 60 * 1000,
            ),
          },
        },
      }),
