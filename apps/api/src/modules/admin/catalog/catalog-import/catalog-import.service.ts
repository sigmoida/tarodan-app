...
export class CatalogImportService {
  ...
  async import(
    ...
    if (!rowNumbers.length) {
      throw new BadRequestException(
        i18nMessage("server.admin.catalogImport.noDataRows", {
    ...
    if (rowNumbers.length > CATALOG_IMPORT_LIMITS.maxRows) {
      throw new BadRequestException(
        i18nMessage("server.admin.catalogImport.tooManyRows", {
          max: CATALOG_IMPORT_LIMITS.maxRows,
  ...
  private assertWorkbookFile(file: Express.Multer.File | undefined): void {
    if (!file)
      throw new BadRequestException(
        i18nMessage("server.admin.catalogImport.fileMissing"),
      );
    if (!file.originalname.toLowerCase().endsWith(".xlsx")) {
      throw new BadRequestException(
        i18nMessage("server.admin.catalogImport.onlyXlsx"),
      );
    }
    ...
    if (file.size > CATALOG_IMPORT_LIMITS.maxFileBytes) {
      throw new BadRequestException(
        i18nMessage("server.admin.catalogImport.fileTooLarge", {
          max: Math.round(CATALOG_IMPORT_LIMITS.maxFileBytes / 1024 / 1024),
        }),
      );
    }
  }
  ...
  private assertHeaders(
    ...
    if (missing.length) {
      throw new BadRequestException(
        i18nMessage("server.admin.catalogImport.missingColumns", {
