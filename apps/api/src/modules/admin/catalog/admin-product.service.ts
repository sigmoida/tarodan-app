...
export class AdminProductService {
  ...
  /**
   * Yöneticinin KAYITLI bir ürün görselini 90° çevirmesi.
   *
   * Yükleme ucuyla aynı sebeple ayrı duruyor: `rotateProductImageVariants`
   * sahipliği anahtarın klasöründen (ya da görselin hâlâ o satıcının ürününe
   * bağlı olmasından) doğrular ve YENİ anahtarı da o kullanıcının klasörüne
   * yazar. Yönetici kendi kimliğiyle çağırsaydı hem doğrulama reddederdi hem
   * de üretilen anahtar kaydetme yolunda sahipsiz kalırdı — bu yüzden ilanın
   * SAHİBİ adına çevriliyor.
   */
  async rotateProductImage(productId: string, detailKey: string) {
    ...
    return {
      ...
      cardUrl: this.resolveProductImageUrl(result.cardKey),
      detailUrl: this.resolveProductImageUrl(result.detailKey),
    };
  }
