...
export class SetRolePermissionsDto {
  @ApiProperty({
    description: 'role → string[] permission map (super_admin, admin, moderator)',
    example: { admin: ['orders', 'products'], moderator: ['products', 'reviews'] },
  })
  @IsObject()
  @IsNotEmpty()
...
/** Eski izin anahtarlarını (noktalı veya orta-granüler format) yeni 36'lı formata çevirir. */
const LEGACY_MAP: Record<string, string[]> = {
  // Eski noktalı format
  'dashboard.view': ['dashboard'],
  'analytics.view': ['analytics'],
  'orders.view': ['orders'], 'orders.manage': ['orders'],
  'trades.view': ['trades'], 'trades.manage': ['trades'],
  'shipping.manage': ['shipping'],
  'refunds.view': ['refund_requests', 'refund_history'],
  'refunds.manage': ['refund_requests', 'refund_history'],
  'products.view': ['products'], 'products.moderate': ['products'],
  'catalog.manage': ['categories', 'brands', 'car_models', 'manufacturers', 'attributes', 'collections'],
  ...
  'tax.manage': ['tax'],
  'messages.view': ['messages'], 'messages.moderate': ['messages'],
  'marketing.manage': ['discounts', 'ads', 'notifications', 'email_templates', 'pages'],
  'ai_moderation.view': ['ai_moderation'], 'ai_moderation.manage': ['ai_moderation'],
  'settings.manage': ['settings', 'payment_settings', 'membership_tiers'],
  'logs.view': ['logs', 'audit_logs'],
  'staff.manage': ['staff'],
  // Orta granüler format (bir önceki sürüm)
  'refunds': ['refund_requests', 'refund_history'],
  'catalog': ['categories', 'brands', 'car_models', 'manufacturers', 'attributes', 'collections'],
...
export function migrateLegacyPermissions(data: Record<string, string[]>): Record<string, string[]> {
