...
describe('10 — Ödeme & Escrow (PAY)', () => {
  ...
  async function makeBuyerSellerProduct(opts: { price?: number; quantity?: number } = {}) {
  ...
  async function buyNow(buyer: { accessToken: string }, productId: string, shippingAddressId: string) {
  ...
  describe('POST /api/payments/process-direct — yeni kart', () => {
    ...
    scenario('PAY-003', async () => {
      ...
      const res = await request(server())
        ...
        .send({ orderId })
        .expect(400);
      expect(JSON.stringify(res.body)).toContain('Kart bilgisi veya kayıtlı kart');
  ...
  describe('POST /api/payments/process-direct — kayıtlı kart (recurring kapalı)', () => {
    ...
    scenario('PAY-011', async () => {
      ...
      const res = await request(server())
        ...
        .expect(410);
      expect(JSON.stringify(res.body)).toContain('Kayıtlı kartla ödeme şu an kullanılamıyor');
    ...
    scenario('PAY-012', async () => {
      ...
      const res = await request(server())
        ...
        .expect(403);
      expect(JSON.stringify(res.body)).toContain('Kayıtlı kartla ödeme için giriş yapmanız gerekiyor');
      expect(ctx.paytr.recurringCalls.length).toBe(0);
    });
  ...
  describe('POST /api/payments/callback/paytr', () => {
    ...
    scenario('PAY-026', async () => {
      const res = await request(server())
        .post('/api/payments/callback/paytr')
        .send(
          signCallback({ merchantOid: 'NONEXISTENTOID12345', status: 'success', totalAmount: 30000 }),
        )
        .expect(200);
      expect(res.text).toBe('OK');
    });
  ...
  describe('Rezervasyon & süre dolumu', () => {
    scenario('PAY-040', async () => {
      ...
      const result = await ctx.app.get(PaymentService).releaseExpiredOrderReservations();
    ...
    scenario('PAY-041', async () => {
      ...
      const orderMid = await prisma.order.findUnique({ where: { id: orderId } });
    ...
    scenario('PAY-045', async () => {
      ...
      await prisma.payment.update({
        where: { id: payment!.id },
        data: { status: PaymentStatus.processing, updatedAt: new Date(Date.now() - 10 * 60 * 1000) },
      });
  ...
  describe('Escrow & payout', () => {
    ...
    scenario('PAY-054', async () => {
      ...
      await expect(ctx.app.get(PaymentService).releasePayment(orderId)).rejects.toThrow();
    ...
    scenario('PAY-056', async () => {
      ...
      await prisma.order.update({
        where: { id: orderId },
        data: { status: OrderStatus.preparing, preparingDeadline: new Date(Date.now() - 1000) },
      });
      ctx.paytr.reset();

      const result = await ctx.app.get(PaymentService).handleExpiredPreparingOrders();
  ...
  describe('Bypass (kapalı ortam)', () => {
    scenario.skip('PAY-060', 'Bypass happy-path PAYMENT_BYPASS=true gerektirir; .env.test\'te false. PAY-063 kapalı durumu kanıtlar.');
    scenario.skip('PAY-061', 'Membership bypass forward PAYMENT_BYPASS=true gerektirir; .env.test\'te false.');
    scenario.skip('PAY-062', 'bypass-complete idempotency PAYMENT_BYPASS=true gerektirir; kapalıyken hep 400.');
    ...
    scenario('PAY-063', async () => {
      ...
      const res = await request(server())
        ...
        .expect(400);
      expect(JSON.stringify(res.body)).toContain('Payment bypass is not enabled');
    });
  ...
  describe('verify / confirm-failed / cancel / retry', () => {
    ...
    scenario('PAY-077', async () => {
      ...
      const res = await request(server())
        ...
        .set(authHeader(buyer))
        .expect(400);
      expect(JSON.stringify(res.body)).toContain('Sadece bekleyen ödemeler iptal edilebilir');
    });
    ...
    scenario('PAY-079', async () => {
      ...
      const res = await request(server())
        ...
        .set(authHeader(buyer))
        .expect(400);
      expect(JSON.stringify(res.body)).toContain('Sadece başarısız ödemeler tekrar denenebilir');
    });
    ...
    scenario('PAY-080', async () => {
      ...
      const fresh = await prisma.payment.findUnique({ where: { id: res.body.newPaymentId } });
  ...
  describe('İade', () => {
    scenario('PAY-090', async () => {
      ...
      const orderId = await buyAndPayDirect(buyer, product.id, addr.id);
      const prisma = getPrisma();
      const productPaid = await prisma.product.findUnique({ where: { id: product.id } });
    ...
    scenario('PAY-091', async () => {
      ...
      const res = await request(server())
        ...
        .send({ orderId })
        .expect(403);
      expect(JSON.stringify(res.body)).toContain('Sadece alıcı iade talebi oluşturabilir');
    });
    ...
    scenario('PAY-092', async () => {
      ...
      const res = await request(server())
        ...
        .send({ orderId })
        .expect(404);
      expect(JSON.stringify(res.body)).toContain('Tamamlanmış ödeme bulunamadı');
    });
    ...
    scenario('PAY-094', async () => {
      ...
      await expect(ctx.app.get(PaymentService).processRefund(orderId)).rejects.toThrow(
        /Transfer zaten başlatılmış/,
      );
    });
  ...
  describe('Grup & takas ödemeleri', () => {
    scenario('PAY-100', async () => {
      ...
      const { buyer, group } = await makeCheckoutGroup({ orderTotals: [200, 250] });
      const res = await request(server())
        .post('/api/payments/process-direct')
        .set(authHeader(buyer))
      ...
      expect(ctx.paytr.directPaymentCalls[0].amount).toBeCloseTo(Number(payment?.amount), 2);
    });
    ...
    scenario('PAY-101', async () => {
      ...
      const { buyer, group } = await makeCheckoutGroup({ orderTotals: [200, 250] });
      const prisma = getPrisma();
      const first = await prisma.order.findFirst({ where: { checkoutGroupId: group.id } });
      await prisma.order.update({ where: { id: first!.id }, data: { status: OrderStatus.cancelled } });
      const res = await request(server())
        .post('/api/payments/process-direct')
        .set(authHeader(buyer))
        .send({ checkoutGroupId: group.id, card: validCard() })
    ...
    scenario('PAY-103', async () => {
      ...
      const { payer, tradeId } = await makeAcceptedCashTrade({ cashAmount: 100 });
      const res = await request(server())
        .post('/api/payments/process-direct')
      ...
      expect(ctx.paytr.directPaymentCalls[0].amount).toBeCloseTo(Number(cp!.totalAmount), 2);
      const payment = await prisma.payment.findUnique({ where: { id: res.body.paymentId } });
    ...
    scenario('PAY-104', async () => {
      ...
      const { other, tradeId } = await makeAcceptedCashTrade({ cashAmount: 100 });
      const res = await request(server())
        .post('/api/payments/process-direct')
    ...
    scenario('PAY-105', async () => {
      ...
      const { payer, receiver, tradeId } = await makeAcceptedCashTrade({ cashAmount: 100 });
      const prisma = getPrisma();
      const cp = await prisma.tradeCashPayment.findUnique({ where: { tradeId } });
      await request(server())
      ...
      const paid = await prisma.tradeCashPayment.findUnique({ where: { tradeId } });
      ...
      const released = await prisma.tradeCashPayment.findUnique({ where: { id: cp!.id } });
      expect(released?.releasedAt).toBeTruthy();

      const payoutsCreated = await ctx.app.get(PayoutService).createPayoutsForReleasedHolds();
      expect(payoutsCreated).toBeGreaterThanOrEqual(1);
      const payout = await prisma.payoutTransfer.findFirst({ where: { tradeCashPaymentId: cp!.id } });
    ...
    scenario('PAY-106', async () => {
      ...
      const { payer, tradeId } = await makeAcceptedCashTrade({ cashAmount: 50 });
      const prisma = getPrisma();
      const cp = await prisma.tradeCashPayment.findUnique({ where: { tradeId } });
      await request(server())
        .post('/api/payments/initiate-trade-cash')
      ...
      const result = await ctx.app.get(PaymentService).refundTradeCashPaymentIfCompleted(tradeId);
      ...
      expect(await prisma.payoutTransfer.count({ where: { tradeCashPaymentId: cp!.id } })).toBe(0);
    });
  ...
  describe('Stok & para tutarlılığı', () => {
    ...
    scenario('PAY-111', async () => {
      ...
      const slowAfter = await prisma.order.findUnique({ where: { id: slowOrder.id } });
    ...
    scenario('PAY-123', async () => {
      // PayTR'a giden tutar ondalık TL'dir (kuruş DEĞİL).
      const { buyer, product, addr } = await makeBuyerSellerProduct({ price: 300 });
      const orderId = await buyNow(buyer, product.id, addr.id);
      await request(server())
        .post('/api/payments/process-direct')
        ...
        .expect(201);
      expect(ctx.paytr.directPaymentCalls.length).toBe(1);
      expect(ctx.paytr.directPaymentCalls[0].amount).toBe(300);
      expect(ctx.paytr.directPaymentCalls[0].amount).not.toBe(30000);
    });
  ...
  describe('Yetki', () => {
    scenario('PAY-130', async () => {
      ...
      const res = await request(server())
        ...
        .send({ orderId, card: validCard() })
        .expect(403);
      expect(JSON.stringify(res.body)).toContain('Bu sipariş için ödeme yapamazsınız');
      expect(ctx.paytr.directPaymentCalls.length).toBe(0);
    });
    ...
    scenario('PAY-131', async () => {
      ...
      const res = await request(server())
        ...
        .send({ orderId, card: validCard() })
        .expect(403);
      expect(JSON.stringify(res.body)).toContain('Bu sipariş için giriş yapmanız gerekiyor');
      expect(ctx.paytr.directPaymentCalls.length).toBe(0);
    });
    ...
    scenario('PAY-133', async () => {
      ...
      await successCallback(orderId).expect(200);
      await request(server()).get(`/api/payments/${payment!.id}/status`).expect(403);
    });
  ...
  describe('i18n / parite / UI', () => {
    scenario.skip('PAY-140', 'TR/EN dil bağlamı istemci (web/mobile) çevirisidir; API ham TR mesaj döner — saf-UI senaryosu.');
    scenario.skip('PAY-141', 'config bypass/recurring web=mobile pariteleri istemci davranışıdır (GET /payments/config aynı değeri döndürür; UI gizleme API e2e dışı).');
    scenario.skip('PAY-142', 'Misafire kayıtlı kart gösterilmemesi web/mobile UI davranışıdır — API e2e kapsamı dışı.');
    scenario.skip('PAY-143', '3DS HTML render (web form / mobile WebView) saf-UI senaryosudur; mock ortamda threeDSHtml null.');
    scenario.skip('PAY-144', 'Yükleniyor / bulunamadı / completed-redirect ekran durumları saf-UI senaryosudur.');
    scenario.skip('PAY-145', 'Success sayfası verify + guest=true taşıma akışı istemci davranışıdır (verify ucu PAY-070/071 ile kapsanır).');
  });
  ...
  describe('Güvenlik & kayıtlı kart', () => {
    scenario('PAY-150', async () => {
      ...
      const prisma = getPrisma();
      const row = (await prisma.savedCard.findUnique({ where: { id: card.id } })) as Record<string, unknown>;
    ...
    scenario('PAY-151', async () => {
      ...
      ctx.paytr.setStoredCards(utoken, [{ ctoken: 'ctok-1', last4: '4358', brand: 'visa' }]);
      const svc = ctx.app.get(PaymentService);
      await svc.syncSavedCardsFromUtoken(user.id, utoken);
      await svc.syncSavedCardsFromUtoken(user.id, utoken);
      const prisma = getPrisma();
      expect(await prisma.savedCard.count({ where: { userId: user.id, ctoken: 'ctok-1' } })).toBe(1);
    });

    scenario('PAY-152', async () => {
      // GET /membership/cards: maskeli, revoked gizli, require_cvv → autoRenewEligible=false.
      const user = await createUser(ctx.module);
      await createSavedCard(user.id, { ctoken: 'c-active1', last4: '1111', requireCvv: false });
      await createSavedCard(user.id, { ctoken: 'c-active2', last4: '2222', requireCvv: true });
      await createSavedCard(user.id, { ctoken: 'c-revoked', last4: '3333', status: 'revoked' });
    ...
    scenario('PAY-153', async () => {
      ...
      const card = await createSavedCard(owner.id, { ctoken: 'c-owned', last4: '9999' });
      const res = await request(server())
  ...
  async function createGuestOrder(productId: string, price: number): Promise<string> {
    const prisma = getPrisma();
    // Misafir alıcı placeholder (sistem misafir kullanıcısı yoksa düz bir kullanıcı yeterli).
    const guestBuyer = await createUser(ctx.module, { email: `guest-${Date.now()}-${Math.random().toString(36).slice(2, 6)}@guest.local` });
    const seller = await prisma.product.findUnique({ where: { id: productId }, select: { sellerId: true } });
    const order = await prisma.order.create({
      data: {
  ...
  async function createSavedCard(
    userId: string,
    over: { ctoken?: string; last4?: string; requireCvv?: boolean; status?: string } = {},
  ): Promise<{ id: string; utoken: string; ctoken: string }> {
    const prisma = getPrisma();
    const utoken = `utok-${userId}`;
    const ctoken = over.ctoken ?? `ctok-${userId}-${Math.random().toString(36).slice(2, 8)}`;
    ...
    ctx.paytr.setStoredCards(utoken, [{ ctoken, last4: card.last4, requireCvv: card.requireCvv }]);
    ...
    for (const amount of opts.orderTotals) {
      ...
      await prisma.order.create({
        data: {
          ...
          status: OrderStatus.pending_payment,
          paymentExpiresAt: new Date(Date.now() + 24 * 60 * 60 * 1000),
          shippingAddress: { addressId: addr.id, city: 'İstanbul', address: 'Test' },
        },
      });
    }
    return { buyer, group };
  ...
  async function makeAcceptedCashTrade(opts: { cashAmount: number }): Promise<{
    ...
    const other = await createUser(ctx.module, { isSeller: true, premium: true });
