...
describe("07 — Sepet & Checkout (CRT)", () => {
      ...
      data: {
        ...
        maxDiscountAmount:
          opts?.maxDiscountAmount != null
            ? new Prisma.Decimal(opts.maxDiscountAmount)
            : null,
        minCartValue:
          opts?.minCartValue != null
            ? new Prisma.Decimal(opts.minCartValue)
            : null,
  ...
  /** Baseline kategori + FREE satıcı için kesin komisyon kuralını günceller. */
  async function addCommissionRule(opts: {
    ...
    return prisma.commissionRule.update({
      ...
      data: {
        sellerCommissionRate: new Prisma.Decimal(opts.sellerRate ?? 0),
        buyerServiceFeeRate: new Prisma.Decimal(opts.buyerRate ?? 0),
      },
    });
  }
  ...
  async function setupCorporateTax(sellerId: string) {
    ...
    const businessTier = await prisma.membershipTier.upsert({
      where: { type: "business" },
  ...
  scenario("CRT-002", async () => {
    ...
    const line = res.body.calculation.items.find(
  ...
  scenario("CRT-003", async () => {
    ...
    const lines = res.body.calculation.items.filter(
      (i: any) => i.productId === product.id,
    );
  ...
  scenario("CRT-006", async () => {
    ...
    expect(String(res.body.message)).toContain(
      "Bu ürün şu an satışa uygun değil",
    );
  });
  ...
  scenario("CRT-009", async () => {
    ...
    expect(String(res.body.message)).toContain(
      "Bu üründen maksimum 3 adet alabilirsiniz",
    );
  });
  ...
  scenario("CRT-010", async () => {
    ...
    const cart = await getCart(buyer).expect(200);
    const line = cart.body.calculation.items.find(
      (i: any) => i.productId === product.id,
    );
    expect(line.quantity).toBe(2);
  });
  ...
  scenario("CRT-011", async () => {
    ...
    const r3 = await addItem(buyer, { productId: p3.id, quantity: "2" }).expect(
      201,
    );
    expect(
      r3.body.calculation.items.find((i: any) => i.productId === p3.id)
        .quantity,
    ).toBe(2);
    ...
    expect(
      r4.body.calculation.items.find((i: any) => i.productId === p4.id)
        .quantity,
    ).toBe(1);
  });
  ...
  scenario("CRT-015", async () => {
    ...
    const res = await request(server())
      ...
      .expect(200);
    expect(
      res.body.calculation.items.find((i: any) => i.productId === product.id),
    ).toBeUndefined();
  });
  ...
  scenario("CRT-016", async () => {
    ...
    const cart = await getCart(buyer).expect(200);
    expect(
      cart.body.calculation.items.find((i: any) => i.productId === product.id)
        .quantity,
    ).toBe(1);
  });
  ...
  scenario("CRT-027", async () => {
    ...
    expect(
      res.body.calculation.warnings.some((w: string) =>
        w.includes("artık satışta değil"),
      ),
    ).toBe(true);
  });
  ...
  scenario("CRT-030", async () => {
    ...
    await prisma.product.update({
      where: { id: product.id },
      data: { price: new Prisma.Decimal(300) },
    });
  ...
  scenario("CRT-032", async () => {
    ...
    expect(res.body.calculation.appliedDiscounts.length).toBeGreaterThanOrEqual(
      1,
    );
  ...
  scenario("CRT-043", async () => {
    ...
    expect(pr.totalAmount).toBeCloseTo(
      pr.subtotal + pr.shippingAmount + pr.buyerFeeAmount + pr.taxAmount,
      2,
    );
  });
  ...
  scenario("CRT-053", async () => {
    ...
    const groups = await prisma.checkoutGroup.count({
      where: { id: res.body.checkoutGroupId },
    });
  ...
  scenario("CRT-059", async () => {
    ...
    const res = await request(server())
      ...
      .send({
        items: [{ productId: product.id }],
        idempotencyKey: uuid(),
        shippingAddressId: otherAddr.id,
      })
      .expect(400);
  ...
  scenario("CRT-067", async () => {
    ...
    const o1 = await prisma.order.findFirst({
      where: { checkoutGroupId: res.body.checkoutGroupId, productId: p1.id },
    });
    const o2 = await prisma.order.findFirst({
      where: { checkoutGroupId: res.body.checkoutGroupId, productId: p2.id },
    });
    ...
    const totalDiscount =
      Number(o1?.discountAmount) + Number(o2?.discountAmount);
  ...
  scenario("CRT-071", async () => {
    ...
    const groupIds = new Set([
      r1.body.checkoutGroupId,
      r2.body.checkoutGroupId,
    ]);
  ...
  scenario("CRT-072", async () => {
    ...
    const firstOrder = await prisma.order.findFirst({
      where: { checkoutGroupId: r1.body.checkoutGroupId },
    });
    expect(firstOrder?.status).toBe("cancelled");
    expect(firstOrder?.cancelReason).toContain(
  ...
  scenario("CRT-075", async () => {
    ...
    expect(String(addrJson?.guestEmail).toLowerCase()).toBe(
  ...
  scenario("CRT-081", async () => {
    ...
    expect(
      cartA.body.calculation.items.find((i: any) => i.productId === product.id),
    ).toBeTruthy();
  });
  ...
  scenario("CRT-082", async () => {
    ...
    await request(server())
      .post("/api/cart/items")
      .send({ productId: ABSENT_UUID })
      .expect(401);
    await request(server())
      .patch(`/api/cart/items/${ABSENT_UUID}`)
      .send({ quantity: 1 })
      .expect(401);
    await request(server())
      .delete(`/api/cart/items/${ABSENT_UUID}`)
      .expect(401);
    await request(server())
      .post("/api/cart/coupon")
      .send({ code: "X" })
  ...
  scenarioSkip(
    "CRT-084",
    'Saf-UI/i18n: boş sepet ekranı (cart.empty / "İlanlara Gözat" / ikon + CTA).',
  );
  scenarioSkip(
    "CRT-085",
    "Saf-UI: sepet yükleniyor animate-pulse skeleton (3 satır).",
  );
  scenarioSkip(
    "CRT-086",
    'Saf-UI/i18n: /cart kargo metni "Ödeme adımında hesaplanır" / "Calculated at checkout".',
  );
  ...
  scenarioSkip(
    "CRT-088",
    'Saf-UI: checkout kargo satırı "Adres seçin" / "Hesaplanıyor..." durumları.',
  );
  scenarioSkip(
    "CRT-089",
  ...
  scenarioSkip(
    "CRT-090",
    'Saf-UI/i18n: indirimli fiyat üstü-çizili gösterim (tr-TR "250,00 TL"), web/mobile parite.',
  );
});
