...
describe('12 — İade & İptal (REF)', () => {
  ...
  async function paidOrder(opts: { price?: number; quantity?: number; sellerAddress?: boolean } = {}) {
    ...
    if (opts.sellerAddress !== false) await createAddress({ userId: seller.id });
  ...
  const createRefund = (orderId: string, buyer: { accessToken: string }, body: Record<string, unknown>) =>
    request(server())
  ...
  describe('Cooling-off / anlık iade talebi oluşturma', () => {
    scenario('REF-001', async () => {
      ...
      const shipmentAfter = await prisma.shipment.findUnique({ where: { id: shipmentBefore!.id } });
    ...
    scenario('REF-004', async () => {
      ...
      const prisma = getPrisma();
      await prisma.shipment.update({ where: { orderId }, data: { status: ShipmentStatus.failed } });
    ...
    scenario('REF-005', async () => {
      ...
      await prisma.order.update({ where: { id: orderId }, data: { status: OrderStatus.delivered, deliveredAt: null } });
      await prisma.shipment.update({ where: { orderId }, data: { status: ShipmentStatus.delivered, deliveredAt: null } });
    ...
    scenario('REF-120', async () => {
      ...
      const fourteenDaysAgo = new Date(Date.now() - 14 * 24 * 3600 * 1000 + 60_000);
  ...
  describe('İade talebi reddi', () => {
    scenario('REF-010', async () => {
      ...
      const r1 = await createRefund(orderId, buyer, { reason: 'damaged', description: 'broken' }).expect(400);
    ...
    scenario('REF-011', async () => {
      ...
      const buyer = await createUser(ctx.module);
      const seller = await createUser(ctx.module, { isSeller: true });
      const product = await createProduct({ sellerId: seller.id, categoryId: baseline.categoryId, price: 50, quantity: 1 });
      const addr = await createAddress({ userId: buyer.id });
      const buyRes = await buyNow(ctx, buyer, product.id, addr.id).expect(201);

      const res = await createRefund(buyRes.body.orderId, buyer, { reason: 'changed_mind' }).expect(400);
    ...
    scenario('REF-012', async () => {
      ...
      await prisma.payment.updateMany({ where: { orderId }, data: { status: PaymentStatus.processing } });
    ...
    scenario('REF-014', async () => {
      ...
      const dup = await createRefund(orderId, buyer, { reason: 'changed_mind' }).expect(400);
    ...
    scenario('REF-016', async () => {
      ...
      const { buyer, orderId } = await paidOrder({ price: 100 });
      await createRefund(orderId, buyer, { reason: 'abuse_reason' }).expect(400);
    });

    scenario('REF-017', async () => {
      // refundQuantity < 1 (0) veya non-int (2.5) → 400 (Min(1) / IsInt).
      const { buyer, orderId } = await paidOrder({ price: 100 });
      await createRefund(orderId, buyer, { reason: 'changed_mind', refundQuantity: 0 }).expect(400);
      await createRefund(orderId, buyer, { reason: 'changed_mind', refundQuantity: 2.5 }).expect(400);
    });

    scenario('REF-018', async () => {
      // evidencePhotoUrls geçersiz URL → 400 (IsUrl each); description > 2000 → 400 (MaxLength).
      const { buyer, orderId } = await paidOrder({ price: 100 });
      await createRefund(orderId, buyer, { reason: 'damaged', evidencePhotoUrls: ['not-a-url'] }).expect(400);
      await createRefund(orderId, buyer, { reason: 'changed_mind', description: 'x'.repeat(2001) }).expect(400);
    });
    ...
    scenario('REF-019', async () => {
      ...
      const res = await request(server())
        .post('/api/orders/a0000000-0000-4000-8000-000000000000/refund-requests')
        .set(authHeader(buyer))
        .send({ reason: 'changed_mind' })
        .expect(404);
  ...
  describe('Yetki & güvenlik (oluşturma/görüntüleme/iptal)', () => {
    scenario('REF-020', async () => {
      ...
      const res = await createRefund(orderId, stranger, { reason: 'changed_mind' }).expect(403);
    ...
    scenario('REF-022', async () => {
      ...
      await request(server()).get(`/api/refund-requests/${id}`).set(authHeader(buyer)).expect(200);
      await request(server()).get(`/api/refund-requests/${id}`).set(authHeader(seller)).expect(200);
      await request(server()).get(`/api/refund-requests/${id}`).set(authHeader(stranger)).expect(403);
    });
    ...
    scenario('REF-141', async () => {
      ...
      await request(server()).get(`/api/refund-requests/${createRes.body.id}`).set(authHeader(kaan)).expect(403);
    });
  ...
  describe('İade talebi iptali', () => {
    ...
    scenario('REF-032', async () => {
      ...
      const res = await request(server())
        ...
        .set(authHeader(buyer))
        .expect(400);
      expect(res.body.message).toMatch(/iade kargosu açılmış veya karara bağlanmış/i);
    });
    ...
    scenario('REF-034', async () => {
      ...
      const buyer = await createUser(ctx.module);
      const res = await request(server())
        .post(`/api/refund-requests/a0000000-0000-4000-8000-000000000000/cancel`)
        .set(authHeader(buyer))
        .expect(404);
    ...
    scenario('REF-132', async () => {
      ...
      const waitRes = await createRefund(o1, b1, { reason: 'changed_mind' }).expect(201);
      expect(waitRes.body.status).toBe(RefundRequestStatus.wait_for_delivery);
      await request(server()).post(`/api/refund-requests/${waitRes.body.id}/cancel`).set(authHeader(b1)).expect(200);
      ...
      await request(server()).post(`/api/refund-requests/${rr.id}/cancel`).set(authHeader(b2)).expect(200);
    });
  });
  ...
  describe('Sipariş iptali', () => {
    ...
    scenario('REF-041', async () => {
      ...
      const res = await request(server())
        ...
        .send({ reason: 'vazgeçtim' })
        .expect(400);
      expect(res.body.message).toMatch(/kargoya verildikten sonra iptal edilemez/i);
    });
    ...
    scenario('REF-042', async () => {
      ...
      const buyer = await createUser(ctx.module);
      const seller = await createUser(ctx.module, { isSeller: true });
      const product = await createProduct({ sellerId: seller.id, categoryId: baseline.categoryId, price: 120, quantity: 5 });
      const addr = await createAddress({ userId: buyer.id });
      const buyRes = await buyNow(ctx, buyer, product.id, addr.id).expect(201);
      const orderId = buyRes.body.orderId;

      const prisma = getPrisma();
      const reservedBefore = await prisma.product.findUnique({ where: { id: product.id } });
      ...
      const order = await prisma.order.findUnique({ where: { id: orderId } });
      expect(order!.status).toBe(OrderStatus.cancelled);
      const reservedAfter = await prisma.product.findUnique({ where: { id: product.id } });
  ...
  describe('PaymentHold ↔ iade kilit etkileşimi', () => {
    ...
    scenario('REF-051', async () => {
      ...
      await request(server()).post(`/api/refund-requests/${createRes.body.id}/cancel`).set(authHeader(buyer)).expect(200);
    ...
    scenario('REF-052', async () => {
      ...
      const payouts = await prisma.payoutTransfer.findMany({ where: { paymentHold: { orderId } } });
  ...
  describe('İade kargosu + finalize', () => {
    scenario('REF-060', async () => {
      ...
      expect(await refundService.findPendingDeliveryToOpenReturn()).toHaveLength(0);
      ...
      const prisma = getPrisma();
      const rr = await prisma.refundRequest.findUnique({ where: { id: createRes.body.id } });
      ...
      const returnCall = ctx.surat.shipmentCalls.find((c) => c.OzelKargoTakipNo === rr!.returnTrackingNumber);
      expect(returnCall).toBeDefined();
      expect(returnCall!.Iademi).toBe(true);
    });
    ...
    scenario('REF-061', async () => {
      ...
      await refundService.applyReturnTrackingUpdate(refundId, { status: ShipmentStatus.in_transit, shippedAt: new Date() });
      expect((await prisma.refundRequest.findUnique({ where: { id: refundId } }))!.status).toBe(RefundRequestStatus.return_in_transit);

      await refundService.applyReturnTrackingUpdate(refundId, { status: ShipmentStatus.delivered, deliveredAt: new Date() });
      expect((await prisma.refundRequest.findUnique({ where: { id: refundId } }))!.status).toBe(RefundRequestStatus.return_delivered);

      // 30dk dolmadan finalize listesinde YOK.
      expect(await refundService.findReturnDeliveredPendingFinalize()).not.toContain(refundId);
      await prisma.refundRequest.update({
        where: { id: refundId },
        data: { returnDeliveredAt: new Date(Date.now() - 31 * 60 * 1000) },
      });
      expect(await refundService.findReturnDeliveredPendingFinalize()).toContain(refundId);
      ...
      expect(ctx.paytr.refundCalls).toHaveLength(1);
      const notif = await prisma.notificationLog.findFirst({ where: { userId: buyer.id, type: 'refund_completed' } });
      expect(notif).toBeTruthy();
    ...
    scenario('REF-062', async () => {
      ...
      const { buyer, orderId } = await paidOrder({ price: 90, sellerAddress: false });
      await markDelivered(orderId);
      const createRes = await createRefund(orderId, buyer, { reason: 'changed_mind' }).expect(201);
      expect(createRes.body.status).toBe(RefundRequestStatus.return_shipment_open);
    ...
    scenario('REF-063', async () => {
      ...
      await prisma.$executeRawUnsafe(`UPDATE "orders" SET "shipping_address" = NULL WHERE id = '${orderId}'`);
      await prisma.address.deleteMany({ where: { userId: buyer.id } });

      const refundService = ctx.app.get(RefundService);
      await expect(refundService.openReturnShipment(refundId)).rejects.toThrow(/teslimat\/kayıtlı adresi bulunamadı/i);
    });
    ...
    scenario('REF-065', async () => {
      ...
      expect(again!.returnTrackingNumber).toBe(createRes.body.returnTrackingNumber);
  ...
  describe('İade bildirimleri', () => {
    scenario('REF-070', async () => {
      ...
      await createRefund(orderId, buyer, { reason: 'changed_mind' }).expect(201);
      const prisma = getPrisma();
      const notif = await prisma.notificationLog.findFirst({ where: { userId: buyer.id, type: 'refund_approved' } });
      expect(notif).toBeTruthy();
    });
    ...
    scenario('REF-071', async () => {
      ...
      await request(server()).post(`/api/refund-requests/${createRes.body.id}/cancel`).set(authHeader(buyer)).expect(200);
      const prisma = getPrisma();
      const notif = await prisma.notificationLog.findFirst({ where: { userId: seller.id, type: 'refund_cancelled' } });
      expect(notif).toBeTruthy();
    });
    ...
    scenario('REF-072', async () => {
      ...
      await prisma.refundRequest.update({
        ...
        data: { status: RefundRequestStatus.return_delivered, returnDeliveredAt: new Date(Date.now() - 31 * 60 * 1000) },
      });
      await refundService.finalizeRefundForReturnedShipment(createRes.body.id);

      const buyerNotif = await prisma.notificationLog.findFirst({ where: { userId: buyer.id, type: 'refund_completed' } });
      expect(buyerNotif).toBeTruthy();
      const sellerNotif = await prisma.notificationLog.findFirst({ where: { userId: seller.id, type: 'refund_completed_seller' } });
    ...
    scenario('REF-073', async () => {
      ...
      await refundService.applyReturnTrackingUpdate(createRes.body.id, { status: ShipmentStatus.in_transit, shippedAt: new Date() });
      expect(await prisma.notificationLog.findFirst({ where: { userId: buyer.id, type: 'refund_return_in_transit' } })).toBeTruthy();
      expect(await prisma.notificationLog.findFirst({ where: { userId: seller.id, type: 'refund_return_shipped_seller' } })).toBeTruthy();

      await refundService.applyReturnTrackingUpdate(createRes.body.id, { status: ShipmentStatus.delivered, deliveredAt: new Date() });
      expect(await prisma.notificationLog.findFirst({ where: { userId: buyer.id, type: 'refund_return_delivered_buyer' } })).toBeTruthy();
      expect(await prisma.notificationLog.findFirst({ where: { userId: seller.id, type: 'refund_return_delivered_seller' } })).toBeTruthy();
    });
  });
  ...
  describe('Sipariş listesi iade durumu', () => {
    scenario('REF-074', async () => {
      ...
      const listRes = await request(server()).get('/api/orders?role=buyer').set(authHeader(buyer)).expect(200);
      const row = (listRes.body.data as any[]).find((o) => o.id === orderId);
      expect(row).toBeTruthy();
      expect(row.activeRefundRequest?.status).toBe(RefundRequestStatus.return_shipment_open);
    });
    ...
    scenario('REF-075', async () => {
      ...
      const def = await request(server()).get('/api/orders?role=buyer').set(authHeader(buyer)).expect(200);
      expect((def.body.data as any[]).find((o) => o.id === orderId)).toBeFalsy();

      const refunds = await request(server()).get('/api/orders?role=buyer&refundsOnly=true').set(authHeader(buyer)).expect(200);
  ...
  describe('Kısmi iade tutarı & admin override-policy', () => {
    scenario('REF-080', async () => {
      ...
      const prisma = getPrisma();
      await prisma.order.update({ where: { id: orderId }, data: { quantity: 3 } });
      const order = await prisma.order.findUnique({ where: { id: orderId } });
      const expected = Math.round((Number(order!.totalAmount) / 3) * 100) / 100;

      const res = await createRefund(orderId, buyer, { reason: 'changed_mind', refundQuantity: 1 }).expect(201);
    ...
    scenario('REF-081', async () => {
      ...
      const { buyer, orderId } = await paidOrder({ price: 200 });
      const prisma = getPrisma();
      await prisma.order.update({ where: { id: orderId }, data: { quantity: 2 } });
      const order = await prisma.order.findUnique({ where: { id: orderId } });

      const res = await createRefund(orderId, buyer, { reason: 'changed_mind', refundQuantity: 99 }).expect(201);
      expect(res.body.refundQuantity).toBe(2);
      expect(Number(res.body.amount)).toBeCloseTo(Number(order!.totalAmount), 2);
    });
    ...
    scenario('REF-082', async () => {
      ...
      const prisma = getPrisma();
      const order = await prisma.order.findUnique({ where: { id: orderId } });
      const expected = Number(order!.totalAmount) - Number(order!.shippingCost) - Number(order!.buyerFeeAmount);

      await request(server())
        .patch(`/api/admin/refund-requests/${refundId}/override-policy`)
        .set(authHeader(admin))
        .send({ refundProductAmount: true, refundShippingFee: false, refundBuyerFee: false, refundSellerCommission: false })
        .expect(200);
    ...
    scenario('REF-083', async () => {
      ...
      await prisma.order.update({
        where: { id: orderId },
        data: { shippingCost: Number(order!.totalAmount), buyerFeeAmount: Number(order!.totalAmount) },
      });

      await request(server())
        .patch(`/api/admin/refund-requests/${refundId}/override-policy`)
        .set(authHeader(admin))
        .send({ refundProductAmount: true, refundShippingFee: false, refundBuyerFee: false })
        .expect(200);
  ...
  describe('processRefundedOrders idempotency & retry', () => {
    ...
    scenario('REF-092', async () => {
      ...
      const first = await createRefund(orderId, buyer, { reason: 'changed_mind' });
      ...
      const second = await createRefund(orderId, buyer, { reason: 'changed_mind' }).expect(201);
  ...
  describe('Admin iade yönetimi & yetki', () => {
    scenario('REF-100', async () => {
      ...
      const res = await request(server())
        .get('/api/admin/refund-requests?status[]=wait_for_delivery&page=1&limit=20')
        .set(authHeader(admin))
        .expect(200);
    ...
    scenario('REF-101', async () => {
      ...
      const mod = await createAdminUser(ctx.module, { role: AdminRole.moderator });
      const res = await request(server())
    ...
    scenario('REF-102', async () => {
      ...
      await request(server()).get(`/api/admin/refund-requests/${fakeId}`).set(authHeader(mod)).expect(403);
      await request(server()).post(`/api/admin/refund-requests/${fakeId}/force-finalize`).set(authHeader(mod)).expect(403);
      await request(server())
    ...
    scenario('REF-103', async () => {
      ...
      const mod = await createAdminUser(ctx.module, { role: AdminRole.moderator });
      const fakeId = 'a0000000-0000-4000-8000-000000000000';
      await request(server()).post(`/api/admin/trades/${fakeId}/retry-refund`).set(authHeader(mod)).expect(403);
    });
    ...
    scenario('REF-106', async () => {
      ...
      await prisma.refundRequest.update({
        where: { id: refundId },
        data: { status: RefundRequestStatus.return_delivered, returnDeliveredAt: new Date() },
      });
    ...
    scenario('REF-107', async () => {
      ...
      const audit = await prisma.auditLog.findFirst({ where: { action: 'payment_manual_refund' } });
  ...
  describe('Takas cash iptal → iade', () => {
    scenario('REF-110', async () => {
      ...
      const ip = await createProduct({ sellerId: initiator.id, categoryId: baseline.categoryId, isTradeEnabled: true, price: 200, quantity: 1 });
      const rp = await createProduct({ sellerId: receiver.id, categoryId: baseline.categoryId, isTradeEnabled: true, price: 200, quantity: 1 });
      ...
      await request(server()).post(`/api/trades/${tradeId}/accept`).set(authHeader(receiver)).send({}).expect(201);
      expect((await prisma.product.findUnique({ where: { id: ip.id } }))!.reservedQuantity).toBe(1);
      expect((await prisma.product.findUnique({ where: { id: rp.id } }))!.reservedQuantity).toBe(1);

      const cashBefore = await prisma.tradeCashPayment.findUnique({ where: { tradeId } });
      await request(server()).post('/api/payments/initiate-trade-cash').set(authHeader(initiator)).send({ tradeId }).expect(201);
      ...
      await request(server()).post(`/api/trades/${tradeId}/cancel`).set(authHeader(initiator)).send({ reason: 'vazgeçtik' }).expect(201);
      ...
      expect((await prisma.product.findUnique({ where: { id: rp.id } }))!.reservedQuantity).toBe(0);
    });
    ...
    scenario('REF-111', async () => {
      ...
      const res = await request(server())
        .post(`/api/admin/trades/a0000000-0000-4000-8000-000000000000/retry-refund`)
        .set(authHeader(admin));
  ...
  scenario.skip('REF-130', 'Saf web UI (i18n metinleri) — API e2e ile doğrulanamaz.');

  // REF-131: Mobil iade ekranı yalnızca TR — saf mobil UI (apps/mobile), API e2e kapsamı dışı.
  scenario.skip('REF-131', 'Saf mobil UI (yalnız-TR etiketler) — API e2e ile doğrulanamaz.');

  // REF-133: Sebep etiketleri parite (counterfeit/lost_in_transit) — web/mobil reasonLabels UI.
  scenario.skip('REF-133', 'Saf web/mobil UI (reasonLabels eşlemesi) — API e2e ile doğrulanamaz.');
  ...
  scenario.skip('REF-134', 'Saf UI rozet metni (İptal/İade Edildi) — cancellationType API tarafı REF-001/REF-072 kapsıyor.');
});
