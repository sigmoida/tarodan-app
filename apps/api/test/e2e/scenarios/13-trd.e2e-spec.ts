...
describe("13 — Takas (TRD)", () => {
  ...
  scenario("TRD-006", async () => {
    ...
    const res = await post("/api/trades", initiator)
      ...
      .expect(400);
    expect(res.body.message).toContain(
      "Takas için bir teslimat adresi ekleyin",
    );
  });
  ...
  scenario("TRD-007", async () => {
    ...
    const foreignAddress = await createAddress({
      userId: f.receiver.id,
      isDefault: false,
    });
  ...
  scenario("TRD-011", async () => {
    ...
    const res = await post("/api/trades", zeynep)
      ...
      .expect(400);
    expect(res.body.message).toContain(
      "Takas özelliği üyeliğinizde mevcut değil",
    );
  });
  ...
  scenario("TRD-012", async () => {
    ...
    const prisma = getPrisma();
    const basic = await prisma.membershipTier.findFirst({
  ...
  scenario("TRD-023", async () => {
    ...
    const tradeId = await createPendingTrade(f);
    const res = await post(`/api/trades/${tradeId}/accept`, kaan)
      .send({})
      .expect(403);
  ...
  scenario("TRD-025", async () => {
    ...
    await post(`/api/trades/${tradeId}/accept`, f.receiver)
      .send({})
      .expect(400);
    await post(`/api/trades/${tradeId}/reject`, f.receiver)
      .send({ reason: "x" })
      .expect(400);
  ...
  scenario("TRD-027", async () => {
    ...
    await post(`/api/trades/${tradeId}/accept`, f.receiver)
      .send({})
      .expect(201);
    const beforeIP = await prisma.product.findUnique({
      where: { id: f.initiatorProduct.id },
    });
    expect(beforeIP?.reservedQuantity).toBe(1);

    // Kargoya verilmeden iptal → rezervasyon geri verilir.
    await post(`/api/trades/${tradeId}/cancel`, f.receiver)
      .send({ reason: "iade" })
      .expect(201);

    const afterIP = await prisma.product.findUnique({
      where: { id: f.initiatorProduct.id },
    });
    const afterRP = await prisma.product.findUnique({
      where: { id: f.receiverProduct.id },
    });
  ...
  scenario("TRD-029", async () => {
    ...
    const f = await setupBilateral();
    const tradeId = await createPendingTrade(f);
    const res = await post(`/api/trades/${tradeId}/counter`, f.initiator).send({
      cashAmount: 50,
      message: "Counter",
    });
  ...
  scenario("TRD-032", async () => {
    ...
    const res = await post(`/api/trades/${created.body.id}/counter`, receiver)
      ...
      .expect(400);
    expect(res.body.message).toContain(
      "Karşı teklif göndermek için Temel veya üstü üyelik gereklidir",
    );
  });
  ...
  scenario("TRD-039", async () => {
    ...
    const res = await post(`/api/trades/${tradeId}/cancel`, f.initiator)
      ...
      .expect(400);
    expect(res.body.message).toContain(
      "Ürün kargoya verildikten sonra takas iptal edilemez",
    );
  });
  ...
  scenario("TRD-040", async () => {
    ...
    await post(`/api/admin/trades/${tradeId}/force-cancel-stuck`, admin)
      .send({
        reason: "Karşı tarafın kargosu şubede sıkıştı, geleni geri yolluyoruz",
      })
      .expect(200);
  ...
  scenario("TRD-042", async () => {
    ...
    const res = await post(
      `/api/trades/${tradeId}/confirm-receipt`,
      f.initiator,
    )
      .send({})
      .expect(400);
  ...
  scenario("TRD-048", async () => {
    ...
    const f = await setupBilateral();
    const tradeId = await createPendingTrade(f);
    const res = await post(`/api/trades/${tradeId}/dispute`, f.initiator).send({
      reason: "damaged",
      description: "x",
    });
  ...
  scenario("TRD-050", async () => {
    ...
    const res = await post(
      `/api/trades/${tradeId}/resolve-dispute`,
      f.initiator,
  ...
  scenario("TRD-060", async () => {
    ...
    await post(`/api/trades/${t1}/reject`, f.receiver)
      .send({ reason: "r" })
      .expect(201);
    ...
    await post(`/api/trades/${t2}/cancel`, f2.initiator)
      .send({ reason: "c" })
      .expect(201);
    await post(`/api/trades/${t2}/accept`, f2.receiver).send({}).expect(400);
    await post(`/api/trades/${t2}/reject`, f2.receiver)
      .send({ reason: "x" })
      .expect(400);
    ...
    await post(`/api/trades/${t3}/cancel`, f3.initiator)
      .send({ reason: "x" })
      .expect(400);
  });
  ...
  scenario("TRD-061", async () => {
    ...
    await post(`/api/trades/${tc}/reject`, c.receiver)
      .send({ reason: "r" })
      .expect(201);
    ...
    await post(`/api/trades/${td}/cancel`, d.initiator)
      .send({ reason: "c" })
      .expect(201);
  ...
  scenario("TRD-062", async () => {
    ...
    await post(`/api/trades/${ta}/cancel`, a.initiator)
      .send({ reason: "vazgeç" })
      .expect(201);
    expect((await prisma.trade.findUnique({ where: { id: ta } }))?.status).toBe(
      TradeStatus.cancelled,
  ...
  scenario("TRD-066", async () => {
    ...
    const empty = await prisma.tradeShipment.findMany({
      where: { tradeId, leg: "to_warehouse" },
    });
  ...
  scenario("TRD-069", async () => {
    ...
    const mine1 = toWarehouse1.find(
      (s: any) => s.senderUserId === f.initiator.id,
    );
    const theirs1 = toWarehouse1.find(
      (s: any) => s.senderUserId === f.receiver.id,
    );
  ...
  scenario("TRD-070", async () => {
    ...
    expect(
      fromWarehouse.some((s: any) => s.recipientUserId === f.receiver.id),
    ).toBe(false);
  });
  ...
  scenario("TRD-072", async () => {
    ...
    const modRes = await post(
      `/api/admin/trades/${tradeId}/mark-return-lost`,
      moderator,
    ).send({
      shipmentId: "a0000000-0000-4000-8000-000000000000",
      reason: "kayıp oldu kargo",
    });
    expect(modRes.status).toBe(403);

    // super_admin rol guard'ı geçer (iş mantığı hatası 403 DEĞİL)
    const saRes = await post(
      `/api/admin/trades/${tradeId}/mark-return-lost`,
      superAdmin,
    ).send({
      shipmentId: "a0000000-0000-4000-8000-000000000000",
  ...
  scenario("TRD-073", async () => {
    ...
    const modRes = await post(
      `/api/admin/trades/${tradeId}/retry-refund`,
      moderator,
    ).send({});
  ...
  scenario("TRD-074", async () => {
    ...
    await request(server())
      .post(`/api/trades/${uuid}/counter`)
      .send({})
      .expect(401);
    await request(server())
      .post(`/api/trades/${uuid}/dispute`)
      .send({ reason: "x" })
      .expect(401);
  });
  ...
  scenario("TRD-082", async () => {
    ...
    await post(`/api/trades/${t1.body.id}/accept`, receiver)
      .send({})
      .expect(201);
  ...
  scenario("TRD-089", async () => {
    ...
    const noAddr = await request(server())
      ...
      .send(createTradeBody(f))
      .expect(400);
    expect(noAddr.body.message).toContain(
      "Takas için bir teslimat adresi ekleyin",
    );
  });
  ...
  scenario("TRD-095", async () => {
    ...
    await post(`/api/trades/${tradeId}/reject`, kaan)
      .send({ reason: "x" })
      .expect(403);
    await post(`/api/trades/${tradeId}/cancel`, kaan)
      .send({ reason: "x" })
      .expect(403);
    // confirm/dispute: yetki gate iş mantığından önce → 403
    await post(`/api/trades/${tradeId}/confirm-receipt`, kaan)
      .send({})
      .expect(403);
    ...
    const shipRes = await post(`/api/trades/${tradeId}/ship`, kaan).send({
      fromAddressId: shipAddr.id,
    });
  ...
  scenario("TRD-099", async () => {
    ...
    await post(`/api/trades/${tCancel}/cancel`, f.initiator)
      .send({ reason: "c" })
      .expect(201);
    await post(`/api/trades/${tCancel}/accept`, f.receiver)
      .send({})
      .expect(400);
    await post(`/api/trades/${tCancel}/confirm-receipt`, f.initiator)
      .send({})
      .expect(400);
    ...
    expect(
      (await prisma.trade.findUnique({ where: { id: tDone } }))?.status,
    ).toBe(TradeStatus.completed);
  });
});
