...
describe('17 — İndirim, Kupon, Reklam, Bülten & Boost (DSC)', () => {
  ...
  const createDiscount = (user: { accessToken: string }, body: Record<string, unknown>) =>
    request(server()).post('/api/discounts').set(authHeader(user)).send(body);

  const validateCoupon = (user: { accessToken: string }, body: Record<string, unknown>) =>
    request(server()).post('/api/discounts/validate').set(authHeader(user)).send(body);
  ...
  async function completeBoostPayment(boostOrderId: string) {
    ...
    if (!payment?.providerConversationId) {
      throw new Error(`boost order ${boostOrderId} için payment/merchantOid yok`);
    }
  ...
  scenario('DSC-001', async () => {
    ...
    const res = await createDiscount(
      seller,
      validDiscount({ code: 'YAZ10', name: 'Yaz İndirimi', type: 'percentage', value: 10, scope: 'seller' }),
    ).expect(201);
  ...
  scenario('DSC-002', async () => {
    ...
    const res = await createDiscount(
      seller,
      validDiscount({ name: '50 TL indirim', code: 'INDIRIM50', type: 'fixed_amount', value: 50, scope: 'seller' }),
    ).expect(201);
  ...
  scenario('DSC-003', async () => {
    ...
    const res = await createDiscount(
      seller,
      validDiscount({ name: 'Seçili ürünler', type: 'percentage', value: 15, scope: 'product', targetProductIds: [p1.id, p2.id] }),
    ).expect(201);
  ...
  scenario('DSC-004', async () => {
    ...
    const res = await createDiscount(
      seller,
      validDiscount({ name: 'Global', type: 'percentage', value: 20, scope: 'global' }),
    ).expect(403);
    expect(String(res.body.message)).toContain('Satıcılar global indirim oluşturamazlar');
    const prisma = getPrisma();
  ...
  scenario('DSC-005', async () => {
    ...
    const res = await createDiscount(
      seller,
      validDiscount({ name: 'Kategori', type: 'percentage', value: 10, scope: 'category', categoryId: baseline.categoryId }),
    ).expect(403);
    expect(String(res.body.message)).toContain('Satıcılar kategori indirimi oluşturamazlar');
  });
  ...
  scenario('DSC-006', async () => {
    ...
    const res = await createDiscount(
      seller,
      validDiscount({ name: 'Boş hedef', type: 'percentage', value: 5, scope: 'product', targetProductIds: [] }),
    ).expect(400);
  ...
  scenario('DSC-007', async () => {
    ...
    const res = await createDiscount(
      seller,
      validDiscount({ name: 'Çapraz', type: 'percentage', value: 10, scope: 'product', targetProductIds: [otherProduct.id] }),
    ).expect(400);
    expect(String(res.body.message)).toContain('Sadece kendi ürünleriniz için indirim oluşturabilirsiniz');
  });
  ...
  scenario('DSC-008', async () => {
    ...
    const res = await createDiscount(
      seller,
      validDiscount({ name: 'Mağaza', type: 'percentage', value: 8, scope: 'seller', targetProductIds: [p.id] }),
    ).expect(201);
    expect(res.body.targetProductIds).toEqual([]);
  });
  ...
  scenario('DSC-009', async () => {
    ...
    const seller = await createUser(ctx.module, { isSeller: true });
    await createDiscount(seller, validDiscount({ code: 'TEKKOD', scope: 'seller' })).expect(201);
    const res = await createDiscount(
      seller,
      validDiscount({ code: 'TEKKOD', name: 'İkinci', scope: 'seller' }),
    ).expect(400);
    expect(String(res.body.message)).toContain('Bu kupon kodu zaten kullanılıyor');
  });

  scenario('DSC-010', async () => {
    // Küçük harf kod → büyük harfe çevrilir.
    const seller = await createUser(ctx.module, { isSeller: true });
    const res = await createDiscount(seller, validDiscount({ code: 'kucuk10', scope: 'seller' })).expect(201);
    expect(res.body.code).toBe('KUCUK10');
    const prisma = getPrisma();
    const row = await prisma.discount.findUnique({ where: { code: 'KUCUK10' } });
    expect(row).toBeTruthy();
  });
  ...
  scenario('DSC-011', async () => {
    ...
    const seller = await createUser(ctx.module, { isSeller: true });
    await createDiscount(seller, validDiscount({ name: 'Bad', type: 'percentage', value: -10, scope: 'seller' })).expect(400);
  });

  scenario('DSC-012', async () => {
    // Geçersiz type enum → 400.
    const seller = await createUser(ctx.module, { isSeller: true });
    await createDiscount(seller, validDiscount({ type: 'totally-fake-type', scope: 'seller' })).expect(400);
  });

  scenario('DSC-013', async () => {
    // Kimliksiz oluşturma → 401.
    await request(server()).post('/api/discounts').send(validDiscount({ scope: 'seller' })).expect(401);
  });
  ...
  scenario('DSC-014', async () => {
    ...
    const res = await createDiscount(
      seller,
      validDiscount({ name: 'BOGO', type: 'bogo', value: 1, scope: 'seller', buyQuantity: 2, getQuantity: 1 }),
    ).expect(201);
  ...
  scenario('DSC-016', async () => {
    ...
    const admin = await createAdminUser(ctx.module, { email: 'admin-dsc16@test.com', role: AdminRole.super_admin });
    const res = await request(server())
      .post('/api/admin/discounts')
      .set(authHeader(admin))
      .send(validDiscount({ name: 'Genel %10', code: 'GLOBAL10', type: 'percentage', value: 10, scope: 'global' }))
      .expect(201);
  ...
  scenario('DSC-017', async () => {
    ...
    const mod = await createAdminUser(ctx.module, { email: 'mod-dsc17@test.com', role: AdminRole.moderator });
    const res = await request(server())
      .post('/api/admin/discounts')
      .set(authHeader(mod))
      .send(validDiscount({ name: 'Mod Kampanya', code: 'MODCAMP', type: 'percentage', value: 5, scope: 'global' }))
      .expect(201);
    expect(res.body.scope).toBe('global');
  });
  ...
  scenario('DSC-018', async () => {
    ...
    const res = await request(server())
      .post('/api/admin/discounts')
      .set(authHeader(user))
      .send(validDiscount({ name: 'Hack', type: 'percentage', value: 100, scope: 'global' }));
  ...
  scenario('DSC-019', async () => {
    ...
    const seller = await createUser(ctx.module, { isSeller: true });
    const res = await createDiscount(seller, validDiscount({ scope: 'global' })).expect(403);
    expect(String(res.body.message)).toContain('Satıcılar global indirim oluşturamazlar');
  });

  scenario('DSC-020', async () => {
    // Admin indirim listesi → 200, sayfalama yapısı.
    const admin = await createAdminUser(ctx.module, { email: 'admin-dsc20@test.com' });
    const res = await request(server()).get('/api/admin/discounts').set(authHeader(admin)).expect(200);
  ...
  scenario('DSC-021', async () => {
    ...
    const admin = await createAdminUser(ctx.module, { email: 'adm-dsc21@test.com', role: AdminRole.admin });
    const mod = await createAdminUser(ctx.module, { email: 'mod-dsc21@test.com', role: AdminRole.moderator });
    const superAdmin = await createAdminUser(ctx.module, { email: 'super-dsc21@test.com', role: AdminRole.super_admin });
    const ruleBody = { name: 'Standart', sellerType: 'ALL', appliesTo: 'SELLER', sellerRate: 5 };

    const r1 = await request(server()).post('/api/admin/commission-rules').set(authHeader(admin)).send(ruleBody);
    expect([401, 403]).toContain(r1.status);
    const r2 = await request(server()).post('/api/admin/commission-rules').set(authHeader(mod)).send(ruleBody);
    expect([401, 403]).toContain(r2.status);
    const r3 = await request(server()).post('/api/admin/commission-rules').set(authHeader(superAdmin)).send(ruleBody);
  ...
  scenario('DSC-022', async () => {
    ...
    const user = await createUser(ctx.module);
    const res = await validateCoupon(user, { code: 'YOK-BOYLE-KOD' }).expect(200);
  ...
  scenario('DSC-023', async () => {
    ...
    const user = await createUser(ctx.module);
    await seedDiscount({ code: 'TESTVAL', scope: 'global', type: 'percentage', value: 10 });
  ...
  scenario('DSC-028', async () => {
    ...
    const user = await createUser(ctx.module);
    const discount = await seedDiscount({ code: 'TEKKULLANIM', usageLimitPerUser: 1 });
    const prisma = getPrisma();
    await prisma.discountUsage.create({
      data: { discountId: discount.id, userId: user.id, amount: new Prisma.Decimal(5) },
    });
  ...
  scenario('DSC-029', async () => {
    ...
    await seedDiscount({ code: 'MIN100', scope: 'global', type: 'percentage', value: 10, minCartValue: new Prisma.Decimal(100) });
  ...
  scenario('DSC-030', async () => {
    ...
    await request(server()).post('/api/discounts/validate').send({ code: 'X' }).expect(401);
  });
  ...
  scenario('DSC-031', async () => {
    ...
    await seedDiscount({ code: 'YAZ10', scope: 'global', type: 'percentage', value: 10 });
    const res = await validateCoupon(user, {
      code: 'YAZ10',
  ...
  scenario('DSC-033', async () => {
    ...
    await request(server())
      ...
      .expect(201);
    await seedDiscount({ code: 'YARIM', scope: 'global', type: 'percentage', value: 100 });
    const res = await request(server())
  ...
  scenario('DSC-034', async () => {
    ...
    await request(server())
      ...
      .send({ productId: product.id, quantity: 1 })
      .expect(201);
    await seedDiscount({ code: 'BUYUKFIX', scope: 'global', type: 'fixed_amount', value: 500 });
    const res = await request(server())
      .post('/api/cart/coupon')
      .set(authHeader(buyer))
      .send({ code: 'BUYUKFIX' })
  ...
  scenario('DSC-035', async () => {
    ...
    await seedDiscount({ code: 'SELLERONLY', scope: 'seller', sellerId: otherSeller.id, type: 'percentage', value: 10 });
    const res = await request(server())
      .post('/api/cart/coupon')
      .set(authHeader(buyer))
    ...
    if (res.status === 200) {
      ...
      expect(
        res.body.calculation.warnings.some((w: string) => w.includes('uygulanamaz')),
      ).toBe(true);
    } else {
  ...
  scenario('DSC-036', async () => {
    ...
    await request(server())
      ...
      .expect(201);
    const cart1 = await request(server()).get('/api/cart').set(authHeader(buyer1)).expect(200);
    ...
    await request(server())
      ...
      .expect(201);
    const cart2 = await request(server()).get('/api/cart').set(authHeader(buyer2)).expect(200);
  ...
  scenario('DSC-037', async () => {
    ...
    const buyer = await createUser(ctx.module);
    const seller = await createUser(ctx.module, { isSeller: true });
    const product = await makeProduct({ sellerId: seller.id, price: 200, quantity: 5 });
    ...
    const usages = await prisma.discountUsage.findMany({ where: { discountId: discount.id, userId: buyer.id } });
    expect(usages).toHaveLength(1);
    expect(usages[0].orderId).toBe(order!.id);
    const refreshed = await prisma.discount.findUnique({ where: { id: discount.id } });
  ...
  scenario('DSC-038', async () => {
    ...
    const buyer = await createUser(ctx.module);
    const seller = await createUser(ctx.module, { isSeller: true });
    const p1 = await makeProduct({ sellerId: seller.id, price: 200, quantity: 5 });
    const p2 = await makeProduct({ sellerId: seller.id, price: 200, quantity: 5 });
    const address = await createAddress({ userId: buyer.id });
    await seedDiscount({ code: 'YAZ10', scope: 'global', type: 'percentage', value: 10, usageLimitPerUser: 1 });

    await request(server())
      .post('/api/orders/checkout')
      .set(authHeader(buyer))
      .send({ items: [{ productId: p1.id }], idempotencyKey: uuid(), shippingAddressId: address.id, couponCode: 'YAZ10' })
      .expect(201);

    const res = await request(server())
      .post('/api/orders/checkout')
      .set(authHeader(buyer))
      .send({ items: [{ productId: p2.id }], idempotencyKey: uuid(), shippingAddressId: address.id, couponCode: 'YAZ10' })
      .expect(400);
    expect(String(res.body.message)).toContain('Bu kuponu zaten kullandınız');
  });

  scenario('DSC-040', async () => {
    // Misafir checkout'ta kupon reddedilir → 400.
    const seller = await createUser(ctx.module, { isSeller: true });
  ...
  scenario('DSC-041', async () => {
    ...
    const p1 = await makeProduct({ sellerId: seller.id, price: 200, quantity: 5 });
    const p2 = await makeProduct({ sellerId: seller.id, price: 200, quantity: 5 });
    const p3 = await makeProduct({ sellerId: seller.id, price: 200, quantity: 5 });
    const address = await createAddress({ userId: buyer.id });
    await seedDiscount({ code: 'IKIKEZ', scope: 'global', type: 'percentage', value: 10, usageLimitPerUser: 2 });

    await request(server())
      .post('/api/orders/checkout')
      .set(authHeader(buyer))
      .send({ items: [{ productId: p1.id }], idempotencyKey: uuid(), shippingAddressId: address.id, couponCode: 'IKIKEZ' })
      .expect(201);
    await request(server())
      .post('/api/orders/checkout')
      .set(authHeader(buyer))
      .send({ items: [{ productId: p2.id }], idempotencyKey: uuid(), shippingAddressId: address.id, couponCode: 'IKIKEZ' })
      .expect(201);
    const res = await request(server())
      .post('/api/orders/checkout')
      .set(authHeader(buyer))
      .send({ items: [{ productId: p3.id }], idempotencyKey: uuid(), shippingAddressId: address.id, couponCode: 'IKIKEZ' })
      .expect(400);
    expect(String(res.body.message)).toContain('Bu kuponu zaten kullandınız');
  });
  ...
  scenario('DSC-042', async () => {
    ...
    await createDiscount(ahmet, validDiscount({ code: 'A-CODE', scope: 'seller' })).expect(201);
    await createDiscount(ali, validDiscount({ code: 'ALI-CODE', scope: 'seller' })).expect(201);

    const ahmetList = await request(server()).get('/api/discounts').set(authHeader(ahmet)).expect(200);
    const aliList = await request(server()).get('/api/discounts').set(authHeader(ali)).expect(200);
  ...
  scenario('DSC-044', async () => {
    ...
    const seller = await createUser(ctx.module, { isSeller: true });
    const created = await createDiscount(seller, validDiscount({ code: 'UPD', scope: 'seller', value: 10 })).expect(201);
    const res = await request(server())
  ...
  scenario('DSC-045', async () => {
    ...
    const ahmet = await createUser(ctx.module, { isSeller: true });
    const ali = await createUser(ctx.module, { isSeller: true });
    const created = await createDiscount(ahmet, validDiscount({ code: 'OWNED', scope: 'seller', name: 'Orijinal' })).expect(201);
    await request(server())
      .patch(`/api/discounts/${created.body.id}`)
      .set(authHeader(ali))
      .send({ name: 'Hacked' })
      .expect(403);
    const prisma = getPrisma();
    const row = await prisma.discount.findUnique({ where: { id: created.body.id } });
  ...
  scenario('DSC-046', async () => {
    ...
    const seller = await createUser(ctx.module, { isSeller: true });
    const created = await createDiscount(seller, validDiscount({ code: 'DEL', scope: 'seller' })).expect(201);
    await request(server()).delete(`/api/discounts/${created.body.id}`).set(authHeader(seller)).expect(204);
    const prisma = getPrisma();
    expect(await prisma.discount.findUnique({ where: { id: created.body.id } })).toBeNull();
  });
  ...
  scenario('DSC-047', async () => {
    ...
    await request(server()).delete(`/api/discounts/${ABSENT_UUID}`).set(authHeader(seller)).expect(404);
  });
  ...
  scenario('DSC-048', async () => {
    ...
    await prisma.discount.create({
      data: {
        name: 'Kuponlu', code: 'CPN1', type: 'percentage' as any, value: new Prisma.Decimal(10),
        scope: 'seller' as any, sellerId: seller.id, isActive: true,
        startDate: new Date(now - 3_600_000), endDate: new Date(now + 30 * 24 * 3_600_000),
      },
    });
    // Kodsuz (otomatik kampanya)
    await prisma.discount.create({
      data: {
        name: 'Otomatik', code: null, type: 'percentage' as any, value: new Prisma.Decimal(5),
        scope: 'seller' as any, sellerId: seller.id, isActive: true,
        startDate: new Date(now - 3_600_000), endDate: new Date(now + 30 * 24 * 3_600_000),
      },
    });
    ...
    await prisma.discount.create({
      data: {
        name: 'Pasif', code: 'PSF1', type: 'percentage' as any, value: new Prisma.Decimal(7),
        scope: 'seller' as any, sellerId: seller.id, isActive: false,
        startDate: new Date(now - 3_600_000), endDate: new Date(now + 30 * 24 * 3_600_000),
      },
    });

    const couponsOnly = await request(server()).get('/api/discounts?couponsOnly=true').set(authHeader(seller)).expect(200);
    expect(couponsOnly.body.items.every((d: any) => d.code != null)).toBe(true);

    const autoOnly = await request(server()).get('/api/discounts?autoOnly=true').set(authHeader(seller)).expect(200);
    expect(autoOnly.body.items.every((d: any) => d.code == null)).toBe(true);
    expect(autoOnly.body.items.length).toBeGreaterThanOrEqual(1);

    const inactive = await request(server()).get('/api/discounts?isActive=false').set(authHeader(seller)).expect(200);
    expect(inactive.body.items.every((d: any) => d.isActive === false)).toBe(true);
  ...
  scenario('DSC-103', async () => {
    ...
    const ahmet = await createUser(ctx.module, { isSeller: true });
    const ali = await createUser(ctx.module, { isSeller: true });
    const created = await createDiscount(ahmet, validDiscount({ code: 'IDOR', scope: 'seller' })).expect(201);
    const res = await request(server()).get(`/api/discounts/${created.body.id}`).set(authHeader(ali)).expect(403);
    expect(String(res.body.message)).toContain('Bu indirimi görüntüleme yetkiniz yok');
  });
  ...
  scenario('DSC-049', async () => {
    // Kampanya yok → [].
    const res = await request(server()).get('/api/discounts/active').expect(200);
  ...
  scenario('DSC-050', async () => {
    ...
    await prisma.discount.create({
      data: {
        name: 'Genel Kampanya', code: null, type: 'percentage' as any, value: new Prisma.Decimal(10),
        scope: 'global' as any, sellerId: null, isActive: true,
        startDate: new Date(now - 3_600_000), endDate: new Date(now + 30 * 24 * 3_600_000),
      },
    });
    ...
    await prisma.discount.create({
      data: {
        name: 'Satıcı Kampanya', code: null, type: 'percentage' as any, value: new Prisma.Decimal(5),
        scope: 'seller' as any, sellerId: seller.id, isActive: true,
        startDate: new Date(now - 3_600_000), endDate: new Date(now + 30 * 24 * 3_600_000),
      },
    });
    ...
    await prisma.discount.create({
      data: {
        name: 'Kuponlu Global', code: 'KUPGLOBAL', type: 'percentage' as any, value: new Prisma.Decimal(20),
        scope: 'global' as any, sellerId: null, isActive: true,
        startDate: new Date(now - 3_600_000), endDate: new Date(now + 30 * 24 * 3_600_000),
      },
    });
  ...
  scenario('DSC-051', async () => {
    ...
    await prisma.discount.create({
      data: {
        name: 'Süresi Geçmiş', code: null, type: 'percentage' as any, value: new Prisma.Decimal(10),
        scope: 'global' as any, sellerId: null, isActive: true,
        startDate: new Date(now - 30 * 24 * 3_600_000), endDate: new Date(now - 24 * 3_600_000),
      },
    });
    const res = await request(server()).get('/api/discounts/active').expect(200);
  ...
  scenario('DSC-052', async () => {
    ...
    await seedAd({ title: 'Aktif Reklam', position: 'header', deviceType: 'all', width: 728, height: 90 });
  ...
  scenario('DSC-053', async () => {
    ...
    const res = await request(server()).get('/api/ads/active?position=header').expect(200);
  ...
  scenario('DSC-054', async () => {
    ...
    const res = await request(server()).get('/api/ads/active?device=mobile').expect(200);
    const titles = res.body.map((a: any) => a.title);
  ...
  scenario('DSC-055', async () => {
    ...
    const res = await request(server()).get('/api/ads/active?device=all').expect(200);
    const titles = res.body.map((a: any) => a.title);
    expect(titles).toEqual(expect.arrayContaining(['D-mobile', 'D-desktop', 'D-all']));
  });
  ...
  scenario('DSC-058', async () => {
    ...
    const prisma = getPrisma();
    const refreshed = await (prisma as any).advertisement.findUnique({ where: { id: ad.id } });
  ...
  scenario('DSC-060', async () => {
    ...
    const click = await request(server()).post(`/api/ads/${ABSENT_UUID}/click`).expect(404);
    expect(String(click.body.message)).toContain('Reklam bulunamadı');
    await request(server()).post(`/api/ads/${ABSENT_UUID}/impression`).expect(404);
  });
  ...
  scenario('DSC-061', async () => {
    ...
    expect(leaderboard).toEqual({ name: 'Leaderboard', width: 728, height: 90 });
  });
  ...
  scenario('DSC-062', async () => {
    ...
    const admin = await createAdminUser(ctx.module, { email: 'admin-dsc62@test.com' });
    const res = await request(server()).post('/api/admin/ads').set(authHeader(admin)).send(iabAd()).expect(201);
  ...
  scenario('DSC-063', async () => {
    ...
    const admin = await createAdminUser(ctx.module, { email: 'admin-dsc63@test.com' });
    const res = await request(server())
      .post('/api/admin/ads')
      .set(authHeader(admin))
  ...
  scenario('DSC-064', async () => {
    ...
    const admin = await createAdminUser(ctx.module, { email: 'admin-dsc64@test.com' });
    await request(server()).post('/api/admin/ads').set(authHeader(admin)).send(iabAd({ width: 2500 })).expect(400);
  });

  scenario('DSC-065', async () => {
    // Sıradan kullanıcı admin reklam oluşturamaz → 401/403.
    const user = await createUser(ctx.module);
    const res = await request(server()).post('/api/admin/ads').set(authHeader(user)).send(iabAd());
    expect([401, 403]).toContain(res.status);
  });

  scenario('DSC-066', async () => {
    // Moderator reklam yönetebilir (list/create/patch/delete hepsi 2xx).
    const mod = await createAdminUser(ctx.module, { email: 'mod-dsc66@test.com', role: AdminRole.moderator });
    await request(server()).get('/api/admin/ads').set(authHeader(mod)).expect(200);
    const created = await request(server()).post('/api/admin/ads').set(authHeader(mod)).send(iabAd({ title: 'Mod Ad' })).expect(201);
    await request(server())
      .patch(`/api/admin/ads/${created.body.id}`)
      .set(authHeader(mod))
      .send({ isActive: false })
      .expect(200);
    await request(server()).delete(`/api/admin/ads/${created.body.id}`).set(authHeader(mod)).expect(204);
  });

  scenario('DSC-067', async () => {
    // Reklam güncelleme (admin) → 200, isActive=false, displayOrder=5.
    const admin = await createAdminUser(ctx.module, { email: 'admin-dsc67@test.com' });
    const created = await request(server()).post('/api/admin/ads').set(authHeader(admin)).send(iabAd()).expect(201);
    const res = await request(server())
      .patch(`/api/admin/ads/${created.body.id}`)
      .set(authHeader(admin))
  ...
  scenario('DSC-068', async () => {
    // Reklam silme → 204.
    const admin = await createAdminUser(ctx.module, { email: 'admin-dsc68@test.com' });
    const created = await request(server()).post('/api/admin/ads').set(authHeader(admin)).send(iabAd()).expect(201);
    await request(server()).delete(`/api/admin/ads/${created.body.id}`).set(authHeader(admin)).expect(204);
    const prisma = getPrisma();
    expect(await (prisma as any).advertisement.findUnique({ where: { id: created.body.id } })).toBeNull();
  });

  scenario('DSC-069', async () => {
    // Olmayan reklam güncelle/sil → 404.
    const admin = await createAdminUser(ctx.module, { email: 'admin-dsc69@test.com' });
    await request(server()).patch(`/api/admin/ads/${ABSENT_UUID}`).set(authHeader(admin)).send({ isActive: false }).expect(404);
    await request(server()).delete(`/api/admin/ads/${ABSENT_UUID}`).set(authHeader(admin)).expect(404);
  });

  scenario('DSC-070', async () => {
    // Reorder → displayOrder atar; idempotent.
    const admin = await createAdminUser(ctx.module, { email: 'admin-dsc70@test.com' });
    ...
    const res = await request(server()).patch('/api/admin/ads/reorder').set(authHeader(admin)).send(body).expect(200);
    expect(Array.isArray(res.body)).toBe(true);
    const prisma = getPrisma();
    const r1 = await (prisma as any).advertisement.findUnique({ where: { id: a1.id } });
    const r2 = await (prisma as any).advertisement.findUnique({ where: { id: a2.id } });
    const r3 = await (prisma as any).advertisement.findUnique({ where: { id: a3.id } });
    ...
    await request(server()).patch('/api/admin/ads/reorder').set(authHeader(admin)).send(body).expect(200);
    const r3b = await (prisma as any).advertisement.findUnique({ where: { id: a3.id } });
  ...
  scenario('DSC-071', async () => {
    ...
    const admin = await createAdminUser(ctx.module, { email: 'admin-dsc71@test.com' });
    await seedAd({ title: 'Stat-A', clickCount: 10, impressionCount: 100, position: 'header', deviceType: 'all' });
    const res = await request(server()).get('/api/admin/ads/statistics').set(authHeader(admin)).expect(200);
  ...
  scenario('DSC-072', async () => {
    ...
    const prisma = getPrisma();
    const row = await prisma.newsletterSubscriber.findUnique({ where: { email: 'yeni@test.com' } });
    expect(row).toBeTruthy();
  ...
  scenario('DSC-073', async () => {
    ...
    await request(server()).post('/api/newsletter/subscribe').send({ email: 'iki@test.com' }).expect(200);
    const res = await request(server())
      .post('/api/newsletter/subscribe')
    ...
    const prisma = getPrisma();
    const rows = await prisma.newsletterSubscriber.findMany({ where: { email: 'iki@test.com' } });
  ...
  scenario('DSC-074', async () => {
    ...
    await prisma.newsletterSubscriber.create({
      data: { email: 'geri@test.com', newsletter: true, promotions: true, unsubscribeToken: 'eski-token', unsubscribedAt: new Date() },
    });
    const res = await request(server()).post('/api/newsletter/subscribe').send({ email: 'geri@test.com' }).expect(200);
    expect(res.body.message).toContain('yeniden aktif');
    const row = await prisma.newsletterSubscriber.findUnique({ where: { email: 'geri@test.com' } });
  ...
  scenario('DSC-075', async () => {
    ...
    await request(server()).post('/api/newsletter/subscribe').send({ email: 'not-an-email' }).expect(400);
  });
  ...
  scenario('DSC-076', async () => {
    ...
    const prisma = getPrisma();
    await prisma.newsletterSubscriber.create({
      data: { email: 'gettoken@test.com', newsletter: true, promotions: true, unsubscribeToken: 'get-token-123' },
    });
    const res = await request(server()).get('/api/newsletter/unsubscribe?token=get-token-123').expect(200);
    expect(res.body.message).toContain('iptal edildi');
    const row = await prisma.newsletterSubscriber.findUnique({ where: { email: 'gettoken@test.com' } });
    expect(row?.unsubscribedAt).toBeTruthy();
  });

  scenario('DSC-077', async () => {
    // Geçersiz token → 404.
    const res = await request(server()).get('/api/newsletter/unsubscribe?token=GECERSIZ').expect(404);
    expect(String(res.body.message)).toContain('Geçersiz veya kullanılmış');
  });

  scenario('DSC-078', async () => {
    // Boş token → 200, { success:false }.
    const res = await request(server()).get('/api/newsletter/unsubscribe').expect(200);
  ...
  scenario('DSC-079', async () => {
    ...
    const prisma = getPrisma();
    await prisma.newsletterSubscriber.create({
      data: { email: 'epcik@test.com', newsletter: true, promotions: true, unsubscribeToken: 'ep-token-1' },
    });
    const res = await request(server()).post('/api/newsletter/unsubscribe').send({ email: 'epcik@test.com' }).expect(200);
    expect(res.body.message).toContain('iptal edildi');
    const row = await prisma.newsletterSubscriber.findUnique({ where: { email: 'epcik@test.com' } });
    expect(row?.unsubscribedAt).toBeTruthy();
  });
  ...
  scenario('DSC-081', async () => {
    ...
    await prisma.newsletterSubscriber.create({
      data: { email: 'zatencik@test.com', newsletter: true, promotions: true, unsubscribeToken: 'zc-token', unsubscribedAt: past },
    });
    const res = await request(server()).post('/api/newsletter/unsubscribe').send({ email: 'zatencik@test.com' }).expect(200);
    expect(res.body.message).toContain('zaten abonelikten çıkarılmış');
    const row = await prisma.newsletterSubscriber.findUnique({ where: { email: 'zatencik@test.com' } });
  ...
  scenario('DSC-105', async () => {
    ...
    await request(server()).get('/api/newsletter/unsubscribe?token=INVALIDTOKEN').expect(404);
  });
  ...
  scenario('DSC-082', async () => {
    ...
    const byDuration = new Map(res.body.options.map((o: any) => [o.durationDays, o.price]));
  ...
  scenario('DSC-083', async () => {
    ...
    const seller = await createUser(ctx.module, { isSeller: true });
    const product = await makeProduct({ sellerId: seller.id, status: 'active' });
    const res = await request(server())
    ...
    const boostOrder = await prisma.order.findUnique({ where: { id: boost!.orderId! } });
  ...
  scenario('DSC-085', async () => {
    ...
    const res = await request(server())
      ...
      .send({ durationDays: 3 })
      .expect(403);
    expect(String(res.body.message)).toContain('Sadece kendi ilanınızı öne çıkarabilirsiniz');
  });
  ...
  scenario('DSC-086', async () => {
    ...
    await ensurePlatformSeller();
    const seller = await createUser(ctx.module, { isSeller: true });
    const product = await makeProduct({ sellerId: seller.id, status: 'inactive' });
    const res = await request(server())
      .post(`/api/products/${product.id}/boost/initiate`)
      .set(authHeader(seller))
      .send({ durationDays: 7 })
      .expect(400);
    expect(String(res.body.message)).toContain('Sadece aktif (yayında) ilanlar öne çıkarılabilir');
  });
  ...
  scenario('DSC-087', async () => {
    ...
    const seller = await createUser(ctx.module, { isSeller: true });
    const product = await makeProduct({ sellerId: seller.id, status: 'active' });
    await request(server()).post(`/api/products/${product.id}/boost/initiate`).send({ durationDays: 7 }).expect(401);
  });
  ...
  scenario('DSC-089', async () => {
    ...
    const prisma = getPrisma();
    const boost = await prisma.productBoost.findUnique({ where: { id: init.body.boostId } });
    await completeBoostPayment(boost!.orderId!);

    const refreshedBoost = await prisma.productBoost.findUnique({ where: { id: boost!.id } });
    ...
    expect(refreshedProduct?.boostedUntil?.getTime()).toBe(refreshedBoost?.endsAt?.getTime());
    const boostOrder = await prisma.order.findUnique({ where: { id: boost!.orderId! } });
  ...
  scenario('DSC-090', async () => {
    ...
    const init1 = await request(server())
      ...
      .send({ durationDays: 7 })
      .expect(201);
    const boost1 = await prisma.productBoost.findUnique({ where: { id: init1.body.boostId } });
    ...
    const init2 = await request(server())
      ...
      .expect(201);
    const boost2 = await prisma.productBoost.findUnique({ where: { id: init2.body.boostId } });
    ...
    expect(Math.abs(afterSecond!.boostedUntil!.getTime() - expected)).toBeLessThan(5000);
  ...
  scenario('DSC-093', async () => {
    ...
    await request(server())
      ...
      .send({ durationDays: 7 })
      .expect(201);
    const res = await request(server()).get('/api/products/boost/my').set(authHeader(seller)).expect(200);
    expect(Array.isArray(res.body)).toBe(true);
  ...
  scenario('DSC-094', async () => {
    ...
    const seller = await createUser(ctx.module, { isSeller: true });
    const res = await createDiscount(seller, validDiscount({ code: 'DEC', scope: 'seller', value: 10.999 })).expect(201);
  ...
  scenario('DSC-095', async () => {
    ...
    await createDiscount(seller, validDiscount({ code: 'SIFIR', scope: 'seller', type: 'percentage', value: 0 })).expect(201);
  ...
  scenario('DSC-096', async () => {
    ...
    await request(server())
      ...
      .send({ productId: product.id, quantity: 1 })
      .expect(201);
    await seedDiscount({ code: 'IKIYUZ', scope: 'global', type: 'percentage', value: 200 });
    const res = await request(server())
      .post('/api/cart/coupon')
      .set(authHeader(buyer))
      .send({ code: 'IKIYUZ' })
  ...
  scenario('DSC-097', async () => {
    ...
    const buyer = await createUser(ctx.module);
    const seller = await createUser(ctx.module, { isSeller: true });
    const product = await makeProduct({ sellerId: seller.id, price: 100, quantity: 5 });
    const address = await createAddress({ userId: buyer.id });
    const prisma = getPrisma();
    ...
    await seedDiscount({ code: 'YIRMI', scope: 'global', type: 'percentage', value: 20, usageLimitPerUser: null });
    const res = await request(server())
      .post('/api/orders/checkout')
      .set(authHeader(buyer))
      .send({ items: [{ productId: product.id }], idempotencyKey: uuid(), shippingAddressId: address.id, couponCode: 'YIRMI' })
      .expect(201);
    const order = await prisma.order.findFirst({ where: { checkoutGroupId: res.body.checkoutGroupId } });
  ...
  scenario('DSC-039', async () => {
    ...
    const [r1, r2] = await Promise.all([
      request(server())
        .post('/api/orders/checkout')
        .set(authHeader(buyer1))
        .send({ items: [{ productId: p1.id }], idempotencyKey: uuid(), shippingAddressId: addr1.id, couponCode: 'YARIS' }),
      request(server())
        .post('/api/orders/checkout')
        .set(authHeader(buyer2))
        .send({ items: [{ productId: p2.id }], idempotencyKey: uuid(), shippingAddressId: addr2.id, couponCode: 'YARIS' }),
    ]);
    ...
    const prisma = getPrisma();
    const usages = await prisma.discountUsage.findMany({ where: { discountId: discount.id } });
  ...
  scenario('DSC-101', async () => {
    const admin = await createAdminUser(ctx.module, { email: 'admin-dsc101@test.com', role: AdminRole.super_admin });
    // Oluşturma formu alanları: scope/type/value/startDate/endDate → 201.
    const created = await request(server())
      .post('/api/admin/discounts')
      .set(authHeader(admin))
      .send(validDiscount({ name: 'Panel Kampanya', code: 'PANEL10', type: 'percentage', value: 10, scope: 'global' }))
      .expect(201);
    ...
    const list = await request(server()).get('/api/admin/discounts').set(authHeader(admin)).expect(200);
  ...
  scenario('DSC-102', async () => {
    const admin = await createAdminUser(ctx.module, { email: 'admin-dsc102@test.com', role: AdminRole.super_admin });
    await seedAd({ title: 'Panel Reklam', position: 'header', deviceType: 'all', width: 728, height: 90 });
    const list = await request(server()).get('/api/admin/ads').set(authHeader(admin)).expect(200);
    ...
    const stats = await request(server()).get('/api/admin/ads/statistics').set(authHeader(admin)).expect(200);
    ...
    const iab = await request(server()).get('/api/admin/ads/iab-sizes').set(authHeader(admin)).expect(200);
