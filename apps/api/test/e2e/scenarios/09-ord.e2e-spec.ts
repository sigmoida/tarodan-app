...
describe("09 — Sipariş Yaşam Döngüsü (ORD)", () => {
   ...
   */
  function makeAutoCompleteScheduler(
    flag: "true" | undefined,
   ...
   */
  async function makeAwaitingOrder(opts: {
    deadlinePast: boolean;
  }): Promise<{ orderId: string; buyer: { id: string; accessToken: string } }> {
  ...
  describe("POST /api/orders/buy", () => {
    ...
    scenario("ORD-003", async () => {
      ...
      const count = await getPrisma().order.count({
  ...
  describe("POST /api/orders/checkout", () => {
    scenario("ORD-013", async () => {
      ...
      const groups = await prisma.checkoutGroup.count({
  ...
  describe("Durum geçişleri", () => {
    ...
    scenario("ORD-031", async () => {
      ...
      const { buyer, seller, product, addr } = await makeBuyerSellerProduct();
      const orderId = (await buyNow(buyer, product.id, addr.id).expect(201))
        .body.orderId;
      const res = await request(server())
        ...
        .expect(400);
      expect(JSON.stringify(res.body)).toContain(
        "Sadece ödenmiş siparişler hazırlanabilir",
      );
    });
    ...
    scenario("ORD-032", async () => {
      ...
      const res = await request(server())
        ...
        .expect(403);
      expect(JSON.stringify(res.body)).toContain(
        "Bu siparişi güncelleme yetkiniz yok",
      );
    });
    ...
    scenario("ORD-033", async () => {
      ...
      const res = await request(server())
        ...
        .expect(400);
      expect(JSON.stringify(res.body)).toContain(
        "Sadece teslim edilmiş siparişler onaylanabilir",
      );
    });
    ...
    scenario("ORD-035", async () => {
      ...
      const res = await request(server())
        ...
        .expect(400);
      expect(JSON.stringify(res.body)).toContain(
        "Sadece ödeme bekleyen siparişlere adres eklenebilir",
      );
    });
    ...
    scenario("ORD-036", async () => {
      ...
      if (res.status === 403) {
        expect(JSON.stringify(res.body)).toContain(
          "Bu siparişe adres ekleme yetkiniz yok",
        );
      }
    });
  ...
  describe("POST /api/orders/:id/confirm-receipt", () => {
    ...
    scenario("ORD-047", async () => {
      ...
      const { orderId, buyer } = await makeAwaitingOrder({
        deadlinePast: true,
      });
  ...
  describe("POST /api/orders/:id/cancel", () => {
    ...
    scenario("ORD-061", async () => {
      ...
      expect(order?.cancellationType).toBe("iptal");
      const ledger = await getPrisma().commissionLedger.findFirst({
        where: { orderId },
      });
      if (ledger) {
  ...
  describe("POST /api/orders/:id/reactivate", () => {
    ...
    scenario("ORD-081", async () => {
      ...
      const { buyer, product, addr } = await makeBuyerSellerProduct();
      const orderId = (await buyNow(buyer, product.id, addr.id).expect(201))
        .body.orderId; // pending_payment
      const res = await request(server())
        .post(`/api/orders/${orderId}/reactivate`)
        .set(authHeader(buyer))
        .expect(400);
      expect(JSON.stringify(res.body)).toContain(
        "Sadece iptal edilmiş siparişler",
      );
    });
  ...
  describe("POST /api/orders/quote & komisyon", () => {
    scenario("ORD-100", async () => {
      ...
      expect(pr.totalAmount).toBeCloseTo(
        pr.subtotal +
          pr.shippingAmount +
          pr.buyerFeeAmount +
          (pr.taxAmount ?? 0),
      ...
      expect(pr.sellerNetAmount).toBeCloseTo(
        pr.subtotal + (pr.taxAmount ?? 0) - pr.sellerFeeAmount,
        2,
      );
    ...
    scenario("ORD-111", async () => {
      ...
      const seller = await createUser(ctx.module, { isSeller: true });
      const below = await createProduct({
        sellerId: seller.id,
        categoryId: baseline.categoryId,
        price: 499,
      });
      const atThreshold = await createProduct({
        sellerId: seller.id,
        categoryId: baseline.categoryId,
        price: 500,
      });
      const r1 = await request(server())
        .post("/api/orders/quote")
        .send({ items: [{ productId: below.id }] })
        .expect(201);
      const r2 = await request(server())
        .post("/api/orders/quote")
        .send({ items: [{ productId: atThreshold.id }] })
        .expect(201);
    ...
    scenario("ORD-118", async () => {
      ...
      expect(
        results[0].sellerFeeAmount + results[0].sellerNetAmount,
      ).toBeCloseTo(100, 2);
      expect(
        results[1].sellerFeeAmount + results[1].sellerNetAmount,
      ).toBeCloseTo(250, 2);
      expect(
        results[2].sellerFeeAmount + results[2].sellerNetAmount,
      ).toBeCloseTo(500, 2);
    });
  });
  ...
  describe("GET /api/orders & gruplar", () => {
    ...
    scenario("ORD-133", async () => {
      ...
      const asBuyer = await request(server())
        ...
        .expect(200);
      expect(
        asBuyer.body.data.every(
          (o: any) => o.isBuyer === true && o.isSeller === false,
        ),
      ).toBe(true);
      const asSeller = await request(server())
        ...
        .expect(200);
      expect(
        asSeller.body.data.every(
          (o: any) => o.isSeller === true && o.isBuyer === false,
        ),
      ).toBe(true);
    });
    ...
    scenario("ORD-134", async () => {
      ...
      expect(
        res.body.data.every((o: any) => o.status === OrderStatus.paid),
      ).toBe(true);
    });
    ...
    scenario("ORD-135", async () => {
      ...
      const pr = await createProduct({
        sellerId: seller.id,
        categoryId: baseline.categoryId,
      });
      const cancelledId = (await buyNow(buyer, pc.id, addr.id).expect(201)).body
        .orderId;
      await request(server())
        .post(`/api/orders/${cancelledId}/cancel`)
        .set(authHeader(buyer))
        .send({})
        .expect(200);
      ...
      const res = await request(server())
        .get("/api/orders?role=buyer&status=cancelled,refunded")
        .set(authHeader(buyer))
        .expect(200);
    ...
    scenario("ORD-136", async () => {
      ...
      const res = await request(server())
        ...
        .expect(200);
      expect(
        res.body.data.every((o: any) => o.status !== OrderStatus.cancelled),
      ).toBe(true);
      expect(res.body.data.length).toBeGreaterThanOrEqual(1);
    });
    ...
    scenario("ORD-140", async () => {
      ...
      expect(
        res.body.data.every(
          (o: any) => !String(o.product?.id ?? "").startsWith("membership-"),
        ),
      ).toBe(true);
    ...
    scenario("ORD-142", async () => {
      ...
      for (const o of co.body.orders) {
        await request(server())
          .post(`/api/orders/${o.orderId}/cancel`)
          .set(authHeader(buyer))
          .send({})
          .expect(200);
      }
      const res = await request(server())
        ...
        .expect(200);
      expect(
        res.body.data.find((g: any) => g.id === co.body.checkoutGroupId),
      ).toBeUndefined();
    });
    ...
    scenario("ORD-143", async () => {
      ...
      const res = await request(server())
        ...
        .expect(403);
      expect(JSON.stringify(res.body)).toContain(
        "Bu sipariş grubunu görüntüleme yetkiniz yok",
      );
    });
    ...
    scenario("ORD-144", async () => {
      ...
      const buyer = await createUser(ctx.module);
      const addr = await createAddress({ userId: buyer.id });
      const pComp = await createProduct({
        sellerId: seller.id,
        categoryId: baseline.categoryId,
        price: 200,
      });
      const pPrep = await createProduct({
        sellerId: seller.id,
        categoryId: baseline.categoryId,
        price: 300,
      });
      ...
      const prepOrder = await getPrisma().order.findUnique({
        where: { id: prepId },
      });
      expect(res.body.totalEarnings).toBeCloseTo(
        Number(completedOrder!.totalAmount),
        2,
      );
      expect(res.body.pendingEarnings).toBeCloseTo(
        Number(prepOrder!.totalAmount),
        2,
      );
    });
    ...
    scenario("ORD-145", async () => {
      ...
      const orderNumber =
        ...
        (await getPrisma().order.findUnique({ where: { id: order.body.id } }))!
          .orderNumber;
  ...
  describe("Eşzamanlılık & idempotency", () => {
    ...
    scenario("ORD-181", async () => {
      ...
      const groups = await getPrisma().checkoutGroup.count({
        where: { buyerId: buyer.id },
      });
    ...
    scenario("ORD-184", async () => {
      ...
      const addr = await createAddress({ userId: buyer.id });
      const staleId = (await buyNow(buyer, product.id, addr.id).expect(201))
        .body.orderId;
      const co = await request(server())
        .post("/api/orders/checkout")
        .set(authHeader(buyer))
    ...
    scenario("ORD-185", async () => {
      ...
      const prisma = getPrisma();
      const order = await prisma.order.findUnique({ where: { id: orderId } });
      expect([OrderStatus.preparing, OrderStatus.paid]).toContain(
        order!.status,
      );
  ...
  describe("Güvenlik", () => {
    scenario("ORD-240", async () => {
      ...
      const orderId = (await buyNow(deniz, product.id, addr.id).expect(201))
        .body.orderId;
      const kaan = await createUser(ctx.module);
      const res = await request(server())
        .get(`/api/orders/${orderId}`)
    ...
    scenario("ORD-241", async () => {
      ...
      const o1 = (await buyNow(buyer, product.id, addr.id).expect(201)).body
        .orderNumber as string;
      const o2 = (await buyNow(buyer, p2.id, addr.id).expect(201)).body
        .orderNumber as string;
    ...
    scenario("ORD-242", async () => {
      ...
      const res = await request(server())
        .post("/api/orders/guest/track")
        .send({ orderNumber, email: "baska@external.com" })
        .expect(404);
  ...
  describe("UI / i18n / parite (skip)", () => {
    scenario.skip(
      "ORD-220",
      "Durum etiketleri TR/EN çevirisi web/mobil katmanında; API yanıtı ham OrderStatus döner (i18n burada test edilmez).",
    );
    scenario.skip(
      "ORD-221",
      "İptal kartı kullanıcı-dostu mesajı frontend cancelCategory→metin eşlemesi; API cancelCategory alanını ORD-060/061 dolaylı kapsar.",
    );
    scenario.skip(
      "ORD-222",
      '"İade" yerine "İptal" rozet gösterimi saf UI (status=refunded + cancellationType=iptal alanları ORD-061/062 ile API\'de kanıtlı).',
    );
    scenario.skip(
      "ORD-223",
      "Liste yükleniyor/hata iskeleti tamamen istemci-tarafı render; API e2e kapsamı dışında.",
    );
    scenario.skip(
      "ORD-224",
      "web/mobil sipariş listesi paritesi cross-client görsel karşılaştırma; tek API harness'inde doğrulanamaz.",
    );
    scenario.skip(
      "ORD-225",
      "Grup detay sayfası parite (web vs mobile) saf UI; grup yanıt şekli ORD-141/143 ile API'de kapsanır.",
    );
  });
});
