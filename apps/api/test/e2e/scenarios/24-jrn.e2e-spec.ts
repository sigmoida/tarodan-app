...
describe("24 — Uçtan Uca Entegrasyon Journeyleri (JRN)", () => {
  ...
  async function subscribeAndPayPremium(
    user: Auth,
    ...
    const payment = await prisma.payment.findUnique({
      where: { id: subRes.body.paymentId },
    });
    const orderId = payment!.orderId!;
    await post("/api/payments/direct-form", user).send({ orderId }).expect(201);
    await successCallback(orderId).expect(200);
    return { orderId };
  }
  ...
  scenario(
    ...
    async () => {
      ...
      await post("/api/payments/initiate", buyer)
        .send({ orderId, provider: "paytr" })
        .expect(201);
      await successCallback(orderId).expect(200);
      ...
      expect([OrderStatus.paid, OrderStatus.preparing]).toContain(
        orderAfterPay?.status as OrderStatus,
      );
      ...
      const releasedHold = await prisma.paymentHold.findFirst({
        where: { orderId },
      });
      ...
      const payout = await prisma.payoutTransfer.findFirst({
        where: { paymentHoldId: releasedHold!.id },
      });
  ...
  scenario(
    ...
    async () => {
      ...
      const stillHeld = await prisma.paymentHold.findFirst({
        where: { orderId },
      });
  ...
  scenario(
    ...
    async () => {
      ...
      const orderAgain = await prisma.order.findUnique({
        where: { id: orderId },
      });
  ...
  scenario(
    ...
    async () => {
      ...
      await post(
        `/api/refund-requests/${refundRes.body.id}/cancel`,
        buyer,
      ).expect(200);
    ...
    async () => {
      ...
      await prisma.order.update({
        ...
        data: {
          ...
          preparingDeadline: new Date(Date.now() - 3600 * 1000),
        },
      });
  ...
  scenario(
    ...
    async () => {
      ...
      const acceptRes = await post(
        `/api/offers/${offerRes.body.id}/accept`,
        seller,
      ).expect(200);
      expect(acceptRes.body.orderId).toBeTruthy();
      const orderId = acceptRes.body.orderId as string;
      expect(
        (await prisma.offer.findUnique({ where: { id: offerRes.body.id } }))
          ?.status,
      ...
      expect(Number(order?.totalAmount)).toBe(
        400 + Number(order?.buyerFeeAmount),
      );
  ...
  scenario(
    ...
    async () => {
      ...
      const counterRes = await post(
        `/api/offers/${offerRes.body.id}/counter`,
        seller,
      )
        .send({ amount: 200 })
        .expect(200);
      ...
      expect(
        (await prisma.offer.findUnique({ where: { id: offerRes.body.id } }))
          ?.status,
      ).toBe(OfferStatus.rejected);

      // 3) Alıcı kabul → order (pending_payment) amount 200.
      const acceptRes = await post(
        `/api/offers/${counterRes.body.id}/accept`,
        buyer,
      ).expect(200);
      const orderId = acceptRes.body.orderId as string;
      const order = await prisma.order.findUnique({ where: { id: orderId } });
      ...
      expect(Number(order?.totalAmount)).toBe(
        200 + Number(order?.buyerFeeAmount),
      );
  ...
  scenario(
    ...
    async () => {
      ...
      expect(
        (await prisma.offer.findUnique({ where: { id: ok.body.id } }))?.status,
      ).toBe(OfferStatus.rejected);
  ...
  scenario(
    ...
    async () => {
      ...
      const { buyer, seller, product } = await makeBuyerSellerProduct({
        price: 200,
        quantity: 1,
      });
      const addr = await createAddress({ userId: buyer.id });
      ...
      expect(
        (await prisma.order.findUnique({ where: { id: orderId } }))?.status,
      ).not.toBe(OrderStatus.pending_payment);
      // Stok tükenme süpürmesi (idempotent güvence).
      await ctx.app.get(PaymentService); // no-op ref
      await request(server())
        .post("/api/dev/run/sweep-out-of-stock")
        .expect(201);

      const p = await prisma.product.findUnique({ where: { id: product.id } });
      expect(p?.quantity).toBe(0);
      // 2) Bekleyen teklif iptal.
      const pendingAfter = await prisma.offer.findUnique({
        where: { id: pending.id },
      });
  ...
  scenario(
    ...
    async () => {
      ...
      const existing = await createUser(ctx.module, {
        email: "kayitli-jrn21@demo.com",
      });
  ...
  scenario(
    ...
    async () => {
      ...
      const prisma = getPrisma();
      expect(
        await prisma.order.count({ where: { product: { id: product.id } } }),
      ).toBe(0);
    },
    LONG,
  );
  ...
  scenario(
    ...
    async () => {
      ...
      await post(`/api/trades/${tradeId}/accept`, f.initiator)
        .send({})
        .expect(201);
      expect(
        (await prisma.trade.findUnique({ where: { id: tradeId } }))?.status,
      ).toBe(TradeStatus.shipping_to_warehouse);
      expect(
        (
          await prisma.product.findUnique({
            where: { id: f.initiatorProduct.id },
          })
        )?.reservedQuantity,
      ...
      await post(`/api/admin/trades/${tradeId}/mark-warehouse-received`, admin)
        .send({ shipmentId: inbound[1].id })
        .expect(200);
      expect(
        (await prisma.trade.findUnique({ where: { id: tradeId } }))?.status,
      ).toBe(TradeStatus.at_warehouse);
      ...
      expect(
        (await prisma.trade.findUnique({ where: { id: tradeId } }))?.status,
      ).toBe(TradeStatus.shipping_to_recipients);
      ...
      await post(`/api/trades/${tradeId}/confirm-receipt`, f.initiator)
        .send({})
        .expect(201);
      await post(`/api/trades/${tradeId}/confirm-receipt`, f.receiver)
        .send({})
        .expect(201);
      ...
      await post("/api/ratings/users", f.initiator)
        .send({ receiverId: f.receiver.id, tradeId, score: 5 })
        .expect(201);
      await post("/api/ratings/users", f.receiver)
        .send({ receiverId: f.initiator.id, tradeId, score: 4 })
        .expect(201);
      expect(
        await prisma.rating.count({ where: { tradeId } }),
  ...
  scenario(
    ...
    async () => {
      ...
      expect(
        await prisma.tradeShipment.count({
          where: { tradeId, leg: "to_warehouse" },
        }),
      ).toBe(0);
      ...
      await request(server())
        ...
        .expect(200);
      const cashPaid = await prisma.tradeCashPayment.findFirst({
        where: { tradeId },
      });
      ...
      for (const s of inbound) {
        await post(
          `/api/admin/trades/${tradeId}/mark-warehouse-received`,
          admin,
        )
      ...
      expect(
        (await prisma.trade.findUnique({ where: { id: tradeId } }))?.status,
      ).toBe(TradeStatus.completed);
      const cashDone = await prisma.tradeCashPayment.findFirst({
        where: { tradeId },
      });
      ...
      expect(
        (await prisma.tradeCashPayment.findUnique({ where: { id: cash!.id } }))
          ?.releasedAt,
      ).toBeTruthy();
      const payoutsCreated = await ctx.app
        .get(PayoutService)
  ...
  scenario(
    ...
    async () => {
      ...
      expect([TradeStatus.returning, TradeStatus.cancelled]).toContain(
        trade?.status as TradeStatus,
      );
      // İade için payout OLUŞMAZ.
      const payouts = await ctx.app
        .get(PayoutService)
        .createPayoutsForReleasedHolds();
  ...
  scenario(
    ...
    async () => {
      ...
      await prisma.trade.update({
        where: { id: tradeId },
        data: {
          shippingDeadline: new Date(Date.now() - 60_000),
  ...
  scenario(
    ...
    async () => {
      ...
      const rpClosed = await createProduct({
        sellerId: receiver.id,
        categoryId: baseline.categoryId,
      ...
      await post("/api/trades", initiator)
        .send({
          ...
          initiatorItems: [{ productId: ip.id, quantity: 1 }],
          receiverItems: [{ productId: rpClosed.id, quantity: 1 }],
        })
        .expect(400);

      // 3) Geçerli takas → pending, stok rezerve değil.
      const rpOpen = await createProduct({
        sellerId: receiver.id,
        categoryId: baseline.categoryId,
        isTradeEnabled: true,
      });
      const ok = await post("/api/trades", initiator)
        .send({
          receiverId: receiver.id,
          initiatorItems: [{ productId: ip.id, quantity: 1 }],
          receiverItems: [{ productId: rpOpen.id, quantity: 1 }],
        })
        .expect(201);
  ...
  scenario(
    ...
    async () => {
      ...
      const { buyer, product, orderId } = await paidOrder({
        price: 200,
        quantity: 1,
      });
      const prisma = getPrisma();
  ...
  scenario(
    ...
    async () => {
      ...
      ]).toContain(rr?.status as RefundRequestStatus);
    },
    LONG,
  );
  ...
  scenario(
    ...
    async () => {
      ...
      // 1) Satıcı → 403.
      const s = await createRefund(orderId, seller, {
        reason: "changed_mind",
      }).expect(403);
      expect(s.body.message).toMatch(/Sadece alıcı/i);
      // 2) Yabancı → 403.
      await createRefund(orderId, stranger, { reason: "changed_mind" }).expect(
        403,
      );
      // 3) Alıcı → 201.
      await createRefund(orderId, buyer, { reason: "changed_mind" }).expect(
  ...
  scenario(
    ...
    async () => {
      ...
      const res = await post(
        `/api/refund-requests/${createRes.body.id}/cancel`,
        buyer,
      ).expect(400);
  ...
  scenario(
    ...
    async () => {
      ...
      await markDelivered(
        orderId,
        new Date(Date.now() - 20 * 24 * 3600 * 1000),
      );
  ...
  scenario(
    ...
    async () => {
      ...
      await post(`/api/trades/${tradeId}/dispute`, stranger)
  ...
  scenario(
    ...
    async () => {
      ...
      expect(
        (await prisma.tradeCashPayment.findFirst({ where: { tradeId } }))
          ?.releasedAt,
      ).toBeNull();

      // 3) Yönetici olmayan token admin JWT stratejisinde doğrulanmaz → 401.
      await post(`/api/admin/payouts/release-trade/${tradeId}`, f.initiator)
        .send({})
        .expect(401);

      // 2) Admin erken serbest bırakma → releasedAt dolar.
      await post(`/api/admin/payouts/release-trade/${tradeId}`, admin)
        .send({})
        .expect(200);
      expect(
        (await prisma.tradeCashPayment.findFirst({ where: { tradeId } }))
  ...
  scenario(
    ...
    async () => {
      ...
      // 1) free durum.
      const meFree = await request(server())
        .get("/api/membership/me")
        .set(authHeader(user))
        .expect(200);
      expect(meFree.body.tier.type).toBe("free");
      const limitsFree = await request(server())
      ...
      const checkFree = await request(server())
      ...
      const limitsPrem = await request(server())
        .get("/api/membership/me/limits")
        .set(authHeader(user))
        .expect(200);
      ...
      const checkPrem = await request(server())
        .get("/api/membership/check/collection")
        .set(authHeader(user))
        .expect(200);
      ...
      await request(server())
        ...
        .expect(200);
      expect(
        (await prisma.userMembership.findUnique({ where: { userId: user.id } }))
          ?.autoRenew,
      ).toBe(false);
    },
    LONG,
  ...
  scenario(
    ...
    async () => {
      ...
      await post("/api/collections", user)
        .send({ name: "Koleksiyonum", isPublic: true })
        .expect(201);
      ...
      await post("/api/messages/threads", user)
        .send({
          recipientId: otherSeller.id,
          productId: otherProduct.id,
          message: "Merhaba, ürün mevcut mu?",
        })
        .expect(201);
  ...
  scenario(
    ...
    async () => {
      ...
      await prisma.userMembership.update({
        where: { userId: user.id },
        data: {
          tierId: (await prisma.membershipTier.findUnique({
            where: { type: MembershipTierType.free },
          }))!.id,
  ...
  scenario(
    ...
    async () => {
      ...
      const limits = await request(server())
        .get("/api/membership/me/limits")
        .set(authHeader(user))
        .expect(200);
  ...
  scenario(
    ...
    async () => {
      ...
      const { payoutsCreated } = await releaseAndPayout(orderId);
      const released = await prisma.paymentHold.findFirst({
        where: { orderId },
      });
  ...
  scenario(
    ...
    async () => {
      ...
      await post(`/api/admin/payouts/${payout!.id}/retry`, admin)
        .send({})
        .expect(200);
      expect(
        (await prisma.payoutTransfer.findUnique({ where: { id: payout!.id } }))
          ?.status,
  ...
  scenario(
    ...
    async () => {
      ...
      await markShipped(orderId);
      const refundRes = await createRefund(orderId, buyer, {
        reason: "changed_mind",
      }).expect(201);
      let hold = await prisma.paymentHold.findFirst({ where: { orderId } });
  ...
  scenario(
    ...
    async () => {
      ...
      expect([403, 404]).toContain(
        (
          await request(server())
            .get(`/api/trades/${tradeRes.body.id}`)
            .set(authHeader(stranger))
        ).status,
      );
      expect([403, 404]).toContain(
  ...
  scenario(
    ...
    async () => {
      ...
      const { orderId, paymentId } = await buyAndInitiate(
        ctx,
        buyer,
        product.id,
        addr.id,
      ...
      expect(
        (await post(`/api/payments/${paymentId}/cancel`, stranger).send({}))
          .status,
      ).toBe(403);
      // 2) Yabancı /confirm → 403.
      expect(
        (await post(`/api/orders/${orderId}/confirm`, stranger)).status,
      ).toBe(403);
  ...
  scenario(
    ...
    async () => {
      ...
      expect(
        (
          await post(`/api/admin/trades/${tradeId}/approve`, moderator).send({
            notes: "x",
          })
        ).status,
      ).toBe(403);
  ...
  scenario(
    ...
    async () => {
      ...
      const offerRes = await post("/api/offers", seller).send({
        productId: ownProduct.id,
        amount: 100,
      });
      expect(offerRes.status).toBe(400);
      expect(JSON.stringify(offerRes.body)).toContain(
      ...
      await post("/api/trades", seller)
        .send({
          ...
          initiatorItems: [{ productId: ownProduct.id, quantity: 1 }],
          receiverItems: [{ productId: ownProduct.id, quantity: 1 }],
        })
        .expect(400);
    },
    LONG,
  ...
  scenario(
    ...
    async () => {
      ...
      const lowTr = await request(server())
        .post("/api/offers")
        .set(authHeader(buyer))
        .set("Accept-Language", "tr")
        .send({ productId: otherProduct.id, amount: 1 });
      const lowEn = await request(server())
        .post("/api/offers")
        .set(authHeader(buyer))
        .set("Accept-Language", "en")
        .send({ productId: otherProduct.id, amount: 1 });
  ...
  scenario(
    ...
    async () => {
      ...
      const invoices = await request(server())
        ...
        .expect(200);
      expect(
        Array.isArray(invoices.body) ? invoices.body.length : invoices.body,
      ).toBeDefined();
