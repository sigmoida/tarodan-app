...
import { OrderStatus, OfferStatus, PaymentStatus, ProductStatus } from '@prisma/client';
...
describe('15 — Stok Bütünlüğü & Rezervasyon (STK)', () => {
  ...
  async function makeProduct(opts: { sellerId: string; price?: number; quantity?: number | null; status?: 'active' | 'inactive' | 'sold' }) {
    ...
    if (opts.quantity === null) {
      await prisma().product.update({ where: { id: p.id }, data: { quantity: null } });
    }
    return p;
  }

  /** Alıcı + satıcı + ürün + alıcı adresi (varsayılan fiyat 100, adet 1). */
  async function setup(opts: { price?: number; quantity?: number | null; status?: 'active' | 'inactive' | 'sold' } = {}) {
    const buyer = await createUser(ctx.module);
    const seller = await createUser(ctx.module, { isSeller: true });
    const product = await makeProduct({ sellerId: seller.id, price: opts.price, quantity: opts.quantity, status: opts.status });
  ...
  const initiateGroup = (buyer: { accessToken: string }, checkoutGroupId: string) =>
    request(server())
      .post('/api/payments/initiate')
      .set(authHeader(buyer))
  ...
  async function paymentByGroup(checkoutGroupId: string) {
    return prisma().payment.findFirst({ where: { checkoutGroupId }, orderBy: { createdAt: 'desc' } });
  }

  /** Başarılı/başarısız PayTR callback'i (kuruş = amount*100). */
  function callback(payment: { providerConversationId: string | null; amount: any }, status: 'success' | 'failed' = 'success') {
  ...
  async function buyAndPay(buyer: { accessToken: string }, productId: string, addrId: string): Promise<string> {
  ...
  function checkout(buyer: { accessToken: string }, productId: string, quantity: number, addrId: string, idempotencyKey = randomUUID()) {
    return request(server())
      .post('/api/orders/checkout')
      .set(authHeader(buyer))
      .send({ items: [{ productId, quantity }], idempotencyKey, shippingAddressId: addrId });
  }
  ...
  async function readProduct(id: string) {
    ...
    return {
      ...
      status: p!.status,
      available: p!.quantity === null ? null : Math.max(0, (p!.quantity ?? 0) - (p!.reservedQuantity ?? 0)),
    };
  }
  ...
  scenario('STK-002', async () => {
    ...
    const offer = await createOfferRow({ productId: product.id, buyerId: buyer.id, sellerId: seller.id, amount: 80 });
  ...
  scenario('STK-004', async () => {
    ...
    await prisma().product.update({ where: { id: product.id }, data: { reservedQuantity: 1 } }); // available=0
    ...
    const count = await prisma().order.count({ where: { productId: product.id } });
    expect(count).toBe(0);
  });
  ...
  scenario('STK-012', async () => {
    ...
    const { buyer: fastBuyer, seller, product, addr } = await setup({ quantity: 1, price: 100 });
    ...
    const offer = await createOfferRow({ productId: product.id, buyerId: slowBuyer.id, sellerId: seller.id, amount: 90 });
    await request(server()).post(`/api/offers/${offer.id}/accept`).set(authHeader(seller)).expect(200);
    const slowOrder = await prisma().order.findFirst({ where: { offerId: offer.id } });
    ...
    const cancelledOffer = await prisma().offer.findUnique({ where: { id: offer.id } });
    expect(cancelledOffer?.status).toBe(OfferStatus.cancelled);
    expect(cancelledOffer?.cancelReason ?? '').toMatch(/Stok/i);
    const cancelledSlowOrder = await prisma().order.findUnique({ where: { id: slowOrder!.id } });
  ...
  scenario('STK-013', async () => {
    ...
    const { buyer: b1, product, addr: a1 } = await setup({ quantity: 2, price: 100 });
    ...
    const order2 = await prisma().order.findUnique({ where: { id: r2.body.orderId } });
  ...
  scenario('STK-022', async () => {
    ...
    const order = await prisma().order.findFirst({ where: { checkoutGroupId: groupId } });
    ...
    await request(server()).post(`/api/orders/${order!.id}/cancel`).set(authHeader(buyer)).send({ reason: 'iade' }).expect(200);
  ...
  scenario('STK-024', async () => {
    ...
    await paymentSvc().processRefund(order!.id, unitTotal, { refundQuantity: 1 });

    p = await readProduct(product.id);
    expect(p.quantity).toBe(3); // 2 + 1
    const openOrder = await prisma().order.findUnique({ where: { id: order!.id } });
  ...
  scenario('STK-031', async () => {
    ...
    expect(res.body.message).toMatch(/yeterli stok yok \(istenen 6, mevcut 5\)/);
  ...
  scenario('STK-033', async () => {
    ...
    const res = await request(server())
      ...
      .expect(400);
    expect(JSON.stringify(res.body)).toContain('Tek siparişte en fazla 20 ürün alınabilir');
  });
  ...
  scenario('STK-034', async () => {
    ...
    const firstOrder = await prisma().order.findFirst({ where: { checkoutGroupId: first.body.checkoutGroupId } });
    let p = await readProduct(product.id);
    expect(p.reserved).toBe(1);

    // Aynı ürünü yeni idempotencyKey ile tekrar checkout → stok hatası YOK.
    const second = await checkout(buyer, product.id, 1, addr.id, randomUUID()).expect(201);
    const secondOrder = await prisma().order.findFirst({ where: { checkoutGroupId: second.body.checkoutGroupId } });
    expect(secondOrder!.id).not.toBe(firstOrder!.id);

    const oldOrder = await prisma().order.findUnique({ where: { id: firstOrder!.id } });
    expect(oldOrder?.status).toBe(OrderStatus.cancelled);
    expect(oldOrder?.cancelReason).toContain('Yeni toplu sipariş ile değiştirildi');
  ...
  scenario('STK-041', async () => {
    ...
    await prisma().order.update({ where: { id: res.body.orderId }, data: { createdAt: new Date(Date.now() - 31 * 60 * 1000) } });
    await runScheduler(ctx.app, 'release-expired-reservations');

    let p = await readProduct(product.id);
    expect(p.reserved).toBe(0);
    const mid = await prisma().order.findUnique({ where: { id: res.body.orderId } });
    ...
    await request(server()).post(`/api/orders/${res.body.orderId}/cancel`).set(authHeader(buyer)).send({ reason: 'vazgeçtim' }).expect(200);
    p = await readProduct(product.id);
  ...
  scenario('STK-050', async () => {
    ...
    expect(p.quantity).toBe(1);
    expect(p.reserved).toBe(1);
    const pending = await prisma().order.count({ where: { productId: product.id, status: OrderStatus.pending_payment } });
  ...
  scenario('STK-051', async () => {
    ...
    const addrs = await Promise.all(buyers.map((b) => createAddress({ userId: b.id })));

    const results = await Promise.all(buyers.map((b, i) => buyNow(b, product.id, addrs[i].id)));
  ...
  scenario('STK-052', async () => {
    ...
    const offer = await createOfferRow({ productId: product.id, buyerId: offerBuyer.id, sellerId: seller.id, amount: 80 });
    ...
    expect([buyRes.status, acceptRes.status].some((s) => s === 200 || s === 201)).toBe(true);
  ...
  scenario('STK-053', async () => {
    ...
    const completed = await prisma().payment.count({ where: { orderId: res.body.orderId, status: PaymentStatus.completed } });
    expect(completed).toBe(1);
    const holds = await prisma().paymentHold.count({ where: { orderId: res.body.orderId } });
    expect(holds).toBe(1);
  ...
  scenario('STK-061', async () => {
    ...
    const res = await request(server()).post(`/api/offers/${offer.id}/accept`).set(authHeader(seller)).expect(400);
    expect(JSON.stringify(res.body)).toMatch(/yeterli müsait adet yok|artık satışta değil/);
  });
  ...
  scenario('STK-063', async () => {
    ...
    const [i1, i2] = await Promise.all([initiate(buyer, order!.id), initiate(buyer, order!.id)]);
  ...
  scenario('STK-071', async () => {
    ...
    await prisma().order.update({ where: { id: order!.id }, data: { createdAt: new Date(Date.now() - 31 * 60 * 1000) } });
    await runScheduler(ctx.app, 'release-expired-reservations');
  ...
  scenario('STK-072', async () => {
    ...
    await prisma().product.update({ where: { id: product.id }, data: { reservedQuantity: 3 } }); // drift
  ...
  scenario('STK-074', async () => {
    ...
    await prisma().order.update({ where: { id: res.body.orderId }, data: { paymentExpiresAt: new Date(Date.now() - 60 * 1000) } });
  ...
  scenario('STK-081', async () => {
    ...
    const seller = await createUser(ctx.module, { isSeller: true });
    const product = await makeProduct({ sellerId: seller.id, quantity: null, price: 100 });
  ...
  scenario('STK-082', async () => {
    ...
    await request(server()).post(`/api/orders/${orderId}/cancel`).set(authHeader(buyer)).send({ reason: 'iade' }).expect(200);
    await paymentSvc().processRefund(orderId);
  ...
  scenario('STK-090', async () => {
    ...
    const offer = await createOfferRow({ productId: product.id, buyerId: loser.id, sellerId: seller.id, amount: 90 });
    await request(server()).post(`/api/offers/${offer.id}/accept`).set(authHeader(seller)).expect(200);

    await buyAndPay(fastBuyer, product.id, addr.id);

    const notif = await prisma().notificationLog.findFirst({
      where: { userId: loser.id, type: 'offer_cancelled_out_of_stock' as any, channel: 'in_app' },
    });
  ...
  scenario('STK-092', async () => {
    ...
    const { buyer, seller, product, addr } = await setup({ quantity: 1, price: 100 });
    ...
    await prisma().product.update({ where: { id: product.id }, data: { quantity: 1, status: ProductStatus.active } });
    const b2 = await createUser(ctx.module);
    const a2 = await createAddress({ userId: b2.id });
    ...
    let count = await prisma().notificationLog.count({
      where: { userId: wisher.id, type: 'back_in_stock' as any, data: { path: ['productId'], equals: product.id } },
    });
    expect(count).toBe(1);
    ...
    await prisma().product.update({ where: { id: product.id }, data: { quantity: 1, status: ProductStatus.active, reservedQuantity: 0 } });
  ...
  const addToCart = (buyer: { accessToken: string }, productId: string, quantity: number) =>
    request(server()).post('/api/cart/items').set(authHeader(buyer)).send({ productId, quantity });
  ...
  scenario('STK-101', async () => {
    ...
    expect(JSON.stringify(res.body)).toContain('Bu üründen en fazla 2 adet sipariş verilebilir');
  });
  ...
  scenario('STK-102', async () => {
    ...
    const buyer = await createUser(ctx.module);
    const seller = await createUser(ctx.module, { isSeller: true });
    const product = await makeProduct({ sellerId: seller.id, quantity: 1, status: 'inactive' });
  ...
  scenario('STK-103', async () => {
    ...
    await addToCart(buyer, pOut.id, 1).expect((r) => expect([200, 201]).toContain(r.status));
    await prisma().product.update({ where: { id: pOut.id }, data: { reservedQuantity: 5 } }); // available=0
    ...
    await addToCart(buyer, pLow.id, 1).expect((r) => expect([200, 201]).toContain(r.status));
    await prisma().cartItem.updateMany({ where: { product: { id: pLow.id } }, data: { quantity: 5 } });
    await prisma().product.update({ where: { id: pLow.id }, data: { quantity: 3 } }); // available=3 < istenen 5

    // available<=5: 'Son birkaç ürün!'.
    const pFew = await makeProduct({ sellerId: seller.id, quantity: 5 });
    await addToCart(buyer, pFew.id, 1).expect((r) => expect([200, 201]).toContain(r.status));

    const cart = await request(server()).get('/api/cart').set(authHeader(buyer)).expect(200);
    ...
    if (byId(pLow.id)) expect(byId(pLow.id)?.stockWarning).toBe('Stokta sadece 3 adet var');
  ...
  scenario('STK-104', async () => {
    ...
    await prisma().product.update({ where: { id: product.id }, data: { maxQuantityPerOrder: 2 } });

    const res = await addToCart(buyer, product.id, 3).expect(400);
    expect(JSON.stringify(res.body)).toContain('Bu üründen maksimum 2 adet alabilirsiniz');
  });
  ...
  scenario('STK-110', async () => {
    ...
    await prisma().product.update({ where: { id: reserved.id }, data: { reservedQuantity: 1 } }); // available=0
  ...
  scenario('STK-112', async () => {
    ...
    const seller = await createUser(ctx.module, { isSeller: true });
    const unlimited = await makeProduct({ sellerId: seller.id, quantity: null });
  ...
  scenario('STK-120', async () => {
    ...
    const before = await readProduct(product.id);
    await request(server()).post('/api/orders/buy').send({ productId: product.id, shippingAddressId: addr.id }).expect(401);
    const after = await readProduct(product.id);
  ...
  scenario('STK-121', async () => {
    ...
    // Üye
    const buyer = await createUser(ctx.module);
    const memberProduct = await makeProduct({ sellerId: seller.id, quantity: 5, price: 100 });
    const addr = await createAddress({ userId: buyer.id });
    const memberRes = await checkout(buyer, memberProduct.id, 3, addr.id).expect(201);
    expect(memberRes.body.checkoutGroupId).toBeTruthy();
    let p = await readProduct(memberProduct.id);
    expect(p.reserved).toBe(3);
    expect(p.quantity).toBe(5);
    ...
    await clearMailbox();
    const guestProduct = await makeProduct({ sellerId: seller.id, quantity: 5, price: 100 });
  ...
  scenario('STK-124', async () => {
    ...
    const seller = await createUser(ctx.module, { isSeller: true });
    const product = await makeProduct({ sellerId: seller.id, quantity: 5, price: 100 });
    ...
    expect(JSON.stringify(buyRes.body)).toContain('Kendi ürününüzü satın alamazsınız');

    const checkoutRes = await checkout(seller, product.id, 1, sellerAddr.id).expect(403);
    expect(JSON.stringify(checkoutRes.body)).toContain('Kendi ürününüzü satın alamazsınız');

    // Sepet: cart.service BadRequestException (400) atar; mesaj aynı.
    const cartRes = await addToCart(seller, product.id, 1).expect(400);
    expect(JSON.stringify(cartRes.body)).toContain('Kendi ürününüzü satın alamazsınız');
  ...
  scenario('STK-130', async () => {
    ...
    const first = await checkout(buyer, product.id, 3, addr.id, key).expect(201);
    const second = await checkout(buyer, product.id, 3, addr.id, key).expect(201);
  ...
  scenario('STK-141', async () => {
    ...
    const unitTotal = Number(order!.totalAmount) / (order!.quantity ?? 3);
    const refund = await paymentSvc().processRefund(order!.id, unitTotal, { refundQuantity: 1 });
  ...
  scenario('STK-150', async () => {
    ...
    const outProduct = await makeProduct({ sellerId: (await createUser(ctx.module, { isSeller: true })).id, quantity: 1 });
    await prisma().product.update({ where: { id: outProduct.id }, data: { reservedQuantity: 1 } });
    const buyRes = await buyNow(buyer, outProduct.id, addr.id).expect(400);
    expect(JSON.stringify(buyRes.body)).toMatch(/stokta bulunmamaktadır|satışta değil/);

    // 2) Fazla checkout.
    const checkoutRes = await checkout(buyer, product.id, 6, addr.id).expect(400);
    expect(checkoutRes.body.message).toMatch(/yeterli stok yok \(istenen 6, mevcut 2\)/);

    // 3) Sepete fazla ekleme.
    const cartRes = await addToCart(buyer, product.id, 3).expect(400);
    expect(JSON.stringify(cartRes.body)).toContain('Bu üründen en fazla 2 adet sipariş verilebilir');
  });

  scenario.skip('STK-151', 'Saf UI (web/mobile/admin stok-bitti/yükleniyor/hata durumları). Frontend kaynağı bu görevde yok; API uyarı alanları STK-103/150 ile kapsandı.');
  ...
  scenario('STK-160', async () => {
    ...
    const create = (body: Record<string, unknown>) =>
      request(server()).post('/api/products').set(authHeader(seller)).send(body);

    const zero = await create({ ...base(), quantity: 0 }).expect(400);
    expect(JSON.stringify(zero.body)).toContain('Stok miktarı en az 1 olmalıdır');
    const neg = await create({ ...base(), quantity: -5 }).expect(400);
    expect(JSON.stringify(neg.body)).toContain('Stok miktarı en az 1 olmalıdır');
    ...
    if (omitted.status === 201) {
      const prod = await prisma().product.findUnique({ where: { id: omitted.body.id } });
  ...
  scenario('STK-161', async () => {
    ...
    await request(server()).post('/api/payments/callback/paytr').send(forged).expect(200); // PayTR 'OK' döner ama işlenmez
  ...
  scenario('STK-163', async () => {
    ...
    const buyers = await Promise.all(Array.from({ length: 10 }, () => createUser(ctx.module)));
    const addrs = await Promise.all(buyers.map((b) => createAddress({ userId: b.id })));
