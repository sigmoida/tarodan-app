...
describe('04 — Ürün/İlan & Katalog (PRD)', () => {
  ...
  const postProduct = (user: { accessToken: string }, body: Record<string, unknown>) =>
  ...
  describe('POST /api/products — oluşturma', () => {
    ...
    scenario('PRD-003', async () => {
      ...
      const res = await postProduct(seller, validProduct({
        price: 500,
        condition: 'like_new',
    ...
    scenario('PRD-004', async () => {
      const seller = await mkSeller();
      const res = await postProduct(seller, validProduct({ quantity: null })).expect(201);
      const prisma = getPrisma();
      const row = await prisma.product.findUnique({ where: { id: res.body.id } });
    ...
    scenario('PRD-011', async () => {
      const seller = await mkSeller();
      const res = await postProduct(seller, validProduct({ title: 'Hi' })).expect(400);
      expect(JSON.stringify(res.body.message)).toContain('Başlık en az 5 karakter olmalıdır');
    });

    scenario('PRD-012', async () => {
      const seller = await mkSeller();
      const over = await postProduct(seller, validProduct({ title: 'x'.repeat(201) })).expect(400);
      expect(JSON.stringify(over.body.message)).toContain('Başlık en fazla 200 karakter olabilir');
      await postProduct(seller, validProduct({ title: 'x'.repeat(200) })).expect(201);
    });

    scenario('PRD-013', async () => {
      const seller = await mkSeller();
      const res = await postProduct(seller, validProduct({ description: 'x'.repeat(5001) })).expect(400);
      expect(JSON.stringify(res.body.message)).toContain('Açıklama en fazla 5000 karakter olabilir');
    });

    scenario('PRD-014', async () => {
      const seller = await mkSeller();
      const zero = await postProduct(seller, validProduct({ price: 0 })).expect(400);
      expect(JSON.stringify(zero.body.message)).toContain('Fiyat en az 1 TL olmalıdır');
    ...
    scenario('PRD-015', async () => {
      const seller = await mkSeller();
      const res = await postProduct(seller, validProduct({ price: 'çok-pahalı' })).expect(400);
      expect(JSON.stringify(res.body.message)).toContain('Fiyat sayı olmalıdır');
    });

    scenario('PRD-016', async () => {
      const seller = await mkSeller();
      const res = await postProduct(seller, validProduct({ condition: 'brand_new' })).expect(400);
      expect(JSON.stringify(res.body.message)).toContain('Geçerli bir durum seçiniz');
    ...
    scenario('PRD-017', async () => {
      const seller = await mkSeller();
      const res = await postProduct(seller, validProduct({ categoryId: '123-not-uuid' })).expect(400);
      expect(JSON.stringify(res.body.message)).toContain('Geçerli bir kategori ID giriniz');
    });
    ...
    scenario('PRD-018', async () => {
      ...
      const res = await postProduct(seller, validProduct({ categoryId: MISSING_UUID })).expect(400);
    ...
    scenario('PRD-019', async () => {
      ...
      await postProduct(seller, validProduct({ isSet: true, bundleSize: 1 })).expect(400);
      // isSet:false + bundleSize:5 → 201; service bundleSize=null yapar.
      const res = await postProduct(seller, validProduct({ isSet: false, bundleSize: 5 })).expect(201);
      const prisma = getPrisma();
      const row = await prisma.product.findUnique({ where: { id: res.body.id } });
    ...
    scenario('PRD-021', async () => {
      ...
      const res = await postProduct(seller, validProduct({ status: 'active', hackField: 1 })).expect(201);
  ...
  describe('POST /api/products — yetki & limit & fiyat', () => {
    scenario('PRD-030', async () => {
      await request(server()).post('/api/products').send(validProduct()).expect(401);
    });

    scenario('PRD-031', async () => {
      const seller = await mkSeller();
      const prisma = getPrisma();
      await prisma.user.update({ where: { id: seller.id }, data: { isBanned: true } });
    ...
    scenario('PRD-032', async () => {
      ...
      const res = await postProduct(seller, validProduct()).expect(403);
      expect(JSON.stringify(res.body.message)).toContain('İlan limitinize ulaştınız');
    });

    scenario('PRD-033', async () => {
      // counts.activeListings = pending+active+reserved; summary.used aynı; remaining = max - active.
      const prisma = getPrisma();
      await prisma.membershipTier.updateMany({ where: { type: 'free' }, data: { maxTotalListings: 50 } });
      const seller = await mkSeller();
      await mkProduct({ sellerId: seller.id, status: ProductStatus.pending });
      await mkProduct({ sellerId: seller.id, status: ProductStatus.active });
    ...
    scenario('PRD-034', async () => {
      // Premium sınırsız: platform setting premium_listing_limit=-1 → remaining=-1, canCreate=true.
      const prisma = getPrisma();
      await prisma.membershipTier.upsert({
        where: { type: 'premium' as any },
        update: { isActive: true },
        create: {
          type: 'premium' as any, name: 'Test Premium', monthlyPrice: 100, yearlyPrice: 1000,
          maxFreeListings: 1000, maxTotalListings: 1000, maxImagesPerListing: 30,
          canCreateCollections: true, canTrade: true, isAdFree: true, isActive: true,
        },
      });
      await prisma.platformSetting.upsert({
        where: { settingKey: 'premium_listing_limit' },
        update: { settingValue: '-1' },
        create: { settingKey: 'premium_listing_limit', settingValue: '-1', settingType: 'number' },
      });
      const seller = await mkSeller();
      const premiumTier = await prisma.membershipTier.findFirst({ where: { type: 'premium' as any } });
      const now = new Date();
      await prisma.userMembership.create({
        data: {
          userId: seller.id, tierId: premiumTier!.id, status: 'active',
          currentPeriodStart: now, currentPeriodEnd: new Date(now.getTime() + 30 * 864e5), autoRenew: false,
        },
      });
      // Çok sayıda ilan
      for (let i = 0; i < 5; i++) await mkProduct({ sellerId: seller.id, status: ProductStatus.active });
    ...
    scenario('PRD-040', async () => {
      ...
      const prisma = getPrisma();
      await prisma.membershipTier.updateMany({ where: { type: 'free' }, data: { maxImagesPerListing: 3 } });
      const seller = await mkSeller();
      const images = Array.from({ length: 4 }, (_, i) => ({ cardKey: `c${i}`, detailKey: `d${i}` }));
      const res = await postProduct(seller, validProduct({ images })).expect(400);
      expect(JSON.stringify(res.body.message)).toContain('4 görsel gönderdiniz');
    });

    scenario('PRD-041', async () => {
      // DTO hard limit @ArrayMaxSize(10): 11 görsel → 400.
      const seller = await mkSeller();
      const images = Array.from({ length: 11 }, (_, i) => ({ cardKey: `c${i}`, detailKey: `d${i}` }));
      const res = await postProduct(seller, validProduct({ images })).expect(400);
      expect(JSON.stringify(res.body.message)).toContain('En fazla 10 resim yüklenebilir');
    });

    scenario('PRD-042', async () => {
      // Nested validation: detailKey eksik → 400.
      const seller = await mkSeller();
      await postProduct(seller, validProduct({ images: [{ cardKey: 'x' }] })).expect(400);
    });
    ...
    scenario('PRD-050', async () => {
      ...
      const seller = await mkSeller();
      const lo = await postProduct(seller, validProduct({ price: 49 })).expect(400);
      expect(JSON.stringify(lo.body.message)).toContain('minimum 50 TL');
      const hi = await postProduct(seller, validProduct({ price: 100001 })).expect(400);
    ...
    scenario('PRD-051', async () => {
      ...
      const seller = await mkSeller();
      const p = await mkProduct({ sellerId: seller.id, price: 200, status: ProductStatus.active });
      const res = await request(server())
    ...
    scenario('PRD-053', async () => {
      ...
      const seller = await mkSeller();
      const p = await mkProduct({ sellerId: seller.id, status: ProductStatus.active, quantity: 3 });
      await request(server())
        .patch(`/api/products/${p.id}`)
        .set(authHeader(seller))
        .send({ quantity: 0 })
    ...
    scenario('PRD-055', async () => {
      ...
      const seller = await mkSeller();
      const res = await postProduct(seller, validProduct({ price: 99.999 })).expect(201);
      const prisma = getPrisma();
      const row = await prisma.product.findUnique({ where: { id: res.body.id } });
  ...
  describe('PATCH /api/products/:id — güncelleme', () => {
    scenario('PRD-060', async () => {
      const seller = await mkSeller();
      const p = await mkProduct({ sellerId: seller.id, status: ProductStatus.active });
      const res = await request(server())
        .patch(`/api/products/${p.id}`)
        .set(authHeader(seller))
    ...
    scenario('PRD-061', async () => {
      ...
      const res = await request(server())
        ...
        .send({ title: 'Hacked' })
        .expect(403);
      expect(JSON.stringify(res.body.message)).toContain('Bu ürünü düzenleme yetkiniz yok');
    });

    scenario('PRD-062', async () => {
      // Satıcı DOĞRUDAN aktifleştiremez: pending ilana status:active → pending kalır.
      const seller = await mkSeller();
      const p = await mkProduct({ sellerId: seller.id, status: ProductStatus.pending });
      await request(server())
        .patch(`/api/products/${p.id}`)
        .set(authHeader(seller))
        .send({ status: 'active' })
    ...
    scenario('PRD-063', async () => {
      ...
      const seller = await mkSeller();
      const p = await mkProduct({ sellerId: seller.id, status: ProductStatus.active, quantity: 5 });
      await request(server())
        .patch(`/api/products/${p.id}`)
        .set(authHeader(seller))
    ...
    scenario('PRD-064', async () => {
      const seller = await mkSeller();
      const p = await mkProduct({ sellerId: seller.id, status: ProductStatus.reserved });
      const res = await request(server())
        .patch(`/api/products/${p.id}`)
        .set(authHeader(seller))
        .send({ title: 'x-değişiklik' })
        .expect(400);
      expect(JSON.stringify(res.body.message)).toContain('Rezerve edilmiş ürünler güncellenemez');
    });

    scenario('PRD-065', async () => {
      const seller = await mkSeller();
      const p = await mkProduct({ sellerId: seller.id, status: ProductStatus.deleted });
      const res = await request(server())
        .patch(`/api/products/${p.id}`)
        .set(authHeader(seller))
        .send({ status: 'active', quantity: 3 })
        .expect(400);
      expect(JSON.stringify(res.body.message)).toContain('kaldırılmış ve yeniden açılamaz');
    });

    scenario('PRD-066', async () => {
      // Sold/inactive ilanı stokla yeniden açma → pending (admin onayına gider), quantity=3.
      const seller = await mkSeller();
      const p = await mkProduct({ sellerId: seller.id, status: ProductStatus.sold, quantity: 0 });
      await request(server())
        .patch(`/api/products/${p.id}`)
        .set(authHeader(seller))
        .send({ status: 'active', quantity: 3 })
    ...
    scenario('PRD-067', async () => {
      const seller = await mkSeller();
      const p = await mkProduct({ sellerId: seller.id, status: ProductStatus.inactive, quantity: 0 });
      const res = await request(server())
        .patch(`/api/products/${p.id}`)
        .set(authHeader(seller))
        .send({ status: 'active', quantity: 0 })
        .expect(400);
      expect(JSON.stringify(res.body.message)).toContain('stok miktarı belirleyin');
    });

    scenario('PRD-068', async () => {
      // Takas açma: free tier canTrade seedBaseline'da true. canTrade=false tier ile 400 üret.
      const prisma = getPrisma();
      await prisma.membershipTier.updateMany({ where: { type: 'free' }, data: { canTrade: false } });
      const seller = await mkSeller();
      const p = await mkProduct({ sellerId: seller.id, status: ProductStatus.active });
      const res = await request(server())
        .patch(`/api/products/${p.id}`)
        ...
        .expect(400);
      expect(JSON.stringify(res.body.message)).toContain('Takas özelliği için Premium üyelik gereklidir');
    });

    scenario('PRD-069', async () => {
      // Düzenleme cache + ES sync tetikler → yeni başlık aramada bulunur.
      const seller = await mkSeller();
      const admin = await createAdminUser(ctx.module, { email: `adm-${Date.now()}@t.local`, role: AdminRole.admin });
      const token = `prdsearch${Date.now().toString(36)}`;
      const p = await mkProduct({ sellerId: seller.id, status: ProductStatus.active, title: 'Eski Başlık' });
      await request(server())
        .patch(`/api/products/${p.id}`)
        .set(authHeader(seller))
        ...
        .expect(200);
      await reindex(admin);
      const res = await request(server()).get(`/api/products?search=${token}`).expect(200);
  ...
  describe('DELETE /api/products/:id — silme', () => {
    ...
    scenario('PRD-081', async () => {
      ...
      const res = await request(server())
        ...
        .expect(403);
      expect(JSON.stringify(res.body.message)).toContain('Bu ürünü silme yetkiniz yok');
    });

    scenario('PRD-082', async () => {
      const seller = await mkSeller();
      const sold = await mkProduct({ sellerId: seller.id, status: ProductStatus.sold });
      const reserved = await mkProduct({ sellerId: seller.id, status: ProductStatus.reserved });
      for (const p of [sold, reserved]) {
        const res = await request(server())
          .delete(`/api/products/${p.id}`)
          .set(authHeader(seller))
          .expect(400);
        expect(JSON.stringify(res.body.message)).toContain('Satılmış veya rezerve edilmiş ürünler silinemez');
      }
    });
    ...
    scenario('PRD-084', async () => {
      ...
      const p = await mkProduct({ sellerId: seller.id, status: ProductStatus.active, title: `${token} Silinecek` });
      await reindex(admin);
      let res = await request(server()).get(`/api/products?search=${token}`).expect(200);
      expect(res.body.data.some((x: any) => x.id === p.id)).toBe(true);
      await request(server()).delete(`/api/products/${p.id}`).set(authHeader(seller)).expect(200);
      await reindex(admin);
      res = await request(server()).get(`/api/products?search=${token}`).expect(200);
  ...
  describe('GET detay & görünürlük', () => {
    scenario('PRD-090', async () => {
      const seller = await mkSeller();
      const p = await mkProduct({ sellerId: seller.id, status: ProductStatus.active });
      const res = await request(server()).get(`/api/products/${p.id}`).expect(200);
      expect(res.body.id).toBe(p.id);
    });

    scenario('PRD-091', async () => {
      const seller = await mkSeller();
      const pending = await mkProduct({ sellerId: seller.id, status: ProductStatus.pending });
      const rejected = await mkProduct({ sellerId: seller.id, status: ProductStatus.rejected });
      const inactiveInStock = await mkProduct({ sellerId: seller.id, status: ProductStatus.inactive, quantity: 5 });
      for (const p of [pending, rejected, inactiveInStock]) {
        const res = await request(server()).get(`/api/products/${p.id}`).expect(404);
    ...
    scenario('PRD-092', async () => {
      ...
      const seller = await mkSeller();
      const sold = await mkProduct({ sellerId: seller.id, status: ProductStatus.sold, quantity: 0 });
      const outOfStock = await mkProduct({ sellerId: seller.id, status: ProductStatus.inactive, quantity: 0 });
    ...
    scenario('PRD-094', async () => {
      ...
      const res = await request(server())
        ...
        .set(authHeader(attacker))
        .expect(403);
      expect(JSON.stringify(res.body.message)).toContain('Bu ürünü görüntüleme yetkiniz yok');
    });

    scenario('PRD-095', async () => {
      // Public liste statü filtresi (status verilmez): yalnız active(stoklu) + inactive+qty=0 + sold.
      const seller = await mkSeller();
      const active = await mkProduct({ sellerId: seller.id, status: ProductStatus.active, quantity: 5 });
      const soldOut = await mkProduct({ sellerId: seller.id, status: ProductStatus.inactive, quantity: 0 });
      const sold = await mkProduct({ sellerId: seller.id, status: ProductStatus.sold, quantity: 0 });
      const pending = await mkProduct({ sellerId: seller.id, status: ProductStatus.pending });
      const reserved = await mkProduct({ sellerId: seller.id, status: ProductStatus.reserved });
      const rejected = await mkProduct({ sellerId: seller.id, status: ProductStatus.rejected });
      ...
      const res = await request(server()).get('/api/products?limit=100').expect(200);
  ...
  describe('like / view / stats', () => {
    ...
    scenario('PRD-101', async () => {
      ...
      const res = await request(server())
        ...
        .set(authHeader(seller))
        .expect(400);
      expect(JSON.stringify(res.body.message)).toContain('Kendi ürününüzü beğenemezsiniz');
    });
    ...
    scenario('PRD-102', async () => {
      ...
      const buyer = await createUser(ctx.module);
      const p = await mkProduct({ sellerId: seller.id, status: ProductStatus.active });
      await request(server()).post(`/api/products/${p.id}/like`).set(authHeader(buyer)).expect(201);
      const res = await request(server())
        .post(`/api/products/${p.id}/like`)
        .set(authHeader(buyer))
        .expect(400);
      expect(JSON.stringify(res.body.message)).toContain('Bu ürünü zaten beğendiniz');
    });

    scenario('PRD-103', async () => {
      const seller = await mkSeller();
      const buyer = await createUser(ctx.module);
      const p = await mkProduct({ sellerId: seller.id, status: ProductStatus.active, likeCount: 0 });
      await request(server()).post(`/api/products/${p.id}/like`).set(authHeader(buyer)).expect(201);
      const res = await request(server())
    ...
    scenario('PRD-104', async () => {
      ...
      const res = await request(server())
        ...
        .set(authHeader(buyer))
        .expect(400);
      expect(JSON.stringify(res.body.message)).toContain('Bu ürünü beğenmemişsiniz');
    });
    ...
    scenario('PRD-106', async () => {
      ...
      const notFound = await request(server())
        ...
        .set(authHeader(buyer))
        .expect(404);
      expect(JSON.stringify(notFound.body.message)).toContain('Ürün bulunamadı');
    ...
    scenario('PRD-110', async () => {
      const seller = await mkSeller();
      const buyer = await createUser(ctx.module);
      const p = await mkProduct({ sellerId: seller.id, status: ProductStatus.active, viewCount: 0 });
      const res = await request(server())
    ...
    scenario('PRD-111', async () => {
      ...
      await request(server()).post(`/api/products/${p.id}/view`).set(authHeader(buyer)).set('User-Agent', ua).expect(201);
      const second = await request(server()).post(`/api/products/${p.id}/view`).set(authHeader(buyer)).set('User-Agent', ua).expect(201);
    ...
    scenario('PRD-113', async () => {
      ...
      const seller = await mkSeller();
      const p = await mkProduct({ sellerId: seller.id, status: ProductStatus.active, viewCount: 7 });
      const res = await request(server())
        .post(`/api/products/${p.id}/view`)
        .set(authHeader(seller))
        .set('User-Agent', 'Mozilla/5.0 (Test Browser)')
    ...
    scenario('PRD-114', async () => {
      ...
      const seller = await mkSeller();
      const attacker = await createUser(ctx.module);
      const p = await mkProduct({ sellerId: seller.id, status: ProductStatus.active, viewCount: 3, likeCount: 2 });
      const forbidden = await request(server())
        .get(`/api/products/${p.id}/stats`)
        .set(authHeader(attacker))
        .expect(403);
      expect(JSON.stringify(forbidden.body.message)).toContain('Bu ürünün istatistiklerini görme yetkiniz yok');
      const ok = await request(server())
        .get(`/api/products/${p.id}/stats`)
        .set(authHeader(seller))
        .expect(200);
      expect(ok.body).toEqual(expect.objectContaining({
        viewCount: 3, likeCount: 2, offersCount: 0, ordersCount: 0,
      }));
    });
  });
  ...
  describe('boost / öne çıkarma', () => {
    ...
    async function ensurePlatformSeller() {
      const prisma = getPrisma();
      const existing = await prisma.user.findFirst({ where: { email: 'platform@tarodan.com' } });
      if (existing) return existing;
      return (await createUser(ctx.module, {
        email: 'platform@tarodan.com', isSeller: true, sellerType: 'platform',
      }));
    }

    scenario('PRD-120', async () => {
      const res = await request(server()).get('/api/products/boost/pricing').expect(200);
    ...
    scenario('PRD-121', async () => {
      ...
      const prisma = getPrisma();
      const boost = await prisma.productBoost.findUnique({ where: { id: res.body.boostId } });
      expect(boost?.status).toBe('pending');
      // boost-<id> sanal ürün + pending_payment order oluşur.
      const virt = await prisma.product.findUnique({ where: { id: `boost-${res.body.boostId}` } });
    ...
    scenario('PRD-122', async () => {
      ...
      const bad = await request(server())
        ...
        .send({ durationDays: 5 })
        .expect(400);
      expect(JSON.stringify(bad.body.message)).toContain('Geçerli bir boost süresi seçiniz (3, 7 veya 30 gün)');
      await request(server())
        .post(`/api/products/${p.id}/boost/initiate`)
        .set(authHeader(seller))
        .send({ durationDays: 0 })
    ...
    scenario('PRD-123', async () => {
      ...
      const res = await request(server())
        ...
        .send({ durationDays: 7 })
        .expect(403);
      expect(JSON.stringify(res.body.message)).toContain('Sadece kendi ilanınızı öne çıkarabilirsiniz');
    });
    ...
    scenario('PRD-124', async () => {
      ...
      const res = await request(server())
        ...
        .send({ durationDays: 7 })
        .expect(400);
      expect(JSON.stringify(res.body.message)).toContain('Sadece aktif (yayında) ilanlar öne çıkarılabilir');
    });
    ...
    scenario('PRD-125', async () => {
      ...
      await prisma.platformSetting.upsert({
        ...
        create: { settingKey: 'boost_enabled', settingValue: 'false', settingType: 'boolean' },
      });
      const seller = await mkSeller();
      const p = await mkProduct({ sellerId: seller.id, status: ProductStatus.active });
      const res = await request(server())
        ...
        .send({ durationDays: 7 })
        .expect(400);
      expect(JSON.stringify(res.body.message)).toContain('Öne çıkarma şu anda kullanılamıyor');
    });
    ...
    scenario('PRD-126', async () => {
      ...
      const p1 = await mkProduct({ sellerId: plain.id, status: ProductStatus.active });
      const r1 = await request(server())
        .post(`/api/products/${p1.id}/boost/initiate`)
        .set(authHeader(plain))
        .send({ durationDays: 7, autoRenew: true })
        .expect(201);
      const b1 = await prisma.productBoost.findUnique({ where: { id: r1.body.boostId } });
      expect(b1?.autoRenew).toBe(false);

      // Premium satıcı → autoRenew=true.
      const prem = await createUser(ctx.module, { isSeller: true, premium: true });
      const p2 = await mkProduct({ sellerId: prem.id, status: ProductStatus.active });
      const r2 = await request(server())
        .post(`/api/products/${p2.id}/boost/initiate`)
        .set(authHeader(prem))
        .send({ durationDays: 7, autoRenew: true })
        .expect(201);
      const b2 = await prisma.productBoost.findUnique({ where: { id: r2.body.boostId } });
    ...
    scenario('PRD-127', async () => {
      ...
      expect(my.body[0]).toEqual(expect.objectContaining({
  ...
  describe('admin onay/ret', () => {
    scenario('PRD-140', async () => {
      ...
      // Audit log yazıldı.
      const audit = await prisma.auditLog.findFirst({ where: { entityType: 'Product', entityId: p.id, action: 'product_approve' } });
    ...
    scenario('PRD-141', async () => {
      ...
      expect(JSON.stringify(res.body.message)).toContain('Sadece bekleyen ürünler onaylanabilir');
    });
    ...
    scenario('PRD-143', async () => {
      ...
      const seller = await mkSeller();
      const p = await mkProduct({ sellerId: seller.id, status: ProductStatus.rejected });
      await request(server())
        .patch(`/api/products/${p.id}`)
        .set(authHeader(seller))
    ...
    scenario('PRD-144', async () => {
      ...
      const seller = await mkSeller();
      const moderator = await createAdminUser(ctx.module, { email: `mod-${Date.now()}@t.local`, role: AdminRole.moderator });
      const normalUser = await createUser(ctx.module);
      const p1 = await mkProduct({ sellerId: seller.id, status: ProductStatus.pending });
      ...
      const p2 = await mkProduct({ sellerId: seller.id, status: ProductStatus.pending });
    ...
    scenario('PRD-145', async () => {
      ...
      const empty = await request(server())
        ...
        .send({ ids: [] })
        .expect(400);
      expect(JSON.stringify(empty.body.message)).toContain('En az bir ürün seçilmelidir');
      // Dolu → sonuç dizisi.
      const p1 = await mkProduct({ sellerId: seller.id, status: ProductStatus.pending });
      const p2 = await mkProduct({ sellerId: seller.id, status: ProductStatus.pending });
      const res = await request(server())
  ...
  describe('liste / filtre / sıralama', () => {
    scenario('PRD-160', async () => {
      ...
      const empty = await request(server()).get('/api/products?page=1&limit=20').expect(200);
      expect(empty.body.data).toEqual([]);
      expect(empty.body.meta).toEqual(expect.objectContaining({
        total: 0, page: 1, limit: 20, totalPages: expect.any(Number),
      }));
    ...
    scenario('PRD-162', async () => {
      ...
      const prisma = getPrisma();
      const cat2 = await prisma.category.create({ data: { name: 'Kat2', slug: `kat2-${Date.now()}`, isActive: true } });
      const seller = await mkSeller();
      const match = await mkProduct({ sellerId: seller.id, status: ProductStatus.active, categoryId: baseline.categoryId, brandId: baseline.brandId });
      const other = await mkProduct({ sellerId: seller.id, status: ProductStatus.active, categoryId: cat2.id });

      const byCat = await request(server()).get(`/api/products?categoryId=${baseline.categoryId}&limit=100`).expect(200);
      ...
      const byBrand = await request(server()).get(`/api/products?brandId=${baseline.brandId}&limit=100`).expect(200);
    ...
    scenario('PRD-163', async () => {
      ...
      const match = await mkProduct({
        sellerId: seller.id, status: ProductStatus.active, price: 300,
        condition: ProductCondition.new, isTradeEnabled: true, isSet: true, isPreorder: true, isLimited: true,
      });
      const noMatch = await mkProduct({
        sellerId: seller.id, status: ProductStatus.active, price: 700, condition: ProductCondition.good,
      });
      const res = await request(server())
        .get('/api/products?minPrice=100&maxPrice=500&condition=new&tradeOnly=true&set=true&preOrder=true&limited=true&limit=100')
        .expect(200);
      const ids = new Set(res.body.data.map((x: any) => x.id));
    ...
    scenario('PRD-164', async () => {
      ...
      const scaleGroup = await prisma.attributeGroup.upsert({
        where: { slug: 'scale' }, update: {}, create: { name: 'Ölçek', slug: 'scale', isActive: true },
      });
      const scaleAttr = await prisma.attribute.upsert({
        where: { groupId_slug: { groupId: scaleGroup.id, slug: '1-64' } },
        update: {}, create: { groupId: scaleGroup.id, value: '1:64', slug: '1-64', isActive: true },
      });
      const matGroup = await prisma.attributeGroup.upsert({
        where: { slug: 'material' }, update: {}, create: { name: 'Malzeme', slug: 'material', isActive: true },
      });
      const matAttr = await prisma.attribute.upsert({
        where: { groupId_slug: { groupId: matGroup.id, slug: 'diecast' } },
        update: {}, create: { groupId: matGroup.id, value: 'diecast', slug: 'diecast', isActive: true },
      });
      const seller = await mkSeller();
      const match = await mkProduct({ sellerId: seller.id, status: ProductStatus.active });
      await prisma.productAttribute.create({ data: { productId: match.id, attributeId: scaleAttr.id } });
      await prisma.productAttribute.create({ data: { productId: match.id, attributeId: matAttr.id } });
      const noMatch = await mkProduct({ sellerId: seller.id, status: ProductStatus.active });

      const res = await request(server()).get('/api/products?scale=1:64&material=diecast&limit=100').expect(200);
      const ids = new Set(res.body.data.map((x: any) => x.id));
      expect(ids.has(match.id)).toBe(true);
      expect(ids.has(noMatch.id)).toBe(false);
    });
    ...
    scenario('PRD-165', async () => {
      ...
      const prisma = getPrisma();
      const rarity = await prisma.attributeGroup.create({ data: { name: 'Rarity', slug: `hw-rarity-${Date.now()}`, isActive: true, manufacturerSlug: baseline.manufacturerId } });
      const color = await prisma.attributeGroup.create({ data: { name: 'Color', slug: `hw-color-${Date.now()}`, isActive: true, manufacturerSlug: baseline.manufacturerId } });
      const th = await prisma.attribute.create({ data: { groupId: rarity.id, value: 'TH', slug: 'treasure-hunt', isActive: true } });
      const red = await prisma.attribute.create({ data: { groupId: color.id, value: 'Red', slug: 'red', isActive: true } });
      const seller = await mkSeller();
      const match = await mkProduct({ sellerId: seller.id, status: ProductStatus.active });
      await prisma.productAttribute.create({ data: { productId: match.id, attributeId: th.id } });
      await prisma.productAttribute.create({ data: { productId: match.id, attributeId: red.id } });
      const onlyRarity = await mkProduct({ sellerId: seller.id, status: ProductStatus.active });
      await prisma.productAttribute.create({ data: { productId: onlyRarity.id, attributeId: th.id } });

      const filter = JSON.stringify({ [rarity.slug]: ['treasure-hunt', 'super-treasure-hunt'], [color.slug]: ['red'] });
      const res = await request(server())
      ...
      await request(server()).get('/api/products?attrGroups={bozuk-json&limit=100').expect(200);
    });

    scenario('PRD-166', async () => {
      // Varsayılan relevance + açık sortBy'lar. price_asc doğrulanır.
      const seller = await mkSeller();
      const cheap = await mkProduct({ sellerId: seller.id, status: ProductStatus.active, price: 10, quantity: 5 });
      const mid = await mkProduct({ sellerId: seller.id, status: ProductStatus.active, price: 100, quantity: 5 });
      const pricey = await mkProduct({ sellerId: seller.id, status: ProductStatus.active, price: 1000, quantity: 5 });

      // Varsayılan (sortBy yok) → 200, geçerli liste.
      await request(server()).get('/api/products?limit=100').expect(200);

      const asc = await request(server()).get('/api/products?sortBy=price_asc&limit=100').expect(200);
      ...
      const desc = await request(server()).get('/api/products?sortBy=price_desc&limit=100').expect(200);
      const descIds = desc.body.data.map((x: any) => x.id);
      expect(descIds.indexOf(pricey.id)).toBeLessThan(descIds.indexOf(cheap.id));

      // Diğer sortBy'lar en azından 200 dönmeli.
      for (const s of ['created_desc', 'view_count_desc', 'rating_desc']) {
        await request(server()).get(`/api/products?sortBy=${s}&limit=100`).expect(200);
      }
    });
    ...
    scenario('PRD-167', async () => {
      ...
      const p = await mkProduct({ sellerId: seller.id, status: ProductStatus.active, title: `${token} 911 GT3` });
      await reindex(admin);
      const res = await request(server()).get(`/api/products?search=${token}`).expect(200);
      expect(res.body.data.some((x: any) => x.id === p.id)).toBe(true);

      const none = await request(server()).get('/api/products?search=zzzzhicbulunmayan9999').expect(200);
    ...
    scenario('PRD-168', async () => {
      ...
      const seller = await mkSeller();
      await mkProduct({ sellerId: seller.id, status: ProductStatus.active, viewCount: 500 });
      const res = await request(server()).get('/api/products/popular?limit=10').expect(200);
    ...
    scenario('PRD-169', async () => {
      ...
      const seller = await mkSeller();
      const base = await mkProduct({ sellerId: seller.id, status: ProductStatus.active, categoryId: baseline.categoryId });
      const sibling = await mkProduct({ sellerId: seller.id, status: ProductStatus.active, categoryId: baseline.categoryId });
      const res = await request(server()).get(`/api/products/${base.id}/similar?limit=12`).expect(200);
    ...
    scenario('PRD-170', async () => {
      ...
      const filters = await request(server()).get('/api/products/filters?manufacturer=test-manufacturer').expect(200);
      expect(filters.body.categories).toBeDefined();
      expect(filters.body.brands).toBeDefined();
      const groups = await request(server()).get('/api/products/attribute-groups?manufacturer=test-manufacturer').expect(200);
  ...
  describe('katalog (kategori/marka/üretici/model)', () => {
    ...
    scenario('PRD-181', async () => {
      const byId = await request(server()).get(`/api/categories/${baseline.categoryId}`).expect(200);
      expect(byId.body.id).toBe(baseline.categoryId);
      const bySlug = await request(server()).get('/api/categories/slug/test-category').expect(200);
      expect(bySlug.body.slug).toBe('test-category');
      await request(server()).get('/api/categories/slug/nonexistent-slug').expect(404);
    });
    ...
    scenario('PRD-182', async () => {
      ...
      const admin = await createAdminUser(ctx.module, { email: `adm-${Date.now()}@t.local`, role: AdminRole.admin });
      await request(server()).post('/api/categories').send({ name: 'Guest Kat', slug: `g-${Date.now()}` }).expect(401);
      await request(server())
        .post('/api/categories')
        .set(authHeader(admin))
    ...
    scenario('PRD-184', async () => {
      const list = await request(server()).get('/api/manufacturers').expect(200);
      expect(Array.isArray(list.body)).toBe(true);
      await request(server()).get('/api/manufacturers/slug/test-manufacturer').expect(200);
    ...
    scenario('PRD-185', async () => {
      ...
      const model = await prisma.carModel.create({
        data: { brandId: baseline.brandId, name: 'BMW M3 E30', slug: `bmw-m3-e30-${Date.now()}`, isActive: true },
      });
      const list = await request(server()).get('/api/car-models').expect(200);
      expect(Array.isArray(list.body)).toBe(true);
      await request(server()).get('/api/car-models?brand=test-brand').expect(200);
      await request(server()).get(`/api/car-models/${model.slug}`).expect(200);
      await request(server()).get('/api/car-models/nonexistent-model-slug').expect(404);
    });
  });
  ...
  describe('eşzamanlılık & zamanlanmış', () => {
    scenario('PRD-200', async () => {
      ...
      const [a, b] = await Promise.all([
        request(server()).patch(`/api/products/${p.id}`).set(authHeader(seller)).send({ title: 'A çakışma testi' }),
        request(server()).patch(`/api/products/${p.id}`).set(authHeader(seller)).send({ title: 'B çakışma testi' }),
      ]);
      const codes = [a.status, b.status].sort();
      expect(codes).toEqual([200, 409]);
      const conflict = a.status === 409 ? a : b;
      expect(JSON.stringify(conflict.body.message)).toContain('başka bir işlem tarafından güncellendi');
    });
    ...
    scenario('PRD-201', async () => {
      ...
      const p = await mkProduct({ sellerId: seller.id, status: ProductStatus.active, likeCount: 0 });
      const [a, b] = await Promise.all([
        request(server()).post(`/api/products/${p.id}/like`).set(authHeader(buyer)),
        request(server()).post(`/api/products/${p.id}/like`).set(authHeader(buyer)),
      ]);
      ...
      const prisma = getPrisma();
      const likeCount = await prisma.productLike.count({ where: { productId: p.id, userId: buyer.id } });
    ...
    scenario('PRD-202', async () => {
      ...
      const ua = 'Mozilla/5.0 (Test Browser)';
      await Promise.all([
        request(server()).post(`/api/products/${p.id}/view`).set(authHeader(buyer)).set('User-Agent', ua),
        request(server()).post(`/api/products/${p.id}/view`).set(authHeader(buyer)).set('User-Agent', ua),
      ]);
      const prisma = getPrisma();
      const row = await prisma.product.findUnique({ where: { id: p.id } });
    ...
    scenario('PRD-203', async () => {
      ...
      const prisma = getPrisma();
      const existing = await prisma.user.findFirst({ where: { email: 'platform@tarodan.com' } });
      if (!existing) await createUser(ctx.module, { email: 'platform@tarodan.com', isSeller: true, sellerType: 'platform' });
      const seller = await mkSeller();
      const p = await mkProduct({ sellerId: seller.id, status: ProductStatus.active });
      const [a, b] = await Promise.all([
        request(server()).post(`/api/products/${p.id}/boost/initiate`).set(authHeader(seller)).send({ durationDays: 7 }),
        request(server()).post(`/api/products/${p.id}/boost/initiate`).set(authHeader(seller)).send({ durationDays: 7 }),
      ]);
      ...
      const boosts = await prisma.productBoost.count({ where: { productId: p.id } });
    ...
    scenario('PRD-220', async () => {
      ...
      const p = await mkProduct({ sellerId: seller.id, status: ProductStatus.active, createdAt: old });
    ...
    scenario('PRD-221', async () => {
      ...
      const p = await mkProduct({ sellerId: seller.id, status: ProductStatus.active, rankTier: 2, boostedUntil: past });
      const prisma = getPrisma();
      await prisma.productBoost.create({
        data: { productId: p.id, userId: seller.id, durationDays: 7, price: new Prisma.Decimal(59), status: 'active', endsAt: past },
      });
      const scheduler = ctx.module.get(ProductSchedulerService);
      await scheduler.runExpireBoosts();
      const row = await prisma.product.findUnique({ where: { id: p.id } });
      expect(row?.rankTier).toBeLessThan(2);
      const boost = await prisma.productBoost.findFirst({ where: { productId: p.id } });
  ...
  describe('i18n / parite / güvenlik', () => {
    scenario('PRD-240', async () => {
      ...
      const seller = await mkSeller();
      const shortTitle = await postProduct(seller, validProduct({ title: 'Hi' })).expect(400);
      expect(JSON.stringify(shortTitle.body.message)).toContain('Başlık en az 5 karakter olmalıdır');
      const lowPrice = await postProduct(seller, validProduct({ price: 0 })).expect(400);
      expect(JSON.stringify(lowPrice.body.message)).toContain('Fiyat en az 1 TL olmalıdır');
      const badCat = await postProduct(seller, validProduct({ categoryId: MISSING_UUID })).expect(400);
      expect(JSON.stringify(badCat.body.message)).toContain('Geçersiz kategori');
    });
    ...
    scenario('PRD-260', async () => {
      ...
      await request(server()).patch(`/api/products/${p.id}`).set(authHeader(attacker)).send({ title: 'Ele geçirildi' }).expect(403);
      await request(server()).delete(`/api/products/${p.id}`).set(authHeader(attacker)).expect(403);
      await request(server()).post(`/api/products/${p.id}/boost/initiate`).set(authHeader(attacker)).send({ durationDays: 7 }).expect(403);
      await request(server()).get(`/api/products/${p.id}/stats`).set(authHeader(attacker)).expect(403);
      await request(server()).get(`/api/products/my/${p.id}`).set(authHeader(attacker)).expect(403);
    });

    scenario('PRD-261', async () => {
      // Create DTO'da status alanı yok → whitelist ayıklar → ilan pending kalır.
      const seller = await mkSeller();
      const res = await postProduct(seller, validProduct({ status: 'active' })).expect(201);
    ...
    scenario('PRD-263', async () => {
      ...
      const res = await postProduct(seller, validProduct({ title: xss })).expect(201);
      const prisma = getPrisma();
      const row = await prisma.product.findUnique({ where: { id: res.body.id } });
