 ...
 *   - GET /orders/commission-preview: kural eşleme önceliği, min/max clamp, yuvarlama, seller-type mapping
 ...
 *   - seedBaseline() komisyon kuralı OLUŞTURMAZ → preview/ledger testleri kuralları inline yaratır
 *     (admin API veya getPrisma()). Manifest'teki "verified-rule/vintage-rule/platform-rule"
 *     demo seed adları yerine eşdeğer kurallar kurulur; davranış aynıdır.
...
describe('11 — Komisyon & Ödeme/Payout (COM)', () => {
  ...
  const postRule = (adm: { accessToken: string }, body: Record<string, unknown>) =>
    request(server()).post('/api/admin/commission-rules').set(authHeader(adm)).send(body);
  ...
  async function seedRule(opts: {
    ...
    return prisma.commissionRule.create({
      data: {
        ...
        appliesTo: opts.appliesTo ?? CommissionAppliesTo.SELLER,
        ...
        sellerMin: opts.sellerMin != null ? new Prisma.Decimal(opts.sellerMin) : null,
        sellerMax: opts.sellerMax != null ? new Prisma.Decimal(opts.sellerMax) : null,
        percentage: new Prisma.Decimal(opts.sellerRate ?? 0),
  ...
  /**
   * Kullanıcıya aktif premium/business tier üyeliği ekle → mapSellerTypeForCommission
   * PREMIUM'a eşler (order.service.ts:656). seedBaseline'daki 'basic' tier PREMIUM'a
   * eşlemez (yalnız premium/business type'lar), bu yüzden ilgili tier yoksa oluşturulur.
   */
  async function attachTierMembership(userId: string, type: 'premium' | 'business') {
    const prisma = getPrisma();
    const tier =
  ...
  function preview(user: { accessToken: string }, amount: number, categoryId?: string) {
  ...
  async function makeBuyerSellerProduct(opts: { price?: number; sellerType?: 'individual' | 'platform' | 'business' } = {}) {
  ...
  async function payOrder(opts: { price?: number; sellerType?: 'individual' | 'platform' | 'business' } = {}) {
  ...
  describe('POST /api/admin/commission-rules', () => {
    scenario('COM-001', async () => {
      ...
      const res = await postRule(adm, ruleBody({ name: 'Standart Komisyon' })).expect(201);
      expect(res.body.sellerRate).toBe(5);
      expect(res.body.appliesTo).toBe('SELLER');
      expect(res.body.sellerType).toBe('ALL');
      expect(res.body.isActive).toBe(true);

      const prisma = getPrisma();
      const audit = await prisma.auditLog.findFirst({
        where: { action: 'commission_rule_create', entityType: 'CommissionRule', entityId: res.body.id },
      });
    ...
    scenario('COM-002', async () => {
      const adm = await admin();
      // appliesTo=BOTH, buyerRate yok → DTO @ValidateIf 400 (buyerRate zorunlu).
      const res = await postRule(
        adm,
        ruleBody({ name: 'İki Taraflı', appliesTo: 'BOTH', sellerRate: 5 }),
      ).expect(400);
      expect(JSON.stringify(res.body)).toMatch(/buyerRate|BOTH/i);
    });

    scenario('COM-003', async () => {
      const adm = await admin();
      // appliesTo=SELLER, sellerRate yok → DTO @ValidateIf 400.
      const body = ruleBody({ name: 'X', appliesTo: 'SELLER' });
      delete (body as Record<string, unknown>).sellerRate;
      const res = await postRule(adm, body).expect(400);
      expect(JSON.stringify(res.body)).toMatch(/sellerRate|SELLER/i);
    });

    scenario('COM-004', async () => {
      const adm = await admin();
      // Aynı (NULL kategori + ALL) kombinasyonu iki kez → ikinci 400.
      await postRule(adm, ruleBody({ name: 'İlk Default', sellerRate: 5 })).expect(201);
      const res = await postRule(adm, ruleBody({ name: 'İkinci Default', sellerRate: 6 })).expect(400);
      expect(JSON.stringify(res.body)).toContain('zaten bir kural mevcut');
    });
    ...
    scenario('COM-005', async () => {
      ...
      const res = await postRule(
        adm,
        ruleBody({ name: 'X', sellerType: 'FREE', appliesTo: 'SELLER', sellerRate: 5, sellerMin: 100, sellerMax: 10 }),
      ).expect(400);
      expect(JSON.stringify(res.body)).toContain('sellerMin cannot be greater than sellerMax');
    });

    scenario('COM-006', async () => {
      const adm = await admin();
      // NULL+ALL kuralı ve NULL+FREE kuralı var. FREE kuralını ALL'a güncelle → çakışma 400.
      await postRule(adm, ruleBody({ name: 'Default ALL', sellerType: 'ALL', sellerRate: 5 })).expect(201);
      const freeRule = await postRule(
        adm,
        ruleBody({ name: 'FREE Kural', sellerType: 'FREE', sellerRate: 4 }),
      ).expect(201);
      const res = await request(server())
        .patch(`/api/admin/commission-rules/${freeRule.body.id}`)
        .set(authHeader(adm))
        .send({ sellerType: 'ALL' })
        .expect(400);
      expect(JSON.stringify(res.body)).toContain('başka bir kural tarafından kullanılıyor');
    });
  });
  ...
  describe('GET /orders/commission-preview — kural eşleme', () => {
    scenario('COM-007', async () => {
      // Kategori-ALL kuralı (%7) NULL varsayılanı ezer; kategori+FREE yok.
      const seller = await freeSeller();
      await seedRule({ name: 'default', sellerType: CommissionSellerType.ALL, sellerRate: 5 });
      await seedRule({
        name: 'kategori-ALL',
    ...
    scenario('COM-008', async () => {
      // Kategori+FREE tam eşleşme (%3) kategori+ALL (%7)'yi ezer.
      const seller = await freeSeller();
      await seedRule({
        name: 'kategori-ALL',
        sellerType: CommissionSellerType.ALL,
    ...
    scenario('COM-009', async () => {
      // Hiç kural yok → 0 komisyon fallback.
      const seller = await freeSeller();
      const res = await preview(seller, 500, baseline.categoryId).expect(200);
    ...
    scenario('COM-010', async () => {
      // appliesTo=BOTH: seller %5, buyer %2. Preview sellerFee=50; commissionAmount=70.
      const seller = await freeSeller();
      await seedRule({
        name: 'İki Taraflı',
        sellerType: CommissionSellerType.ALL,
    ...
    scenario('COM-011', async () => {
      // Platform satıcı (BUSINESS eşlemesi) → BUSINESS+NULL %0 kural → sellerFee=0.
      const platform = await createUser(ctx.module, { isSeller: true, sellerType: 'platform' });
      await seedRule({ name: 'default', sellerType: CommissionSellerType.ALL, sellerRate: 5 });
      await seedRule({ name: 'platform', sellerType: CommissionSellerType.BUSINESS, sellerRate: 0 });
    ...
    scenario('COM-012', async () => {
      ...
      const seller = await createUser(ctx.module, { isSeller: true, sellerType: 'individual' });
      await attachTierMembership(seller.id, 'premium');
      await seedRule({ name: 'default', sellerType: CommissionSellerType.ALL, sellerRate: 5 });
      // Kural eklenmeden önce: PREMIUM yok → default %5 → sellerFee=50.
      const before = await preview(seller, 1000).expect(200);
      expect(before.body.sellerFeeAmount).toBeCloseTo(50, 2);
      // PREMIUM+NULL %2 ekle → sellerFee=20.
      await seedRule({ name: 'premium', sellerType: CommissionSellerType.PREMIUM, sellerRate: 2 });
      const after = await preview(seller, 1000).expect(200);
      expect(after.body.sellerFeeAmount).toBeCloseTo(20, 2);
    });

    scenario('COM-013', async () => {
      // Bireysel (individual → FREE): NULL+FREE (%4) NULL+ALL (%5)'ten önce eşleşir.
      const seller = await freeSeller();
      await seedRule({ name: 'default', sellerType: CommissionSellerType.ALL, sellerRate: 5 });
      await seedRule({ name: 'verified', sellerType: CommissionSellerType.FREE, sellerRate: 4 });
      const res = await preview(seller, 1000).expect(200);
      expect(res.body.sellerFeeAmount).toBeCloseTo(40, 2);
    });

    scenario('COM-014', async () => {
      // FREE satıcı + kategori: kategori-ALL (%7) NULL+FREE (%4)'ten önce (kategori önceliği).
      const seller = await freeSeller();
      await seedRule({ name: 'verified', sellerType: CommissionSellerType.FREE, sellerRate: 4 });
      await seedRule({
        name: 'kategori-ALL',
    ...
    scenario('COM-015', async () => {
      // Business üyelik → PREMIUM eşlemesi. PREMIUM kuralı yoksa default %5.
      const seller = await createUser(ctx.module, { isSeller: true, sellerType: 'individual' });
  ...
  describe('Sipariş komisyonu & ledger', () => {
    scenario('COM-021', async () => {
      ...
      const product = await createProduct({ sellerId: seller.id, categoryId: baseline.categoryId, price: 1000, quantity: 1 });
      const addr = await createAddress({ userId: buyer.id });
    ...
    scenario('COM-022', async () => {
      ...
      expect(Number(ledger!.sellerCommission)).toBeCloseTo(Number(order!.commissionAmount), 2);
    ...
    scenario('COM-030', async () => {
      ...
      const { orderId } = await buyAndPay(ctx, buyer, product.id, addr.id);
      const prisma = getPrisma();
      const before = await prisma.commissionLedger.findUnique({ where: { orderId } });

      // İkinci başarılı callback (aynı merchantOid).
      const payment = await prisma.payment.findFirst({ where: { orderId }, orderBy: { createdAt: 'desc' } });      await request(server())
      ...
      const after = await prisma.commissionLedger.findUnique({ where: { orderId } });
      expect(after!.status).toBe(before!.status);
      expect(Number(after!.sellerCommission)).toBeCloseTo(Number(before!.sellerCommission), 2);
      expect(await prisma.commissionLedger.count({ where: { orderId } })).toBe(1);
    });
    ...
    scenario('COM-098', async () => {
      ...
      expect(Number(ledger!.sellerCommission)).toBeCloseTo(Number(order!.commissionAmount), 2);
      expect(Number(ledger!.buyerFee)).toBeCloseTo(Number(order!.buyerFeeAmount), 2);
      expect(Number(ledger!.totalPlatformRevenue)).toBeCloseTo(
        Number(ledger!.sellerCommission) + Number(ledger!.buyerFee),
        2,
      );
  ...
  scenario.skip('COM-027', 'Ledger backfill migration SQL (production migration) — e2e app stack üzerinden koşulamaz; unit/migration testi kapsamında.');
  scenario.skip('COM-028', 'Ledger backfill durum eşlemesi migration SQL — e2e app stack dışı.');
  scenario.skip('COM-029', 'Ledger backfill idempotency migration SQL — e2e app stack dışı.');
  ...
  scenario('COM-045', async () => {
    ...
    await getPrisma().sellerBankAccount.create({
      data: { userId: seller.id, accountHolder: 'Mehmet Yılmaz', iban: IBAN_A, isVerified: false },
    });
    ...
    const pendingPayout = await prisma.payoutTransfer.findFirst({ where: { sellerId: seller.id } });
    expect(pendingPayout!.status).toBe(PayoutStatus.pending);
    await prisma.payoutTransfer.update({
      where: { id: pendingPayout!.id },
      data: { status: PayoutStatus.retry_pending, nextRetryAt: new Date(Date.now() - 1000), retryCount: 1 },
    });

    ctx.paytr.reset();
    const result = await ctx.app.get(PayoutSchedulerService).runProcessPayouts();
    ...
    const stats = result.stats as any as { retried: number; processed: number; failed: number; stuck: number };
    expect(stats.retried).toBe(1);
    expect(stats.processed).toBe(1);
    expect(stats.failed).toBe(0);
    expect(ctx.paytr.transferCalls.length).toBe(1);
    const after = await prisma.payoutTransfer.findUnique({ where: { id: pendingPayout!.id } });
  ...
  describe('Escrow & hold release', () => {
    scenario('COM-031', async () => {
      ...
      await prisma.order.update({
        where: { id: orderId },
        data: { status: OrderStatus.delivered, deliveredAt: new Date(), version: { increment: 1 } },
      });
      ...
      const released = await prisma.paymentHold.findUnique({ where: { id: hold!.id } });
    ...
    scenario('COM-032', async () => {
      ...
      const after = await prisma.paymentHold.findUnique({ where: { id: hold!.id } });
  ...
  describe('Payout üretimi & işleme', () => {
    ...
    async function setBankAccount(userId: string, iban = IBAN_A, accountHolder = 'Mehmet Yılmaz') {
      const prisma = getPrisma();
    ...
    scenario('COM-037', async () => {
      ...
      await prisma.sellerBankAccount.create({
        data: { userId: recipient.id, accountHolder: 'Recipient User', iban: IBAN_A, isVerified: false },
      });
      ...
      const created = await ctx.app.get(PayoutService).createPayoutsForReleasedHolds();
      expect(created).toBeGreaterThanOrEqual(1);
      const payout = await prisma.payoutTransfer.findFirst({ where: { tradeCashPaymentId: tcp.id } });
    ...
    scenario('COM-040', async () => {
      ...
      const bank = await prisma.sellerBankAccount.findUnique({ where: { userId: seller.id } });
    ...
    scenario('COM-041', async () => {
      ...
      const transferCall = ctx.paytr.transferCalls.find((c) => c.transferIban === newIban);
    ...
    scenario('COM-049', async () => {
      ...
      await prisma.payoutTransfer.update({
        where: { id: payout!.id },
        data: { status: PayoutStatus.processing, updatedAt: new Date(Date.now() - 60 * 60 * 1000) },
      });
      const stuck = await ctx.app.get(PayoutService).detectStuckProcessingPayouts(30);
    ...
    scenario('COM-052', async () => {
      ...
      const [r1, r2] = await Promise.all([svc.processPendingPayouts(), svc.processPendingPayouts()]);
    ...
    scenario('COM-054', async () => {
      ...
      await ctx.app.get(PayoutService).createPayoutsForReleasedHolds();
      const prisma = getPrisma();
      expect(await prisma.payoutTransfer.count({ where: { paymentHoldId: hold.id } })).toBe(1);
    });
    ...
    scenario('COM-099', async () => {
      ...
      await getPrisma().sellerBankAccount.create({
        data: { userId: seller.id, accountHolder: 'Platform', iban: IBAN_A, isVerified: false },
      });
      const product = await createProduct({ sellerId: seller.id, categoryId: baseline.categoryId, price: 1000, quantity: 1 });
      const addr = await createAddress({ userId: buyer.id });
      const { orderId } = await buyAndPay(ctx, buyer, product.id, addr.id);
      ...
      const order = await prisma.order.findUnique({ where: { id: orderId } });
      expect(Number(payout!.netAmount)).toBeCloseTo(Number(order!.totalAmount), 2);
    });
  ...
  scenario('COM-034', async () => {
    ...
    const p1 = await createProduct({ sellerId: seller.id, categoryId: cat, price: 100, quantity: 1 });
    const p2 = await createProduct({ sellerId: seller.id, categoryId: cat, price: 100, quantity: 1 });
    const expires = new Date(Date.now() + 864e5);
    await prisma.order.create({
      data: {
        orderNumber: `ORD-${Date.now()}-1`, productId: p1.id, buyerId: buyer.id, sellerId: seller.id,
        totalAmount: 500, commissionAmount: 0, status: OrderStatus.delivered, shippingAddressId: addr.id,
        paymentExpiresAt: expires,
      },
    });
    await prisma.order.create({
      data: {
        orderNumber: `ORD-${Date.now()}-2`, productId: p2.id, buyerId: buyer.id, sellerId: seller.id,
        totalAmount: 300, commissionAmount: 0, status: OrderStatus.paid, shippingAddressId: addr.id,
        paymentExpiresAt: expires,
      },
    });
  ...
  scenario('COM-050', async () => {
    ...
    await prisma.sellerBankAccount.create({
      data: { userId: seller.id, accountHolder: 'X', iban: IBAN_A, isVerified: true, verifiedAt: new Date() },
    });
    ...
    const spy = jest
      ...
      .mockResolvedValueOnce({ status: 'success', data: [{ trans_id: transId, reason: 'hesap kapalı' }] });
    try {
  ...
  describe('Banka hesabı', () => {
    const patchBank = (user: { accessToken: string }, body: Record<string, unknown>) =>
      request(server()).patch('/api/users/me/bank-account').set(authHeader(user)).send(body);

    scenario('COM-056', async () => {
      const user = await createUser(ctx.module, { isSeller: true });
      const res = await patchBank(user, { accountHolder: 'Ahmet Yılmaz', iban: IBAN_A }).expect(200);
    ...
    scenario('COM-058', async () => {
      const user = await createUser(ctx.module, { isSeller: true });
      await patchBank(user, { accountHolder: 'Test', iban: 'DE89370400440532013000' }).expect(400);
    });

    scenario('COM-059', async () => {
      const user = await createUser(ctx.module, { isSeller: true });
      await patchBank(user, { accountHolder: 'Test', iban: 'TR12345' }).expect(400);
    });

    scenario('COM-060', async () => {
      const user = await createUser(ctx.module, { isSeller: true });
      const res = await patchBank(user, { accountHolder: 'Test', iban: IBAN_A, tcKimlikNo: '123' }).expect(400);
      expect(JSON.stringify(res.body)).toContain('TC Kimlik numarası 11 haneli olmalıdır');
    });

    scenario('COM-061', async () => {
      const user = await createUser(ctx.module, { isSeller: true });
      const res = await patchBank(user, { accountHolder: 'A', iban: IBAN_A }).expect(400);
      expect(JSON.stringify(res.body)).toContain('Hesap sahibi adı 2-150 karakter olmalıdır');
    });

    scenario('COM-062', async () => {
      const user = await createUser(ctx.module, { isSeller: true });
      // Önce hesap oluştur ve doğrula.
      await patchBank(user, { accountHolder: 'Old Name', iban: IBAN_A }).expect(200);
      const prisma = getPrisma();
      await prisma.sellerBankAccount.update({ where: { userId: user.id }, data: { isVerified: true, verifiedAt: new Date() } });
      // IBAN güncelle → isVerified false'a döner.
      const newIban = VALID_IBANS[1];
      const res = await patchBank(user, { accountHolder: 'New Name', iban: newIban }).expect(200);
    ...
    scenario('COM-063', async () => {
      ...
      await patchBank(user, { accountHolder: 'Test User', iban: 'tr330006100519786457841326' }).expect(400);
      await patchBank(user, { accountHolder: 'Test User', iban: 'TR33 0006 1005 1978 6457 8413 26' }).expect(400);
    });

    scenario('COM-064', async () => {
      const user = await createUser(ctx.module, { isSeller: true });
      // Yokken GET → boş (id undefined).
      const empty = await request(server()).get('/api/users/me/bank-account').set(authHeader(user)).expect(200);
      expect(empty.body?.id).toBeUndefined();
      // Oluştur, tekrar GET.
      await patchBank(user, { accountHolder: 'Ahmet', iban: IBAN_A }).expect(200);
      const filled = await request(server()).get('/api/users/me/bank-account').set(authHeader(user)).expect(200);
    ...
    scenario('COM-065', async () => {
      ...
      await request(server()).delete('/api/users/me/bank-account').set(authHeader(user)).expect(200);
      const prisma = getPrisma();
      expect(await prisma.sellerBankAccount.findUnique({ where: { userId: user.id } })).toBeNull();
      // Yokken DELETE → 404.
      const res = await request(server()).delete('/api/users/me/bank-account').set(authHeader(user)).expect(404);
      expect(JSON.stringify(res.body)).toContain('Banka hesabı bulunamadı');
    });

    scenario('COM-066', async () => {
      // Kimliksiz PATCH/GET/DELETE → 401.
      await request(server()).patch('/api/users/me/bank-account').send({ accountHolder: 'X', iban: IBAN_A }).expect(401);
    ...
    scenario('COM-087', async () => {
      ...
      const prisma = getPrisma();
      await prisma.sellerBankAccount.create({ data: { userId: sellerB.id, accountHolder: 'B User', iban: IBAN_A, isVerified: false } });
      // A kendi bank-account'ını sorar → B'nin IBAN'ı GÖRÜNMEZ (A'nın hesabı yok → boş).
      const aBank = await request(server()).get('/api/users/me/bank-account').set(authHeader(sellerA)).expect(200);
      expect(aBank.body?.iban).toBeUndefined();
      // A kendi earnings'i → yalnız A'nın verisi (0).
      const aEarn = await request(server()).get('/api/orders/seller/earnings').set(authHeader(sellerA)).expect(200);
    ...
    scenario('COM-095', async () => {
      ...
      const res = await request(server())
        ...
        .expect(400);
      expect(JSON.stringify(res.body)).toContain('Geçerli bir TR IBAN numarası giriniz');
    });
    ...
    scenario('COM-096', async () => {
      ...
      const user = await createUser(ctx.module, { isSeller: true });
      const prisma = getPrisma();
      await prisma.sellerBankAccount.create({ data: { userId: user.id, accountHolder: 'X', iban: IBAN_A, isVerified: false } });
  ...
  describe('Admin payout uçları', () => {
    ...
    scenario('COM-068', async () => {
      ...
      const audit = await prisma.auditLog.findFirst({
        where: { action: 'payout_release', entityType: 'PaymentHold', entityId: orderId },
      });
      expect(audit).toBeTruthy();
    });
    ...
    scenario('COM-076', async () => {
      ...
      const trade = await prisma.trade.create({
        data: { tradeNumber: `TRD-${Date.now()}`, initiatorId: a.id, receiverId: b.id, status: 'completed' as any, responseDeadline: new Date(Date.now() + 864e5) },
      });
      await prisma.tradeCashPayment.create({
        data: { tradeId: trade.id, payerId: a.id, recipientId: b.id, amount: 100, commission: 5, totalAmount: 105, provider: 'paytr', status: PaymentStatus.completed, refundedAt: new Date() },
      });
      const res = await request(server()).post(`/api/admin/payouts/release-trade/${trade.id}`).set(authHeader(adm)).expect(400);
      expect(JSON.stringify(res.body)).toContain('İade edilmiş ödeme serbest bırakılamaz');
    });
    ...
    scenario('COM-077', async () => {
      ...
      const tcp = await prisma.tradeCashPayment.create({
        data: { tradeId: trade.id, payerId: a.id, recipientId: b.id, amount: 100, commission: 5, totalAmount: 105, provider: 'paytr', status: PaymentStatus.completed },
      });
      const res = await request(server()).post(`/api/admin/payouts/release-trade/${trade.id}`).set(authHeader(adm)).expect(200);
      expect(res.body.success).toBe(true);
      const after = await prisma.tradeCashPayment.findUnique({ where: { id: tcp.id } });
      expect(after!.releasedAt).toBeTruthy();
      const audit = await prisma.auditLog.findFirst({ where: { action: 'trade_cash_hold_release', entityId: tcp.id } });
      expect(audit).toBeTruthy();
    });
    ...
    scenario('COM-078', async () => {
      ...
      await prisma.tradeCashPayment.create({
        data: { tradeId: trade.id, payerId: a.id, recipientId: b.id, amount: 100, commission: 5, totalAmount: 105, provider: 'paytr', status: PaymentStatus.completed, releasedAt: new Date() },
      });
      const res = await request(server()).post(`/api/admin/payouts/release-trade/${trade.id}`).set(authHeader(adm)).expect(200);
      expect(res.body.success).toBe(true);
    ...
    scenario('COM-072', async () => {
      ...
      const prisma = getPrisma();
      const transfer = await prisma.payoutTransfer.create({
        data: { sellerId: seller.id, amount: 100, commission: 4, netAmount: 96, merchantOid: 'OID1', transId: `T${Date.now()}`, transferIban: IBAN_A, transferName: 'X', status: PayoutStatus.failed, failureReason: 'PayTR error', retryCount: 2 },
      });
      const res = await request(server()).post(`/api/admin/payouts/${transfer.id}/retry`).set(authHeader(adm)).expect(200);
      expect(res.body.success).toBe(true);
      const after = await prisma.payoutTransfer.findUnique({ where: { id: transfer.id } });
      expect(after!.status).toBe(PayoutStatus.pending);
    ...
    scenario('COM-073', async () => {
      ...
      const transfer = await prisma.payoutTransfer.create({
        data: { sellerId: seller.id, amount: 100, commission: 4, netAmount: 96, merchantOid: 'OID1', transId: oldTransId, transferIban: IBAN_A, transferName: 'X', status: PayoutStatus.returned, failureReason: 'geri döndü' },
      });
      await request(server()).post(`/api/admin/payouts/${transfer.id}/retry`).set(authHeader(adm)).expect(200);
      const after = await prisma.payoutTransfer.findUnique({ where: { id: transfer.id } });
      expect(after!.status).toBe(PayoutStatus.pending);
    ...
    scenario('COM-074', async () => {
      ...
      const prisma = getPrisma();
      const transfer = await prisma.payoutTransfer.create({
        data: { sellerId: seller.id, amount: 100, commission: 4, netAmount: 96, merchantOid: 'OID1', transId: `T${Date.now()}`, transferIban: IBAN_A, transferName: 'X', status: PayoutStatus.completed, processedAt: new Date() },
      });
      const res = await request(server()).post(`/api/admin/payouts/${transfer.id}/retry`).set(authHeader(adm)).expect(400);
    ...
    scenario('COM-075', async () => {
      ...
      const seller = await createUser(ctx.module, { isSeller: true });
      const prisma = getPrisma();
      const base = { sellerId: seller.id, amount: 100, commission: 4, netAmount: 96, merchantOid: 'O', transferIban: IBAN_A, transferName: 'X' };
      await prisma.payoutTransfer.create({ data: { ...base, transId: `A${Date.now()}`, status: PayoutStatus.failed } });
      await prisma.payoutTransfer.create({ data: { ...base, transId: `B${Date.now()}`, status: PayoutStatus.returned } });
      await prisma.payoutTransfer.create({ data: { ...base, transId: `C${Date.now()}`, status: PayoutStatus.completed, processedAt: new Date() } });
      const res = await request(server()).get('/api/admin/payouts/failed').set(authHeader(adm)).expect(200);
    ...
    scenario('COM-079', async () => {
      ...
      const prisma = getPrisma();
      const hold = await prisma.paymentHold.findFirst({ where: { orderId } });
      await prisma.paymentHold.update({ where: { id: hold!.id }, data: { releaseAt: new Date(Date.now() + 5 * 864e5) } });
      const res = await request(server()).get('/api/admin/payouts/summary').set(authHeader(adm)).expect(200);
    ...
    scenario('COM-081', async () => {
      ...
      expect(firstLine).toBe('id,orderId,orderNumber,sellerId,sellerName,sellerEmail,amount,status,releaseAt,releasedAt,createdAt');
    });
  });
  ...
  describe('Yetki (Authz)', () => {
    scenario('COM-082', async () => {
      ...
      const moderator = await createAdminUser(ctx.module, { role: 'moderator' as any });
      const nonSuper = await createAdminUser(ctx.module, { role: 'admin' as any });
    ...
    scenario('COM-083', async () => {
      ...
      const adminRole = await createAdminUser(ctx.module, { role: 'admin' as any });
      const moderator = await createAdminUser(ctx.module, { role: 'moderator' as any });
      const normal = await createUser(ctx.module);
      await request(server()).get('/api/admin/commission-rules').set(authHeader(superAdmin)).expect(200);
      await request(server()).get('/api/admin/commission-rules').set(authHeader(adminRole)).expect(200);
      const modRes = await request(server()).get('/api/admin/commission-rules').set(authHeader(moderator));
      expect([401, 403]).toContain(modRes.status);
      const res = await request(server()).get('/api/admin/commission-rules').set(authHeader(normal));
    ...
    scenario('COM-085', async () => {
      ...
      const moderator = await createAdminUser(ctx.module, { role: 'moderator' as any });
      const adminRole = await createAdminUser(ctx.module, { role: 'admin' as any });
      const modSummary = await request(server()).get('/api/admin/payouts/summary').set(authHeader(moderator));
      expect([401, 403]).toContain(modSummary.status);
      const modFailed = await request(server()).get('/api/admin/payouts/failed').set(authHeader(moderator));
      expect([401, 403]).toContain(modFailed.status);
      // admin rolü 'payouts' iznine sahip → 200 (matris geçer).
      await request(server()).get('/api/admin/payouts/summary').set(authHeader(adminRole)).expect(200);
      await request(server()).get('/api/admin/payouts/failed').set(authHeader(adminRole)).expect(200);
    });
    ...
    scenario('COM-086', async () => {
      ...
      await request(server())
        .post('/api/admin/payouts/release-trade/00000000-0000-0000-0000-000000000000')
  ...
  scenario('COM-044', async () => {
    ...
    await releaseHold(orderId);
    await ctx.app.get(PayoutService).createPayoutsForReleasedHolds();
    const before = await prisma.payoutTransfer.findFirst({ where: { sellerId: seller.id } });
    ...
    try {
      ...
      expect(ctx.paytr.transferCalls.length).toBe(0);
      const after = await prisma.payoutTransfer.findUnique({ where: { id: before!.id } });
      expect(after!.status).toBe(PayoutStatus.pending);
    } finally {
      spy.mockRestore();
    }
  });
  scenario.skip('COM-070', 'Saf-UI parite/bug: admin web "Serbest Bırak" butonu reason göndermiyor (admin/src/lib/api.ts). Backend zorunlu-reason davranışı COM-067 kapsıyor; buton parite kontrolü frontend testinde.');
  scenario.skip('COM-071', 'Saf-UI: admin payouts sayfası frozen/open_refund satırda butonu disable eder (payouts/page.tsx). Backend savunma katmanı COM-069 kapsıyor.');
  scenario.skip('COM-088', 'Saf-UI: web IBAN sayfası client doğrulama + toast (bank-account/page.tsx). API davranışı COM-056/058/059 kapsıyor.');
  scenario.skip('COM-089', 'Saf-UI: web IBAN doğrulama rozeti (bank-account/page.tsx). API isVerified alanı COM-056/062 kapsıyor.');
  scenario.skip('COM-090', 'Saf-UI: web IBAN sil onay diyaloğu (bank-account/page.tsx). DELETE davranışı COM-065 kapsıyor.');
  scenario.skip('COM-091', 'Parite (mobile): mobile IBAN ekranı zod şeması web ile aynı (bank-account.tsx). API uç davranışı COM-056..063 kapsıyor.');
  scenario.skip('COM-092', 'Saf-UI: admin Satıcı Ödemeleri özet kartları yükleniyor/boş durumları (payouts/page.tsx). Summary API COM-079 kapsıyor.');
  scenario.skip('COM-093', 'Saf-UI: admin Komisyon Kuralları formu appliesTo preview hesabı (commission/page.tsx). Backend preview COM-007..020 kapsıyor.');
  scenario.skip('COM-094', 'Parite sınırı: mobile\'da admin payout/komisyon yönetimi yoktur (tasarım gereği). Uygulanamaz — assertion yok.');
});
