...
describe("02 — Kullanıcı Profili & Hesap Yönetimi (USR)", () => {
  ...
  describe("PATCH /api/users/me", () => {
    ...
    scenario("USR-006", async () => {
      ...
      expect(JSON.stringify(res.body.message)).toContain(
        "Geçerli bir Türkiye telefon numarası",
      );
    });
    ...
    scenario("USR-077", async () => {
      ...
      // body'ye id enjekte → controller @CurrentUser('id') kullanır + whitelist body id'yi
      // strip eder. (displayName @MinLength(2) → geçerli 2+ karakterli isim gönder.)
      await request(server())
        .patch("/api/users/me")
        .set(authHeader(me))
        .send({ id: other.id, displayName: "Yeni" })
        .expect(200);
      // Başka kullanıcı etkilenmedi.
      const otherDb = await getPrisma().user.findUnique({
        where: { id: other.id },
      });
      expect(otherDb?.displayName).toBe("Başka");
      const meDb = await getPrisma().user.findUnique({ where: { id: me.id } });
      expect(meDb?.displayName).toBe("Yeni");
    });
  ...
  describe("Addresses", () => {
    ...
    scenario("USR-026", async () => {
      ...
      const list = await request(server())
        ...
        .expect(200);
      const byId = new Map(
        (list.body as any[]).map((x) => [x.id, x.isDefault]),
      );
  ...
  describe("Follow", () => {
    scenario("USR-036", async () => {
      ...
      const following = await request(server())
        ...
        .set(authHeader(me))
        .expect(200);
      const ids = (following.body.following as any[]).map(
        (f) => f.following?.id ?? f.id,
      );
  ...
  describe("Account deletion", () => {
    scenario("USR-040", async () => {
      ...
      expect(await prisma.address.count({ where: { userId: user.id } })).toBe(
        0,
      );
    });
  ...
  describe("Bank account", () => {
    ...
    scenario("USR-063", async () => {
      ...
      await request(server())
        ...
        .send({
          accountHolder: "Ahmet Yılmaz",
          iban: "TR000006100519786457841399",
        })
        .expect(200);
      ...
      expect(count).toBe(1);
      const row = await prisma.sellerBankAccount.findUnique({
        where: { userId: user.id },
      });
    ...
    scenario("USR-066", async () => {
      ...
      const del2 = await request(server())
        ...
        .set(authHeader(user))
        .expect(404);
      expect(JSON.stringify(del2.body.message)).toContain(
  ...
  describe("Stats & analytics", () => {
    ...
    scenario("USR-085", async () => {
      ...
      const a = await request(server())
        ...
        .set(authHeader(free))
        .expect(400);
      expect(JSON.stringify(a.body.message)).toContain(
        "işletme üyeliğine sahip",
      );
      ...
      const b = await request(server())
        ...
        .set(authHeader(biz))
        .expect(400);
      expect(JSON.stringify(b.body.message)).toContain(
        "şirket adı bilgisi gereklidir",
      );
    });
    ...
    scenario("USR-086", async () => {
      ...
      expect(res.body.topProducts).toEqual(
        expect.objectContaining({
          byViews: expect.any(Array),
          byLikes: expect.any(Array),
        }),
      );
  ...
  describe("Public featured endpoints", () => {
    scenario("USR-087", async () => {
      ...
      const collector = await request(server())
        .get("/api/users/featured-collector")
        .expect(200);
      expect(
        collector.body === null || typeof collector.body === "object",
      ).toBe(true);

      const business = await request(server())
        .get("/api/users/featured-business")
        .expect(200);
      expect(business.body === null || typeof business.body === "object").toBe(
        true,
      );
    });
  });
  ...
  describe("Parite & i18n", () => {
    scenario("USR-079", async () => {
      ...
      const phone = await request(server())
        ...
        .send({ phone: "12345" })
        .expect(400);
      expect(JSON.stringify(phone.body.message)).toContain(
        "Geçerli bir Türkiye telefon",
      );
      for (let i = 0; i < 10; i++)
        await createAddress({ userId: user.id, isDefault: false });
