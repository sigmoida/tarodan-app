...
describe('05 — Koleksiyon & Favori (Wishlist) (COL)', () => {
  ...
  async function seedCollection(opts: {
    ...
    const slug =
      opts.slug ??
      opts.name.toLowerCase().replace(/[^a-z0-9çğıöşü]+/g, '-').replace(/^-|-$/g, '');
  ...
  const createBody = (over: Record<string, unknown> = {}) => ({ name: 'Test Koleksiyon', ...over });
  ...
  describe('POST /api/collections', () => {
    scenario('COL-001', async () => {
      ...
      const res = await request(server())
        ...
        .send({ name: 'Porsche Vitrinim', description: 'Tüm Porsche modellerim', isPublic: true })
        .expect(201);
      ...
      const prisma = getPrisma();
      const count = await prisma.collection.count({ where: { userId: ahmet.id } });
      expect(count).toBe(1);
    });
    ...
    scenario('COL-009', async () => {
      ...
      await request(server())
        ...
        .expect(400);
      const prisma = getPrisma();
      expect(await prisma.collection.count({ where: { userId: u.id } })).toBe(0);
    });
    ...
    scenario('COL-010', async () => {
      ...
      const prisma = getPrisma();
      expect(await prisma.collection.findFirst({ where: { userId: u.id, name: 'KatTest2' } })).toBeNull();
    });
    ...
    scenario('COL-101', async () => {
      ...
      const res = await request(server())
        ...
        .expect(201);
      const prisma = getPrisma();
      const col = await prisma.collection.findUnique({ where: { id: res.body.id } });
  ...
  describe('PATCH/DELETE /api/collections/:id', () => {
    scenario('COL-011', async () => {
      const ahmet = await premiumUser();
      const col = await seedCollection({ userId: ahmet.id, name: 'Porsche Vitrinim' });
      const res = await request(server())
    ...
    scenario('COL-012', async () => {
      ...
      const b = await seedCollection({ userId: ahmet.id, name: 'B Koleksiyonu' });
      const res = await request(server())
    ...
    scenario('COL-015', async () => {
      const ahmet = await premiumUser();
      const col = await seedCollection({ userId: ahmet.id, name: 'Açıklamalı' });
      const res = await request(server())
        .patch(`/api/collections/${col.id}`)
        .set(authHeader(ahmet))
        .send({ description: '' })
    ...
    scenario('COL-016', async () => {
      ...
      await request(server()).post(`/api/collections/${col.id}/like`).set(authHeader(ayse)).expect(200);

      await request(server()).delete(`/api/collections/${col.id}`).set(authHeader(ahmet)).expect(204);

      const prisma = getPrisma();
      expect(await prisma.collection.findUnique({ where: { id: col.id } })).toBeNull();
      expect(await prisma.collectionItem.count({ where: { collectionId: col.id } })).toBe(0);
      expect(await prisma.collectionLike.count({ where: { collectionId: col.id } })).toBe(0);
    });
  ...
  describe('Collection items & reorder', () => {
    ...
    scenario('COL-023', async () => {
      ...
      const res = await request(server())
        ...
        .send({})
        .expect(400);
      expect(res.body.message).toBe('Ürün ID veya custom ürün bilgileri gerekli');
    });

    scenario('COL-024', async () => {
      const ahmet = await premiumUser('ahmet@demo.com');
      const ayse = await premiumUser('ayse@demo.com');
      const col = await seedCollection({ userId: ahmet.id, name: 'Sahip-Korumalı' });
      const seller = await createUser(ctx.module, { isSeller: true });
      const product = await createProduct({ sellerId: seller.id, categoryId: baseline.categoryId });
      const res = await request(server())
        .post(`/api/collections/${col.id}/items`)
    ...
    scenario('COL-025', async () => {
      ...
      const item = await seedItem({ collectionId: col.id, customTitle: 'Custom X' });
      await request(server())
    ...
    scenario('COL-026', async () => {
      ...
      const itemB = await seedItem({ collectionId: colB.id, customTitle: 'B-item' });
      const res = await request(server())
      ...
      const prisma = getPrisma();
      expect(await prisma.collectionItem.findUnique({ where: { id: itemB.id } })).toBeTruthy();
    });
    ...
    scenario('COL-028', async () => {
      ...
      const a = await seedItem({ collectionId: col.id, customTitle: 'A', sortOrder: 0 });
      const b = await seedItem({ collectionId: col.id, customTitle: 'B', sortOrder: 1 });
      await request(server())
        .post(`/api/collections/${col.id}/reorder`)
        .set(authHeader(ahmet))
        .send({ items: [{ itemId: a.id, sortOrder: 2 }, { itemId: b.id, sortOrder: 1 }] })
        .expect(204);
      // Detayda yeni sırayla (sortOrder asc): B(1) önce, A(2) sonra.
      const detail = await request(server()).get(`/api/collections/${col.id}`).expect(200);
    ...
    scenario('COL-029', async () => {
      ...
      const colA = await seedCollection({ userId: ahmet.id, name: 'A' });
      const colB = await seedCollection({ userId: ahmet.id, name: 'B' });
      const itemB = await seedItem({ collectionId: colB.id, customTitle: 'B-item', sortOrder: 0 });
      await request(server())
        ...
        .expect(204);
      const prisma = getPrisma();
      const fresh = await prisma.collectionItem.findUnique({ where: { id: itemB.id } });
    ...
    scenario('COL-030', async () => {
      const ahmet = await premiumUser('ahmet@demo.com');
      const ayse = await premiumUser('ayse@demo.com');
      const col = await seedCollection({ userId: ahmet.id, name: 'Sahip-Reorder' });
      const item = await seedItem({ collectionId: col.id, customTitle: 'X', sortOrder: 0 });
      const res = await request(server())
        .post(`/api/collections/${col.id}/reorder`)
        .set(authHeader(ayse))
    ...
    scenario('COL-031', async () => {
      ...
      const res = await request(server())
        .post(`/api/collections/${col.id}/items`)
        .set(authHeader(ahmet))
        .send({ customTitle: 'Nadir Tomica', customBrand: 'Tomica', customYear: 1980, customScale: '1:64' })
        .expect(201);
    ...
    scenario('COL-032', async () => {
      ...
      const ahmet = await premiumUser();
      const col = await seedCollection({ userId: ahmet.id, name: 'Resimli-Custom' });
      const res = await request(server())
        .post(`/api/collections/${col.id}/items`)
        .set(authHeader(ahmet))
        .field('customTitle', 'Nadir Görselli')
        .attach('image', jpegBuffer(), { filename: 'x.jpg', contentType: 'image/jpeg' })
        .expect(201);
      expect(res.body.isCustom).toBe(true);
      expect(res.body.productId).toBeUndefined();
      ...
      const prisma = getPrisma();
      const item = await prisma.collectionItem.findUnique({ where: { id: res.body.id } });
    ...
    scenario('COL-033', async () => {
      const ahmet = await premiumUser();
      const col = await seedCollection({ userId: ahmet.id, name: 'Boş-Custom' });
    ...
    scenario('COL-034', async () => {
      ...
      const post = (body: Record<string, unknown>) =>
        request(server()).post(`/api/collections/${col.id}/items`).set(authHeader(ahmet)).send(body);
    ...
    scenario('COL-035', async () => {
      const ahmet = await premiumUser();
      const col = await seedCollection({ userId: ahmet.id, name: 'Alan-Sınır' });
      const post = (body: Record<string, unknown>) =>
        request(server()).post(`/api/collections/${col.id}/items`).set(authHeader(ahmet)).send(body);
      await post({ customTitle: 'x'.repeat(201) }).expect(400); // customTitle MaxLength 200
      await post({ customTitle: 'ok1', customDescription: 'd'.repeat(1001) }).expect(400); // 1000
      await post({ customTitle: 'ok2', customBrand: 'b'.repeat(101) }).expect(400); // 100
      await post({ customTitle: 'ok3', customModel: 'm'.repeat(101) }).expect(400); // 100
      await post({ customTitle: 'ok4', customManufacturer: 'n'.repeat(101) }).expect(400); // 100
      await post({ customTitle: 'ok5', customScale: 's'.repeat(51) }).expect(400); // 50
      await post({ customTitle: 'ok6', customMaterial: 'r'.repeat(51) }).expect(400); // 50
    });

    scenario('COL-036', async () => {
      const ahmet = await premiumUser();
      const col = await seedCollection({ userId: ahmet.id, name: 'Mükerrer-Custom' });
      const post = () =>
        request(server())
          .post(`/api/collections/${col.id}/items`)
          .set(authHeader(ahmet))
      ...
      const prisma = getPrisma();
      expect(
        await prisma.collectionItem.count({ where: { collectionId: col.id, customTitle: 'Nadir Tomica' } }),
      ).toBe(2);
    });

    scenario('COL-090', async () => {
      const ahmet = await premiumUser();
      const col = await seedCollection({ userId: ahmet.id, name: 'SortOrder-Sınır' });
      await request(server())
        .post(`/api/collections/${col.id}/items`)
        .set(authHeader(ahmet))
    ...
    scenario('COL-091', async () => {
      const ahmet = await premiumUser();
      const col = await seedCollection({ userId: ahmet.id, name: 'Boş-Reorder' });
      await request(server())
        .post(`/api/collections/${col.id}/reorder`)
        .set(authHeader(ahmet))
        .send({ items: [] })
    ...
    scenario('COL-085', async () => {
      ...
      const prisma = getPrisma();
      expect(
        await prisma.collectionItem.count({ where: { collectionId: col.id, productId: product.id } }),
      ).toBe(1);
    });
    ...
    scenario('COL-098', async () => {
      ...
      const ahmet = await premiumUser('ahmet@demo.com');
      const ayse = await premiumUser('ayse@demo.com');
      const ahmetCol = await seedCollection({ userId: ahmet.id, name: 'Ahmet' });
      const ahmetItem = await seedItem({ collectionId: ahmetCol.id, customTitle: 'Ah-item', sortOrder: 0 });
      ...
      await request(server())
        ...
        .expect(204);
      const prisma = getPrisma();
      const fresh = await prisma.collectionItem.findUnique({ where: { id: ahmetItem.id } });
  ...
  describe('Collection viewing & privacy', () => {
    scenario('COL-037', async () => {
      const ahmet = await premiumUser();
      const col = await seedCollection({ userId: ahmet.id, name: 'Best JDM', slug: 'best-jdm', isPublic: true });
      const res = await request(server()).get('/api/collections/slug/best-jdm').expect(200);
      expect(res.body.id).toBe(col.id);
      const prisma = getPrisma();
      const after = await prisma.collection.findUnique({ where: { id: col.id } });
      expect(after?.viewCount).toBe(1); // anonim sahip değil → +1
    });

    scenario('COL-038', async () => {
      const ahmet = await premiumUser('ahmet@demo.com');
      const ayse = await premiumUser('ayse@demo.com');
      const priv = await seedCollection({ userId: ahmet.id, name: 'Gizli', isPublic: false });
      // 1) Anonim.
      const anon = await request(server()).get(`/api/collections/${priv.id}`).expect(403);
      expect(anon.body.message).toBe('Bu koleksiyon özel');
      // 2) Yabancı üye.
      const other = await request(server()).get(`/api/collections/${priv.id}`).set(authHeader(ayse)).expect(403);
      expect(other.body.message).toBe('Bu koleksiyon özel');
    });

    scenario('COL-039', async () => {
      const ahmet = await premiumUser();
      const priv = await seedCollection({ userId: ahmet.id, name: 'Sahip-Gizli', isPublic: false });
      const res = await request(server()).get(`/api/collections/${priv.id}`).set(authHeader(ahmet)).expect(200);
      expect(res.body.id).toBe(priv.id);
      const prisma = getPrisma();
      const after = await prisma.collection.findUnique({ where: { id: priv.id } });
    ...
    scenario('COL-040', async () => {
      const ahmet = await premiumUser('ahmet@demo.com');
      const ayse = await premiumUser('ayse@demo.com');
      const pub = await seedCollection({ userId: ahmet.id, name: 'Açık', isPublic: true });
      await seedCollection({ userId: ahmet.id, name: 'Kapalı', isPublic: false });
      // 1) ayse ahmet'in listesini görür → yalnızca public.
      const r1 = await request(server()).get(`/api/collections/user/${ahmet.id}`).set(authHeader(ayse)).expect(200);
      expect(r1.body.total).toBe(1);
      expect(r1.body.collections[0].id).toBe(pub.id);
      // 2) ahmet kendi listesini görür → public+private.
      const r2 = await request(server()).get(`/api/collections/user/${ahmet.id}`).set(authHeader(ahmet)).expect(200);
    ...
    scenario('COL-041', async () => {
      ...
      await seedCollection({ userId: ahmet.id, name: 'Kapalı', isPublic: false });
      const res = await request(server()).get('/api/collections/me').set(authHeader(ahmet)).expect(200);
      expect(res.body.total).toBe(2); // public+private
      // createdAt desc — en yeni "Kapalı" önce.
      const dates = res.body.collections.map((c: any) => new Date(c.createdAt).getTime());
    ...
    scenario('COL-051', async () => {
      ...
      const col = await seedCollection({ userId: ahmet.id, name: 'Liked-Flag', isPublic: true });
      await request(server()).post(`/api/collections/${col.id}/like`).set(authHeader(ayse)).expect(200);
      // ayse → isLiked true
      const a = await request(server()).get(`/api/collections/${col.id}`).set(authHeader(ayse)).expect(200);
      expect(a.body.isLiked).toBe(true);
      // kaan → isLiked false
      const k = await request(server()).get(`/api/collections/${col.id}`).set(authHeader(kaan)).expect(200);
      expect(k.body.isLiked).toBe(false);
      // anonim → isLiked false
      const anon = await request(server()).get(`/api/collections/${col.id}`).expect(200);
    ...
    scenario('COL-100', async () => {
      ...
      const ahmet = await premiumUser('ahmet@demo.com');
      const ayse = await premiumUser('ayse@demo.com');
      const priv = await seedCollection({ userId: ahmet.id, name: 'Sızıntı', slug: 'sizinti', isPublic: false });
      await seedItem({ collectionId: priv.id, customTitle: 'gizli-item' });
      const byId = await request(server()).get(`/api/collections/${priv.id}`).set(authHeader(ayse)).expect(403);
      expect(byId.body.message).toBe('Bu koleksiyon özel');
      expect(byId.body.items).toBeUndefined();
      const bySlug = await request(server()).get('/api/collections/slug/sizinti').set(authHeader(ayse)).expect(403);
  ...
  describe('Collection likes', () => {
    scenario('COL-043', async () => {
      const ahmet = await premiumUser('ahmet@demo.com');
      const ayse = await premiumUser('ayse@demo.com');
      const col = await seedCollection({ userId: ahmet.id, name: 'Beğenilecek', isPublic: true });
      const res = await request(server())
      ...
      const prisma = getPrisma();
      expect(await prisma.collectionLike.count({ where: { collectionId: col.id, userId: ayse.id } })).toBe(1);
    });

    scenario('COL-044', async () => {
      const ahmet = await premiumUser('ahmet@demo.com');
      const ayse = await premiumUser('ayse@demo.com');
      const col = await seedCollection({ userId: ahmet.id, name: 'Toggle', isPublic: true });
      await request(server()).post(`/api/collections/${col.id}/like`).set(authHeader(ayse)).expect(200);
      // İkinci POST → toggle off.
      const res = await request(server()).post(`/api/collections/${col.id}/like`).set(authHeader(ayse)).expect(200);
      expect(res.body.liked).toBe(false);
      const prisma = getPrisma();
      expect(await prisma.collectionLike.count({ where: { collectionId: col.id, userId: ayse.id } })).toBe(0);
    });

    scenario('COL-045', async () => {
      const ahmet = await premiumUser('ahmet@demo.com');
      const ayse = await premiumUser('ayse@demo.com');
      const col = await seedCollection({ userId: ahmet.id, name: 'Delete-Like', isPublic: true });
      await request(server()).post(`/api/collections/${col.id}/like`).set(authHeader(ayse)).expect(200);
      const res = await request(server()).delete(`/api/collections/${col.id}/like`).set(authHeader(ayse)).expect(200);
      expect(res.body.liked).toBe(false);
      expect(res.body.likeCount).toBe(0);
    });
    ...
    scenario('COL-046', async () => {
      const ahmet = await premiumUser('ahmet@demo.com');
      const ayse = await premiumUser('ayse@demo.com');
      const col = await seedCollection({ userId: ahmet.id, name: 'Delete-NoLike', isPublic: true });
      const res = await request(server()).delete(`/api/collections/${col.id}/like`).set(authHeader(ayse)).expect(200);
      expect(res.body.liked).toBe(false);
    ...
    scenario('COL-047', async () => {
      const ahmet = await premiumUser();
      const col = await seedCollection({ userId: ahmet.id, name: 'Kendi', isPublic: true });
      const res = await request(server()).post(`/api/collections/${col.id}/like`).set(authHeader(ahmet)).expect(400);
    ...
    scenario('COL-048', async () => {
      const ahmet = await premiumUser('ahmet@demo.com');
      const ayse = await premiumUser('ayse@demo.com');
      const priv = await seedCollection({ userId: ahmet.id, name: 'Gizli-Beğeni', slug: 'gizli-begeni', isPublic: false });
      const res = await request(server()).post('/api/collections/gizli-begeni/like').set(authHeader(ayse)).expect(403);
      expect(res.body.message).toBe('Bu koleksiyon özel');
    });

    scenario('COL-049', async () => {
      const ahmet = await premiumUser();
      const col = await seedCollection({ userId: ahmet.id, name: 'Anon-Like', isPublic: true });
      await request(server()).post(`/api/collections/${col.id}/like`).expect(401);
    });

    scenario('COL-050', async () => {
      const ayse = await premiumUser();
      const res = await request(server()).post(`/api/collections/${RANDOM_UUID}/like`).set(authHeader(ayse)).expect(404);
    ...
    scenario('COL-056', async () => {
      const ahmet = await premiumUser('ahmet@demo.com');
      const ayse = await premiumUser('ayse@demo.com');
      const c1 = await seedCollection({ userId: ahmet.id, name: 'L1', isPublic: true });
      const c2 = await seedCollection({ userId: ahmet.id, name: 'L2', isPublic: true });
      await request(server()).post(`/api/collections/${c1.id}/like`).set(authHeader(ayse)).expect(200);
      await request(server()).post(`/api/collections/${c2.id}/like`).set(authHeader(ayse)).expect(200);
      const res = await request(server()).get('/api/collections/liked').set(authHeader(ayse)).expect(200);
      expect(res.body.total).toBe(2);
      expect(res.body.collections.every((c: any) => c.isLiked === true)).toBe(true);
    ...
    scenario('COL-057', async () => {
      ...
      const ahmet = await premiumUser('ahmet@demo.com');
      const ayse = await premiumUser('ayse@demo.com');
      const pub = await seedCollection({ userId: ahmet.id, name: 'Pub', isPublic: true });
      const priv = await seedCollection({ userId: ahmet.id, name: 'Priv', isPublic: true });
      await request(server()).post(`/api/collections/${pub.id}/like`).set(authHeader(ayse)).expect(200);
      await request(server()).post(`/api/collections/${priv.id}/like`).set(authHeader(ayse)).expect(200);
      // Sonradan priv'i gizli yap.
      const prisma = getPrisma();
      await prisma.collection.update({ where: { id: priv.id }, data: { isPublic: false } });
      const res = await request(server()).get('/api/collections/liked').set(authHeader(ayse)).expect(200);
    ...
    scenario('COL-087', async () => {
      ...
      const ayse = await premiumUser('ayse@demo.com');
      const kaan = await premiumUser('kaan@demo.com');
      const col = await seedCollection({ userId: ahmet.id, name: 'Concurrent-Like', isPublic: true });
      await Promise.all([
        request(server()).post(`/api/collections/${col.id}/like`).set(authHeader(ayse)),
        request(server()).post(`/api/collections/${col.id}/like`).set(authHeader(kaan)),
      ]);
      const prisma = getPrisma();
      const after = await prisma.collection.findUnique({ where: { id: col.id } });
      expect(after?.likeCount).toBe(2);
      expect(await prisma.collectionLike.count({ where: { collectionId: col.id } })).toBe(2);
    });
  });
  ...
  describe('Browse public collections', () => {
    scenario('COL-052', async () => {
      const ahmet = await premiumUser();
      await seedCollection({ userId: ahmet.id, name: 'Açık-1', isPublic: true });
      await seedCollection({ userId: ahmet.id, name: 'Gizli-1', isPublic: false });
      const res = await request(server()).get('/api/collections/browse?page=1&pageSize=20').expect(200);
    ...
    scenario('COL-053', async () => {
      ...
      for (const sortBy of ['popular', 'recent', 'name', 'items_asc', 'items_desc']) {
        const res = await request(server()).get(`/api/collections/browse?sortBy=${sortBy}`).expect(200);
        expect(res.body.total).toBeGreaterThanOrEqual(3);
      }
      // name sıralaması TR collator: Aslan, Bizon, Çita
      const byName = await request(server()).get('/api/collections/browse?sortBy=name&pageSize=100').expect(200);
    ...
    scenario('COL-054', async () => {
      const ahmet = await createUser(ctx.module, { email: 'ahmet@demo.com', displayName: 'Ahmet' });
      await attachMembership(ahmet.id, 'premium', true);
      await seedCollection({ userId: ahmet.id, name: 'JDM Severler', isPublic: true });
      // 1) İsim eşleşmesi.
      const r1 = await request(server()).get('/api/collections/browse?search=JDM').expect(200);
      expect(r1.body.collections.some((c: any) => c.name === 'JDM Severler')).toBe(true);
      // 2) Kullanıcı adı eşleşmesi.
      const r2 = await request(server()).get('/api/collections/browse?search=Ahmet').expect(200);
      expect(r2.body.collections.some((c: any) => c.userName === 'Ahmet')).toBe(true);
      // 3) Eşleşmeyen → boş.
      const r3 = await request(server()).get('/api/collections/browse?search=zzzz-yok').expect(200);
    ...
    scenario('COL-055', async () => {
      ...
      await seedCollection({ userId: ahmet.id, name: 'Araba-Koleksiyon', isPublic: true, categoryId: araba.id });
      await seedCollection({ userId: ahmet.id, name: 'Kategorisiz', isPublic: true });
      const res = await request(server()).get('/api/collections/browse?category=araba').expect(200);
      expect(res.body.collections.every((c: any) => c.categoryId === araba.id)).toBe(true);
      expect(res.body.collections.some((c: any) => c.name === 'Araba-Koleksiyon')).toBe(true);
    });

    scenario('COL-060', async () => {
      const ahmet = await premiumUser();
      await seedCollection({ userId: ahmet.id, name: 'Sayfalama', isPublic: true });
      // pageSize=0 → clamp 20 (Math.max(1,..)||20 → 20'ye/1'e clamp); 500 → 100; page<1 → 1; page=abc → 1.
      const r1 = await request(server()).get('/api/collections/browse?pageSize=0').expect(200);
      expect(r1.body.pageSize).toBeLessThanOrEqual(100);
      expect(r1.body.pageSize).toBeGreaterThanOrEqual(1);
      const r2 = await request(server()).get('/api/collections/browse?pageSize=500').expect(200);
      expect(r2.body.pageSize).toBe(100);
      const r3 = await request(server()).get('/api/collections/browse?page=-1').expect(200);
      expect(r3.body.page).toBe(1);
      const r4 = await request(server()).get('/api/collections/browse?page=abc').expect(200);
  ...
  describe('Item visibility by product status', () => {
    scenario('COL-059', async () => {
      ...
      const ahmet = await premiumUser();
      const col = await seedCollection({ userId: ahmet.id, name: 'Tutarlı', isPublic: true });
      const seller = await createUser(ctx.module, { isSeller: true });
      for (let i = 0; i < 3; i++) {
        const p = await createProduct({ sellerId: seller.id, categoryId: baseline.categoryId, status: 'active' });
        await seedItem({ collectionId: col.id, productId: p.id, sortOrder: i });
      }
      const draft = await createProduct({ sellerId: seller.id, categoryId: baseline.categoryId, status: 'inactive' });
      await seedItem({ collectionId: col.id, productId: draft.id, sortOrder: 3 });
      await seedItem({ collectionId: col.id, customTitle: 'Custom-1', sortOrder: 4 });

      const detail = await request(server()).get(`/api/collections/${col.id}`).expect(200);
      expect(detail.body.itemCount).toBe(4); // 3 active + 1 custom

      const me = await request(server()).get('/api/collections/me').set(authHeader(ahmet)).expect(200);
    ...
    scenario('COL-061', async () => {
      const ahmet = await premiumUser();
      const col = await seedCollection({ userId: ahmet.id, name: 'Sold-Görünür', isPublic: true });
      const seller = await createUser(ctx.module, { isSeller: true });
      const sold = await createProduct({ sellerId: seller.id, categoryId: baseline.categoryId, status: 'sold' });
      await seedItem({ collectionId: col.id, productId: sold.id, sortOrder: 0 });
      const detail = await request(server()).get(`/api/collections/${col.id}`).expect(200);
    ...
    scenario('COL-062', async () => {
      const ahmet = await premiumUser();
      const col = await seedCollection({ userId: ahmet.id, name: 'Draft-Gizli', isPublic: true });
      const seller = await createUser(ctx.module, { isSeller: true });
      const draft = await createProduct({ sellerId: seller.id, categoryId: baseline.categoryId, status: 'inactive' });
      await seedItem({ collectionId: col.id, productId: draft.id, sortOrder: 0 });
      await seedItem({ collectionId: col.id, customTitle: 'Custom-Kalır', sortOrder: 1 });
      const detail = await request(server()).get(`/api/collections/${col.id}`).expect(200);
      ...
      const browse = await request(server()).get('/api/collections/browse?pageSize=100').expect(200);
    ...
    scenario('COL-063', async () => {
      const ahmet = await premiumUser();
      const col = await seedCollection({ userId: ahmet.id, name: 'Cascade', isPublic: true });
      const seller = await createUser(ctx.module, { isSeller: true });
      const p = await createProduct({ sellerId: seller.id, categoryId: baseline.categoryId, status: 'active' });
      ...
      expect(await prisma.collectionItem.count({ where: { collectionId: col.id, productId: p.id } })).toBe(0);
      const detail = await request(server()).get(`/api/collections/${col.id}`).expect(200);
      expect(detail.body.items.find((i: any) => i.productId === p.id)).toBeUndefined();
      expect(detail.body.itemCount).toBe(0);
    });

    scenario('COL-064', async () => {
      const ahmet = await premiumUser();
      const col = await seedCollection({ userId: ahmet.id, name: 'Reactivate', isPublic: true });
      const seller = await createUser(ctx.module, { isSeller: true });
      const p = await createProduct({ sellerId: seller.id, categoryId: baseline.categoryId, status: 'inactive' });
      await seedItem({ collectionId: col.id, productId: p.id, sortOrder: 0 });
      // İlk durum: gizli.
      const before = await request(server()).get(`/api/collections/${col.id}`).expect(200);
      expect(before.body.itemCount).toBe(0);
      // Active yap.
      const prisma = getPrisma();
      await prisma.product.update({ where: { id: p.id }, data: { status: 'active' as any } });
      const after = await request(server()).get(`/api/collections/${col.id}`).expect(200);
      expect(after.body.items.some((i: any) => i.productId === p.id)).toBe(true);
  ...
  describe('Wishlist', () => {
    scenario('COL-065', async () => {
      const ahmet = await createUser(ctx.module, { email: 'ahmet@demo.com', isSeller: true });
      const deniz = await createUser(ctx.module, { email: 'deniz@demo.com' });
      const product = await createProduct({ sellerId: ahmet.id, categoryId: baseline.categoryId });
      const res = await request(server())
        .post('/api/wishlist')
    ...
    scenario('COL-066', async () => {
      ...
      const deniz = await createUser(ctx.module, { email: 'deniz@demo.com' });
      const product = await createProduct({ sellerId: ahmet.id, categoryId: baseline.categoryId });
      await request(server()).post('/api/wishlist').set(authHeader(deniz)).send({ productId: product.id }).expect(201);
    ...
    scenario('COL-067', async () => {
      ...
      const res = await request(server())
        ...
        .send({ productId: product.id })
        .expect(400);
      expect(res.body.message).toBe('Kendi ürününüzü istek listesine ekleyemezsiniz');
    });
    ...
    scenario('COL-069', async () => {
      const deniz = await createUser(ctx.module);
      // 1) Geçersiz UUID.
      const r1 = await request(server()).post('/api/wishlist').set(authHeader(deniz)).send({ productId: '123' }).expect(400);
      expect(JSON.stringify(r1.body.message)).toContain('Geçerli bir ürün ID giriniz');
      // 2) Boş body.
      const r2 = await request(server()).post('/api/wishlist').set(authHeader(deniz)).send({}).expect(400);
    ...
    scenario('COL-070', async () => {
      ...
      const product = await createProduct({ sellerId: ahmet.id, categoryId: baseline.categoryId });
      await request(server()).post('/api/wishlist').set(authHeader(deniz)).send({ productId: product.id }).expect(201);
      await request(server()).delete(`/api/wishlist/${product.id}`).set(authHeader(deniz)).expect(204);
      const check = await request(server()).get(`/api/wishlist/check/${product.id}`).set(authHeader(deniz)).expect(200);
    ...
    scenario('COL-071', async () => {
      ...
      const deniz = await createUser(ctx.module, { email: 'deniz@demo.com' });
      const product = await createProduct({ sellerId: ahmet.id, categoryId: baseline.categoryId });
      const other = await createProduct({ sellerId: ahmet.id, categoryId: baseline.categoryId });
      // deniz'in wishlist'i var ama bu ürün listede değil → 404 "Ürün istek listenizde değil".
      await request(server()).post('/api/wishlist').set(authHeader(deniz)).send({ productId: product.id }).expect(201);
      const res = await request(server()).delete(`/api/wishlist/${other.id}`).set(authHeader(deniz)).expect(404);
    ...
    scenario('COL-072', async () => {
      ...
      const res = await request(server()).delete(`/api/wishlist/${product.id}`).set(authHeader(ceren)).expect(404);
    ...
    scenario('COL-073', async () => {
      const ahmet = await createUser(ctx.module, { email: 'ahmet@demo.com', isSeller: true });
      const deniz = await createUser(ctx.module, { email: 'deniz@demo.com' });
      const inList = await createProduct({ sellerId: ahmet.id, categoryId: baseline.categoryId });
      const notIn = await createProduct({ sellerId: ahmet.id, categoryId: baseline.categoryId });
      await request(server()).post('/api/wishlist').set(authHeader(deniz)).send({ productId: inList.id }).expect(201);
      const r1 = await request(server()).get(`/api/wishlist/check/${inList.id}`).set(authHeader(deniz)).expect(200);
      expect(r1.body.inWishlist).toBe(true);
      const r2 = await request(server()).get(`/api/wishlist/check/${notIn.id}`).set(authHeader(deniz)).expect(200);
    ...
    scenario('COL-074', async () => {
      ...
      const empty = await request(server()).get('/api/wishlist').set(authHeader(deniz)).expect(200);
      expect(empty.body.items).toEqual([]);
      expect(empty.body.totalItems).toBe(0);
      // Dolu.
      const p1 = await createProduct({ sellerId: ahmet.id, categoryId: baseline.categoryId });
      const p2 = await createProduct({ sellerId: ahmet.id, categoryId: baseline.categoryId });
      await request(server()).post('/api/wishlist').set(authHeader(deniz)).send({ productId: p1.id }).expect(201);
      await request(server()).post('/api/wishlist').set(authHeader(deniz)).send({ productId: p2.id }).expect(201);
      const full = await request(server()).get('/api/wishlist').set(authHeader(deniz)).expect(200);
    ...
    scenario('COL-075', async () => {
      ...
      const product = await createProduct({ sellerId: ahmet.id, categoryId: baseline.categoryId });
      await request(server()).post('/api/wishlist').set(authHeader(deniz)).send({ productId: product.id }).expect(201);
      await request(server()).delete('/api/wishlist').set(authHeader(deniz)).expect(204);
      const after = await request(server()).get('/api/wishlist').set(authHeader(deniz)).expect(200);
    ...
    scenario('COL-076', async () => {
      ...
      await request(server()).delete('/api/wishlist').set(authHeader(kaan)).expect(204);
    });

    scenario('COL-077', async () => {
      // Tüm favori uçları anonim → 401.
      await request(server()).get('/api/wishlist').expect(401);
      await request(server()).post('/api/wishlist').send({ productId: RANDOM_UUID }).expect(401);
      await request(server()).delete(`/api/wishlist/${RANDOM_UUID}`).expect(401);
      await request(server()).get(`/api/wishlist/check/${RANDOM_UUID}`).expect(401);
    ...
    scenario('COL-078', async () => {
      ...
      await request(server()).post('/api/wishlist').set(authHeader(deniz)).send({ productId: product.id }).expect(201);
      const res = await request(server()).get('/api/wishlist').set(authHeader(deniz)).expect(200);
    ...
    scenario('COL-079', async () => {
      ...
      const ahmet = await createUser(ctx.module, { email: 'ahmet@demo.com', isSeller: true });
      const deniz = await createUser(ctx.module, { email: 'deniz@demo.com' });
      const product = await createProduct({ sellerId: ahmet.id, categoryId: baseline.categoryId, status: 'active' });
      await request(server()).post('/api/wishlist').set(authHeader(deniz)).send({ productId: product.id }).expect(201);
      const prisma = getPrisma();
    ...
    scenario('COL-080', async () => {
      ...
      const ahmet = await createUser(ctx.module, { email: 'ahmet@demo.com', isSeller: true });
      const deniz = await createUser(ctx.module, { email: 'deniz@demo.com' });
      const keep = await createProduct({ sellerId: ahmet.id, categoryId: baseline.categoryId });
      const drop = await createProduct({ sellerId: ahmet.id, categoryId: baseline.categoryId });
      await request(server()).post('/api/wishlist').set(authHeader(deniz)).send({ productId: keep.id }).expect(201);
      await request(server()).post('/api/wishlist').set(authHeader(deniz)).send({ productId: drop.id }).expect(201);
      const prisma = getPrisma();
    ...
    scenario('COL-086', async () => {
      ...
      const product = await createProduct({ sellerId: ahmet.id, categoryId: baseline.categoryId });
      const fire = () =>
        request(server()).post('/api/wishlist').set(authHeader(deniz)).send({ productId: product.id });
    ...
    scenario('COL-099', async () => {
      ...
      const product = await createProduct({ sellerId: ahmet.id, categoryId: baseline.categoryId });
      const prisma = getPrisma();
      const read = async () => (await prisma.product.findUnique({ where: { id: product.id } }))?.likeCount;
      ...
      expect(await read()).toBe(1);
      await request(server()).delete('/api/wishlist').set(authHeader(deniz)).expect(204);
      expect(await prisma.wishlistItem.count({ where: { productId: product.id } })).toBe(0);
  ...
  describe('Authorization matrix & i18n', () => {
    scenario('COL-081', async () => {
      ...
      const post = (u: { accessToken: string }) =>
        request(server()).post('/api/collections').set(authHeader(u)).send(createBody({ name: `Mat-${Math.random().toString(36).slice(2, 8)}` }));
    ...
    scenario('COL-082', async () => {
      ...
      await disableFreeTierCollections();
      const platform = await createUser(ctx.module, { email: 'platform@tarodan.com', isSeller: true, sellerType: 'platform' });
      const res = await request(server())
        .post('/api/collections')
    ...
    scenario('COL-083', async () => {
      ...
      const ahmet = await premiumUser();
      const col = await seedCollection({ userId: ahmet.id, name: 'Misafir', slug: 'misafir', isPublic: true });
      // Public okuma uçları.
      await request(server()).get('/api/collections/browse').expect(200);
      await request(server()).get(`/api/collections/user/${ahmet.id}`).expect(200);
      ...
      await request(server()).get('/api/collections/liked').expect(401);
      await request(server()).post(`/api/collections/${col.id}/like`).expect(401);
      await request(server()).post('/api/collections').send(createBody()).expect(401);
    });
    ...
    scenario('COL-084', async () => {
      ...
      await request(server()).post('/api/wishlist').set(authHeader(deniz)).send({ productId: product.id }).expect(201);
      // 2) Koleksiyon → 403.
      await request(server()).post('/api/collections').set(authHeader(deniz)).send(createBody({ name: 'Deniz Set' })).expect(403);
    });
  ...
  // ════════════════════════════════════════════════════════════════════════
  scenario.skip('COL-094', 'Web /wishlist → /favorites yönlendirmesi: Next.js rota davranışı (apps/web), API kapsamı dışı.');
  scenario.skip('COL-095', 'Koleksiyon/favori i18n anahtar paritesi (tr.json vs en.json): statik dosya karşılaştırması, e2e API testi değil.');
  scenario.skip('COL-096', 'Web vs Mobile koleksiyon oluşturma parite (premium modal vs snackbar): istemci-UI davranışı; backend 403 COL-002/COL-081/COL-084 ile kapsanır.');
  scenario.skip('COL-097', 'Web vs Mobile favori statü badge paritesi: istemci-UI render karşılaştırması; backend statüsü COL-079 ile kapsanır.');
  scenario.skip('COL-102', 'Koleksiyon listesi boş durumu: UI empty-state + CTA görünümü, API kapsamı dışı.');
  scenario.skip('COL-103', 'Favori listesi boş durumu: UI empty-state metni, API kapsamı dışı.');
  scenario.skip('COL-104', 'Favori/Koleksiyon yükleniyor & hata UI: istemci loading/retry davranışı; GET /wishlist hata-tolerans dalı backend tarafında yapay hata enjeksiyonu gerektirir, saf-API e2e değil.');
  scenario.skip('COL-105', 'Misafire favori/beğeni CTA & login yönlendirme: UI akışı; API 401 davranışı COL-077/COL-083/COL-049 ile kapsanır.');
});
