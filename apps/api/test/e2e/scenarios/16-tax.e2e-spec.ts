...
describe('16 — Vergi & Fatura (TAX)', () => {
   ...
   */
  async function seedTaxRegionWithDefaultRate(opts: {
    countryCode?: string;
  ...
  async function makeCorporate(sellerId: string, taxId = '1234567890'): Promise<void> {
    await getPrisma().user.update({
      where: { id: sellerId },
      data: { businessStatus: 'approved' as any, taxId, sellerType: 'business' as any },
    });
  }
  ...
  describe('GET /api/tax/calculate (public)', () => {
    scenario('TAX-001', async () => {
      await seedTaxRegionWithDefaultRate({ countryCode: 'TR', regionCode: null, rate: 20, name: 'KDV Standart' });
    ...
    scenario('TAX-002', async () => {
      ...
      const res = await request(server()).get('/api/tax/calculate?countryCode=TR').expect(200);
      expect(res.body.rate).toBe(20);
    ...
    scenario('TAX-005', async () => {
      ...
      await seedTaxRegionWithDefaultRate({ countryCode: 'TR', regionCode: null, rate: 20, name: 'TR-Genel' });
      await seedTaxRegionWithDefaultRate({ countryCode: 'TR', regionCode: '34', rate: 18, name: 'TR-34' });
      const res = await request(server())
    ...
    scenario('TAX-007', async () => {
      ...
      await seedTaxRegionWithDefaultRate({ countryCode: 'TR', regionCode: null, rate: 20, isDefault: true, name: 'TR-Default' });
      const res = await request(server())
    ...
    scenario('TAX-008', async () => {
      ...
      const { regionId } = await seedTaxRegionWithDefaultRate({ rate: 20, name: 'Default' });
      const prisma = getPrisma() as any;
      const catRate = await prisma.taxRate.create({
        data: { taxRegionId: regionId, name: 'Kategori %10', rate: 10, isActive: true },
      });
      await prisma.taxRule.create({
        data: { taxRegionId: regionId, taxRateId: catRate.id, scope: 'category', categoryId: baseline.categoryId, priority: 1, isActive: true },
      });
      const res = await request(server())
        .get(`/api/tax/calculate?countryCode=TR&categoryId=${baseline.categoryId}&subtotal=1000`)
        .expect(200);
    ...
    scenario('TAX-009', async () => {
      ...
      const otherCat = await prisma.category.create({
        data: { name: 'Other Cat', slug: 'other-cat', sortOrder: 1, isActive: true },
      });
      const res = await request(server())
        .get(`/api/tax/calculate?countryCode=TR&categoryId=${otherCat.id}&subtotal=1000`)
        .expect(200);
      expect(res.body.rate).toBe(20);
      expect(res.body.taxAmount).toBe(200);
    });
    ...
    scenario('TAX-011', async () => {
      ...
      const rate8 = await prisma.taxRate.create({
        data: { taxRegionId: regionId, name: 'Kat %8', rate: 8, isActive: true },
      });
      const rate12 = await prisma.taxRate.create({
        data: { taxRegionId: regionId, name: 'Kat %12', rate: 12, isActive: true },
      });
      await prisma.taxRule.create({
        data: { taxRegionId: regionId, taxRateId: rate12.id, scope: 'category', categoryId: baseline.categoryId, priority: 1, isActive: true },
      });
      await prisma.taxRule.create({
        data: { taxRegionId: regionId, taxRateId: rate8.id, scope: 'category', categoryId: baseline.categoryId, priority: 5, isActive: true },
      });
      const res = await request(server())
        .get(`/api/tax/calculate?countryCode=TR&categoryId=${baseline.categoryId}&subtotal=1000`)
        .expect(200);
    ...
    scenario('TAX-087', async () => {
      ...
      const { regionId } = await seedTaxRegionWithDefaultRate({ rate: 20, name: 'Expired' });
      const prisma = getPrisma() as any;
      // Rate'e geçmiş effectiveTo ata (resolveTaxRate tarih filtresi yapmaz).
      await prisma.taxRate.updateMany({
        where: { taxRegionId: regionId },
        data: { effectiveFrom: new Date('2000-01-01'), effectiveTo: new Date('2001-01-01') },
      });
      const res = await request(server())
        .get('/api/tax/calculate?countryCode=TR&subtotal=1000')
        .expect(200);
    ...
    scenario('TAX-088', async () => {
      ...
      const productRate = await prisma.taxRate.create({
        data: { taxRegionId: regionId, name: 'Product %5', rate: 5, isActive: true },
      });
      await prisma.taxRule.create({
        data: { taxRegionId: regionId, taxRateId: productRate.id, scope: 'product', priority: 10, isActive: true },
      });
      const res = await request(server())
        .get(`/api/tax/calculate?countryCode=TR&categoryId=${baseline.categoryId}&subtotal=1000`)
        .expect(200);
    ...
    scenario('TAX-089', async () => {
      ...
      expect(Math.round(res.body.taxAmount * 100) / 100).toBe(res.body.taxAmount);
    });
  });
  ...
  describe('Sipariş bazlı KDV', () => {
    scenario('TAX-012', async () => {
      ...
      const { buyer, seller, product, addr } = await makeBuyerSellerProduct({ price: 1000 });
    ...
    scenario('TAX-014', async () => {
      ...
      await getPrisma().user.update({
        where: { id: seller.id },
        data: { businessStatus: 'approved' as any, taxId: null, sellerType: 'business' as any },
      });
      await seedTaxRegionWithDefaultRate({ rate: 20 });
      const { orderId } = await buyAndPay(buyer, product.id, addr.id);
    ...
    scenario('TAX-015', async () => {
      ...
      await getPrisma().user.update({
        where: { id: seller.id },
        data: { businessStatus: 'approved' as any, taxId: '9999999999', sellerType: 'platform' as any },
      });
      await seedTaxRegionWithDefaultRate({ rate: 20 });
      const { orderId } = await buyAndPay(buyer, product.id, addr.id);
    ...
    scenario('TAX-018', async () => {
      ...
      expect(p.totalAmount).toBe(p.subtotal + p.shippingAmount + p.buyerFeeAmount + p.taxAmount);
    });
  });
  ...
  describe('Otomatik fatura üretimi (ödeme başarısı)', () => {
    ...
    scenario('TAX-095', async () => {
      ...
      const group = await prisma.checkoutGroup.create({
        data: { groupNumber: `GRPGUEST${Date.now()}`, buyerId: guestUser.id, totalAmount: 500, isGuest: true },
      });
      ...
      const { InvoiceService } = await import('../../../src/modules/invoice/invoice.service');
  ...
  describe('Fatura numarası / mükerrer', () => {
    scenario('TAX-037', async () => {
      ...
      const product2 = await createProduct({ sellerId: seller2.id, categoryId: baseline.categoryId, price: 200, quantity: 1 });
    ...
    scenario('TAX-039', async () => {
      ...
      const [a, b] = await Promise.all([
        request(server()).post(`/api/invoices/generate/${orderId}`).set(authHeader(buyer)),
        request(server()).post(`/api/invoices/generate/${orderId}`).set(authHeader(buyer)),
      ]);
    ...
    scenario('TAX-040', async () => {
      ...
      const g1 = await request(server()).post(`/api/invoices/generate/${orderId}`).set(authHeader(buyer)).expect(201);
      const g2 = await request(server()).post(`/api/invoices/generate/${orderId}`).set(authHeader(buyer)).expect(201);
  ...
  describe('Public fatura uçları (paymentId doğrulaması)', () => {
    scenario('TAX-047', async () => {
      const { buyer, product, addr } = await makeBuyerSellerProduct({ price: 200 });
      const { orderId, paymentId } = await buyAndPay(buyer, product.id, addr.id);
      const res = await request(server())
    ...
    scenario('TAX-049', async () => {
      ...
      const { orderId } = await buyAndPay(buyer, product.id, addr.id);
      await request(server())
        .get(`/api/invoices/order/${orderId}/public?paymentId=${NONEXISTENT_UUID}`)
        .expect(404);
    });
    ...
    scenario('TAX-050', async () => {
      ...
      expect(invoice).toBeTruthy();
      const res = await request(server())
        .get(`/api/invoices/download/${invoice!.id}/public?paymentId=${paymentId}`)
        .expect(200);
      expect(res.headers['content-type']).toContain('application/pdf');
      expect(res.headers['content-disposition']).toContain('attachment');
      expect(res.headers['content-disposition']).toContain(`fatura-${invoice!.id}.pdf`);
    });
    ...
    scenario('TAX-092', async () => {
      ...
      await request(server())
        ...
        .expect(404);
      await request(server())
        .get(`/api/invoices/download/${invoice!.id}/public?paymentId=${NONEXISTENT_UUID}`)
        .expect(404);
    });
    ...
    scenario('TAX-093', async () => {
      ...
      expect(dbInvoice!.pdfUrl === '' || dbInvoice!.pdfUrl?.startsWith('http') === false).toBe(true);
  ...
  describe('PDF indirme (GET /invoices/download/:id)', () => {
    ...
    scenario('TAX-031', async () => {
      ...
      const sellerRow = await prisma.user.findUnique({ where: { id: seller.id } });
    ...
    scenario('TAX-022', async () => {
      ...
      const prisma = getPrisma();
      const invoice = await prisma.invoice.findFirst({ where: { orderId } });
      const derivedRate = Math.round((Number(invoice!.taxAmount) / Number(invoice!.subtotal)) * 100);
    ...
    scenario('TAX-033', async () => {
      ...
      const { buyer, product, addr } = await makeBuyerSellerProduct({ price: 500 });
      const { orderId } = await buyAndPay(buyer, product.id, addr.id);
      const prisma = getPrisma();
      const invoice = await prisma.invoice.findFirst({ where: { orderId } });
      const res = await request(server())
    ...
    scenario('TAX-035', async () => {
      ...
      const { buyer, product, addr } = await makeBuyerSellerProduct({ price: 750, quantity: 5 });
      const { orderId } = await buyAndPay(buyer, product.id, addr.id);
      const prisma = getPrisma();
    ...
    scenario('TAX-036', async () => {
      ...
      const { orderId } = await buyAndPay(buyer, product.id, addr.id);
      const prisma = getPrisma();
      await prisma.order.update({ where: { id: orderId }, data: { quantity: 3, unitPrice: 250, subtotal: 750 } });
      const invoice = await prisma.invoice.findFirst({ where: { orderId } });
  ...
  describe('İade faturası (generateRefundInvoice)', () => {
    scenario('TAX-054', async () => {
      ...
      const { InvoiceService } = await import('../../../src/modules/invoice/invoice.service');
      const invoiceService = ctx.module.get(InvoiceService);
      const refundNumber = await invoiceService.generateRefundInvoice(orderId, 300);
      expect(refundNumber).toBe(`${original!.invoiceNumber}-R`);

      const refund = await prisma.invoice.findFirst({ where: { invoiceNumber: refundNumber } });
    ...
    scenario('TAX-055', async () => {
      ...
      const invoiceService = ctx.module.get(InvoiceService);
      await expect(invoiceService.generateRefundInvoice(orderId, 100)).rejects.toBeInstanceOf(NotFoundException);
    });
  ...
  describe('Admin vergi bölgeleri', () => {
    scenario('TAX-058', async () => {
      const admin = await createAdminUser(ctx.module, { email: 'tax-super@test.com' });
      const res = await request(server())
      ...
      const prisma = getPrisma();
      const audit = await prisma.auditLog.findFirst({ where: { action: 'tax_region_create' } });
      expect(audit).toBeTruthy();
    });

    scenario('TAX-059', async () => {
      const { AdminRole } = await import('@prisma/client');
      const moderator = await createAdminUser(ctx.module, { email: 'tax-mod@test.com', role: AdminRole.moderator });
      await request(server())
        .post('/api/admin/tax/regions')
    ...
    scenario('TAX-060', async () => {
      const { AdminRole } = await import('@prisma/client');
      const moderator = await createAdminUser(ctx.module, { email: 'tax-mod-read@test.com', role: AdminRole.moderator });
      await request(server()).get('/api/admin/tax/regions').set(authHeader(moderator)).expect(200);
      await request(server()).get('/api/admin/tax/rates').set(authHeader(moderator)).expect(200);
      await request(server()).get('/api/admin/tax/rules').set(authHeader(moderator)).expect(200);
    });
    ...
    scenario('TAX-062', async () => {
      ...
      const admin = await createAdminUser(ctx.module, { email: 'tax-dup@test.com' });
      await request(server())
        .post('/api/admin/tax/regions')
        .set(authHeader(admin))
    ...
    scenario('TAX-063', async () => {
      ...
      const admin = await createAdminUser(ctx.module, { email: 'tax-def@test.com' });
      const a = await request(server())
        .post('/api/admin/tax/regions')
        .set(authHeader(admin))
        .send({ name: 'A', countryCode: 'TR', regionCode: '01', isDefault: true })
        .expect(201);
      const b = await request(server())
        .post('/api/admin/tax/regions')
        .set(authHeader(admin))
        .send({ name: 'B', countryCode: 'TR', regionCode: '02', isDefault: true })
        .expect(201);
      expect(b.body.isDefault).toBe(true);
      const prisma = getPrisma() as any;
      const aRow = await prisma.taxRegion.findUnique({ where: { id: a.body.id } });
      expect(aRow.isDefault).toBe(false);
    });

    scenario('TAX-064', async () => {
      // Oranı olan bölgeyi silme → 400.
      const admin = await createAdminUser(ctx.module, { email: 'tax-del-rate@test.com' });
    ...
    scenario('TAX-065', async () => {
      ...
      const admin = await createAdminUser(ctx.module, { email: 'tax-del-empty@test.com' });
      const prisma = getPrisma() as any;
      const region = await prisma.taxRegion.create({
        data: { name: 'Bos Bölge', countryCode: 'TR', regionCode: '99', sortOrder: 0, isActive: true },
      });
      await request(server())
        .delete(`/api/admin/tax/regions/${region.id}`)
        .set(authHeader(admin))
        .expect(204);
      const gone = await prisma.taxRegion.findUnique({ where: { id: region.id } });
      expect(gone).toBeNull();
      const audit = await getPrisma().auditLog.findFirst({ where: { action: 'tax_region_delete' } });
      expect(audit).toBeTruthy();
    });
  });
  ...
  describe('Admin vergi oranları', () => {
    async function regionFor(): Promise<string> {
      const prisma = getPrisma() as any;
      const region = await prisma.taxRegion.create({
        data: { name: 'Oran Bölge', countryCode: 'TR', regionCode: null, sortOrder: 0, isActive: true },
      });
      return region.id;
    }

    scenario('TAX-066', async () => {
      const admin = await createAdminUser(ctx.module, { email: 'rate-hi@test.com' });
    ...
    scenario('TAX-067', async () => {
      const admin = await createAdminUser(ctx.module, { email: 'rate-neg@test.com' });
      const taxRegionId = await regionFor();
      await request(server())
        .post('/api/admin/tax/rates')
        .set(authHeader(admin))
    ...
    scenario('TAX-068', async () => {
      ...
      const admin = await createAdminUser(ctx.module, { email: 'rate-bound@test.com' });
      const taxRegionId = await regionFor();
      await request(server())
        .post('/api/admin/tax/rates')
        .set(authHeader(admin))
    ...
    scenario('TAX-069', async () => {
      ...
      const admin = await createAdminUser(ctx.module, { email: 'rate-404@test.com' });
      await request(server())
        .post('/api/admin/tax/rates')
        .set(authHeader(admin))
  ...
  describe('Admin vergi kuralları', () => {
    async function seedRegionAndRate(): Promise<{ regionId: string; rateId: string }> {
      const prisma = getPrisma() as any;
      const region = await prisma.taxRegion.create({
        data: { name: 'Kural Bölge', countryCode: 'TR', regionCode: null, sortOrder: 0, isActive: true },
      });
      const rate = await prisma.taxRate.create({
    ...
    scenario('TAX-070', async () => {
      ...
      const admin = await createAdminUser(ctx.module, { email: 'rule-cat@test.com' });
    ...
    scenario('TAX-071', async () => {
      ...
      const admin = await createAdminUser(ctx.module, { email: 'rule-mismatch@test.com' });
      const { regionId: regionA } = await seedRegionAndRate();
      const prisma = getPrisma() as any;
      const regionB = await prisma.taxRegion.create({
        data: { name: 'B', countryCode: 'TR', regionCode: '35', sortOrder: 0, isActive: true },
      });
      const rateB = await prisma.taxRate.create({
        data: { taxRegionId: regionB.id, name: 'B-Rate', rate: 10, isActive: true },
      });
      await request(server())
        .post('/api/admin/tax/rules')
        .set(authHeader(admin))
        .send({ taxRegionId: regionA, taxRateId: rateB.id, scope: 'default_rate' })
        .expect(400);
    });

    scenario('TAX-072', async () => {
      // scope geçersiz enum → 400 (@IsEnum).
      const admin = await createAdminUser(ctx.module, { email: 'rule-enum@test.com' });
      const { regionId, rateId } = await seedRegionAndRate();
      await request(server())
        .post('/api/admin/tax/rules')
        .set(authHeader(admin))
    ...
    scenario('TAX-073', async () => {
      ...
      const { AdminRole } = await import('@prisma/client');
      const superAdmin = await createAdminUser(ctx.module, { email: 'rule-super@test.com' });
      const moderator = await createAdminUser(ctx.module, { email: 'rule-mod@test.com', role: AdminRole.moderator });
      const { regionId, rateId } = await seedRegionAndRate();
      const created = await request(server())
        .post('/api/admin/tax/rules')
        .set(authHeader(superAdmin))
        .send({ taxRegionId: regionId, taxRateId: rateId, scope: 'default_rate' })
        .expect(201);
      await request(server())
  ...
  describe('Admin vergi raporu (GET /admin/tax/report)', () => {
    scenario('TAX-074', async () => {
      ...
      const admin = await createAdminUser(ctx.module, { email: 'report-month@test.com' });
      const res = await request(server())
    ...
    scenario('TAX-075', async () => {
      ...
      await seedTaxRegionWithDefaultRate({ rate: 20 });
      await buyAndPay(buyer, product.id, addr.id);
      const admin = await createAdminUser(ctx.module, { email: 'report-group@test.com' });

      const day = await request(server())
        .get('/api/admin/tax/report?fromDate=2026-01-01&toDate=2026-12-31&groupBy=day')
        .set(authHeader(admin))
        .expect(200);
      expect(day.body.breakdown[0].period).toMatch(/^\d{4}-\d{2}-\d{2}$/);

      const year = await request(server())
        .get('/api/admin/tax/report?fromDate=2026-01-01&toDate=2026-12-31&groupBy=year')
        .set(authHeader(admin))
        .expect(200);
    ...
    scenario('TAX-076', async () => {
      const admin = await createAdminUser(ctx.module, { email: 'report-range@test.com' });
      await request(server())
    ...
    scenario('TAX-077', async () => {
      const { AdminRole } = await import('@prisma/client');
      const moderator = await createAdminUser(ctx.module, { email: 'report-mod@test.com', role: AdminRole.moderator });
      await request(server())
    ...
    scenario('TAX-078', async () => {
      ...
      const admin = await createAdminUser(ctx.module, { email: 'report-default@test.com' });
      const res = await request(server())
        .get('/api/admin/tax/report')
        .set(authHeader(admin))
        .expect(200);
    ...
    scenario('TAX-079', async () => {
      ...
      const admin = await createAdminUser(ctx.module, { email: 'report-zero@test.com' });
      await seedTaxRegionWithDefaultRate({ rate: 20 });
      ...
      const corpProduct = await createProduct({ sellerId: corpSeller.id, categoryId: baseline.categoryId, price: 1000, quantity: 1 });
      ...
      const indProduct = await createProduct({ sellerId: indSeller.id, categoryId: baseline.categoryId, price: 500, quantity: 1 });
    ...
    scenario('TAX-057', async () => {
      ...
      const admin = await createAdminUser(ctx.module, { email: 'report-cancel@test.com' });
      await seedTaxRegionWithDefaultRate({ rate: 20 });
      const buyer = await createUser(ctx.module);
      const seller = await createUser(ctx.module, { isSeller: true });
      await makeCorporate(seller.id);
      ...
      const prisma = getPrisma();
      // Faturayı iptal et.
      await prisma.invoice.updateMany({ where: { orderId }, data: { status: 'cancelled', cancelledAt: new Date() } });
  ...
  scenario.skip('TAX-080', 'Saf-UI: web sipariş detayı KDV satırı (TR/EN) — apps/web bileşeni, API e2e kapsamı dışında');
  scenario.skip('TAX-081', 'Saf-UI parite: mobil sipariş detayı KDV satırı + PDF indirme yok — apps/mobile bileşeni');
  scenario.skip('TAX-082', 'Saf-UI parite: web fatura indir butonu yalnız alıcıda — apps/web bileşeni');
  scenario.skip('TAX-083', 'Saf-UI parite: misafir payment/success public fatura indirme — apps/web sayfası (API yolları TAX-047/050/051 ile kapsanır)');
  scenario.skip('TAX-084', 'Saf-UI: success sayfası loading→error durumu — apps/web sayfa state');
  scenario.skip('TAX-085', 'Saf-UI parite: admin tax sayfası 4 sekme — apps/admin bileşeni');
});
