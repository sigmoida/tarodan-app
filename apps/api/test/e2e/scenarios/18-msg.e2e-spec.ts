...
describe('18 — Mesajlaşma & Bildirim (MSG)', () => {
  ...
  describe('POST /api/messages/threads', () => {
    scenario('MSG-001', async () => {
      ...
      expect(res.body.lastMessage?.content).toBe('Merhaba, ürününüzle ilgileniyorum');
    ...
    scenario('MSG-004', async () => {
      ...
      const noProduct = await createThread(deniz, { recipientId: ahmet.id }).expect(201);
    ...
    scenario('MSG-005', async () => {
      const deniz = await createUser(ctx.module);
      const res = await createThread(deniz, { recipientId: deniz.id }).expect(400);
      expect(JSON.stringify(res.body)).toContain('Kendinize mesaj gönderemezsiniz');
    });

    scenario('MSG-006', async () => {
      const deniz = await createUser(ctx.module);
      const res = await createThread(deniz, { recipientId: NIL_UUID }).expect(404);
      expect(JSON.stringify(res.body)).toContain('Alıcı kullanıcı bulunamadı');
    });

    scenario('MSG-007', async () => {
      const deniz = await createUser(ctx.module);
      const res = await createThread(deniz, { recipientId: 'not-a-uuid' }).expect(400);
      expect(JSON.stringify(res.body)).toContain('Geçerli bir kullanıcı ID giriniz');
    });
    ...
    scenario('MSG-010', async () => {
      ...
      const deniz = await createUser(ctx.module);
      const ahmet = await createUser(ctx.module);
      const res = await createThread(deniz, { recipientId: ahmet.id }).expect(201);
      expect(res.body.lastMessage).toBeUndefined();

      const prisma = getPrisma();
      const msgCount = await prisma.message.count({ where: { threadId: res.body.id } });
    ...
    scenario('MSG-099', async () => {
      ...
      const results = await Promise.allSettled([
        createThread(deniz, { recipientId: ahmet.id, productId: product.id, message: 'a' }),
        createThread(deniz, { recipientId: ahmet.id, productId: product.id, message: 'b' }),
      ]);
      // En az biri başarılı olmalı; çift thread OLUŞMAMALI (@@unique).
      const ok = results.filter((r) => r.status === 'fulfilled' && (r.value as any).status === 201);
      expect(ok.length).toBeGreaterThanOrEqual(1);

      const prisma = getPrisma();
      const count = await prisma.messageThread.count({ where: { productId: product.id } });
      expect(count).toBe(1);
    });
  });
  ...
  describe('POST /api/messages/threads/:id/messages', () => {
    scenario('MSG-011', async () => {
      ...
      const res = await sendMessage(deniz, threadId, 'Fiyatta indirim olur mu?').expect(201);
    ...
    scenario('MSG-012', async () => {
      ...
      const res = await sendMessage(deniz, threadId, 'a'.repeat(1000)).expect(201);
      expect(res.body.status).toBe('sent');
    });
    ...
    scenario('MSG-016', async () => {
      ...
      await prisma.platformSetting.create({
        data: { settingKey: 'max_message_length', settingValue: '200', settingType: 'number' },
      });
      const { deniz, threadId } = await twoUsersWithThread();
      const res = await sendMessage(deniz, threadId, 'a'.repeat(201)).expect(400);
      expect(JSON.stringify(res.body)).toContain('Mesaj uzunluğu maksimum 200 karakter olabilir');
  ...
  describe('GET messages & unread-count', () => {
    ...
    scenario('MSG-020', async () => {
      ...
      const deniz = await createUser(ctx.module);
      const ahmet = await createUser(ctx.module);
      const t = await createThread(ahmet, { recipientId: deniz.id, message: 'm1' }).expect(201);
    ...
    scenario('MSG-022', async () => {
      ...
      await prisma.message.createMany({
        data: [
          { threadId, senderId: ahmet.id, receiverId: deniz.id, content: 'temiz', status: MessageStatus.sent },
          { threadId, senderId: ahmet.id, receiverId: deniz.id, content: 'bekleyen', status: MessageStatus.pending_approval },
          { threadId, senderId: ahmet.id, receiverId: deniz.id, content: 'red', status: MessageStatus.rejected },
        ],
      });
      const res = await request(server())
        .get('/api/messages/unread-count')
    ...
    scenario('MSG-023', async () => {
      ...
      await prisma.message.create({
        data: { threadId, senderId: ahmet.id, receiverId: deniz.id, content: 'eski', status: MessageStatus.sent, createdAt: old },
      });
      await sendMessage(ahmet, threadId, 'yeni').expect(201);

      const res = await getMessages(deniz, threadId, '?since=2026-06-29T00:00:00.000Z').expect(200);
    ...
    scenario('MSG-102', async () => {
      ...
      for (let i = 0; i < 25; i++) {
        ...
        await createThread(deniz, { recipientId: other.id, message: `t${i}` }).expect(201);
      }
      const res = await request(server())
  ...
  describe('Yetki (Authz)', () => {
    scenario('MSG-025', async () => {
      ...
      await request(server()).post('/api/messages/threads').send({ recipientId: NIL_UUID }).expect(401);
    ...
    scenario('MSG-103', async () => {
      ...
      const { threadId } = await twoUsersWithThread({ message: 'çok gizli içerik' });
      const kaan = await createUser(ctx.module);
  ...
  describe('İçerik filtresi & moderasyon', () => {
    scenario('MSG-030', async () => {
      await clearFilters();
      const { deniz, threadId } = await twoUsersWithThread();
      const res = await sendMessage(deniz, threadId, 'Merhaba, teşekkürler').expect(201);
      expect(res.body.status).toBe('sent');
    });
    ...
    scenario('MSG-034', async () => {
      ...
      await clearFilters();
      const { deniz, threadId } = await twoUsersWithThread();
      const res = await sendMessage(deniz, threadId, 'Ürün elimde, ne zaman gönderelim?').expect(201);
      expect(res.body.status).toBe('sent');
    ...
    scenario('MSG-031', async () => {
      ...
      try {
        const { deniz, ahmet, threadId } = await twoUsersWithThread();
        const res = await sendMessage(deniz, threadId, 'Beni 0532 123 45 67 numaradan ara').expect(201);
        expect(res.body.status).toBe('sent');
    ...
    scenario('MSG-032', async () => {
      ...
      await clearFilters();
      const { deniz, threadId } = await twoUsersWithThread();
      const res = await sendMessage(deniz, threadId, 'seni orospu çocuğu').expect(201);
    ...
    scenario('MSG-033', async () => {
      ...
      await clearFilters();
      const { deniz, threadId } = await twoUsersWithThread();
      const clean = await sendMessage(deniz, threadId, 'Ürünü kargoya götür lütfen').expect(201);
    ...
    scenario('MSG-036', async () => {
      ...
      const prisma = getPrisma();
      const dbMsg = await prisma.message.findUnique({ where: { id: sent.body.id } });
    ...
    scenario('MSG-038', async () => {
      ...
      const again = await request(server())
        ...
        .send({ action: 'reject' })
        .expect(400);
      expect(JSON.stringify(again.body)).toContain('Bu mesaj onay beklemiyordu');
    });
    ...
    scenario('MSG-040', async () => {
      ...
      try {
        ...
        expect(list.body.some((f: any) => f.name === 'Test Numara Filtresi')).toBe(true);
    ...
    scenario('MSG-104', async () => {
      ...
      await clearFilters();
      const { deniz, ahmet, threadId } = await twoUsersWithThread();
      const sent = await sendMessage(deniz, threadId, 'lan orospu gizli-ifsa').expect(201);
      expect(sent.body.status).toBe('pending_approval');
      const msgs = await getMessages(ahmet, threadId).expect(200);
    ...
    scenario('MSG-105', async () => {
      ...
      try {
        const { deniz, threadId } = await twoUsersWithThread();
        const res = await sendMessage(deniz, threadId, '0 5 3 2 1 2 3 4 5 6 7').expect(201);
        // Kalıp eşleşti → onaya düşer veya maskelenir; her hâlükârda temiz 'sent' DEĞİL
        // veya flaggedReason dolu.
        expect(res.body.status === 'pending_approval' || !!res.body.flaggedReason).toBe(true);
      } finally {
        await clearFilters();
      }
    });
  ...
  describe('Günlük limit', () => {
    ...
    scenario('MSG-043', async () => {
      ...
      const prisma = getPrisma();
      await prisma.platformSetting.create({
        data: { settingKey: 'daily_message_limit', settingValue: '1', settingType: 'number' },
      });
      const { deniz, threadId } = await twoUsersWithThread();
  ...
  describe('Bildirimler (/api/notifications)', () => {
    async function seedNotif(userId: string, opts: { read?: boolean; type?: string; data?: any } = {}) {
    ...
    scenario('MSG-058', async () => {
      ...
      const prisma = getPrisma();
      const refreshed = await prisma.notificationLog.findUnique({ where: { id: notif.id } });
    ...
    scenario('MSG-059', async () => {
      ...
      const prisma = getPrisma();
      const refreshed = await prisma.notificationLog.findUnique({ where: { id: cerenNotif.id } });
    ...
    scenario('MSG-061', async () => {
      ...
      const res = await request(server())
        ...
        .set(authHeader(deniz))
        .expect(200);
      const newMsgNotifs = res.body.notifications.filter((n: any) => n.type === 'new_message');
    ...
    scenario('MSG-062', async () => {
      ...
      const deniz = await createUser(ctx.module, { displayName: 'Deniz' });
      const ahmet = await createUser(ctx.module, { displayName: 'Ahmet' });
      const t = await createThread(ahmet, { recipientId: deniz.id, message: 'selam' }).expect(201);

      const res = await request(server())
        .get('/api/notifications')
        .set(authHeader(deniz))
        .expect(200);
      const n = res.body.notifications.find((x: any) => x.type === 'new_message');
      expect(n).toBeTruthy();
      expect(n.icon).toBe('💬');
    ...
    scenario('MSG-063', async () => {
      ...
      const { deniz, ahmet, threadId } = await twoUsersWithThread();
      await sendMessage(deniz, threadId, 'Merhaba bu bir test mesajıdır').expect(201);
      const prisma = getPrisma();
      const notif = await prisma.notificationLog.findFirst({
        where: { userId: ahmet.id, type: 'new_message', channel: 'in_app' },
      });
  ...
  describe('Push token (/api/notifications/push-token)', () => {
    const registerToken = (user: CreatedTestUser, body: Record<string, unknown>) =>
      request(server())
    ...
    scenario('MSG-068', async () => {
      ...
      const prisma = getPrisma();
      const row = await prisma.pushToken.findFirst({ where: { userId: deniz.id } });
    ...
    scenario('MSG-069', async () => {
      ...
      const deniz = await createUser(ctx.module);
      const res = await registerToken(deniz, { token: 'random-token', platform: 'ios' }).expect(200);
      expect(res.body.success).toBe(false);
      expect(res.body.error).toContain('Invalid Expo push token format');

      const prisma = getPrisma();
      const count = await prisma.pushToken.count({ where: { userId: deniz.id } });
      expect(count).toBe(0);
    });
    ...
    scenario('MSG-071', async () => {
      ...
      const prisma = getPrisma();
      const rows = await prisma.pushToken.findMany({ where: { userId: deniz.id, token } });
    ...
    scenario('MSG-072', async () => {
      ...
      await registerToken(deniz, { token, platform: 'ios' }).expect(200);
      const res = await registerToken(deniz, { token, revoke: true }).expect(200);
      expect(res.body.success).toBe(true);
      expect(res.body.revoked).toBe(true);

      const prisma = getPrisma();
      const row = await prisma.pushToken.findFirst({ where: { userId: deniz.id, token } });
    ...
    scenario('MSG-073', async () => {
      ...
      const gonderen = await createUser(ctx.module, { displayName: 'Gönderen' });
      const mustafa = await createUser(ctx.module, { displayName: 'Mustafa' });
      const prisma = getPrisma();
      // mustafa'da push token OLMADIĞINI teyit et.
      const tokenCount = await prisma.pushToken.count({ where: { userId: mustafa.id } });
      expect(tokenCount).toBe(0);

      const t = await createThread(gonderen, { recipientId: mustafa.id, message: 'push token yok' }).expect(201);
  ...
  describe('Tercih gating', () => {
    const setSettings = (user: CreatedTestUser, body: Record<string, unknown>) =>
      request(server())
    ...
    scenario('MSG-083', async () => {
      ...
      const deniz = await createUser(ctx.module);
      const res = await setSettings(deniz, { messageAlerts: false }).expect(200);
    ...
    scenario('MSG-079', async () => {
      ...
      const t = await createThread(deniz, { recipientId: ahmet.id, message: 'ilk' }).expect(201);
    ...
    scenario('MSG-080', async () => {
      ...
      const t = await createThread(deniz, { recipientId: ahmet.id, message: 'selam' }).expect(201);
    ...
    scenario('MSG-064', async () => {
      ...
      await createAddress({ userId: seller.id, isDefault: true, city: 'İstanbul' });
      const buyer = await createUser(ctx.module);
      const addr = await createAddress({ userId: buyer.id, isDefault: true, city: 'Ankara' });
      ...
      // Satıcı shipment oluşturur, sonra takip no girer → ORDER_SHIPPED.
  ...
  describe('Admin broadcast & scheduled', () => {
    scenario('MSG-084', async () => {
      ...
      const res = await request(server())
        ...
        .set(authHeader(admin))
        .send({ title: 'Duyuru', body: 'Herkese', channels: ['in_app'], targetType: 'all' })
        .expect(200);
      expect(res.body.success).toBe(true);
      ...
      // Audit log yazıldı.
      const audit = await prisma.auditLog.count({ where: { action: 'notification_send' } });
    ...
    scenario('MSG-085', async () => {
      ...
      await request(server())
        ...
        .send({ pushNotifications: false, orderUpdates: false, messageAlerts: false })
        .expect(200);

      await request(server())
        .post('/api/admin/notifications/send')
        .set(authHeader(admin))
        .send({ title: 'Zorunlu', body: 'Muaf', channels: ['in_app'], targetType: 'all' })
        .expect(200);
    ...
    scenario('MSG-087', async () => {
      ...
      const res = await request(server())
        .post('/api/admin/notifications/send')
        .set(authHeader(admin))
        .send({ title: 't', body: 'b', channels: ['in_app'], targetType: 'user_ids', userIds: [] })
        .expect(400);
    ...
    scenario('MSG-088', async () => {
      ...
      const res = await request(server())
        ...
        .set(authHeader(admin))
        .send({ title: 'Planlı', body: 'İleri', channels: ['in_app'], targetType: 'all', scheduledFor: future })
        .expect(201);
      expect(res.body.status).toBe('pending');
      expect(res.body.id).toBeTruthy();

      const prisma = getPrisma();
      const row = await prisma.scheduledNotification.findUnique({ where: { id: res.body.id } });
      expect(row?.status).toBe('pending');
      const audit = await prisma.auditLog.count({ where: { action: 'notification_schedule' } });
      expect(audit).toBeGreaterThanOrEqual(1);
    });
    ...
    scenario('MSG-089', async () => {
      ...
      const res = await request(server())
        .post('/api/admin/notifications/schedule')
        .set(authHeader(admin))
        .send({ title: 'x', body: 'y', channels: ['in_app'], targetType: 'all', scheduledFor: past })
        .expect(400);
      expect(JSON.stringify(res.body)).toContain('Zamanlama tarihi gelecekte olmalıdır');
    });
    ...
    scenario('MSG-092', async () => {
      ...
      const created = await request(server())
        .post('/api/admin/notifications/schedule')
        .set(authHeader(admin))
        .send({ title: 'İptal edilecek', body: 'b', channels: ['in_app'], targetType: 'all', scheduledFor: future })
        .expect(201);
      ...
      const prisma = getPrisma();
      const row = await prisma.scheduledNotification.findUnique({ where: { id: created.body.id } });
    ...
    scenario('MSG-093', async () => {
      ...
      await request(server())
        .post('/api/admin/notifications/send')
        .set(authHeader(moderator))
        .send({ title: 'Mod', body: 'b', channels: ['in_app'], targetType: 'all' })
        .expect(200);
      ...
      const res = await request(server())
        .post('/api/admin/notifications/send')
        .set(authHeader(deniz))
        .send({ title: 'x', body: 'y', channels: ['in_app'], targetType: 'all' });
  ...
  describe('Kullanıcı engelleme', () => {
    scenario('MSG-053', async () => {
      ...
      const self = await request(server())
        ...
        .set(authHeader(deniz))
        .expect(400);
      expect(JSON.stringify(self.body)).toContain('Kendinizi engelleyemezsiniz');
    });
    ...
    scenario('MSG-054', async () => {
      ...
      const thread = await createThread(deniz, { recipientId: ahmet.id, message: 'engelli olsam da' }).expect(201);
  ...
  // ════════════════════════════════════════════════════════════════════════
  scenario.skip('MSG-035', 'Salt web-client ön-uyarısı (i18n toast); backend kararı MSG-031/105 ile kaplı, API tarafında gözlemlenecek davranış yok.');
  scenario.skip('MSG-041', 'Salt admin-UI paneli (rozet/aksiyon render); API tarafı MSG-029/036/037 ile kaplı.');
  scenario.skip('MSG-044', 'WS bağlantı reddi: paylaşılan test app app.listen() çağırmaz + socket.io-client paketi yok → handshake gözlemlenemez.');
  scenario.skip('MSG-045', 'WS geçersiz token reddi: aynı WS harness kısıtı (bkz. MSG-044).');
  scenario.skip('MSG-046', 'WS connected/kişisel oda: WS istemcisi yok → event alınamaz.');
  scenario.skip('MSG-047', 'WS join:thread yetki reddi: WS istemcisi yok.');
  scenario.skip('MSG-048', 'WS message:new + thread:updated yayılımı: WS istemcisi yok (senkron in-app bildirim MSG-063 ile kaplı).');
  scenario.skip('MSG-049', 'WS pending mesajda emit olmaması: WS istemcisi yok (pending davranışı MSG-032/104 ile kaplı).');
  scenario.skip('MSG-050', 'WS message:read yayılımı: WS istemcisi yok (okundu DB davranışı MSG-019 ile kaplı).');
  scenario.skip('MSG-051', 'WS typing:start/stop: WS istemcisi yok.');
  scenario.skip('MSG-052', 'WS çoklu cihaz/isUserOnline: WS istemcisi yok, in-memory socket haritası gözlemlenemez.');
  scenario.skip('MSG-055', 'Engellemenin API restart’ta kaybı: paylaşılan test app restart edilemez (in-memory Map).');
  scenario.skip('MSG-065', 'OFFER_RECEIVED (create) BullMQ emailQueue/pushQueue üzerinden asenkron; create akışı senkron in-app NotificationLog yazmaz → deterministik assert yok.');
  scenario.skip('MSG-066', 'REFUND_* bildirimleri event/queue tabanlı çok-adımlı akış; test worker deterministik koşmaz → senkron NotificationLog doğrulanamaz.');
  scenario.skip('MSG-067', 'Stokout/BACK_IN_STOCK bildirimleri sweep-cron + queue + 24s debounce ile asenkron; harness’te deterministik değil.');
  scenario.skip('MSG-075', 'Sipariş-oluşturuldu e-postası emailQueue üzerinden asenkron işlenir; MailHog’a düşmesi test worker’a bağlı → deterministik değil.');
  scenario.skip('MSG-076', 'emailNotifications=false EMAIL atlama: EMAIL kanalı queue tabanlı; kategori/kanal gating mantığı MSG-079/080/081 ile fiilen kaplı.');
  scenario.skip('MSG-077', 'PASSWORD_RESET (ALWAYS_SEND) e-postası: e-posta gönderimi queue/SMTP’ye bağlı asenkron; auth domaininde (01) test edilir.');
  scenario.skip('MSG-078', 'E-postaların yalnız TR olması: dokümante kısıt, saf içerik/i18n gözlemi; API tarafında assert edilecek davranış yok.');
  scenario.skip('MSG-090', 'Cron planlı bildirim gönderimi: scheduled-notification cron/processor için /dev hook’u yok (runScheduler listesinde değil) → tetiklenemez.');
  scenario.skip('MSG-091', 'Cron çift-işlem koruması (isProcessing flag): manuel cron tetikleme hook’u yok → gözlemlenemez.');
  scenario.skip('MSG-094', 'Mesaj önizleme i18n "📷 Fotoğraf"/"📷 Photo": salt-UI/preview render pariteи; getThreadPreview backend’i mesaj listesi uçlarında ayrı DTO döndürmez.');
  scenario.skip('MSG-095', 'Web vs Mobile mesaj ekranı paritesi: salt istemci karşılaştırması.');
  scenario.skip('MSG-096', 'Bildirim metinlerinin TR olması: dokümante i18n kısıtı, saf içerik gözlemi.');
  scenario.skip('MSG-097', 'Mobil bildirim ayarları ekranı paritesi: salt istemci; ayar backend’i MSG-083 ile kaplı.');
  scenario.skip('MSG-101', 'Yükleniyor/hata UI durumları: salt istemci render/hata-sınırı davranışı.');
  scenario.skip('MSG-106', 'WS oda izolasyonu (yetkisiz kullanıcı event almaz): WS istemcisi yok; thread erişim izolasyonu MSG-026/103 ile kaplı.');
  scenario.skip('MSG-107', 'Mesaj/bildirim XSS koruması: React varsayılan escaping (render katmanı); backend içeriği ham saklar, API’de assert edilecek dönüşüm yok.');
});
