...
import { getLastEmailTo, extractLink, clearMailbox } from '../../test-utils/mail';
...
describe('01 — Kimlik Doğrulama & Hesap Güvenliği (AUTH)', () => {
  ...
  describe('POST /api/auth/register', () => {
    ...
    scenario('AUTH-004', async () => {
      ...
      await register(validRegister({ email: 'weak@test.com', password: 'weakpassword1' })).expect(400);
    });

    scenario('AUTH-005', async () => {
      // 'Ab1!' 4 karakter → MinLength(8) 400.
      await register(validRegister({ email: 'short@test.com', password: 'Ab1!' })).expect(400);
    });

    scenario('AUTH-006', async () => {
      // Sınır değer: 7 karakter reddedilir (MinLength(8)); 8 karakter kabul.
      await register(validRegister({ email: 'len7@test.com', password: 'Ab1cdef' })).expect(400); // 7
      await register(validRegister({ email: 'len8@test.com', password: 'Abc1defg' })).expect(201); // 8
      // 50 kabul, 51 red (MaxLength(50)). Hepsinde upper+lower+digit var.
      const p50 = 'Ab1' + 'c'.repeat(47); // 50
      const p51 = 'Ab1' + 'c'.repeat(48); // 51
      await register(validRegister({ email: 'len50@test.com', password: p50 })).expect(201);
      await register(validRegister({ email: 'len51@test.com', password: p51 })).expect(400);
    });
    ...
    scenario('AUTH-007', async () => {
      ...
      await register(
        validRegister({ email: 'young@test.com', birthDate: underage.toISOString().split('T')[0] }),
      ).expect(400);
    });
    ...
    scenario('AUTH-008', async () => {
      ...
      await register(
        validRegister({ email: 'age18@test.com', birthDate: exactly18.toISOString().split('T')[0] }),
      ).expect(201);
      // 18'e bir gün kala → red (400).
      const almost18 = new Date(today.getFullYear() - 18, today.getMonth(), today.getDate() + 1);
      await register(
        validRegister({ email: 'almost18@test.com', birthDate: almost18.toISOString().split('T')[0] }),
      ).expect(400);
    });
    ...
    scenario('AUTH-012', async () => {
      const res = await register(validRegister({ email: 'seller@test.com', isSeller: true })).expect(201);
      const prisma = getPrisma();
      const user = await prisma.user.findUnique({ where: { email: 'seller@test.com' } });
    ...
    scenario('AUTH-013', async () => {
      ...
      const res = await register(validRegister({ email: 'extra@test.com', isAdmin: true, role: 'super_admin' })).expect(201);
      expect(res.body.user?.email).toBe('extra@test.com');
      const prisma = getPrisma();
      const admin = await prisma.adminUser.findFirst({ where: { user: { email: 'extra@test.com' } } });
  ...
  describe('POST /api/auth/register/business', () => {
    scenario('AUTH-014', async () => {
      ...
      const prisma = getPrisma();
      const user = await prisma.user.findUnique({ where: { email: 'business@test.com' } });
    ...
    scenario('AUTH-015', async () => {
      ...
      await registerBusiness(
        validBusiness({ email: 'b2@test.com', phone: '+905550000001', taxId: '9999999999' }),
      ).expect(409);
    });
    ...
    scenario('AUTH-016', async () => {
      ...
      await registerBusiness(
        validBusiness({ companyName: 'Farklı Şirket', email: 'b3@test.com', phone: '+905550000002' }),
      ).expect(409);
    });

    scenario('AUTH-017', async () => {
      // Geçersiz telefon formatı → @Matches(/^\+90[0-9]{10}$/) 400.
      await registerBusiness(validBusiness({ phone: '05551234567' })).expect(400);
    });

    scenario('AUTH-018', async () => {
      // taxId @Matches(/^[0-9]{10,11}$/): 9 hane red, 10 kabul, 11 kabul, 12 red.
      await registerBusiness(validBusiness({ email: 't9@test.com', phone: '+905550000010', companyName: 'T9', taxId: '123456789' })).expect(400);
      await registerBusiness(validBusiness({ email: 't10@test.com', phone: '+905550000011', companyName: 'T10', taxId: '1234567890' })).expect(201);
      await registerBusiness(validBusiness({ email: 't11@test.com', phone: '+905550000012', companyName: 'T11', taxId: '12345678901' })).expect(201);
      await registerBusiness(validBusiness({ email: 't12@test.com', phone: '+905550000013', companyName: 'T12', taxId: '123456789012' })).expect(400);
    });
  });
  ...
  describe('POST /api/auth/login', () => {
    scenario('AUTH-019', async () => {
      const user = await createUser(ctx.module, { email: 'login@test.com', password: 'Demo123!' });
    ...
    scenario('AUTH-020', async () => {
      await createUser(ctx.module, { email: 'wrongpass@test.com', password: 'CorrectPass123!' });
    ...
    scenario('AUTH-024', async () => {
      await createUser(ctx.module, { email: 'emptypass@test.com', password: 'Demo123!' });
    ...
    scenario('AUTH-107', async () => {
      ...
      await createUser(ctx.module, { email: 'inj@test.com', password: 'Demo123!' });
    ...
    scenario('AUTH-108', async () => {
      ...
      const res2 = await register(validRegister({ email: 'long@test.com', displayName: 'x'.repeat(5000) }));
    ...
    scenario('AUTH-115', async () => {
      ...
      const user = await createUser(ctx.module, { email: 'jwt@test.com', password: 'Demo123!' });
      ...
      await request(server()).get('/api/auth/profile').set('Authorization', `Bearer ${tampered}`).expect(401);
    });
  });
  ...
  describe('Google login', () => {
    ...
    scenario.skip('AUTH-031', 'GoogleAuthService test app’te mock edilemiyor (gerçek Google doğrulaması gerekir)');
    scenario.skip('AUTH-032', 'GoogleAuthService test app’te mock edilemiyor (oto-bağlama gerçek token ister)');
    scenario.skip('AUTH-033', 'GoogleAuthService test app’te mock edilemiyor (idempotent OAuthAccount gerçek token ister)');
    scenario.skip('AUTH-035', 'email_verified=false reddi gerçek Google payload’ı ister; mock yok');
  });
  ...
  describe('GET /api/auth/profile', () => {
    scenario('AUTH-026', async () => {
      ...
      const res = await request(server()).get('/api/auth/profile').set(authHeader(user)).expect(200);
  ...
  describe('Admin login/profile', () => {
    ...
    scenario('AUTH-030', async () => {
      ...
      await request(server()).get('/api/auth/admin/profile').set(authHeader(user)).expect(401);
    });

    scenario('AUTH-100', async () => {
      // Non-admin kullanıcı admin korumalı uçta 401/403.
      const user = await createUser(ctx.module);
      const res = await request(server()).get('/api/auth/admin/profile').set(authHeader(user));
  ...
  describe('Email verification', () => {
    ...
    scenario('AUTH-037', async () => {
      ...
      const token = new URL(extractLink(mail.body, 'verify')!).searchParams.get('token');
      await request(server()).post('/api/auth/verify-email').send({ token }).expect(200);
    ...
    scenario('AUTH-038', async () => {
      ...
      const prisma = getPrisma();
      const user = await createUser(ctx.module, { email: 'used-unverified@test.com', isEmailVerified: false });
      ...
      await request(server()).post('/api/auth/verify-email').send({ token: raw }).expect(400);
    });

    scenario('AUTH-039', async () => {
      // Süresi dolmuş doğrulama tokeni → 400.
      const prisma = getPrisma();
      const user = await createUser(ctx.module, { email: 'expired-verify@test.com', isEmailVerified: false });
      const raw = crypto.randomBytes(32).toString('hex');
      const hashed = crypto.createHash('sha256').update(raw).digest('hex');
      await prisma.emailVerificationToken.create({
        data: {
    ...
    scenario('AUTH-042', async () => {
      ...
      await createUser(ctx.module, { email: 'already-verified@test.com', isEmailVerified: true });
      await request(server())
        .post('/api/auth/resend-verification')
    ...
    scenario('AUTH-043', async () => {
      // Oturumlu e-posta doğrulama durumu (security/email/status).
      const user = await createUser(ctx.module, { email: 'status@test.com', isEmailVerified: true });
      const res = await request(server())
        .get('/api/security/email/status')
        .set(authHeader(user))
        .expect(200);
      expect(res.body.isVerified).toBe(true);
      expect(res.body.email).toBe('status@test.com');
    });
  });
  ...
  describe('Password reset & change', () => {
    ...
    scenario('AUTH-046', async () => {
      ...
      const prisma = getPrisma();
      const user = await createUser(ctx.module, { email: 'reset-ok@test.com', password: 'Old123!' });
      const raw = crypto.randomBytes(32).toString('hex');
      const hashed = crypto.createHash('sha256').update(raw).digest('hex');
      await prisma.passwordResetToken.create({
        data: { userId: user.id, token: hashed, expiresAt: new Date(Date.now() + 3600_000) },
      });
      await request(server())
    ...
    scenario('AUTH-047', async () => {
      ...
      const prisma = getPrisma();
      const user = await createUser(ctx.module, { email: 'reset-nospecial@test.com', password: 'Old123!' });
      const raw = crypto.randomBytes(32).toString('hex');
      const hashed = crypto.createHash('sha256').update(raw).digest('hex');
      await prisma.passwordResetToken.create({
        data: { userId: user.id, token: hashed, expiresAt: new Date(Date.now() + 3600_000) },
    ...
    scenario('AUTH-048', async () => {
      ...
      const prisma = getPrisma();
      const user = await createUser(ctx.module, { email: 'used-reset@test.com' });
      const raw = crypto.randomBytes(32).toString('hex');
      const hashed = crypto.createHash('sha256').update(raw).digest('hex');
      await prisma.passwordResetToken.create({
        data: {
    ...
    scenario('AUTH-049', async () => {
      ...
      const prisma = getPrisma();
      const user = await createUser(ctx.module, { email: 'expired-reset@test.com' });
      const raw = crypto.randomBytes(32).toString('hex');
      const hashed = crypto.createHash('sha256').update(raw).digest('hex');
      await prisma.passwordResetToken.create({
        data: { userId: user.id, token: hashed, expiresAt: new Date(Date.now() - 3600_000) },
    ...
    scenario('AUTH-051', async () => {
      ...
      const prisma = getPrisma();
      const user = await createUser(ctx.module, { email: 'reset-revoke@test.com', password: 'Old123!' });
      // Bir oturum aç → refresh token DB'de (revokedAt null).
      const loginRes = await login('reset-revoke@test.com', 'Old123!').expect(200);
      const activeBefore = await prisma.refreshToken.count({ where: { userId: user.id, revokedAt: null } });
      ...
      const activeAfter = await prisma.refreshToken.count({ where: { userId: user.id, revokedAt: null } });
    ...
    scenario('AUTH-052', async () => {
      ...
      const prisma = getPrisma();
      const user = await createUser(ctx.module, { email: 'reset-noRevoke@test.com', password: 'Old123!' });
    ...
    scenario('AUTH-053', async () => {
      const user = await createUser(ctx.module, { email: 'chpw@test.com', password: 'Demo123!' });
      await request(server())
    ...
    scenario('AUTH-054', async () => {
      const user = await createUser(ctx.module, { email: 'chpw2@test.com', password: 'Demo123!' });
      await request(server())
        .post('/api/security/password/change')
        .set(authHeader(user))
    ...
    scenario('AUTH-055', async () => {
      ...
      const user = await createUser(ctx.module, { email: 'chpw-weak@test.com', password: 'Demo123!' });
      await request(server())
        .post('/api/security/password/change')
        .set(authHeader(user))
  ...
  describe('Refresh & logout', () => {
    scenario('AUTH-057', async () => {
      await createUser(ctx.module, { email: 'refresh@test.com', password: 'Demo123!' });
      ...
      const r1 = await request(server()).post('/api/auth/refresh').send({ refreshToken: first }).expect(200);
      expect(r1.body.accessToken).toBeTruthy();
      const second = r1.body.refreshToken;
      expect(second).toBeTruthy();
      expect(second).not.toBe(first);

      await request(server()).post('/api/auth/refresh').send({ refreshToken: second }).expect(200);
    ...
    scenario('AUTH-059', async () => {
      ...
      const user = await createUser(ctx.module, { email: 'expired-refresh@test.com', password: 'Demo123!' });
      const secret =
      ...
      await request(server()).post('/api/auth/refresh').send({ refreshToken: expired }).expect(401);
    });
    ...
    scenario('AUTH-060', async () => {
      ...
      const user = await createUser(ctx.module, { email: 'acc@test.com', password: 'Demo123!' });
      await request(server())
        .post('/api/auth/refresh')
    ...
    scenario('AUTH-062', async () => {
      ...
      const user = await createUser(ctx.module, { email: 'deleted-refresh@test.com', password: 'Demo123!' });
      const loginRes = await login('deleted-refresh@test.com', 'Demo123!').expect(200);
    ...
    scenario('AUTH-063', async () => {
      ...
      const admin = await createAdminUser(ctx.module, { email: 'admin-refresh@test.com' });
    ...
    scenario('AUTH-064', async () => {
      ...
      const prisma = getPrisma();
      const admin = await createAdminUser(ctx.module, { email: 'admin-deact@test.com' });
      const loginRes = await request(server())
        .post('/api/auth/admin/login')
      ...
      const rt = loginRes.body.tokens.refreshToken;
      // Admin'i deaktif et.
      await prisma.adminUser.updateMany({ where: { userId: admin.id }, data: { isActive: false } });
      await request(server()).post('/api/auth/admin/refresh').send({ refreshToken: rt }).expect(401);
    });
    ...
    scenario('AUTH-066', async () => {
      ...
      const jwt = ctx.module.get<JwtService>(JwtService);
      const config = ctx.module.get<ConfigService>(ConfigService);
      const user = await createUser(ctx.module, { email: 'expiry-flow@test.com', password: 'Demo123!' });
      ...
      await request(server()).get('/api/auth/profile').set('Authorization', `Bearer ${expiredAccess}`).expect(401);
      // Refresh → yeni access → 200.
      const loginRes = await login('expiry-flow@test.com', 'Demo123!').expect(200);
      const r1 = await request(server())
        .post('/api/auth/refresh')
        .send({ refreshToken: loginRes.body.tokens.refreshToken })
        .expect(200);
      await request(server()).get('/api/auth/profile').set('Authorization', `Bearer ${r1.body.accessToken}`).expect(200);
    });

    scenario('AUTH-067', async () => {
      await createUser(ctx.module, { email: 'logout@test.com', password: 'Demo123!' });
    ...
    scenario('AUTH-070', async () => {
      await createUser(ctx.module, { email: 'everywhere@test.com', password: 'Demo123!' });
      ...
      await request(server()).delete('/api/security/tokens').set(authHeader({ accessToken: me })).expect(204);

      await request(server()).post('/api/auth/refresh').send({ refreshToken: a.body.tokens.refreshToken }).expect(401);
      await request(server()).post('/api/auth/refresh').send({ refreshToken: b.body.tokens.refreshToken }).expect(401);
    });
    ...
    scenario('AUTH-071', async () => {
      ...
      const prisma = getPrisma();
      const user = await createUser(ctx.module, { email: 'reset-session@test.com', password: 'Old123!' });
      const loginRes = await login('reset-session@test.com', 'Old123!').expect(200);
      const raw = crypto.randomBytes(32).toString('hex');
      const hashed = crypto.createHash('sha256').update(raw).digest('hex');
      await prisma.passwordResetToken.create({
        data: { userId: user.id, token: hashed, expiresAt: new Date(Date.now() + 3600_000) },
  ...
  describe('Two-factor auth', () => {
    scenario('AUTH-072', async () => {
      ...
      const status = await request(server()).get('/api/security/2fa/status').set(authHeader(user)).expect(200);
    ...
    scenario('AUTH-074', async () => {
      ...
      await request(server())
        ...
        .expect(201);
      await request(server()).post('/api/security/2fa/enable').set(authHeader(user)).expect(400);
    });

    scenario('AUTH-075', async () => {
      const user = await createUser(ctx.module);
      await request(server()).post('/api/security/2fa/enable').set(authHeader(user)).expect(201);
      await request(server())
        .post('/api/security/2fa/verify')
        .set(authHeader(user))
        .send({ code: '000000' })
    ...
    scenario('AUTH-080', async () => {
      ...
      await request(server()).post('/api/security/2fa/verify').send({ code: '123456' }).expect(401);
  ...
  describe('CSRF token', () => {
    scenario('AUTH-082', async () => {
      ...
      const user = await createUser(ctx.module);
      const res = await request(server()).get('/api/security/csrf-token').set(authHeader(user)).expect(200);
      expect(res.body.token).toBeTruthy();
      expect(res.body.expiresAt).toBeTruthy();
      const prisma = getPrisma();
      const row = await prisma.csrfToken.findUnique({ where: { token: res.body.token } });
    ...
    scenario.skip('AUTH-083', 'CSRF validate/silme public API ucu yok (yalnız servis-içi validateCsrfToken)');
    scenario.skip('AUTH-084', 'CSRF yanlış-session reddi public API ucu yok (yalnız servis-içi validateCsrfToken)');
  });
  ...
  describe('Rate limiting', () => {
    ...
    scenario.skip('AUTH-090', 'Throttle test env’de kapalı (skipIf NODE_ENV=test) → 429 üretilemez');
    scenario.skip('AUTH-091', 'Throttle test env’de kapalı → register 429 üretilemez');
    scenario.skip('AUTH-092', 'Throttle test env’de kapalı → forgot-password 429 üretilemez');
    scenario.skip('AUTH-093', 'Throttle test env’de kapalı → resend 429 üretilemez');
    ...
    scenario('AUTH-094', async () => {
      ...
      for (let i = 0; i < 8; i++) {
        const r = await request(server()).post('/api/auth/refresh').send({ refreshToken: rt });
        expect(r.status).not.toBe(429);
        if (r.status === 200) rt = r.body.refreshToken;
      }
      for (let i = 0; i < 8; i++) {
        const p = await request(server()).get('/api/auth/profile').set(authHeader(user));
  ...
  describe('Ban behavior', () => {
    ...
    scenario('AUTH-097', async () => {
      ...
      const prisma = getPrisma();
      const user = await createUser(ctx.module, { email: 'banned-logout@test.com', password: 'Demo123!' });
      await prisma.user.update({ where: { id: user.id }, data: { isBanned: true, bannedReason: 'test' } });
      await request(server()).post('/api/auth/logout').set(authHeader(user)).expect(200);
    });

    scenario('AUTH-098', async () => {
      // Banlı cookie/token varken BAŞKA hesapla login yapılabilir (login Public → ban bloklamaz).
      const prisma = getPrisma();
      const banned = await createUser(ctx.module, { email: 'banned-cookie@test.com', password: 'Demo123!' });
      await prisma.user.update({ where: { id: banned.id }, data: { isBanned: true } });
      await createUser(ctx.module, { email: 'other-acct@test.com', password: 'Demo123!' });
  ...
  describe('Security storage', () => {
    scenario('AUTH-109', async () => {
      await createUser(ctx.module, { email: 'hash@test.com', password: 'Demo123!' });
    ...
    scenario('AUTH-112', async () => {
      ...
      await clearMailbox();
      const user = await createUser(ctx.module, { email: 'token-hash@test.com' });
      ...
      await request(server())
        ...
        .expect(200);
      const prisma = getPrisma();
      const prt = await prisma.passwordResetToken.findFirst({ where: { userId: user.id } });
  ...
  describe('Guest order linking', () => {
    ...
    scenario.skip('AUTH-114', 'Guest sipariş bağlama tam Order seed’i (checkoutGroup/seller/ürün/tutarlar) ister — sipariş domaininde kapsanır');
  });
  ...
  describe('i18n & UI', () => {
    scenario('AUTH-101', async () => {
      ...
      await createUser(ctx.module, { email: 'i18n@test.com', password: 'Demo123!' });
    ...
    scenario('AUTH-103', async () => {
      ...
      await createUser(ctx.module, { email: 'parity@test.com', password: 'Demo123!' });
      const l = await login('parity@test.com', 'Demo123!').expect(200);
      await request(server()).post('/api/auth/refresh').send({ refreshToken: l.body.tokens.refreshToken }).expect(200);
    ...
    scenario('AUTH-104', async () => {
      ...
      await createUser(ctx.module, { email: 'cookie-vs-header@test.com', password: 'Demo123!' });
      const res = await login('cookie-vs-header@test.com', 'Demo123!').expect(200);
      const setCookie = (res.headers['set-cookie'] as unknown as string[]) ?? [];
      expect(setCookie.join(';')).toContain('access_token=');
    ...
    scenario.skip('AUTH-105', 'Admin ayrı login EKRANI saf UI (API tarafı AUTH-029/069 ile kapsanıyor)');
    scenario.skip('AUTH-106', 'Boş/Yükleniyor/Hata UI durumları saf istemci sunumu — API’den assert edilemez');
    ...
    scenario('AUTH-102', async () => {
      ...
      const prisma = getPrisma();
      const user = await createUser(ctx.module, { email: 'reset-parity@test.com', password: 'Old123!' });
      const raw = crypto.randomBytes(32).toString('hex');
      const hashed = crypto.createHash('sha256').update(raw).digest('hex');
      await prisma.passwordResetToken.create({
        data: { userId: user.id, token: hashed, expiresAt: new Date(Date.now() + 3600_000) },
    ...
    scenario('AUTH-099', async () => {
      ...
      const admin = await createAdminUser(ctx.module, { email: 'super@test.com', role: AdminRole.super_admin });
      await request(server()).get('/api/auth/admin/profile').set(authHeader(admin)).expect(200);
      const normal = await createUser(ctx.module, { email: 'nonadmin-99@test.com' });
      await request(server()).get('/api/auth/admin/profile').set(authHeader(normal)).expect(401);
    });
  });
});
