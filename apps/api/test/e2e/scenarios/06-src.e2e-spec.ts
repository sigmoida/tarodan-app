...
describe("06 — Arama, Filtre & Keşif (SRC)", () => {
  ...
  beforeEach(async () => {
    ...
    const seller = await createUser(ctx.module, {
      isSeller: true,
      displayName: "Satıcı Demo",
    });
    sellerId = seller.id;
    admin = await createAdminUser(ctx.module, {
      email: `admin-src-${Date.now()}@test.local`,
  ...
  async function mkProduct(opts: MkProductOpts = {}): Promise<string> {
    ...
    const p = await prisma.product.create({
      data: {
        ...
        kind: opts.kind ?? ProductKind.listing,
  ...
  async function attachScale(productId: string, value: string): Promise<void> {
    ...
    await prisma.productAttribute.create({
      data: { productId, attributeId: attr.id },
    });
  }
    ...
    const res = await request(server())
      ...
      .expect((r) => {
        if (![200, 201].includes(r.status)) {
          throw new Error(
            `reindex beklenmeyen status ${r.status}: ${JSON.stringify(r.body)}`,
          );
        }
      });
  ...
  const search = (qs = "") =>
    request(server()).get(`/api/search/products${qs}`);
  ...
  scenario.skip(
    "SRC-002",
    "Saf web parite (tarayıcı yönlendirme /search → /listings); API kapsamı yok.",
  );
  ...
  scenario("SRC-003", async () => {
    ...
    const accentQ = await search(
      "?q=" + encodeURIComponent("şahin doğan"),
    ).expect(200);
    expect(asciiQ.body.total).toBeGreaterThanOrEqual(1);
    expect(accentQ.body.total).toBeGreaterThanOrEqual(1);
    expect(
      asciiQ.body.results.some((r: any) => r.title === "Şahin Doğan Modeli"),
    ).toBe(true);
    expect(
      accentQ.body.results.some((r: any) => r.title === "Şahin Doğan Modeli"),
    ).toBe(true);
  });
  ...
  scenario("SRC-004", async () => {
    ...
    expect(upper.body.results.some((r: any) => r.title.includes("BMW"))).toBe(
      true,
    );
  });
  ...
  scenario("SRC-005", async () => {
    ...
    expect(
      res.body.results.some((r: any) => /Lamborghini Aventador/.test(r.title)),
    ).toBe(true);
  ...
  scenario("SRC-006", async () => {
    ...
    await mkProduct({
      title: "Gri Kupa Model",
      categoryId: cat.id,
      price: 150,
    });
    await reindex();
    const res = await search(
      "?q=" + encodeURIComponent("spor arabalar"),
    ).expect(200);
    expect(res.body.total).toBeGreaterThanOrEqual(1);
    expect(
      res.body.results.some((r: any) => r.categoryName === "Spor Arabalar"),
    ).toBe(true);
  });
  ...
  scenario("SRC-007", async () => {
    ...
    expect(res.body.results.every((r: any) => r.highlight === undefined)).toBe(
      true,
    );
  ...
  scenario.skip(
    "SRC-009",
    "Saf web parite (boş durum UI kutusu / Filtreleri Temizle butonu); API kapsamı yok.",
  );
  scenario.skip(
    "SRC-010",
    "Saf web parite (loading skeleton animate-pulse); API kapsamı yok.",
  );
  scenario.skip(
    "SRC-011",
  ...
  scenario("SRC-012", async () => {
    ...
    expect(res.body.results.every((r: any) => r.categoryId === cat.id)).toBe(
      true,
    );
    // aggregations.categories.buckets içinde bu kategori sayımı.
    const bucket = res.body.aggregations?.categories?.buckets?.find(
      (b: any) => b.key === cat.id,
    );
  ...
  scenario("SRC-013", async () => {
    const ferrari = await getPrisma().brand.create({
      data: { name: "Ferrari", slug: "ferrari", isActive: true },
    });
    const bmw = await getPrisma().brand.create({
      data: { name: "BMW", slug: "bmw", isActive: true },
    });
    ...
    const conflict = await search(`?brandId=${ferrari.id}&brand=BMW`).expect(
      200,
    );
  ...
  scenario("SRC-014", async () => {
    const brand = await getPrisma().brand.create({
      data: { name: "Honda", slug: "honda", isActive: true },
    });
    ...
    const mfg = await getPrisma().manufacturer.create({
      data: { name: "Tomica", slug: "tomica", isActive: true },
    });
    await mkProduct({
      title: "Civic Model",
    ...
    expect(
      byMfg.body.results.every((r: any) => r.manufacturerId === mfg.id),
    ).toBe(true);
  ...
  scenario("SRC-015", async () => {
    ...
    expect(
      range.body.results.every((r: any) => r.price >= 100 && r.price <= 500),
    ).toBe(true);
  ...
  scenario("SRC-016", async () => {
    ...
    expect(isNew.body.results.every((r: any) => r.condition === "new")).toBe(
      true,
    );
    expect(isNew.body.total).toBeGreaterThanOrEqual(1);

    const vg = await search("?condition=very_good").expect(200);
    expect(vg.body.results.every((r: any) => r.condition === "very_good")).toBe(
      true,
    );
  ...
  scenario("SRC-017", async () => {
    ...
    expect(colon.body.results.every((r: any) => /1:18/.test(r.title))).toBe(
      true,
    );
  ...
  scenario("SRC-019", async () => {
    ...
    expect(
      (await search("?tradeOnly=true").expect(200)).body.results.map(
        (r: any) => r.title,
      ),
    ).toEqual(["Takaslı"]);
    expect(
      (await search("?preOrder=true").expect(200)).body.results.map(
        (r: any) => r.title,
      ),
    ).toEqual(["Ön Sipariş"]);
    expect(
      (await search("?limited=true").expect(200)).body.results.map(
        (r: any) => r.title,
      ),
    ).toEqual(["Limitli"]);
    expect(
      (await search("?set=true").expect(200)).body.results.map(
        (r: any) => r.title,
      ),
    ).toEqual(["Set"]);
  });
  ...
  scenario("SRC-021", async () => {
    ...
    const res = await search(
      "?condition=new&tradeOnly=true&minPrice=200&maxPrice=800",
    ).expect(200);
  ...
  scenario.skip(
    "SRC-022",
    "Saf web parite (SidebarFilters URL senk + rozet/activeFilterCount); API kapsamı yok.",
  );
  scenario.skip(
    "SRC-023",
    "Saf web parite (marka adı yazınca brand facet uygulanır, sayfa state); API kapsamı yok.",
  );
  ...
  scenario("SRC-024", async () => {
    ...
    const inStockAsc = asc.body.results.filter(
    ...
    const inStockDesc = desc.body.results.filter(
      (r: any) => r.status === "active",
    );
  ...
  scenario("SRC-025", async () => {
    ...
    const titleAsc = (
      await search("?sortBy=title_asc").expect(200)
    ).body.results.map((r: any) => r.title);
    expect(titleAsc).toEqual([...titleAsc].sort());
    const titleDesc = (
      await search("?sortBy=title_desc").expect(200)
    ).body.results.map((r: any) => r.title);
    expect(titleDesc).toEqual([...titleDesc].sort().reverse());

    const viewDesc = (await search("?sortBy=view_count_desc").expect(200)).body
      .results;
    const vc = viewDesc.map((r: any) => r.title);
    expect(vc[0]).toBe("Charlie"); // en çok görüntülenen

    const viewAsc = (
      await search("?sortBy=view_count_asc").expect(200)
    ).body.results.map((r: any) => r.title);
    expect(viewAsc[0]).toBe("Alpha");

    // created_desc / created_asc hata vermez ve ters sırada.
    const cd = (
      await search("?sortBy=created_desc").expect(200)
    ).body.results.map((r: any) => r.id);
    const ca = (
      await search("?sortBy=created_asc").expect(200)
    ).body.results.map((r: any) => r.id);
  ...
  scenario("SRC-027", async () => {
    ...
    expect(titles.indexOf("Sponsorlu")).toBeLessThan(
      titles.indexOf("Standart"),
    );
  });
  ...
  scenario("SRC-028", async () => {
    for (let i = 0; i < 25; i++)
      await mkProduct({
        title: `Sayfalı ${String(i).padStart(2, "0")}`,
        viewCount: 100 - i,
      });
    await reindex();

    const p1 = await search(
      "?page=1&pageSize=10&sortBy=view_count_desc",
    ).expect(200);
    const p2 = await search(
      "?page=2&pageSize=10&sortBy=view_count_desc",
    ).expect(200);
  ...
  scenario.skip(
    "SRC-030",
    "Saf web parite (sayfalama gezinme, 48 sayfa boyutu, Önceki disabled); API kapsamı yok.",
  );
  ...
  scenario("SRC-033", async () => {
    ...
    expect(titles.indexOf("Satılan Ürün")).toBeGreaterThan(
      titles.indexOf("Aktif Stoklu"),
    );
  });
  ...
  scenario("SRC-034", async () => {
    ...
    expect(titles.indexOf("Tam Rezerve")).toBeGreaterThan(
      titles.indexOf("Stoklu Aktif"),
    );
  });

  scenario("SRC-035", async () => {
    await mkProduct({
      id: "membership-virtual-1",
      kind: ProductKind.membership,
      title: "Membership Ürünü",
      status: ProductStatus.active,
    });
    await mkProduct({
      id: "boost-virtual-1",
    ...
    for (const res of [m, b, all]) {
      expect(
        res.body.results.every(
          (r: any) =>
            !r.id.startsWith("membership-") && !r.id.startsWith("boost-"),
        ),
      ).toBe(true);
    }
  });
  ...
  scenario("SRC-036", async () => {
    ...
    await reindex();
    const res = await request(server())
      .get("/api/search/autocomplete?q=por")
      .expect(200);
    expect(Array.isArray(res.body.suggestions)).toBe(true);
    expect(res.body.suggestions.length).toBeLessThanOrEqual(10);
    expect(res.body.suggestions.some((s: string) => /Porsche/i.test(s))).toBe(
      true,
    );
    // Başlık bazında tekil.
    expect(new Set(res.body.suggestions).size).toBe(
      res.body.suggestions.length,
    );
  });
  ...
  scenario("SRC-037", async () => {
    ...
    await reindex();
    const res = await request(server())
      .get("/api/search/autocomplete?q=bmw&limit=3")
      .expect(200);
  ...
  scenario("SRC-039", async () => {
    ...
    await reindex();
    const res = await request(server())
      .get("/api/search/autocomplete-rich?q=ferrari")
      .expect(200);
    for (const key of [
      "products",
      "brands",
  ...
  scenario("SRC-040", async () => {
    ...
    for (const q of empties) {
      const res = await request(server())
        .get(`/api/search/autocomplete-rich?q=${q}`)
        .expect(200);
      for (const key of [
        "products",
        "brands",
  ...
  scenario("SRC-041", async () => {
    ...
    const res = await request(server())
      .get("/api/search/autocomplete-rich?q=bugatti")
      .expect(200);
  ...
  scenario.skip(
    "SRC-042",
    "Saf mobil parite (debounce 400ms + ≥2 karakter öneri paneli); API kapsamı yok.",
  );
  ...
  scenario("SRC-043", async () => {
    ...
    await mkProduct({
      title: "Satılan Popüler",
      status: ProductStatus.sold,
      quantity: 0,
      viewCount: 200,
    });
    await mkProduct({
      title: "Tam Rezerve Popüler",
      status: ProductStatus.active,
      quantity: 1,
      reservedQuantity: 1,
      viewCount: 300,
    ...
    const res = await request(server())
      .get("/api/products/popular?limit=20&page=1")
      .expect(200);
  ...
  scenario("SRC-044", async () => {
    ...
    const res = await request(server())
      .get("/api/products/popular?limit=10&page=2")
      .expect(200);
  ...
  scenario("SRC-045", async () => {
    ...
    const res = await request(server())
      .get("/api/products/popular?limit=50")
      .expect(200);
    expect(
      res.body.data.every(
        (p: any) =>
          !p.id.startsWith("membership-") && !p.id.startsWith("boost-"),
      ),
    ).toBe(true);
  });
  ...
  scenario("SRC-046", async () => {
    const cat = await getPrisma().category.create({
      data: {
        name: "Benzer Kategori",
        slug: "benzer-kat",
        sortOrder: 3,
        isActive: true,
      },
    });
    const source = await mkProduct({
      title: "Kaynak Ürün",
      categoryId: cat.id,
      status: ProductStatus.active,
      quantity: 5,
    ...
    const res = await request(server())
      .get(`/api/products/${source}/similar?limit=6`)
      .expect(200);
    ...
    expect(
      res.body.every(
        (p: any) => p.category?.id === cat.id || p.categoryId === cat.id,
      ),
    ).toBe(true);
  });
  ...
  scenario("SRC-047", async () => {
    ...
    const res = await request(server())
      .get(`/api/products/${source}/similar?limit=100`)
      .expect(200);
  ...
  scenario("SRC-048", async () => {
    const res = await request(server())
      .get("/api/products/not-a-uuid/similar")
      .expect(400);
  ...
  scenario("SRC-049", async () => {
    ...
    const id = await mkProduct({
      title: "Benzersizurun Yeni",
      status: ProductStatus.active,
    });
    await reindex();
    const res = await search(
      "?q=" + encodeURIComponent("benzersizurun"),
    ).expect(200);
  ...
  scenario("SRC-050", async () => {
    ...
    await reindex();
    expect(
      (await search("?q=silinecekurun").expect(200)).body.results.some(
        (r: any) => r.id === id,
      ),
    ).toBe(true);
  ...
  scenario("SRC-051", async () => {
    const id = await mkProduct({
      title: "Stoktanbitenurun",
      status: ProductStatus.active,
      quantity: 1,
      price: 100,
    });
    await mkProduct({
      title: "Stoktanbitenurun Diğer",
      status: ProductStatus.active,
      quantity: 5,
      price: 50,
    });
    ...
    await getPrisma().product.update({
      where: { id },
      data: { status: ProductStatus.sold, quantity: 0 },
    });
    await reindex();
    const res = await search("?q=stoktanbitenurun&sortBy=price_asc").expect(
      200,
    );
    ...
    const inStockIdx = res.body.results.findIndex(
      (r: any) => r.status === "active",
    );
  ...
  scenario("SRC-052", async () => {
    ...
    await getPrisma().carModel.update({
      where: { id: carModel.id },
      data: { name: "Civic Type R" },
    });
    await reindex();
    const res = await search("?q=" + encodeURIComponent("civic type r")).expect(
      200,
    );
    expect(res.body.results.some((r: any) => r.id === id)).toBe(true);
  });
  ...
  scenario("SRC-053", async () => {
    ...
    expect(
      Math.abs(res.body.dbCount - res.body.documentCount),
  ...
  scenario("SRC-054", async () => {
    ...
    await mkProduct({
      title: "Postgres Fallback A",
      status: ProductStatus.active,
      quantity: 5,
    });
    await mkProduct({
      title: "Postgres Fallback B",
      status: ProductStatus.active,
      quantity: 5,
    });
    ...
    expect(
      res.body.data.every((p: any) =>
        ["active", "sold", "inactive"].includes(p.status),
      ),
    ).toBe(true);
  });
  ...
  scenario("SRC-055", async () => {
    ...
    const owner = await createUser(ctx.module, {
      email: `col-owner-${Date.now()}@test.local`,
      displayName: "Koleksiyoncu",
    });
    await getPrisma().collection.create({
      data: {
        userId: owner.id,
        name: "Herkese Açık Koleksiyon A",
        slug: `pub-a-${Date.now()}`,
        isPublic: true,
      },
    });
    await getPrisma().collection.create({
      data: {
        userId: owner.id,
        name: "Herkese Açık Koleksiyon B",
        slug: `pub-b-${Date.now()}`,
        isPublic: true,
      },
    });
    ...
    const res = await request(server())
      .get("/api/collections/browse?page=1&pageSize=50")
      .expect(200);
  ...
  scenario("SRC-057", async () => {
    ...
    await request(server())
      .get("/api/search/autocomplete-rich?q=por")
      .expect(200);
  ...
  scenario("SRC-058", async () => {
    ...
    const normal = await createUser(ctx.module, {
      email: `normal-${Date.now()}@test.local`,
    });
    await request(server())
      .post("/api/search/admin/reindex")
      ...
      .expect((r) => {
        if (![200, 201].includes(r.status)) {
          throw new Error(
            `SRC-058 normal kullanıcı beklenen 200/201 (gap), alınan ${r.status}`,
          );
        }
      });
    ...
    await request(server())
      ...
      .expect((r) => {
        if (![200, 201].includes(r.status)) {
          throw new Error(
            `SRC-058 moderator beklenen 200/201 (gap), alınan ${r.status}`,
          );
        }
      });
  });
  ...
  scenario("SRC-060", async () => {
    ...
    const found = await search(
      "?q=" + encodeURIComponent("tekil index"),
    ).expect(200);
  ...
  scenario("SRC-067", async () => {
    await mkProduct({
      title: "Porsche Güvenlik",
      status: ProductStatus.active,
    });
    await reindex();
    await search("?q=" + encodeURIComponent("' OR 1=1 --")).expect(200);
    await search("?q=" + encodeURIComponent("porsche & ferrari | bmw")).expect(
      200,
    );
  ...
  scenario.skip(
    "SRC-073",
    "Web vs mobil aynı param kümesi paritesi; iki istemci ağ isteği karşılaştırması, API kapsamı yok.",
  );
  scenario.skip(
    "SRC-074",
    "Sıralama seçenekleri parite (web title A-Z/Z-A var, mobil yok); UI menü incelemesi.",
  );
  ...
  scenario("SRC-075", async () => {
    ...
    const cat = await getPrisma().category.create({
      data: {
        name: `LikeNew Kat ${Date.now()}`,
        slug: `likenew-kat-${Date.now()}`,
        sortOrder: 8,
        isActive: true,
      },
    });
    await mkProduct({
      title: "Yeni Gibi Ürün",
      categoryId: cat.id,
    ...
    const res = await products(
      `?condition=like_new&categoryId=${cat.id}&limit=50`,
    ).expect(200);
    // Enum geçerli → 200; dönen ürünlerin hepsi like_new (veya boş).
    expect(res.body.data.every((p: any) => p.condition === "like_new")).toBe(
      true,
    );
  });
  ...
  scenario.skip(
    "SRC-076",
    "Saf web parite (TR/EN etiket ve sayı i18n stringleri); API kapsamı yok.",
  );
  scenario.skip(
    "SRC-077",
    "Saf web/mobil parite (kategori slug→id çözümü ve sayfa render); API kapsamı yok.",
  );
  ...
  scenario("SRC-078", async () => {
    ...
    expect(
      res.body.results.every((r: any) => r.price >= 149.95 && r.price <= 150.0),
    ).toBe(true);
  });

  scenario.skip(
    "SRC-079",
    "Saf web parite (indirimli kart fiyat gösterimi: üstü çizili + % rozeti, tr-TR biçim); API kapsamı yok.",
  );
  ...
  scenario("SRC-080", async () => {
    ...
    const cat = await getPrisma().category.create({
      data: {
        name: `İndirim Kat ${Date.now()}`,
        slug: `indirim-kat-${Date.now()}`,
        sortOrder: 9,
        isActive: true,
      },
    });
    ...
    const res = await products(
      `?discountOnly=true&categoryId=${cat.id}&limit=50`,
    ).expect(200);
    const titles = res.body.data.map((p: any) => p.title);
