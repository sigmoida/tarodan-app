 ...
 *   - app.module.ts (ThrottlerModule skipIf test; GraphQLAppModule DEVRE DIŞI; global prefix)
 ...
 *   - GraphQLAppModule app.module.ts:181'de yorum satırı → /graphql route YOK → OPS-041/43/44/45 skip.
...
describe('23 — Platform: i18n, Sağlık, Cache, GraphQL, Monitoring (OPS)', () => {
  ...
  describe('GET /api/i18n/*', () => {
    scenario('OPS-001', async () => {
      const res = await request(server()).get('/api/i18n/languages').expect(200);
    ...
    scenario('OPS-003', async () => {
      ...
      const res = await request(server()).get('/api/i18n/translations').expect(200);
      expect(res.body.language).toBe('tr');
      expect(res.body.translations['common.success']).toBe('İşlem başarılı');
      expect(res.body.translations['auth.invalidCredentials']).toBe('E-posta veya şifre hatalı');
    });

    scenario('OPS-004', async () => {
      const res = await request(server()).get('/api/i18n/translations?lang=en').expect(200);
      expect(res.body.language).toBe('en');
      expect(res.body.translations['common.success']).toBe('Operation successful');
      expect(res.body.translations['auth.invalidCredentials']).toBe('Invalid email or password');
    });
    ...
    scenario('OPS-005', async () => {
      const res = await request(server())
        ...
        .expect(200);
      expect(res.body).toEqual({ key: 'order.created', language: 'en', translation: 'Order created' });
    });
    ...
    scenario('OPS-006', async () => {
      ...
      const de = await request(server()).get('/api/i18n/translations?lang=de').expect(200);
      ...
      const zzzz = await request(server()).get('/api/i18n/translations?lang=zzzz').expect(200);
    ...
    scenario('OPS-012', async () => {
      ...
      const res = await request(server()).get('/api/i18n/translate?lang=tr').expect(200);
    ...
    scenario('OPS-013', async () => {
      ...
      const tr = await request(server()).get('/api/i18n/translations?lang=tr').expect(200);
      const en = await request(server()).get('/api/i18n/translations?lang=en').expect(200);
  ...
  describe('GET /api/health*', () => {
    ...
    scenario('OPS-025', async () => {
      ...
      const res = await request(server()).get('/api/health/detailed').expect(200);
      ...
      expect(typeof res.body.services.redis.details.memoryUsedMB).toBe('number');
      expect(['healthy', 'degraded', 'unhealthy']).toContain(res.body.services.elasticsearch.status);
  ...
  describe('Cache — categories:all', () => {
    ...
    scenario('OPS-085', async () => {
      ...
      const names = await waitFor(
        async () => {
          const final = await request(server()).get('/api/categories').expect(200);
  ...
  describe('Routing, validation & error shape', () => {
    scenario('OPS-064', async () => {
      ...
      const res = await request(server()).get('/api/this-does-not-exist').expect(404);
    ...
    scenario('OPS-068', async () => {
      ...
      const res = await request(server())
        .post('/api/auth/login')
        .send({ email: 'nobody@test.local', password: 'SomePass123!', hacker: 'x' });
    ...
    scenario('OPS-069', async () => {
      ...
      await createProduct({ sellerId: seller.id, categoryId: baseline.categoryId, status: 'active' });
  ...
  describe('Throttle skipped in test env', () => {
    scenario('OPS-060', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        const res = await request(server())
          .post('/api/auth/login')
          .send({ email: `no-such-${i}@test.local`, password: 'WrongPass123!' });
  ...
  scenario.skip('OPS-014', 'Saf web UI: ilk render dil (localhost:3000 DOM) — API kapsamı dışı.');
  scenario.skip('OPS-015', 'Saf web UI: tarayıcı dili → otomatik EN (DOM) — API kapsamı dışı.');
  scenario.skip('OPS-016', 'Saf web UI: dil değiştirici localStorage kalıcılığı — API kapsamı dışı.');
  scenario.skip('OPS-017', 'Saf web UI: eksik anahtar TR fallback (web i18n) — API kapsamı dışı.');
  scenario.skip('OPS-019', 'Saf web UI: tanımsız anahtar ham gösterim + console.warn — API kapsamı dışı.');
  ...
  scenario.skip('OPS-026', 'Redis durdurma gerektirir; paylaşılan in-process app\'te Redis canlı → degraded dalı üretilemez.');
  scenario.skip('OPS-027', 'PostgreSQL durdurma gerektirir; PG canlı olmadan Prisma/test çalışmaz → unhealthy dalı üretilemez.');
  scenario.skip('OPS-028', 'Elasticsearch\'i yellow\'a zorlamak gerekir; ES cluster durumu test kontrolünde değil.');
  scenario.skip('OPS-029', 'Elasticsearch durdurma + timeout ölçümü gerektirir; ES canlı ve durdurulamaz.');
  scenario.skip('OPS-035', 'Redis durdurma gerektirir (graceful degradation); paylaşılan app\'te Redis canlı, kapatmak diğer testleri bozar.');

  // 429 / ThrottlerGuard — app.module.ts skipIf(NODE_ENV==='test') ile TAMAMEN kapalı → 429 asla üretilemez.
  scenario.skip('OPS-031', 'ThrottlerGuard test\'te skipIf ile kapalı + 1000+ istek pratik değil → 429 üretilemez.');
  scenario.skip('OPS-055', 'Login 5/dk brute-force limiti: throttle test\'te kapalı → 429 üretilemez.');
  scenario.skip('OPS-056', 'Register 5/dk limiti: throttle test\'te kapalı → 429 üretilemez.');
  scenario.skip('OPS-057', 'Forgot/Reset 3/dk limiti: throttle test\'te kapalı → 429 üretilemez.');
  scenario.skip('OPS-058', 'Ödeme uçları 10/dk: throttle test\'te kapalı → 429 üretilemez.');
  scenario.skip('OPS-059', 'Global 1000/dk: throttle test\'te kapalı + 1000+ istek pratik değil → 429 üretilemez.');
  scenario.skip('OPS-061', 'X-Forwarded-For başına ayrı kova: throttle test\'te kapalı → doğrulanamaz.');
  scenario.skip('OPS-063', 'Throttle guard auth\'tan önce (429>401): throttle test\'te kapalı → 429 üretilemez.');

  // GraphQL — app.module.ts:181 GraphQLAppModule YORUM SATIRI → /graphql route mount edilmez.
  scenario.skip('OPS-041', 'GraphQLAppModule devre dışı (app.module.ts:181 yorumlu) → /graphql route yok.');
  scenario.skip('OPS-043', 'GraphQL introspection: GraphQLAppModule devre dışı → /graphql route yok.');
  scenario.skip('OPS-044', 'GraphQL formatError: GraphQLAppModule devre dışı → /graphql route yok.');
  scenario.skip('OPS-045', 'GraphQL sayfalama sınırları: GraphQLAppModule devre dışı → /graphql route yok.');

  // Sentry — SentryInterceptor global APP_INTERCEPTOR olarak kayıtlı DEĞİL + harici Sentry/boot-log gerektirir.
  scenario.skip('OPS-046', 'Sentry DSN yokken init atlama: boot-log incelemesi gerektirir, API\'den doğrulanamaz.');
  scenario.skip('OPS-047', 'Sentry 5xx yakalama: harici Sentry projesi + init gerektirir, API\'den doğrulanamaz.');
  scenario.skip('OPS-048', 'Sentry 4xx yakalamaz: harici Sentry event doğrulaması gerektirir.');
  scenario.skip('OPS-049', 'Sentry /health event drop: harici Sentry beforeSend doğrulaması gerektirir.');
  scenario.skip('OPS-050', 'Sentry body redaction ([REDACTED]): harici Sentry request-context incelemesi gerektirir.');

  // Harness (create-app.ts) main.ts'i replike ETMEZ: helmet/CORS/BullBoard/callback-exclude YOK.
  scenario.skip('OPS-066', 'Test harness setGlobalPrefix callback-exclude uygulamaz (create-app.ts) → prefix\'siz /callback yok.');
  scenario.skip('OPS-070', 'Cron dashboard /admin/jobs/api Basic-Auth: BullBoard/dashboard harness\'te mount edilmez.');
  scenario.skip('OPS-071', 'Cron dashboard /admin/jobs (HTML): dashboard harness\'te mount edilmez.');
  scenario.skip('OPS-074', 'Cron tracker restart\'ta sıfırlama: app restart + in-memory tracker; harness kapsamı dışı.');
  scenario.skip('OPS-075', 'Prod\'da BULLBOARD_PASS yoksa mount edilmez: prod boot yapılandırması gerektirir.');
  scenario.skip('OPS-078', 'Bağımlı UI/servis portları (Web/Admin/Swagger/MailHog/Kibana) — manuel çok-servis smoke, API kapsamı dışı.');
  scenario.skip('OPS-079', 'Swagger prod\'da kapalı: prod boot + /api/docs harness\'te setup edilmez.');
  scenario.skip('OPS-080', 'PAYMENT_BYPASS+production fatal guard: NODE_ENV=production ile ayrı process boot gerektirir (process.exit).');
  scenario.skip('OPS-081', 'helmet header\'ları: test harness helmet() uygulamaz (create-app.ts ≠ main.ts).');
  scenario.skip('OPS-082', 'CORS izinli origin: test harness enableCors() uygulamaz (create-app.ts ≠ main.ts).');
});
