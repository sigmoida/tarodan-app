...
describe("22 — Admin Paneli & Yetkilendirme (ADM)", () => {
  ...
  scenario("ADM-009", async () => {
    ...
    const admin = await createAdminUser(ctx.module, {
      email: "admin-refresh@tarodan.com",
    });
    ...
    const payload = JSON.parse(
      Buffer.from(r.body.accessToken.split(".")[1], "base64").toString("utf8"),
    );
  ...
  scenario("ADM-072", async () => {
    ...
    await request(server())
      ...
      .set(authHeader(superAdmin))
      .expect(204);
    const gone = await prisma.commissionRule.findUnique({
      where: { id: rule.id },
    });
  ...
  scenario("ADM-063", async () => {
    ...
    await request(server())
      ...
      .expect(200);
    const premium = await prisma.membershipTier.findUnique({
      where: { type: "premium" },
    });
  ...
  scenario("ADM-064", async () => {
    ...
    await request(server())
      ...
      .expect(200);
    const basic = await prisma.membershipTier.findUnique({
      where: { type: "basic" },
    });
    const premium = await prisma.membershipTier.findUnique({
      where: { type: "premium" },
  ...
  scenario("ADM-065", async () => {
    ...
    await request(server())
      ...
      .expect(200);
    const after = await prisma.membershipTier.findUnique({
      where: { type: "premium" },
    });
  ...
  scenario("ADM-043", async () => {
    ...
    const setting = await prisma.platformSetting.findUnique({
      where: { settingKey: "robustness_key" },
    });
  ...
  scenario("ADM-113", async () => {
    ...
    await prisma.user.update({
      where: { id: target.id },
      data: { isBanned: true, bannedReason: "ilk" },
    });
    const res = await request(server())
      .post(`/api/admin/users/${target.id}/ban`)
      .set(authHeader(admin))
  ...
  scenario("ADM-114", async () => {
    ...
    await prisma.product.update({
      where: { id: p1.id },
      data: { reservedQuantity: 1, status: "reserved" },
    });
    ...
    const acc = await prisma.trade.findUnique({
      where: { id: acceptedTrade.id },
    });
    const wh = await prisma.trade.findUnique({
      where: { id: warehouseTrade.id },
    });
  ...
  scenario("ADM-115", async () => {
    ...
    const activeP = await createProduct({
      sellerId: seller.id,
      categoryId: baseline.categoryId,
      status: "active",
    });
    const pendingP = await createProduct({
      sellerId: seller.id,
      categoryId: baseline.categoryId,
      status: "pending",
    });
    // buyer olarak bekleyen bir teklif oluştur.
    const targetP = await createProduct({
      sellerId: seller.id,
      categoryId: baseline.categoryId,
      status: "active",
    });
    ...
    expect(
      (await prisma.product.findUnique({ where: { id: activeP.id } }))?.status,
    ).toBe("suspended");
    expect(
      (await prisma.product.findUnique({ where: { id: pendingP.id } }))?.status,
    ).toBe("rejected");
    ...
    await request(server())
      ...
      .expect(200);
    expect(
      (await prisma.offer.findUnique({ where: { id: off2.id } }))?.status,
    ).toBe("cancelled");
  });
  ...
  scenario("ADM-116", async () => {
    ...
    await prisma.user.update({
      where: { id: seller.id },
      data: { isBanned: true, bannedReason: "x", bannedAt: new Date() },
    });
    ...
    expect(
      (await prisma.product.findUnique({ where: { id: suspended.id } }))
        ?.status,
    ).toBe("active");
    const log = await prisma.auditLog.findFirst({
      where: { action: "user_unban", entityId: seller.id },
    });
    expect(log).toBeTruthy();
  });
  ...
  scenario("ADM-118", async () => {
    ...
    const target = await createUser(ctx.module);
    const prisma = getPrisma();
    const adminUserRow = await prisma.adminUser.findFirst({
  ...
  scenario("ADM-021", async () => {
    ...
    const prisma = getPrisma();
    expect(
      (await prisma.product.findUnique({ where: { id: product.id } }))?.status,
    ).toBe("active");
  });
  ...
  scenario("ADM-025", async () => {
    ...
    const moderator = await createAdminUser(ctx.module, {
      email: "al-mod@test.com",
      role: AdminRole.moderator,
    });
    const admin = await createAdminUser(ctx.module, {
      email: "al-admin@test.com",
      role: AdminRole.admin,
    });
    const superAdmin = await createAdminUser(ctx.module, {
      email: "al-sa@test.com",
      role: AdminRole.super_admin,
    });
    await request(server())
  ...
  scenario("ADM-028", async () => {
    ...
    const admin = await createAdminUser(ctx.module, {
      email: "ip-admin@test.com",
      role: AdminRole.admin,
    });
    const superAdmin = await createAdminUser(ctx.module, {
      email: "ip-sa@test.com",
      role: AdminRole.super_admin,
    });
    await request(server())
  ...
  scenario("ADM-057", async () => {
    ...
    await request(server())
      .get("/api/reports/access/realtime")
      .set(authHeader(moderator))
      .expect(200);
  });
  ...
  scenario("ADM-059", async () => {
    ...
    const diffDays =
      (new Date(res.body.period.end).getTime() -
        new Date(res.body.period.start).getTime()) /
      86400_000;
  ...
  scenario("ADM-052", async () => {
    ...
    const res = await request(server())
      .get(
        "/api/admin/analytics/revenue?startDate=2020-01-01&endDate=2030-01-01",
      )
      .set(authHeader(admin))
      .expect(200);
    expect(res.body).toBeTruthy();
  ...
  scenario("ADM-053", async () => {
    const admin = await createAdminUser(ctx.module, {
      email: "snap-admin@test.com",
      role: AdminRole.admin,
    });
    const superAdmin = await createAdminUser(ctx.module, {
      email: "snap-sa@test.com",
      role: AdminRole.super_admin,
    });
  ...
  scenario("ADM-060", async () => {
    ...
    await request(server())
      .get("/api/admin/commission/revenue")
      .set(authHeader(moderator))
      .expect(403);
  });
  ...
  scenario("ADM-032", async () => {
    ...
    const admin = await createAdminUser(ctx.module, {
      email: "tax-admin@test.com",
      role: AdminRole.admin,
    });
    const superAdmin = await createAdminUser(ctx.module, {
      email: "tax-sa@test.com",
      role: AdminRole.super_admin,
    });
    await request(server())
  ...
  scenario("ADM-066", async () => {
    ...
    const admin = await createAdminUser(ctx.module, {
      email: "kdv-admin@test.com",
      role: AdminRole.admin,
    });
    const superAdmin = await createAdminUser(ctx.module, {
      email: "kdv-sa@test.com",
      role: AdminRole.super_admin,
    });
    await request(server())
      .post("/api/admin/tax/rates")
  ...
  scenario("ADM-071", async () => {
    ...
    const admin = await createAdminUser(ctx.module, {
      email: "txr-admin@test.com",
      role: AdminRole.admin,
    });
    const superAdmin = await createAdminUser(ctx.module, {
      email: "txr-sa@test.com",
      role: AdminRole.super_admin,
    });
    await request(server())
  ...
  scenario("ADM-033", async () => {
    ...
    const admin = await createAdminUser(ctx.module, {
      email: "mt-admin@test.com",
      role: AdminRole.admin,
    });
    const moderator = await createAdminUser(ctx.module, {
      email: "mt-mod@test.com",
      role: AdminRole.moderator,
    });
    const superAdmin = await createAdminUser(ctx.module, {
      email: "mt-sa@test.com",
      role: AdminRole.super_admin,
    });
    const prisma = getPrisma();
  ...
  scenario("ADM-042", async () => {
    ...
    if (res.status === 200) {
      const log = await prisma.auditLog.findFirst({
        where: { action: "payout_release", entityId: orderId },
      });
  ...
  scenario("ADM-092", async () => {
    ...
    const prisma = getPrisma();
    for (const [st, oid] of [
      [PayoutStatus.failed, "F1"],
      [PayoutStatus.returned, "F2"],
      [PayoutStatus.completed, "C1"],
    ] as const) {
  ...
  scenario("ADM-093", async () => {
    ...
    const after1 = await prisma.tradeCashPayment.findFirst({
    ...
    const after2 = await prisma.tradeCashPayment.findFirst({
      where: { tradeId: trade.id },
    });
  ...
  scenario("ADM-077", async () => {
    ...
    const prisma = getPrisma();
    expect(
      await prisma.staticPage.findUnique({ where: { slug: "kvkk" } }),
    ).toBeTruthy();
  });
  ...
  scenario("ADM-080", async () => {
    ...
    const prisma = getPrisma();
    expect(
      await prisma.staticPage.findUnique({ where: { id: page.id } }),
    ).toBeNull();
  ...
  scenario("ADM-082", async () => {
    ...
    const moderator = await createAdminUser(ctx.module, {
      email: "et-mod@test.com",
      role: AdminRole.moderator,
    });
    const admin = await createAdminUser(ctx.module, {
      email: "et-admin@test.com",
      role: AdminRole.admin,
    });
    await request(server())
  ...
  scenario("ADM-044", async () => {
    ...
    const created = await prisma.adminUser.findFirst({
      where: { user: { email: "yeni-mod@tarodan.com" } },
    });
  ...
  scenario("ADM-098", async () => {
    ...
    const prisma = getPrisma();
    const created = await prisma.adminUser.findFirst({
      where: { user: { email: "temp-mod@tarodan.com" } },
    });
    expect(created?.role).toBe("moderator");
    expect(created?.isActive).toBe(true);
  });
  ...
  scenario("ADM-101", async () => {
    ...
    await prisma.platformSetting.upsert({
      ...
      create: {
        settingKey: "allow_admin_assign_roles",
        settingValue: "true",
        settingType: "boolean",
      },
    });
  ...
  scenario("ADM-103", async () => {
    ...
    const prisma = getPrisma();
    const adminRow = await prisma.adminUser.findFirst({
      where: { userId: superAdmin.id },
    });
    const res = await request(server())
  ...
  scenario.skip(
    "ADM-104",
    'removeAdminStaff son-süper dalı erişilemez: actor≠target super_admin ⇒ activeSupers>1; actor===target ⇒ önce "kendi yetkini" hatası (aynı garanti ADM-103’te)',
  );
  ...
  scenario("ADM-105", async () => {
    ...
    await createAdminUser(ctx.module, {
      email: "sa-second@test.com",
      role: AdminRole.super_admin,
    });
    const prisma = getPrisma();
    const selfRow = await prisma.adminUser.findFirst({
      where: { userId: superAdmin.id },
    });
    const res = await request(server())
  ...
  scenario("ADM-107", async () => {
    ...
    const res = await request(server())
      ...
      .expect(200);
    expect(res.body.super_admin).toEqual(
      expect.arrayContaining([...ADMIN_PERMISSION_KEYS]),
    );
  ...
  scenario("ADM-108", async () => {
    ...
    const moderator = await createAdminUser(ctx.module, {
      email: "ci-mod@test.com",
      role: AdminRole.moderator,
    });
    const superAdmin = await createAdminUser(ctx.module, {
      email: "ci-sa@test.com",
      role: AdminRole.super_admin,
    });
