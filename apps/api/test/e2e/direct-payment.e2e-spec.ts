...
describe("Direct API card payment (tek yol, E2E)", () => {
  ...
  it("kayıtlı kartla ödeme: capiPaymentByRegisteredCard çağrılır (utoken/ctoken) ve success döner", async () => {
    setRecurringFlag(true);
    const { buyer, orderId } = await makeBuyerWithOrder();
    const card = await seedSavedCard(buyer.id);
    ctx.paytr.nextRegisteredCardResult = { status: "success" };
    ...
    // Kayıtlı-kart CIT yolu kullanılır — recurring DEĞİL (o yalnız oto-yenilemede).
    expect(ctx.paytr.registeredCardCalls.length).toBe(1);
    expect(ctx.paytr.registeredCardCalls[0].utoken).toBe(card.utoken);
    expect(ctx.paytr.registeredCardCalls[0].ctoken).toBe(card.ctoken);
  ...
  it("kayıtlı kartla ödeme başarısızsa status=failed + payment.failureReason yazılır", async () => {
    ...
    const { buyer, orderId } = await makeBuyerWithOrder();
    const card = await seedSavedCard(buyer.id);
    ctx.paytr.nextRegisteredCardResult = {
      status: "failed",
      reason: "Kart kapalı",
      tryAgain: false,
    };
