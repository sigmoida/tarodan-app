...
describe('Direct API scenarios (E2E)', () => {
  ...
  it('SATIN ALMA: Direct (yeni kart) → callback → sipariş ödendi + escrow hold', async () => {
    ...
    const product = await createProduct({ sellerId: seller.id, categoryId: baseline.categoryId, price: 400, quantity: 1 });
    ...
    const payment = await prisma.payment.findUnique({ where: { id: pay.body.paymentId } });
  ...
  it('GRUP (sepet): checkout → process-direct(checkoutGroupId) → PayTR çağrıldı + payment gruba bağlı', async () => {
    ...
    const buyer = await createUser(ctx.module);
    const seller = await createUser(ctx.module, { isSeller: true });
    const p1 = await createProduct({ sellerId: seller.id, categoryId: baseline.categoryId, price: 150, quantity: 1 });
    const p2 = await createProduct({ sellerId: seller.id, categoryId: baseline.categoryId, price: 250, quantity: 1 });
    const addr = await createAddress({ userId: buyer.id });
    ...
    expect(ctx.paytr.directPaymentCalls[0].amount).toBe(Number(payment!.amount));
  ...
  it('TAKAS nakit: process-direct(tradeId) → PayTR çağrıldı + payment trade-cash’e bağlı', async () => {
    ...
    const trade = await seedAwaitingCashTrade(initiator.id, receiver.id, 'TR-SCEN-PAY');
    ...
    const cashPayment = await prisma.tradeCashPayment.findUnique({ where: { tradeId: trade.id } });
    const payment = await prisma.payment.findUnique({ where: { id: pay.body.paymentId } });
