...
export function useSelection(selectable = false): SelectionState {
  ...
  return {
    ...
    toggleAll: (ids) =>
      setSet((prev) => (ids.every((id) => prev.has(id)) ? new Set() : new Set(ids))),
...
export const ResourceListContext = createContext<ResourceListContextValue<any> | null>(null);
...
export function useResourceList<T = any>(): ResourceListContextValue<T> {
  ...
  if (!ctx) throw new Error('useResourceList must be used within <ResourceList>');
  return ctx;
}
