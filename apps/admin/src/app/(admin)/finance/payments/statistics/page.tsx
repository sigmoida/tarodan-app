...
export default function PaymentStatisticsPage() {
  ...
  return (
    <AdminPage>
      <PageHeader
        title={t("admin.finance.payments.statisticsTitle")}
        backHref="/finance/payments"
        description={
          data
            ? `${new Date(data.startDate).toLocaleDateString(t("common.dateLocale"))} - ${new Date(
                data.endDate,
              ).toLocaleDateString(t("common.dateLocale"))}`
            : undefined
        }
      />
