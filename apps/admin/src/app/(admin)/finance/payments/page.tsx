...
export default function PaymentsPage() {
  ...
  return (
    <AdminPage>
      <PageHeader title="Ödeme Yönetimi" description="Tüm ödeme işlemleri ve durumları">
        <Button
          variant="secondary"
      ...
      <ResourceList<Payment>
        ...
        fetcher={(p) =>
          adminApi.getPayments(p).then((res) => {
            ...
            return { ...res, data: { data: mapPayments(raw), meta: { total } } };
          })
        }
        getRowId={(p) => p.id}
        syncUrl
        initialFilters={{ status: 'all', provider: 'all', startDate: '', endDate: '' }}
        errorMessage="Ödemeler yüklenemedi"
      >
        <ResourceList.Toolbar>
          <ResourceList.Search placeholder="İşlem no, sipariş no veya kullanıcı ara..." />
        ...
        <ResourceList.Table
          ...
          onRowClick={(p) => router.push(`/finance/payments/${p.id}`)}
