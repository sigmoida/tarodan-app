...
export const elogoColumns = [
  col.custom<Invoice>(
    ...
    (i) => (
      <div className="text-sm">
        ...
        <span className="mt-1 inline-flex rounded bg-surface-alt px-2 py-0.5 text-[11px] font-medium text-muted">
          {i.documentTypeLabel}
        </span>
        {i.isReturn && i.billingReference && (
          <p className="mt-1 text-xs text-danger-600">İade → {i.billingReference}</p>
        )}
      </div>
    ),
    { grow: 2, minWidth: 150 },
  ),
  col.badge<Invoice>('Tür', (i) => (
    <Badge variant={i.isReturn ? 'danger' : 'secondary'} size="sm">
      {i.typeLabel}
    </Badge>
  )),
  col.user<Invoice>('Alıcı', (i) => ({
    name: i.recipientName || '—',
    secondary: i.recipientVknTckn ? `VKN/TCKN: ${i.recipientVknTckn}` : undefined,
  })),
  col.custom<Invoice>(
    'Tutar',
    (i) => (
  ...
  col.custom<Invoice>('Durum', (i) => (
    <div>
      <StatusBadge status={i.status} config={invoiceStatusConfig} />
      {i.status === 'failed' && i.resultMsg && (
        <p className="mt-1 max-w-[200px] truncate text-xs text-danger-600">{i.resultMsg}</p>
      )}
    </div>
  )),
  ...
  col.custom<Invoice>('PDF', (i) =>
    i.hasPdf ? <InvoicePdfButton id={i.id} seller={false} /> : <span className="text-xs text-muted">—</span>,
  ),
];
...
export const sellerColumns = [
  col.custom<SellerInvoice>(
    ...
    (s) => (
      <div className="text-sm">
        ...
        <p className="max-w-[220px] truncate text-xs text-muted">{s.fileName}</p>
        {s.replacedAt && (
  ...
  col.user<SellerInvoice>('Alıcı', (s) => ({ name: s.buyerName, secondary: s.buyerEmail })),
  ...
  col.date<SellerInvoice>('Yüklenme', (s) => s.uploadedAt),
  col.custom<SellerInvoice>('PDF', (s) => <InvoicePdfButton id={s.id} seller />),
];
