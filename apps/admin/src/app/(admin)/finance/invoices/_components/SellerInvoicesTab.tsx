  ...
  return (
    <ResourceList<SellerInvoice>
      ...
      fetcher={(p) =>
        adminApi.getSellerInvoices(p).then((res) => {
          ...
          return { ...res, data: { data: mapSellerInvoices(raw), meta: { total } } };
        })
      }
      getRowId={(s) => s.id}
      syncUrl
    ...
    >
      <ResourceList.Toolbar>
        <ResourceList.Search placeholder="Sipariş no, satıcı, alıcı veya dosya ara..." />
        <ResourceList.DateRange />
      </ResourceList.Toolbar>
      <ResourceList.Total unit="fatura" />
      <ResourceList.Table columns={sellerColumns} emptyText="Fatura bulunamadı" />
