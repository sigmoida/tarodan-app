...
import { ProductsPendingBadge } from './_components/ProductsPendingBadge';
...
export default function ProductsPage() {
  ...
  const approve = useAdminMutation((id: string) => adminApi.approveProduct(id), {
    invalidates: ['products'],
    successMessage: 'Ürün onaylandı',
  });
  const reject = useAdminMutation(
    (v: { id: string; reason: string }) => adminApi.rejectProduct(v.id, v.reason),
  ...
  const restore = useAdminMutation((id: string) => adminApi.restoreProduct(id), {
    invalidates: ['products'],
  ...
  return (
    ...
    >
      <ResourceList.Header
        title="Ürünler"
        badge={<ProductsPendingBadge />}
        description={<ProductsCountText />}
        actions={<ProductsExport />}
        tabs={PRODUCT_TABS}
        activeTab={tab}
        onTabChange={setTab}
      />
      ...
      <ProductsTable onApprove={onApprove} onReject={onReject} onDelete={onDelete} onRestore={onRestore} />
