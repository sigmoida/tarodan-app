...
export default function AttributesPage() {
    ...
    return (
        <>
                        ...
                        {loading ? <div className="text-center py-8"><Spinner size="md" className="mx-auto" /></div>
                            : groups.filter((g) => g.slug !== 'vehicle_type').length === 0 ? <div className="text-center py-8 text-muted">Grup yok</div>
                                : <div className="space-y-2">{groups.filter((g) => g.slug !== 'vehicle_type').map((g) => (
                                    <div key={g.id} onClick={() => selectGroup(g)} className={`p-3 rounded-lg cursor-pointer flex items-center justify-between gap-3 ${selectedGroup?.id === g.id ? 'bg-primary-50 border border-primary-600' : 'bg-surface-alt hover:bg-surface-alt'}`}>
                                        <div className="flex items-center gap-2 min-w-0"><Squares2X2Icon className="h-5 w-5 text-muted shrink-0" /><span className="text-heading truncate">{g.name}</span>{!g.isActive && <span className="px-1.5 text-xs bg-body text-muted rounded shrink-0">Pasif</span>}</div>
                        ...
                        {!selectedGroup ? <div className="text-center py-12 text-muted">Değerleri görmek için bir grup seçin</div>
                            : (<>
                                <div className="flex items-center justify-between gap-3 mb-4">
                                    <h3 className="text-lg font-medium text-heading truncate min-w-0">{selectedGroup.name} Değerleri</h3>
                                    <Button variant="primary" size="sm" onClick={openAttrCreate} className="shrink-0"><PlusIcon className="w-4 h-4" />Değer Ekle</Button>
                                </div>
            ...
            {/* Group Modal */}
            {showGroupModal && (<div className="fixed inset-0 bg-heading bg-opacity-50 flex items-center justify-center z-50"><div className="bg-surface-elevated rounded-xl px-6 pb-6 pt-5 max-w-md w-full mx-4 border border-border">
                <h2 className="text-xl font-semibold text-heading mb-4 leading-tight">{editingGroup ? 'Grubu Düzenle' : 'Yeni Grup'}</h2>
            ...
            {/* Attribute Modal */}
            {showAttrModal && (<div className="fixed inset-0 bg-heading bg-opacity-50 flex items-center justify-center z-50"><div className="bg-surface-elevated rounded-xl px-6 pb-6 pt-5 max-w-md w-full mx-4 border border-border">
                <h2 className="text-xl font-semibold text-heading mb-4 leading-tight">{editingAttr ? 'Değeri Düzenle' : 'Yeni Değer'}</h2>
            ...
            {deleteConfirm && (<div className="fixed inset-0 bg-heading bg-opacity-50 flex items-center justify-center z-50"><div className="bg-surface-elevated rounded-xl px-6 pb-6 pt-5 max-w-md w-full mx-4 border border-border">
                <h3 className="text-lg font-semibold text-heading mb-4 leading-tight">{deleteConfirm.type === 'group' ? 'Grubu Sil' : 'Değeri Sil'}</h3><p className="text-muted mb-6">Silmek istediğinizden emin misiniz?</p>
