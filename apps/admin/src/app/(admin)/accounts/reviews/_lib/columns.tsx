...
import { type Review, type UserRating, type ReviewStatus, reviewStatusConfig } from './types';
...
export function productReviewColumns(act: Act) {
  return [
        ...
        <div className="flex items-center gap-3">
          {r.product.images?.[0] ? (
            <div className="relative h-10 w-10 shrink-0 overflow-hidden rounded">
              <Image src={r.product.images[0].url} alt={r.product.title} fill className="object-cover" />
            </div>
          ) : (
            <div className="h-10 w-10 shrink-0 rounded bg-surface-alt" />
          )}
          <span className="min-w-0 truncate text-sm font-medium text-heading" title={r.product.title}>
            {r.product.title}
          </span>
        </div>
      ),
    ...
    col.badge<Review>('Durum', (r) => <Badge status={r.status} config={reviewStatusConfig} />),
