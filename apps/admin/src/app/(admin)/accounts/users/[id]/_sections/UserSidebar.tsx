  ...
  const quickLinks = [
    {
      ...
      label: t("admin.users.detail.allOrders", {
        count: user.stats?.ordersCount || 0,
      }),
    },
    {
      href: `/catalog/products?sellerId=${user.id}`,
      icon: CubeIcon,
      label: t("admin.users.detail.allProducts", {
        count: user.stats?.productsCount || 0,
      }),
    },
    {
      href: ordersTabHref("trades", { userId: user.id }),
      icon: ArrowPathIcon,
      label: t("admin.users.detail.allTrades", {
        count: user.stats?.tradesCount || 0,
      }),
    },
  ];
    ...
    <>
      ...
      <SectionCard title={t("admin.users.detail.verificationStatusTitle")}>
        <div className="space-y-3">
          ...
          <VerifiedRow
            label={t("admin.users.detail.identityLabel")}
            ok={user.isVerified}
          />
      ...
      {user.addresses && user.addresses.length > 0 && (
        <SectionCard
          title={t("admin.users.detail.addressesTitle", {
