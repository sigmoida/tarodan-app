...
export function AdminFinancialSummary({
  ...
  const sub =
    ...
    fallback?.subtotal ??
    (fallback &&
    fallback.totalAmount != null &&
    fallback.shippingCost != null &&
    fallback.buyerFeeAmount != null
      ? fallback.totalAmount - fallback.shippingCost - fallback.buyerFeeAmount
      : 0);
  ...
  const sellerNet =
    p?.sellerNetAmount ?? fallback?.sellerNetAmount ?? sub - sellerFee;
  ...
  return (
    <div className={className}>
      <h3 className="text-sm font-semibold text-body mb-3">
        {t("admin.operations.orders.financial.title")}
      </h3>
      <dl className="space-y-2 text-sm">
        ...
        {discountAmount > 0 && (
          <div className="flex justify-between gap-2 min-w-0">
            <dt className="text-muted truncate">
              {t("admin.operations.orders.financial.discount", {
        ...
        <div className="flex justify-between gap-2 min-w-0 border-t pt-2 mt-2">
          <dt className="text-heading font-semibold truncate">
