            ...
            <li key={part.key}>
              ...
              >
                <span className="truncate">
                  {t(
                    `admin.dashboard.queueParts.${part.key}` as MessageKey,
                  )}
                </span>
...
export function QueuesZone({
  ...
  return (
    <section className="flex flex-col gap-3">
      ...
      {isError ? (
        <EmptyState size="compact" title={t("admin.dashboard.zones.loadFailed")} />
      ) : isLoading ? (
        ...
        <EmptyState size="compact" title={t("admin.dashboard.zones.queuesEmpty")} />
      ) : (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {queues.map((tile) => (
