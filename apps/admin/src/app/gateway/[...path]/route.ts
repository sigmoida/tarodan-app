...
/**
 * Gateway proxy. Every client data call goes to same-origin `/gateway/*` (no
 * CORS) and is forwarded to the NestJS API with a server-side `Authorization:
 * Bearer` header. Access tokens are refreshed transparently on 401. The browser
 * never holds or sees the API tokens. The proxy logic lives in the shared
