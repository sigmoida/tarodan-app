...
const INITIAL_ROLES = [
    ...
    },
    {
        ...
        permissions: ['users.view', 'users.manage', 'products.view', 'products.manage', 'orders.view', 'orders.manage'],
        usersCount: 5,
        isSystem: true,
    },
    {
...
const INITIAL_ADMIN_USERS = [
    { id: '1', name: 'Görkem', email: 'gorkem@tarotaro.com', role: 'super_admin', status: 'active', lastLogin: '2024-02-09 18:30' },
    { id: '2', name: 'Ahmet', email: 'ahmet@tarotaro.com', role: 'admin', status: 'active', lastLogin: '2024-02-08 14:20' },
    { id: '3', name: 'Ayşe', email: 'ayse@tarotaro.com', role: 'moderator', status: 'inactive', lastLogin: '2024-01-15 09:45' },
];
...
export default function RolesPage() {
    ...
    const handleAssignUser = (e: React.FormEvent) => {
        e.preventDefault();
        if (editingUser) {
            setAdminUsers(prev => prev.map(u => u.id === editingUser.id ? { ...u, role: userForm.role } : u));
                ...
                {activeTab === 'roles' ? (
                    ...
                    <div className="space-y-6">
                        <div className="flex justify-end">
                            <button onClick={handleCreateRole} className="btn-primary flex items-center">
                                ...
                                <tbody>
                                    {adminUsers.map((user) => (
                                        <tr key={user.id} className="border-b border-gray-200 hover:bg-gray-100/50">
                                            ...
                                            <td className="p-4">
                                                <span className={`px-2 py-1 rounded-full text-xs font-medium ${user.status === 'active' ? 'bg-green-500/10 text-green-500' : 'bg-gray-500/10 text-gray-500'
                                                    }`}>
