...
export default function Error({
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center gap-4 bg-surface px-6 text-center">
      ...
      <h1 className="text-2xl font-semibold text-heading">{t('admin.shared.errors.title')}</h1>
      <p className="max-w-md text-muted">{t('admin.shared.errors.description')}</p>
      {error.digest && (
        <p className="text-xs text-subtle">{t('admin.shared.errors.errorCode', { code: error.digest })}</p>
      )}
