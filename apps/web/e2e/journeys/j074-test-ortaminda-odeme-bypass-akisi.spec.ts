...
test.describe('J74 — Test ortamında ödeme bypass akışı', () => {
  test('bypass iki kez çağrılır ama durum bozulmaz, tek payment kalır', async ({ request }) => {
    ...
    // İdempotency = çift tamamlamada state BOZULMAZ. App duplicate'i 400 ('Payment already completed')
    // ile reddediyor (de geçerli idempotency). Önemli olan TEK payment/TEK hold (aşağıda doğrulanıyor).
    expect([200, 201, 400, 409], `bypass 2 (${second.status()})`).toContain(second.status());
