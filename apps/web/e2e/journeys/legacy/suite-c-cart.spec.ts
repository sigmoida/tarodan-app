...
test.describe("J58 — Sepette kupon denemeleri", () => {
  test("bos ve gecersiz kupon reddedilir, gecerli kupon indirim yansitir, kaldirma idempotent", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    ...
    const myProdsRes = await request.get(`${API}/products/my`, {
      headers: auth(sellerToken),
      params: { status: "active", limit: "50" },
    });
    const myList: any[] = (await myProdsRes.json())?.data ?? [];
    const sellerProducts = myList
      .filter(
        (p) => p.status === "active" && (p.quantity == null || p.quantity > 0),
      )
      .slice(0, 2);
    expect(
      sellerProducts.length,
      "saticinin >=2 aktif urunu var",
    ...
    const cartGet = await request.get(`${API}/cart`, {
      headers: auth(buyerToken),
    });
    ...
    const emptyRes = await request.post(`${API}/cart/coupon`, {
      headers: auth(buyerToken),
      data: { code: "" },
    });
    ...
    const badRes = await request.post(`${API}/cart/coupon`, {
      headers: auth(buyerToken),
      data: { code: "GECERSIZKUPON_" + Date.now() },
    });
    ...
    const dbDiscount = await dbFind(
      request,
      "discount",
      { code },
      { id: true, sellerId: true, value: true, type: true },
    );
    expect(
      dbDiscount?.code === undefined ? created?.code : code,
      "kupon DB de",
    ).toBeTruthy();
    // sellerId, kuponu yaratan saticinin id'si olmali (seller-scope)
    expect(dbDiscount.sellerId).toBe(sellerMe.id);

    const applyRes = await request.post(`${API}/cart/coupon`, {
      headers: auth(buyerToken),
      data: { code },
    });
    ...
    const couponDiscount = Number(
      applied?.calculation?.couponDiscountTotal ?? 0,
    );
    expect(
      couponDiscount,
      "kupon indirimi sepete yansidi (>0)",
    ).toBeGreaterThan(0);
    ...
    const remove1 = await request.delete(`${API}/cart/coupon`, {
      headers: auth(buyerToken),
    });
    ...
    const remove2 = await request.delete(`${API}/cart/coupon`, {
      headers: auth(buyerToken),
    });
    expect(
      remove2.ok(),
      "ikinci kaldirma da sorunsuz (idempotent)",
    ).toBeTruthy();
...
test.describe("J59 — Sepet izolasyonu (IDOR)", () => {
  test("A sepete urun ekler, B A`nin sepetini goremez, B kendi bos sepetiyle alir", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    ...
    const aAdd = await request.post(`${API}/cart/items`, {
      headers: auth(aToken),
      data: { productId: aProduct.id, quantity: 1 },
    });
    expect(aAdd.ok(), "A sepete urun ekledi").toBeTruthy();

    // DB: A nin sepetinde item var
    const aCartGet = await request.get(`${API}/cart`, {
      headers: auth(aToken),
    });
    const aCart = await aCartGet.json();
    expect(
      aCart?.calculation?.itemCount,
      "A sepetinde >=1 urun",
    ...
    const bSeesOwn = await request.get(`${API}/cart`, {
      headers: auth(bToken),
    });
    ...
    const idorDelete = await request.delete(
      `${API}/cart/items/${aProduct.id}`,
      { headers: auth(bToken) },
    );
    ...
    const aCartAfter = await (
      await request.get(`${API}/cart`, { headers: auth(aToken) })
    ).json();
    expect(
      aCartAfter?.calculation?.itemCount,
      "A sepeti etkilenmedi",
    ).toBeGreaterThanOrEqual(1);
...
test.describe("J60 — Kendi urunu engeli (sepet / teklif / Hemen Al)", () => {
  test("satici kendi urununu sepete/teklif/hemen-al ile alamaz, baskasininkini normal alir", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    ...
    const cartAdd = await request.post(`${API}/cart/items`, {
      headers: auth(sellerToken),
      data: { productId: own.id, quantity: 1 },
    });
    ...
    const offerRes = await request.post(`${API}/offers`, {
      ...
      data: {
        productId: own.id,
        amount: Math.max(1, Math.round(Number(own.price) * 0.8)),
      },
    });
    ...
    const offerCount = await dbCount(request, "offer", {
      productId: own.id,
      buyerId: sellerMe.id,
    });
    ...
    const buyRes = await request.post(`${API}/orders/buy`, {
      headers: auth(sellerToken),
      data: { productId: own.id, shippingAddressId: addressId },
    });
...
test.describe("J62 — Cift Hemen Al tek siparis (idempotency)", () => {
  test("ayni urune iki kez Hemen Al ayni pending siparisi doner, ode -> tamamlanir", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    ...
    const buy1 = await request.post(`${API}/orders/buy`, {
      headers: auth(token),
      data: { productId: product.id, shippingAddressId: addressId },
    });
    ...
    const order1 = await dbFind(
      request,
      "order",
      { id: orderId1 },
      { id: true, status: true },
    );
    expect(order1.status, "ilk siparis pending_payment").toBe(
      "pending_payment",
    );
    ...
    const pendingCount = await dbCount(request, "order", {
    ...
    const initRes = await request.post(`${API}/payments/initiate`, {
      headers: auth(token),
      data: { orderId: orderId1, provider: "paytr" },
    });
...
test.describe("J65 — Odeme oncesi adres degisimi + IDOR", () => {
  test("alici pending siparis adresini degistirir, baska kullanici degistiremez, ode -> ilerler", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    ...
    const before = await dbFind(
      request,
      "order",
      { id: orderId },
      { id: true, status: true, shippingAddress: true },
    );
    ...
    const idorPatch = await request.patch(
      `${API}/orders/${orderId}/shipping-address`,
      {
        headers: auth(bToken),
    ...
    expect(
      (afterIdor?.shippingAddress as any)?.city,
      "IDOR sonrasi adres korundu",
    ).toBe("Ankara");
    ...
    expect(
      (finalOrder.shippingAddress as any)?.city,
      "odeme sonrasi yeni adres korundu",
    ).toBe("Ankara");
  });
});
