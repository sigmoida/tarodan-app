...
test.describe("J14 — Üyelik yükseltme: limit dolunca pakete geçip daha çok hak", () => {
  test("free limit dolu → premium abone+öde → limit açılır → auto-renew kapatılır", async ({
    page,
    request,
  }) => {
    ...
    expect(
      freeLimits.maxFreeListings,
      "free ücretsiz ilan tavanı sınırlı",
    ).toBeGreaterThan(0);
    expect(freeLimits.canCreateCollection, "free: koleksiyon hakkı yok").toBe(
      false,
    );
    expect(freeLimits.canTrade, "free: takas yok").toBe(false);
    const freeColCheck = await checkCollection(request, token);
    expect(
      freeColCheck.allowed,
      "free: koleksiyon oluşturma reddi (limit hayır)",
    ).toBe(false);
    ...
    const memRow = await dbFind(
      request,
      "userMembership",
      { userId: me.id },
      { status: true, autoRenew: true, tier: { select: { type: true } } },
    );
    ...
    expect(
      paidLimits.canCreateCollection,
      "premium: koleksiyon hakkı açıldı",
    ).toBe(true);
    expect(paidLimits.canTrade, "premium: takas açıldı").toBe(true);
    // Ücretsiz slot sayısı premium'da arttı (>= free 5).
    expect(paidLimits.maxFreeListings).toBeGreaterThanOrEqual(
      freeLimits.maxFreeListings,
    );
    expect(
      paidLimits.remainingFreeListings,
      "premium ücretsiz slot tekrar açıldı",
    ).toBeGreaterThan(0);
...
test.describe("J15 — Koleksiyon oluşturup paylaşma ve beğeni alma", () => {
  test("premium oluşturur+ürün ekler+public yapar; başkası beğenir; yabancı eklenemez", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    ...
    const afterItems = await (
      await request.get(`${API}/collections/${col.id}`, {
        headers: auth(ownerToken),
      })
    ).json();
    ...
    const pubRes = await request.patch(`${API}/collections/${col.id}`, {
      headers: auth(ownerToken),
      data: { isPublic: true },
    });
    ...
    const likeRes = await request.post(`${API}/collections/${col.id}/like`, {
    ...
    const updRes = await request.patch(`${API}/collections/${col.id}`, {
      headers: auth(ownerToken),
      data: { name: newName },
    });
    ...
    await expect
      .poll(
        async () => {
          const browse = await request.get(`${API}/collections/browse`, {
            params: { search: newName, pageSize: "50" },
          });
          const browseBody = await browse.json();
          return (browseBody.collections ?? []).some(
            (c: any) => c.id === col.id,
          );
        },
        { timeout: 8000, message: "koleksiyon browse listesinde görünmeli" },
      )
      .toBe(true);
  });
});
...
test.describe("J30 — Premium üye showcase için koleksiyonunu öne çıkarıyor", () => {
  test("premium abone → 2 koleksiyon → public → beğeni → biri silinir, diğeri güncellenir", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    ...
    const colA = await (
      await request.post(`${API}/collections`, {
        headers: auth(token),
        data: { name: `J30 Showcase A ${Date.now()}`, isPublic: false },
      })
    ).json();
    const colB = await (
      await request.post(`${API}/collections`, {
        headers: auth(token),
        data: { name: `J30 Showcase B ${Date.now()}`, isPublic: false },
      })
    ).json();
    expect(colA.id && colB.id).toBeTruthy();
    for (const t of ["Tomica Skyline", "Maisto Lambo", "Greenlight GTO"]) {
      const r = await request.post(`${API}/collections/${colA.id}/items`, {
        headers: auth(token),
        data: { customTitle: t },
      });
      expect(r.ok()).toBeTruthy();
    }
    const colAFull = await (
      await request.get(`${API}/collections/${colA.id}`, {
        headers: auth(token),
      })
    ).json();
    ...
    await request.patch(`${API}/collections/${colA.id}`, {
      headers: auth(token),
      data: { isPublic: true },
    });
    await request.patch(`${API}/collections/${colB.id}`, {
      headers: auth(token),
      data: { isPublic: true },
    });
    expect(
      (await (await request.get(`${API}/collections/${colA.id}`)).json())
        .isPublic,
    ).toBe(true);
    ...
    const l1 = await (
      await request.post(`${API}/collections/${colA.id}/like`, {
        headers: auth(fan1),
        data: {},
      })
    ).json();
    ...
    const del = await request.delete(`${API}/collections/${colB.id}`, {
      headers: auth(token),
    });
    expect([200, 204]).toContain(del.status());
    const gone = await request.get(`${API}/collections/${colB.id}`, {
      headers: auth(token),
    });
    expect(gone.ok(), "silinen koleksiyon bulunamaz").toBeFalsy();

    const upd = await request.patch(`${API}/collections/${colA.id}`, {
      headers: auth(token),
      data: { description: "Vitrin güncellendi" },
    });
...
test.describe("J104 — Günlük mesaj limiti kontrolü", () => {
  test("mesaj gönderdikçe kalan hak azalır; yükseltme sonrası limit (platform) sabit kalır", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    ...
    const prodsRes = await request.get(`${API}/products`, {
      params: { sellerId: seller.id, limit: "5" },
    });
    ...
    const threadRes = await request.post(`${API}/messages/threads`, {
      ...
      data: {
        ...
        message: "Merhaba, bu ürün müsait mi?",
      },
    });
    ...
    const before = await (
      await request.get(`${API}/messages/daily-limit`, {
        headers: auth(buyerToken),
      })
    ).json();
    ...
    for (const c of ["Fiyatta esneklik var mı?", "Kargo ne zaman çıkar?"]) {
      const m = await request.post(
        ...
        { headers: auth(buyerToken), data: { content: c } },
      );
    ...
    expect(after.remaining, "kalan mesaj hakkı azaldı").toBeLessThan(
      before.remaining,
    );
    expect(
    ...
    expect(afterUpgrade.limit, "günlük limit tier-bağımsız → sabit").toBe(
      before.limit,
    );

    // 4) Mesajlaşmaya devam edebilir (hâlâ hak var).
    const cont = await request.post(
      `${API}/messages/threads/${thread.id}/messages`,
      {
        headers: auth(buyerToken),
        data: { content: "Anlaştık, teşekkürler." },
      },
    );
...
test.describe("J105 — Koleksiyon sahipliği: yabancı düzenleyemiyor", () => {
  test("yabancı update/item-ekle/delete denemeleri 403; sahibi günceller+siler", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    ...
    const col = await (
      await request.post(`${API}/collections`, {
        headers: auth(ownerToken),
        data: { name: `J105 IDOR ${Date.now()}`, isPublic: true },
      })
    ).json();
    expect(col.id).toBeTruthy();
    const it = await request.post(`${API}/collections/${col.id}/items`, {
      headers: auth(ownerToken),
      data: { customTitle: "Sahibin ürünü" },
    });
    ...
    const upd = await request.patch(`${API}/collections/${col.id}`, {
      headers: auth(ownerToken),
      data: { name: newName },
    });
    expect(upd.ok()).toBeTruthy();
    ...
    const del = await request.delete(`${API}/collections/${col.id}`, {
      headers: auth(ownerToken),
    });
    expect([200, 204]).toContain(del.status());
    const afterDelete = await request.get(`${API}/collections/${col.id}`, {
      headers: auth(ownerToken),
    });
...
test.describe("J106 — Adsız koleksiyon oluşturulamıyor", () => {
  test("boş/çok kısa ad reddedilir; geçerli adla oluşur, ürün+public, başkası beğenir", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    ...
    const empty = await request.post(`${API}/collections`, {
      headers: auth(token),
    ...
    const tooShort = await request.post(`${API}/collections`, {
      headers: auth(token),
      data: { name: "ab", isPublic: true },
    });
    expect(tooShort.ok()).toBeFalsy();
    expect([400, 422]).toContain(tooShort.status());

    // 2) Geçerli adla oluştur.
    const okRes = await request.post(`${API}/collections`, {
      headers: auth(token),
      data: { name: `J106 Geçerli ${Date.now()}`, isPublic: false },
    });
    ...
    const it = await request.post(`${API}/collections/${col.id}/items`, {
      headers: auth(token),
      data: { customTitle: "İlk ürün" },
    });
    expect(it.ok()).toBeTruthy();
    const pub = await request.patch(`${API}/collections/${col.id}`, {
      headers: auth(token),
      data: { isPublic: true },
    });
    ...
    const like = await (
      await request.post(`${API}/collections/${col.id}/like`, {
        headers: auth(fan),
        data: {},
      })
    ).json();
...
test.describe("J107 — Üyelik paketi iptali ve yeniden abonelik", () => {
  test("abone ol → iptal → tekrar iptal red → yeniden abone → auto-renew kapat", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    ...
    expect(
      (await getLimits(request, token)).canCreateCollection,
      "basic koleksiyon hakkı",
    ).toBe(true);

    // 2) Aboneliği iptal et → status=cancelled + cancelledAt.
    const cancel = await request.post(`${API}/membership/cancel`, {
      headers: auth(token),
      data: {},
    });
    ...
    const ar = await request.patch(`${API}/membership/auto-renew`, {
      headers: auth(token),
      data: { autoRenew: false },
    });
...
test.describe("J108 — Geçersiz paket tipiyle abonelik denemesi", () => {
  test("geçersiz tier red + auth'suz red + geçerli abone + limit kontrolleri", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    ...
    const invalid = await request.post(`${API}/membership/subscribe`, {
      headers: auth(token),
      data: { tierType: "diamond", billingPeriod: "monthly" },
    });
    ...
    const noBilling = await request.post(`${API}/membership/subscribe`, {
      headers: auth(token),
    ...
    const noAuth = await request.post(`${API}/membership/subscribe`, {
      data: { tierType: "premium", billingPeriod: "monthly" },
    });
    ...
    const listing = await (
      await request.get(`${API}/membership/check/listing`, {
        headers: auth(token),
      })
    ).json();
    const trade = await (
      await request.get(`${API}/membership/check/trade`, {
        headers: auth(token),
      })
    ).json();
    const collection = await (
      await request.get(`${API}/membership/check/collection`, {
        headers: auth(token),
      })
    ).json();
...
test.describe("DOWNGRADE — süresi dolan üyelik free'ye düşer", () => {
  test("premium abone → dönem sonu geçmişe → scheduler → tier=free, autoRenew=false", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    ...
    expect(limits.canCreateCollection, "free: koleksiyon hakkı kapandı").toBe(
      false,
    );
  });
});
