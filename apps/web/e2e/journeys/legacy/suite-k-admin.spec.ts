...
test.describe("J18 — Ürün moderasyon red/düzelt/onay/satış", () => {
  test("satıcı ilan açar, admin reddeder, satıcı düzeltir, admin onaylar, alıcı satın alır", async ({
    request,
  }) => {
    ...
    const fixRes = await request.patch(`${API}/products/${productId}`, {
      ...
      data: {
        description:
          "Kurallara uygun, detaylı ve net açıklama eklendi. Orijinal ürün.",
        price: 249.99,
      },
    });
...
test.describe("J19 — Şikayet, ban, banlı engel, itiraz kaldır", () => {
  test("admin kullanıcıyı banlar, banlı engellenir, ban kalkar; normal kullanıcı banlayamaz", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    ...
    const target = await registerFreshUser(request, {
      displayName: "Kotuye Kullanan",
    });
    ...
    const reportRes = await request.post(`${API}/user-reports`, {
      ...
      data: {
        ...
        reason: "harassment",
        description:
          "Bu kullanıcı taciz edici davranıyor ve kuralları ihlal ediyor.",
      },
    });
    expect(reportRes.ok(), "kullanıcı şikayet edildi").toBeTruthy();

    // 2) Admin şikayet listesini + istatistikleri görür
    const listRes = await request.get(`${API}/user-reports/admin`, {
      headers: authHeader(adminToken),
      params: { type: "user" },
    });
    expect(listRes.ok(), "admin şikayet listesi").toBeTruthy();
    const statsRes = await request.get(`${API}/user-reports/admin/stats`, {
      headers: authHeader(adminToken),
    });
    ...
    const blocked = await request.get(`${API}/users/me`, {
    ...
    const unbanRes = await request.post(
      `${API}/admin/users/${target.id}/unban`,
      { headers: authHeader(adminToken) },
    );
    ...
    const afterUnban = await request.get(`${API}/users/me`, {
      headers: authHeader(target.token),
    });
    ...
    const normalBan = await request.post(
...
test.describe("J20 — Destek talebi yaşam döngüsü", () => {
  test("üye talep açar, yanıt yazar, yabancı engellenir, admin atar/çözer, üye görür", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    ...
    const createRes = await request.post(`${API}/support/tickets`, {
      ...
      data: {
        subject: "Siparişim hakkında sorum var",
        category: "general",
        message:
          "Siparişimin durumu ile ilgili bilgi almak istiyorum, yardımcı olur musunuz?",
      },
    });
    ...
    const replyRes = await request.post(
      `${API}/support/tickets/${ticketId}/messages`,
      {
        ...
        data: {
          content:
            "Ek bilgi: sipariş numaram panelimde görünüyor ancak kargo bilgisi yok.",
        },
      },
    );
    ...
    const peek = await request.get(`${API}/support/tickets/${ticketId}`, {
    ...
    const moderatorUser = await request.post(`${API}/auth/admin/login`, {
      data: USERS.moderator,
    });
    const moderatorId = (await moderatorUser.json())?.user?.id;
    expect(moderatorId, "moderatör user id").toBeTruthy();

    const prioRes = await request.patch(
    ...
    const adminReply = await request.post(
      `${API}/support/tickets/${ticketId}/messages`,
      {
        headers: authHeader(adminToken),
        data: {
          content:
            "Merhaba, siparişiniz kargoya verildi. Takip numarası en geç yarın iletilecektir.",
        },
      },
    );
    ...
    const memberView = await request.get(`${API}/support/tickets/${ticketId}`, {
      headers: authHeader(memberToken),
    });
    expect(memberView.ok(), "üye talebini görür").toBeTruthy();
    const viewed = await memberView.json();
    expect(["resolved", "closed"], "durum çözüldü/kapandı").toContain(
      viewed?.status,
    );
  });
});
...
test.describe("J111 — Şikayet yönetimi", () => {
  test("üye ürün+kullanıcı şikayet eder, kendi şikayetlerini listeler, admin görür ve aksiyon alır", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    ...
    const repProduct = await request.post(`${API}/user-reports`, {
      ...
      data: {
        ...
        targetId: product.id,
        reason: "counterfeit",
        description: "Bu ürün orijinal görünmüyor, sahte olabilir.",
      },
    });
    ...
    const seller = await apiMe(
      request,
      await apiLogin(request, USERS.sellerPremium),
    );
    const repUser = await request.post(`${API}/user-reports`, {
      ...
      data: {
        ...
        targetId: seller.id,
        reason: "scam",
        description: "Bu satıcı dolandırıcılık yaptığından şüpheleniyorum.",
      },
    });
    expect(repUser.ok(), "kullanıcı şikayet edildi").toBeTruthy();

    // 3) Üye kendi şikayetlerini listeler (en az 2 kayıt)
    const mine = await request.get(`${API}/user-reports/me`, {
      headers: authHeader(memberToken),
    });
    ...
    const all = await request.get(`${API}/user-reports/admin`, {
      headers: authHeader(adminToken),
    });
    expect(all.ok(), "admin tüm şikayetler").toBeTruthy();
    const stats = await request.get(`${API}/user-reports/admin/stats`, {
      headers: authHeader(adminToken),
    });
    ...
    const resolve = await request.patch(
      `${API}/user-reports/admin/${reportId}`,
      {
        headers: authHeader(adminToken),
        data: {
          status: "resolved",
          adminNote: "İncelendi, gerekli aksiyon alındı.",
        },
      },
    );
...
test.describe("J115 — Misafir destek formu + üye talebi", () => {
  test("misafir form gönderir, boş mesaj reddedilir, üye talep açar, admin çözer/kapatır", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    ...
    const guestRes = await request.post(`${API}/support/contact`, {
      data: {
        ...
        message:
          "Sitenizdeki ürünler hakkında genel bilgi almak istiyorum, teşekkürler.",
      },
    });
    ...
    const tRes = await request.post(`${API}/support/tickets`, {
      headers: authHeader(memberToken),
      data: {
        subject: "Üyelik paketim hakkında",
        category: "membership",
        message: "Üyelik paketimi yükseltmek istiyorum, nasıl ilerlemeliyim?",
      },
    });
    ...
    const reply = await request.post(
      `${API}/support/tickets/${ticketId}/messages`,
      {
        headers: authHeader(memberToken),
        data: {
          content: "Premium pakete geçmek istiyorum, fiyatlandırma nedir?",
        },
      },
    );
    ...
    const view = await request.get(`${API}/support/tickets/${ticketId}`, {
      headers: authHeader(memberToken),
    });
...
test.describe("J116 — Destek IDOR", () => {
  test("yabancı talebi göremez/yanıtlayamaz, admin atar+öncelik günceller, çözer", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    ...
    const tRes = await request.post(`${API}/support/tickets`, {
      ...
      data: {
        subject: "Gizli talep",
        category: "general",
        message: "Bu talebi yalnızca ben ve yetkililer görebilmeli.",
      },
    });
    ...
    const intrude = await request.post(
      `${API}/support/tickets/${ticketId}/messages`,
      {
        headers: authHeader(strangerToken),
    ...
    const moderatorId = (
      await (
        await request.post(`${API}/auth/admin/login`, { data: USERS.moderator })
      ).json()
    )?.user?.id;
    ...
    const prio = await request.patch(
      `${API}/support/admin/tickets/${ticketId}/priority`,
      {
        headers: authHeader(adminToken),
...
test.describe("J120 — Admin sipariş yönetimi", () => {
  test("admin tüm siparişleri listeler, normal engellenir, detay görür, yok-olan 404, günceller + dashboard", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    ...
    const userList = await request.get(`${API}/admin/orders`, {
...
test.describe("J121 — Toplu ürün onay", () => {
  test("birkaç ilan onaya düşer, admin listeler, toplu onaylar; normal kullanıcı toplu onay yapamaz", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    ...
    const id1 = await createPendingProduct(
      request,
      sellerToken,
      categoryId,
      "J121-a",
    ...
    const id2 = await createPendingProduct(
      request,
      sellerToken2,
      categoryId,
      "J121-b",
...
test.describe("J122 — Komisyon kuralı rol-bazlı", () => {
  test("super listeler+oluşturur, (normal admin oluşturamaz - seed kısıtı), moderatör göremez, global indirim açılır", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    ...
    const listRes = await request.get(`${API}/admin/commission-rules`, {
      headers: authHeader(superToken),
    });
    ...
    expect(
      modCreate.ok(),
      "super_admin olmayan komisyon oluşturamamalı",
    ).toBeFalsy();
    expect([401, 403]).toContain(modCreate.status());

    // 4) Moderatör komisyon kurallarını GÖRMEYE çalışır → engellenir (moderator listede yok)
    const modList = await request.get(`${API}/admin/commission-rules`, {
      headers: authHeader(moderatorToken),
    });
...
test.describe("J124 — Filtreye takılan mesajlar", () => {
  test("içerik filtresi telefonu yakalar (düz + aralıklı), admin bekleyenleri görür, normal kullanıcı engellenir", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    ...
    const phonePattern =
      "(\\+?9?0?5\\d{2}[\\s.-]?\\d{3}[\\s.-]?\\d{2}[\\s.-]?\\d{2})";
    const direct = await request.post(`${API}/messages/admin/filters/test`, {
      headers: authHeader(adminToken),
      data: {
        pattern: phonePattern,
        testContent: "Beni ara 0532 123 45 67 lütfen",
      },
    });
    ...
    const spaced = await request.post(`${API}/messages/admin/filters/test`, {
      ...
      data: {
        pattern: phonePattern,
        testContent: "numaram 0 5 3 2 - 1 2 3 . 4 5 . 6 7 yazdim".replace(
          / /g,
          "",
        ),
      },
    ...
    const pending = await request.get(`${API}/messages/admin/pending`, {
      headers: authHeader(adminToken),
    });
    expect(pending.ok(), "admin bekleyen mesajlar").toBeTruthy();

    // 4) Normal kullanıcı bu ekrana girmeye çalışır → engellenir
    const userPending = await request.get(`${API}/messages/admin/pending`, {
      headers: authHeader(userToken),
    });
    expect(
      userPending.ok(),
      "normal kullanıcı bekleyen mesajları görememeli",
    ).toBeFalsy();
    expect([401, 403]).toContain(userPending.status());

    // 5) Yönetici filtre listesini görür (gerekli işlem zemini)
    const filters = await request.get(`${API}/messages/admin/filters`, {
      headers: authHeader(adminToken),
    });
...
test.describe("J125 — Sistem sağlığı kontrolleri", () => {
  test("health/detailed/live/ready başarılı döner, test DB erişilebilir", async ({
    request,
  }) => {
...
test.describe("J136 — Admin günü tam tur", () => {
  test("admin bekleyenleri inceler, onay+red, şikayetleri görür, kullanıcı banlar+kaldırır, raporlar", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    ...
    const approveId = await createPendingProduct(
      request,
      sellerToken,
      categoryId,
      "J136-onay",
    );
    const rejectId = await createPendingProduct(
      request,
      sellerToken,
      categoryId,
      "J136-red",
    );
    const queue = await request.get(`${API}/admin/products`, {
      headers: authHeader(adminToken),
      params: { status: "pending", limit: "50" },
    });
    ...
    const ap = await request.post(
      `${API}/admin/products/${approveId}/approve`,
      { headers: authHeader(adminToken), data: { note: "Uygun" } },
    );
    expect(ap.ok(), "onay").toBeTruthy();
    const rj = await request.post(`${API}/admin/products/${rejectId}/reject`, {
      headers: authHeader(adminToken),
      data: { reason: "Telif ihlali şüphesi" },
    });
    expect(rj.ok(), "red").toBeTruthy();
    expect(
      (await dbFind(request, "product", { id: approveId }, { status: true }))
        ?.status,
    ).toBe("active");
    expect(
      (await dbFind(request, "product", { id: rejectId }, { status: true }))
        ?.status,
    ).toBe("rejected");

    // 3) Gelen kullanıcı şikayetlerini inceler
    const reports = await request.get(`${API}/user-reports/admin`, {
      headers: authHeader(adminToken),
    });
    ...
    const target = await registerFreshUser(request, {
      displayName: "Ihlal Eden",
    });
    const ban = await request.post(`${API}/admin/users/${target.id}/ban`, {
      headers: authHeader(adminToken),
      data: { reason: "Kural ihlali" },
    });
    ...
    const unban = await request.post(`${API}/admin/users/${target.id}/unban`, {
      headers: authHeader(adminToken),
    });
