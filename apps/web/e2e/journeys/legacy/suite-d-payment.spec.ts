...
test.describe("J11 — Ödeme süresi doluyor, kullanıcı geri dönüp ödüyor", () => {
  test("timeout payment iptal, sipariş yaşar, yeniden ödeme tamamlanır", async ({
    request,
  }) => {
    ...
    expect(order1.status, "sipariş hâlâ ödeme bekliyor").toBe(
      "pending_payment",
    );

    // 3) Kullanıcı geri döndü, yeniden initiate + bu kez bypass ile öder
    const reInit = await request.post(`${API}/payments/initiate`, {
      headers: auth(buyerToken),
      data: { orderId, provider: "paytr" },
    });
    ...
    const done = await request.post(
      `${API}/payments/${newPaymentId}/bypass-complete`,
      { data: {} },
    );
    ...
    expect(["paid", "preparing"], "yeniden ödeme sonrası").toContain(
      order2.status,
    );
    const newPay = await dbFind(
      request,
      "payment",
      { id: newPaymentId },
      { status: true },
    ...
    const ownerSellerToken =
      order2.sellerId === sellerMe.id
        ? sellerToken
        : await tokenForSeller(request, order2.sellerId);
...
async function tokenForSeller(
  request: APIRequestContext,
  sellerId: string,
): Promise<string> {
  for (const u of [
    USERS.sellerPremium,
    USERS.sellerBusiness,
    USERS.sellerFree,
    USERS.buyer,
  ]) {
...
test.describe("J25 — Misafir üye olmadan alışveriş yapıyor", () => {
  test("misafir OTP doğrular, sipariş oluşur, ödenir", async ({ request }) => {
    ...
    const sendCode = await request.post(
      `${API}/orders/guest/send-verification-code`,
    ...
    const initRes = await request.post(`${API}/payments/initiate-guest`, {
      data: { orderId, provider: "paytr" },
    });
...
test.describe("J63 — Satıcı ödemeden hazırlamaya çalışıyor", () => {
  test("ödenmemiş sipariş prepare reddedilir, ödeme sonrası akış ilerler", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    const buyerToken = await apiLogin(request, USERS.buyerClean);
    ...
    const earlyPrep = await request.post(`${API}/orders/${orderId}/prepare`, {
      headers: auth(sellerToken),
    });
...
test.describe("J64 — Alıcı olmayan teslimatı onaylayamıyor", () => {
  test("üçüncü kişi confirm reddedilir, gerçek alıcı onaylar", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    const buyerToken = await apiLogin(request, USERS.buyerClean);
    ...
    const sellerToken = await tokenForSeller(request, dbOrder.sellerId);
    await request.post(`${API}/orders/${orderId}/prepare`, {
      headers: auth(sellerToken),
    });
    await backdate(
      request,
    ...
    const idor = await request.post(`${API}/orders/${orderId}/confirm`, {
    ...
    const sellerConfirm = await request.post(
    ...
    const ok = await request.post(`${API}/orders/${orderId}/confirm`, {
      headers: auth(buyerToken),
    });
...
test.describe("J69 — Ödeme iptali ve rezervasyon serbest kalması", () => {
  test("alıcı pending ödemeyi iptal eder, başka alıcı aynı ürünü alır", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    const buyerToken = await apiLogin(request, USERS.buyerClean);
    ...
    expect(
      (await dbFind(request, "payment", { id: paymentId }, { status: true }))
        .status,
    ).toBe("pending");

    // 2) Bekleyen ödemeyi iptal et -> payment failed + sipariş iptal (rezervasyon serbest)
    const cancel = await request.post(`${API}/payments/${paymentId}/cancel`, {
      headers: auth(buyerToken),
    });
    ...
    const cancelledOrder = await dbFind(
      request,
      "order",
      { id: orderId },
      { status: true },
    );
    expect(cancelledOrder.status, "iptal sonrası sipariş").toMatch(
    ...
    const { orderId: order2Id } = await apiBuyAndPay(
      request,
      buyer2,
      product.id,
    );
    const order2 = await dbFind(
      request,
      "order",
      { id: order2Id },
      { status: true },
    );
    expect(["paid", "preparing"], "ikinci alıcı ödeyebildi").toContain(
      order2.status,
    );

    // 4) İkinci alıcı teslim alır, onaylar -> completed
    const seller = await tokenForSeller(
      request,
      (await dbFind(request, "order", { id: order2Id }, { sellerId: true }))
        .sellerId,
    );
    await driveToCompleted(request, buyer2, seller, order2Id);
    expect(
      (await dbFind(request, "order", { id: order2Id }, { status: true }))
        .status,
    ).toBe("completed");
  });
});
...
test.describe("J70 — Başkasının ödemesini iptal etme engeli", () => {
  test("B, A nın ödemesini iptal edemez; A tamamlanmış ödemeyi iptal edemez", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    ...
    const idor = await request.post(`${API}/payments/${paymentId}/cancel`, {
      headers: auth(tokenB),
    });
...
test.describe("J71 — Başarısız ödeme onayı ile rezervasyon iadesi", () => {
  test("confirm-failed rezervasyonu serbest bırakır, başka alıcı ürünü alır", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    const buyerToken = await apiLogin(request, USERS.buyerClean);
    ...
    const failRes = await request.post(
      `${API}/payments/${paymentId}/confirm-failed`,
      { data: {} },
    );
    expect(failRes.ok(), "confirm-failed").toBeTruthy();
    expect(
      (await failRes.json())?.released,
    ...
    const failAgain = await request.post(
      `${API}/payments/${paymentId}/confirm-failed`,
      { data: {} },
    );
    ...
    expect(["paid", "preparing"]).toContain(
      (await dbFind(request, "order", { id: order2Id }, { status: true }))
        .status,
    );
  });
...
test.describe("J74 — Test ortamında ödeme bypass akışı", () => {
  test("bypass iki kez çağrılır ama durum bozulmaz, tek payment kalır", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    const buyerToken = await apiLogin(request, USERS.buyerClean);
    ...
    const orderAfter1 = await dbFind(
      request,
      "order",
      { id: orderId },
      { status: true },
    ...
    expect(
      await dbCount(request, "payment", { orderId }),
      "sipariş için tek payment",
    ).toBe(1);
    expect(
      await dbCount(request, "paymentHold", { orderId }),
      "sipariş için tek hold",
    ).toBe(1);
  });
});
...
test.describe("J75 — Para akışı: ödeme tutuldu, süre sonunda serbest", () => {
  test("ödeme hold da tutulur, süre dolmadan serbest değil, dolunca released", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    const buyerToken = await apiLogin(request, USERS.buyerClean);
    ...
    const { orderId, paymentId } = await apiBuyAndPay(
      request,
      buyerToken,
      product.id,
    );
    const hold = await dbFind(
      ...
      "paymentHold",
      { paymentId },
      { id: true, status: true, releaseAt: true, amount: true },
    );
    ...
    const sellerToken = await tokenForSeller(
      request,
      (await dbFind(request, "order", { id: orderId }, { sellerId: true }))
        .sellerId,
    );
    await driveToCompleted(request, buyerToken, sellerToken, orderId);
    ...
    expect(
      (await dbFind(request, "paymentHold", { paymentId }, { status: true }))
        .status,
      "süre dolmadan held kalır",
    ).toBe("held");
...
test.describe("J28 — Tekrarlı ödeme bildirimi: sistem bir kez işliyor", () => {
  test("aynı success callback 3x gelir, sipariş tam bir kez kesinleşir", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    const buyerToken = await apiLogin(request, USERS.buyerClean);
    ...
    const { orderId, paymentId, amount } = await buyAndInitiate(
      request,
      buyerToken,
      product.id,
    );
    ...
    for (let i = 0; i < 3; i++) {
      const res = await request.post(`${API}/payments/callback/paytr`, {
        form: cb,
      });
    ...
    expect(await dbCount(request, "payment", { orderId }), "tek payment").toBe(
      1,
    );
    expect(
      await dbCount(request, "paymentHold", { orderId }),
      "tek hold (çift tahsilat yok)",
    ).toBe(1);
  });
});
...
test.describe("J29 — Sahte ödeme bildirimi reddediliyor", () => {
  test("imzası bozuk callback sipariş durumunu değiştirmez, gerçek ödeme ilerler", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    const buyerToken = await apiLogin(request, USERS.buyerClean);
    ...
    const forged = {
      ...
      status: "success",
      total_amount: String(totalKurus),
      hash: "sahteimzaBOZUK==",
    };
    const res = await request.post(`${API}/payments/callback/paytr`, {
      form: forged,
...
test.describe("J88 — Webhook güvenliği: yanlış anahtar reddediliyor", () => {
  test("anahtarsız ve yanlış anahtarlı bildirim reddedilir, doğru anahtarla ilerler", async ({
    request,
  }) => {
    test.setTimeout(60_000);
    const buyerToken = await apiLogin(request, USERS.buyerClean);
    ...
    const r1 = await request.post(`${API}/payments/callback/paytr`, {
      form: noHash,
    });
    ...
    const wrongKey = signPaytrCallback(
      orderId,
      "success",
      totalKurus,
      "YANLIS-KEY",
    ...
    const r2 = await request.post(`${API}/payments/callback/paytr`, {
      form: wrongKey,
    });
    ...
    const validCb = signPaytrCallback(
      orderId,
      "success",
      totalKurus,
      PAYTR_KEY,
    ...
    const r3 = await request.post(`${API}/payments/callback/paytr`, {
      form: validCb,
    });
