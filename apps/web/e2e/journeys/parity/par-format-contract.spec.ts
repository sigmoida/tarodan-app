 ...
 *
 * Web `apps/web/src/lib/format.ts` helper'ını DOĞRUDAN import edip çıktısını
 * doğrular. Harici mobil istemcinin format paritesi kendi repository'sinde
 * API kontratına karşı doğrulanır.
 *
...
test.describe("PAR-040 [P1] — web sipariş statü etiketleri", () => {
  ...
  test("PAR-040 [P1] — awaiting_buyer_confirmation web'de map'siz → title-case (mobile \"Alıcı Onayı Bekleniyor\")", () => {
    expect(formatOrderStatus("awaiting_buyer_confirmation")).toBe(
      "Awaiting buyer confirmation",
    );
    expect(formatOrderStatus("awaiting_buyer_confirmation")).not.toBe(
...
test.describe("PAR-041 [P0] — web takas escrow statüleri Türkçe (mobile çiğ → R-PAR-1)", () => {
  test("PAR-041 [P0] — at_warehouse/shipping_to_warehouse/awaiting_payment web'de TR map'li", () => {
    ...
    expect(formatTradeStatus("shipping_to_warehouse")).toBe(
      "Depoya Gönderiliyor",
    );
