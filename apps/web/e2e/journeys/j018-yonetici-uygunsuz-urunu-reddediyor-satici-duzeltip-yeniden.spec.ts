...
import { dbFind, dbCount, backdate } from '../support/db';
...
test.describe('J18 — Ürün moderasyon red/düzelt/onay/satış', () => {
  test('satıcı ilan açar, admin reddeder, satıcı düzeltir, admin onaylar, alıcı satın alır', async ({ request }) => {
    ...
    if (!approveRes.ok()) {
      const _ps = (await dbFind(request, 'product', { id: productId }, { status: true }))?.status;
      expect(approveRes.ok(), `admin onayladı (${approveRes.status()}) prodStatus=${_ps} ${(await approveRes.text()).slice(0, 90)}`).toBeTruthy();
    }
    p = await dbFind(request, 'product', { id: productId }, { status: true });
