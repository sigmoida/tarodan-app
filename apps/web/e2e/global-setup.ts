...
export default async function globalSetup(): Promise<void> {
  ...
  if (process.env.E2E_SKIP_SETUP === "1") {
    console.log(
      "[e2e] E2E_SKIP_SETUP=1 — global setup atlandı (runner yönetiyor)",
    );
    return;
  }
  ...
  try {
    ...
    console.warn(
      "[e2e] createdb tarodan_test atlandı (CI servis DB veya zaten var):",
      (e as Error).message,
    );
  }
  ...
  // 3) Seed (SEED_SKIP_IMAGES=1 → görsel adımları atlanır, S3 gerekmez).
  //    Seed hatası ARTIK YUTULMAZ: yarım seed'le koşan e2e, sonraki bölümlerin
  //    (order/trade/refund...) yokluğunu alakasız test hatası olarak gösteriyordu.
  ...
  execSync("pnpm exec ts-node prisma/e2e-prepare.ts", {
    cwd: apiDir,
    env,
    stdio: "inherit",
  });
