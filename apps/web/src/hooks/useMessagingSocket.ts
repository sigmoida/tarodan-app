  ...
  useEffect(() => {
    ...
    const onMessageNew = (payload: MessageNewEvent) => {
      ...
      if (payload.threadId === activeRef.current) {
        queryClient.setQueryData<any[]>(
          ...
          (old) => mergeMessages(old ?? [], payload.message as any),
        );
        ...
        if (
          ...
          queryClient.invalidateQueries({
            queryKey: queryKeys.messages.thread(payload.threadId),
          });
        }
      }
    ...
    const onTypingStart = (t: TypingEvent) => {
      if (t.threadId === activeRef.current && t.userId !== currentUserId) {
        setTypingUserIds((ids) =>
          ids.includes(t.userId) ? ids : [...ids, t.userId],
        );
      }
    };
    ...
    const onReconnect = () => {
      if (activeRef.current) {
        ...
        queryClient.invalidateQueries({
          queryKey: queryKeys.messages.thread(activeRef.current),
        });
      }
      queryClient.invalidateQueries({ queryKey: queryKeys.messages.threads() });
    };
