    ...
    if (!brand) {
        return (
            <div className="min-h-screen bg-surface flex flex-col items-center justify-center p-4 text-center">
                ...
                <h2 className="text-3xl font-bold text-heading mb-2">Marka Bulunamadı</h2>
                <Link href="/brands" className="mt-8 px-8 py-3 bg-heading text-inverted rounded-full font-bold hover:bg-primary-600 transition-all shadow-lg hover:shadow-primary-500/25">
                    Tüm Markalara Dön
                </Link>
            </div>
        );
    ...
    return (
        <div className="min-h-screen bg-surface pb-24 pt-8">
            <div className="container mx-auto px-4 max-w-7xl">
                ...
                <Link href="/brands" className="inline-flex items-center gap-2 text-subtle hover:text-primary-600 font-medium mb-8 transition-colors group">
                ...
                {models.length > 0 && (
                    <div className="mb-12">
                        <div className="flex items-center justify-between mb-6 px-2">
                            <h3 className="text-xl font-bold text-heading">Modeller</h3>
                            {selectedModelId && (
                        ...
                        <div className="flex flex-wrap gap-3">
                            <Button variant="secondary" onClick={() => setSelectedModelId(null)}
                                className={`px-5 py-2.5 rounded-full font-bold text-sm transition-all duration-300 ${selectedModelId === null
                                    ? 'bg-heading text-inverted shadow-lg shadow-heading/20'
                                    : 'bg-surface-elevated text-muted hover:bg-surface-alt border border-border-subtle'
                                    }`}>
                                Tümü
                            </Button>
                            ...
                            {models.map((model) => (
                                <Button variant="secondary" key={model.id}
                                    ...
                                    className={`px-5 py-2.5 rounded-full font-bold text-sm transition-all duration-300 ${selectedModelId === model.id
                                        ? 'bg-primary-500 text-inverted shadow-lg shadow-primary-500/30'
                                        : 'bg-surface-elevated text-muted hover:bg-primary-50 hover:text-primary-600 border border-border-subtle'
                                        }`}>
                                    {model.name}
                                </Button>
                            ))}
