...
export function buildListingFormData(
  ...
  const materialFromAttrs = (listing as any).attributes?.find(
    (a: any) =>
      a.label === "Malzeme" || a.group === "Malzeme" || a.group === "material",
  )?.name;
  ...
  const newFormData: EditListingFormData = {
    ...
    categoryId:
      listing.categoryId || listing.category?.id || prev.categoryId || "",
    ...
    quantity:
      listing.quantity !== undefined && listing.quantity !== null
        ...
        : (prev.quantity ?? ""),
    shippingPackageTier:
      (listing as any).shippingPackageTier ??
      prev.shippingPackageTier ??
      "small",
    images:
  ...
  const previewUrls =
    listing.images?.map(
      (img: any) => img.cardUrl ?? img.detailUrl ?? img.url ?? img,
    ) || [];

  return { newFormData, previewUrls };
}

export function buildSaleDataFromListing(listing: any): {
  saleData: SaleData;
  saleActive: boolean;
} {
  ...
  const orig =
    (listing as any).oldPrice != null
      ? Number((listing as any).oldPrice)
      : listing.originalPrice != null
        ? Number(listing.originalPrice)
        : null;
  const onSale =
    ((listing as any).oldPrice != null && listing.price != null) ||
    (listing as any).isOnSale === true;
  const sale = onSale
    ? Number(listing.price)
    : listing.salePrice != null
      ? Number(listing.salePrice)
      : null;
  const start = listing.saleStartDate
    ? typeof listing.saleStartDate === "string"
      ? listing.saleStartDate.split("T")[0]
      : new Date(listing.saleStartDate).toISOString().split("T")[0]
    : new Date().toISOString().split("T")[0];
  const end = listing.saleEndDate
    ? typeof listing.saleEndDate === "string"
      ? listing.saleEndDate.split("T")[0]
      : new Date(listing.saleEndDate).toISOString().split("T")[0]
