...
export type SortOption =
  "popular" | "recent" | "name" | "items_asc" | "items_desc";
...
export function unwrapPublicCollections(raw: any): PublicCollectionsResult {
  ...
  const total =
    raw?.total ?? (Array.isArray(collections) ? collections.length : 0);
...
export async function fetchPublicCollectionsServer(
  ...
  try {
    const qs = new URLSearchParams(
      buildBrowseParams(sortBy, search, categoryId),
    ).toString();
...
export async function fetchPublicCollectionsClient(
  ...
  const response = await collectionsApi.browse(
    buildBrowseParams(sortBy, search, categoryId),
  );
