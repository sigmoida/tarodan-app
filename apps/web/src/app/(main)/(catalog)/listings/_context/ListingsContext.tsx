...
function useListingsValue(): ListingsContextValue {
  ...
  const { data: listingsData, isLoading } = useQuery({
    ...
    queryFn: () => fetchListingsClient(filters, resolvedCategoryId, currentPage),
    meta: { page: 'listings' },
  });

  const listings: Listing[] = (listingsData?.listings ?? []) as Listing[];
  const pagination: Pagination = (listingsData?.meta as Pagination) ?? { total: 0, page: currentPage, limit: pageLimit, totalPages: 1 };
