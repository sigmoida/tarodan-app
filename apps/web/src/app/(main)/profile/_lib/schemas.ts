...
export const addressSchema = z.object({
	...
	phone: z
		.string()
		.refine((v) => v.replace(/\D/g, '').length >= 12, 'Geçerli bir telefon giriniz'),
...
export const bankAccountSchema = z.object({
	accountHolder: z.string().trim().min(2, 'Hesap sahibi en az 2 karakter olmalı'),
	iban: z.string().refine(isValidTrIban, 'Geçerli bir TR IBAN giriniz (TR + 24 rakam)'),
	tcKimlikNo: z
		.string()
		.optional()
		.or(z.literal(''))
