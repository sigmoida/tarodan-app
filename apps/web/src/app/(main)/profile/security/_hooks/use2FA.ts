...
export function use2FA() {
  ...
  const statusQuery = useQuery({
    queryKey: STATUS_KEY,
    queryFn: async (): Promise<TwoFactorStatus> => (await api.get('/security/2fa/status')).data,
  });

  const invalidateStatus = () => queryClient.invalidateQueries({ queryKey: STATUS_KEY });

  const enable = useMutation({
    mutationFn: async (): Promise<SetupResponse> => (await api.post('/security/2fa/enable')).data,
    onMutate: () => setError(''),
    onSuccess: (data) => setSetupData(data),
    onError: (e: any) => setError(e?.response?.data?.message || '2FA kurulumu başlatılamadı'),
  });

  const verify = useMutation({
    mutationFn: async (code: string) => (await api.post('/security/2fa/verify', { code })).data,
    onMutate: () => setError(''),
    onSuccess: (data) => {
    ...
    onError: (e: any) => setError(e?.response?.data?.message || 'Doğrulama başarısız'),
  });

  const disable = useMutation({
    mutationFn: async (code: string) => (await api.post('/security/2fa/disable', { code })).data,
    onMutate: () => setError(''),
    onSuccess: () => invalidateStatus(),
    onError: (e: any) => setError(e?.response?.data?.message || '2FA devre dışı bırakılamadı'),
  });

  const regenerateBackupCodes = useMutation({
    mutationFn: async (code: string) => (await api.post('/security/2fa/backup-codes', { code })).data,
    onMutate: () => setError(''),
    onSuccess: (data) => {
    ...
    onError: (e: any) => setError(e?.response?.data?.message || 'Yedek kodlar oluşturulamadı'),
  });
