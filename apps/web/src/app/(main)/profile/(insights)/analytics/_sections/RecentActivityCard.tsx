...
export default function RecentActivityCard({ activity }: { activity: Activity[] }) {
	return (
		<SectionCard title='Son Aktiviteler' badge={<FireIcon className='h-5 w-5 text-primary-500' />}>
						...
						<div
							...
							<div className='min-w-0 flex-1'>
								<p className='line-clamp-2 text-sm text-heading'>
									<span className='font-medium'>{item.productTitle}</span> {cfg.text}
									{item.amount ? (
										<span className='font-semibold text-success-600'> {formatTL(item.amount)}</span>
									) : null}
								</p>
								{item.userDisplayName && (
									<p className='text-xs text-muted'>{item.userDisplayName}</p>
								)}
								<p className='mt-0.5 text-xs text-subtle'>{formatTimeAgo(item.timestamp)}</p>
							</div>
						</div>
					);
				})}
