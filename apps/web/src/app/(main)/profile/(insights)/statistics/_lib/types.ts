...
export function aggregateStats(
	...
	const totalFavorites = products.reduce((sum, p) => sum + (p.likeCount || 0), 0);
	const totalRevenue = soldProducts.reduce((sum, p) => sum + (parseFloat(p.price) || 0), 0);
...
export function membershipDuration(memberSince: string): { days: number; months: number } {
