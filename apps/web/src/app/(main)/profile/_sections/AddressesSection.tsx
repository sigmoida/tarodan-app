...
export default function AddressesSection() {
	...
	return (
		<SectionCard
			title='Adreslerim'
			badge={<Badge variant='secondary' size='sm'>{addresses.length}/{MAX_ADDRESSES}</Badge>}
			action={
						...
						<div
							...
							<div className='min-w-0'>
								<div className='mb-1 flex items-center gap-2'>
									{address.title && (
										<span className='font-semibold text-heading'>{address.title}</span>
									)}
									{address.isDefault && (
								...
								<p className='text-sm font-medium text-heading'>{address.fullName}</p>
