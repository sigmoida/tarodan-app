...
import { useProfileInfo, useUpdateProfile, useUploadAvatar } from '../_hooks/useProfileInfo';
...
export default function ProfileInfoSection() {
	...
	const isBusiness = (profile?.membershipTier ?? user?.membershipTier) === 'business';
	...
	return (
		<SectionCard
			...
			<div className='mb-5 flex items-center gap-4'>
				...
				<div>
					<p className='font-medium text-heading'>{profile?.displayName || '—'}</p>
			...
			{isLoading ? (
				...
				<Form form={form} onSubmit={onSubmit} className='space-y-4'>
					<FormInput name='displayName' label='Görünen İsim' placeholder='Adınız' />
					<div className='grid gap-4 md:grid-cols-2'>
						<FormInput name='phone' label='Telefon' placeholder='+90 555 123 4567' />
					...
					{isBusiness && (
						<div className='space-y-4 rounded-lg border border-border-subtle bg-surface p-4'>
							<p className='text-sm font-semibold text-heading'>İşletme Bilgileri</p>
							<FormInput name='companyName' label='Şirket / Ticari Unvan' placeholder='ABC Ltd. Şti.' />
							<div className='grid gap-4 md:grid-cols-2'>
								<FormInput name='taxId' label='Vergi Kimlik No' placeholder='1234567890' />
								<FormInput name='taxOffice' label='Vergi Dairesi' placeholder='Kadıköy VD' />
							</div>
						</div>
					)}
				</Form>
