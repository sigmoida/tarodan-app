...
import { changePasswordSchema, type ChangePasswordValues } from '../_lib/schemas';
import {
	useChangePassword,
			...
			{step === 'enter' ? (
				<div className='space-y-4'>
					...
					<Button
						...
						onClick={() => sendCode.mutate(phone, { onSuccess: () => setStep('verify') })}>
						Kod Gönder
					</Button>
				</div>
			) : (
		...
		<SectionCard
						...
						return (
							<span
								...
								className={`rounded-full px-2 py-0.5 text-xs ${
									met ? 'bg-success-100 text-success-700' : 'bg-surface-alt text-subtle'
								}`}>
								{met ? '✓' : '○'} {r.label}
							</span>
						);
			...
			<div className='mt-6 flex items-center justify-between border-t border-border-subtle pt-5'>
				<div className='flex items-center gap-3'>
					...
					<div>
						<p className='text-sm font-medium text-heading'>Telefon Doğrulama</p>
			...
			<div className='mt-5 flex items-center justify-between border-t border-border-subtle pt-5'>
				<div className='flex items-center gap-3'>
					...
					<div>
						<p className='text-sm font-medium text-heading'>İki Adımlı Doğrulama (2FA)</p>
						<p className='text-xs text-muted'>
							{is2faEnabled
