		...
		<div className='flex items-center gap-4 rounded-lg border border-border bg-surface-elevated p-4'>
			<Link
				...
				<div className='flex-1'>
					<h3 className='font-semibold text-heading'>{item.following.displayName}</h3>
					{item.following.bio && (
						<p className='line-clamp-1 text-sm text-muted'>{item.following.bio}</p>
					)}
					<p className='mt-1 text-sm text-subtle'>{item.following._count?.products || 0} ilan</p>
				</div>
			</Link>
			<Button
				variant='outline'
