...
const TONE: Record<Tone, { wrap: string; icon: string; title: string; body: string }> = {
	success: {
	...
	if (refund.status === 'refunded') {
		return (
			<Callout
				...
				title={
					locale === 'en' ? 'Refund completed successfully' : 'İadeniz başarıyla tamamlandı'
				}>
				<p>
					{locale === 'en'
				...
				{refund.refundedAt && (
					<p className='mt-2 text-xs'>
						...
						{new Date(refund.refundedAt).toLocaleString(locale === 'en' ? 'en-US' : 'tr-TR')}
					</p>
				)}
			</Callout>
		);
	...
	if (refund.status === 'rejected') {
		return (
			<Callout
				...
				title={locale === 'en' ? 'Request rejected' : 'Talep reddedildi'}>
				<p>
					{locale === 'en'
	...
	if (refund.status === 'disputed') {
		return (
			<Callout
				...
				title={locale === 'en' ? 'Under admin review' : 'Admin İncelemesinde'}>
				<p>
					{locale === 'en'
