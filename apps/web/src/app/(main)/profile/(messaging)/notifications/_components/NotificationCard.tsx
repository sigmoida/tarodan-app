...
export default function NotificationCard({
	...
	const { isRead, icon, data, title, message, createdAt, link } = notification;
	...
	return (
		<SectionCard
			...
			<div className='flex items-start gap-3'>
				<div className='flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-surface-alt text-xl'>
					{icon || data?.icon || '🔔'}
				</div>
						...
						<div className='flex flex-shrink-0 items-center gap-1'>
							...
							{!isRead && (
								<IconButton
									...
									aria-label={
										locale === 'en' ? 'Mark as read' : 'Okundu işaretle'
									}>
