	...
	return (
		<PageShell className='pb-16'>
			...
			<div className='grid grid-cols-2 gap-4 md:grid-cols-4'>
				<MetricCard icon={ReceiptPercentIcon} label='Toplam İndirim' value={metrics.total} accent='text-heading' />
				<MetricCard icon={CheckCircleIcon} label='Aktif' value={metrics.active} accent='text-success-600' />
				<MetricCard icon={ClockIcon} label='Süresi Dolmuş' value={metrics.expired} accent='text-danger-600' />
				<MetricCard icon={FireIcon} label='Toplam Kullanım' value={metrics.usage} accent='text-primary-600' />
			</div>

			<Tabs value={filter} onValueChange={(v) => setFilter(v as DiscountFilter)}>
			...
			{isLoading ? (
				...
				<div className='rounded-lg border border-border bg-surface-elevated p-12 text-center'>
					...
					<h3 className='mb-2 text-xl font-semibold text-heading'>
						{filter !== 'all' ? 'Bu filtreye uygun indirim yok' : 'Henüz indirim oluşturmadınız'}
					</h3>
