...
/** The order itself — queryKey ['order', orderId]. */
...
export function useElogoInvoice(orderId: string, order: OrderDetail | null | undefined) {
	return useQuery({
...
export function useSellerInvoice(orderId: string, order: OrderDetail | null | undefined) {
	return useQuery({
		...
		onError: (error: any) => {
			...
			toast.error(
				error?.response?.data?.message ||
					(locale === 'en' ? 'Failed to update status' : 'Durum güncellenemedi'),
			);
		},
	});
}
...
export function useReactivateOrder(orderId: string) {
	...
	return useMutation({
		...
		onError: (err: any) => {
			toast.error(
				err?.response?.data?.message ||
					(locale === 'en' ? 'Could not reactivate order' : 'Sipariş aktive edilemedi'),
			);
		},
	});
}
...
export function useSetAddressAndPay(orderId: string) {
	...
	const initiatePayment = async (order: OrderDetail): Promise<void> => {
		try {
			...
			toast.error(
				err.response?.data?.message ||
					(locale === 'en' ? 'Payment initiation failed' : 'Ödeme başlatılamadı'),
			);
		}
	};
	...
	return useMutation({
		mutationFn: async (input: CheckoutInput) => {
			const { order, showNewAddressForm, newAddress, selectedAddressId, savedAddresses } =
				input;
