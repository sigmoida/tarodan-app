...
export default function ReviewModal({ order, onClose }: ReviewModalProps) {
  ...
  const locale = useLocale();
	...
	const primary = order ? getOrderPrimary(order) : { product: undefined, image: undefined };
				...
				{primary.product && (
					<div className='mb-4 flex items-center gap-3 rounded-lg bg-surface p-3'>
						<div className='relative h-12 w-12 flex-shrink-0 overflow-hidden rounded bg-surface-alt'>
							<OptimizedImage
								src={primary.image || 'https://placehold.co/96x96/f3f4f6/9ca3af?text=%F0%9F%9A%97'}
						...
						<p className='font-medium text-heading'>{primary.product.title}</p>
					</div>
				)}
