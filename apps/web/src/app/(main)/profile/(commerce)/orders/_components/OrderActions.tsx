...
export default function OrderActions({
	...
	const isTerminal = ['delivered', 'completed', 'cancelled', 'refunded'].includes(
		order.status,
	);
	...
	const show = {
		...
		invoiceBuyer:
			isBuyer &&
			['paid', 'preparing', 'shipped', 'delivered', 'completed'].includes(order.status),
		reorder: isBuyer && hasProduct && isTerminal,
		refundLink: !!refundId,
		cancel:
			!refundId && isBuyer && ['paid', 'preparing'].includes(order.status) && !shipped,
		requestRefund:
			!refundId &&
			isBuyer &&
			shipped &&
		...
		sellerShip:
			isSeller && ['paid', 'preparing'].includes(order.status) && !order.shipment,
	...
	return (
		<div className='mt-4 flex flex-wrap gap-2'>
			<ButtonLink href={`/profile/orders/${order.id}`} variant='secondary' size='sm'>
				{t('common.details')}
			</ButtonLink>
