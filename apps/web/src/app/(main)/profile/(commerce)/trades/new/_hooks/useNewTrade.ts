...
export function useTradeTarget(listingId: string | null, enabled: boolean) {
	const query = useQuery({
		queryKey: ['trade-target', listingId],
		queryFn: async (): Promise<TradeProduct> => {
			const res = await listingsApi.getOne(listingId!);
			return res.data.product || res.data;
		},
		enabled: enabled && !!listingId,
		meta: { page: 'trades-new-target' },
	});
	return { target: query.data ?? null, isLoading: query.isLoading };
}

/** The user's active, trade-enabled products (excluding the target). */
export function useTradeableProducts(listingId: string | null, enabled: boolean) {
	const query = useQuery({
		queryKey: ['trade-eligible-products', listingId],
		queryFn: async (): Promise<TradeProduct[]> => {
			const res = await userApi.getMyProducts({ status: 'active', tradeEligible: true });
	...
	return useMutation({
		...
		onError: async (error: any) => {
			const msg =
				...
				error?.message ||
				(locale === 'en' ? 'Failed to send trade offer' : 'Takas teklifi gönderilemedi');
			if (
