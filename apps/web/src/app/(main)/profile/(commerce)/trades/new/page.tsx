...
function NewTradeContent() {
	...
	const { target, isLoading: targetLoading } = useTradeTarget(listingId, enabled);
	...
	const submit = () => {
		if (selectedIds.length === 0 && !cash) return toast.error(t('trade.selectAtLeastOne'));
		if (!target) return toast.error(t('trade.targetNotFound'));
		if (!addressId) return toast.error('Lütfen bir teslimat adresi seçin veya ekleyin');
		...
		<PageShell className='pb-16'>
			...
			<div className='flex flex-col items-stretch gap-6 lg:flex-row'>
				<SectionCard title='İstediğiniz Ürün' className='flex-1'>
					<div className='flex flex-col items-center gap-4'>
						...
						<div className='w-full text-center'>
							<h3 className='mb-2 line-clamp-2 font-medium text-heading'>{target.title}</h3>
				...
				<SectionCard title='Teklif Edeceğiniz Ürünler' className='flex-1'>
					<TradeProductPicker products={products} selectedIds={selectedIds} onToggle={toggle} />
				</SectionCard>
			</div>

			<SectionCard title={`${t('trade.cashDifference')} (${t('common.optional')})`}>
				...
				<div className='space-y-4'>
					...
					{cash > 0 && (
						<div className='space-y-2'>
							<p className='text-sm font-medium text-body'>Nakit farkı kim ödeyecek?</p>
			...
			<SectionCard title={`${t('trade.message')} (${t('common.optional')})`}>
				<Textarea
					...
					maxLength={500}
				/>
				<p className='mt-2 text-right text-sm text-muted'>{message.length}/500</p>
			</SectionCard>
