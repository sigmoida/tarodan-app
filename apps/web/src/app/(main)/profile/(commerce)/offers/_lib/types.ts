...
const PAID_ORDER_STATUSES = ['paid', 'preparing', 'shipped', 'delivered', 'completed'];

export const isOfferOrderPaid = (offer: Offer): boolean =>
	offer.status === 'accepted' && PAID_ORDER_STATUSES.includes(offer.orderStatus ?? '');

export const getOfferImage = (offer: Offer): string | undefined => {
	const img0 = offer.product.images?.[0];
	return img0?.cardUrl ?? img0?.detailUrl ?? img0?.url ?? offer.product.imageUrl;
};

export const calculateDiscount = (offerAmount: number, listingPrice: number): number =>
	listingPrice > 0 ? Math.round(((listingPrice - offerAmount) / listingPrice) * 100) : 0;
...
export function formatTimeAgo(timestamp: string, locale: string = 'tr'): string {
	...
	if (days < 30)
		return locale === 'en' ? `${days} day${days === 1 ? '' : 's'} ago` : `${days} gün önce`;
...
export const offerStatusLabel = (status: string, locale: string, trFallback: string): string =>
	locale === 'en' ? OFFER_STATUS_EN[status] || status : trFallback;
