...
export default function CounterOfferModal({ open, onClose, offer, mode }: CounterOfferModalProps) {
	...
	const submit = () => {
		...
		if (mode === 'buyer' && value >= base) {
			toast.error(`Satıcının karşı teklifinden (${baseLabel}) düşük olmalıdır.`);
			return;
		}
		if (mode === 'seller' && value <= base) {
			toast.error(`Karşı teklifiniz alıcının teklifinden (${baseLabel}) yüksek olmalıdır.`);
			return;
		}
		counter.mutate({ offerId: offer.id, amount: value, mode }, { onSuccess: onClose });
	};
