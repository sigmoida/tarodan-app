...
export default function OfferCard({
	...
	const statusLabel = paid
		? locale === 'en'
			? 'Paid'
			: 'Ödendi'
		: offerStatusLabel(offer.status, locale, offerStatusConfig[offer.status]?.label || offer.status);
		...
		if (offer.status === 'accepted') {
			...
			return (
				<Button asChild variant={showPay ? 'success' : 'primary'} size='sm'>
					<Link href={offer.orderId ? `/profile/orders/${offer.orderId}` : '/profile/orders'}>
		...
		return (
			<DropdownMenu>
				<DropdownMenuTrigger asChild>
					<IconButton aria-label='İşlemler' variant='ghost' size='sm' isLoading={busy}>
				...
				<DropdownMenuContent align='end'>
					{isReceived ? (
						<>
							...
							<DropdownMenuSeparator />
							<DropdownMenuItem danger onSelect={() => onAction(offer.id, 'reject')}>
						...
						<DropdownMenuItem danger onSelect={() => onAction(offer.id, 'cancel')}>
							<XMarkIcon className='mr-2 h-4 w-4' />
							İptal Et
						</DropdownMenuItem>
					)}
				...
				<Link
					...
					{discount > 0 && (
						<div className='absolute left-3 top-3'>
							<Badge variant='danger' size='sm' icon={<ArrowTrendingDownIcon className='h-3.5 w-3.5' />}>
								%{discount}
							</Badge>
						</div>
					)}
				...
				<div className='flex-1 p-5'>
					<div className='mb-4 flex items-start justify-between gap-4'>
						<div className='min-w-0 flex-1'>
							...
							<p className='mt-1 text-sm text-muted'>
								İlan Fiyatı:{' '}
								<span className='line-through'>₺{listingPrice.toLocaleString('tr-TR')}</span>
							</p>
						</div>
						<div className='flex flex-col items-end gap-1'>
							<StatusBadge status={offer.status} config={offerStatusConfig} label={statusLabel} />
					...
					<div className='mb-4 flex flex-wrap items-center gap-3 sm:gap-6'>
						<div className='rounded-lg border border-primary-200 bg-primary-50 px-3 py-2 sm:px-4 sm:py-3'>
							<p className='mb-0.5 text-[10px] text-muted sm:mb-1 sm:text-xs'>Teklif Tutarı</p>
							<p className='text-lg font-bold text-primary-600 sm:text-2xl'>
								₺{offer.amount.toLocaleString('tr-TR')}
							</p>
							{isReceived && offer.status === 'pending' && estimatedNet != null && (
