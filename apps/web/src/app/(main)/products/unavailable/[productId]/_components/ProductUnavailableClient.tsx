...
export default function ProductUnavailableClient() {
	...
	const { product, similar, isLoading, isBackInStock } = useUnavailableProduct(productId);
	...
	return (
		<PageShell>
				...
				{isBackInStock ? (
					...
					<>
						...
						{product?.category?.slug && (
							<ButtonLink variant='primary' href={`/category/${product.category.slug}`}>
			...
			<SectionCard title='Benzer ürünler'>
				{similar.length === 0 ? (
					<p className='text-muted'>Bu kategoride başka aktif ürün bulunamadı.</p>
				) : (
