...
import type { Seller, UserRating, RatingStats, SellerCollection } from '../_lib/types';
...
export function useSellerProfile() {
			...
			if (profileRes?.data) {
				return {
					...
					stats: {
						...
						averageRating:
							statsData?.averageScore ?? profileRes.data.stats?.averageRating ?? 0,
						totalRatings:
							statsData?.totalRatings ?? profileRes.data.stats?.totalRatings ?? 0,
					},
				};
			}
	...
	const membershipDuration = (() => {
		...
		return locale === 'en' ? `${Math.floor(months / 12)}+ years` : `${Math.floor(months / 12)}+ yıl`;
	})();
