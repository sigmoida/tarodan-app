...
export default function CardPaymentForm({
	...
	return (
		<SectionCard
			title='Kart ile Öde'
			action={
				<Badge variant='success' icon={<LockClosedIcon className='h-3.5 w-3.5' />}>
					Güvenli
				</Badge>
			}>
			<p className='-mt-3 mb-4 text-sm text-muted'>PayTR güvenli ödeme altyapısı</p>
			...
			<div className='mt-4 flex flex-wrap items-center justify-center gap-x-4 gap-y-2 text-xs text-muted'>
				...
				<span className='inline-flex items-center gap-1.5'>
					<CheckCircleIcon className='h-4 w-4 text-success-500' /> PayTR güvencesi
				</span>
				<span className='inline-flex items-center gap-1.5'>
					<LockClosedIcon className='h-4 w-4 text-success-500' /> Kart bilgileri saklanmaz
				</span>
			</div>
		</SectionCard>
	);
