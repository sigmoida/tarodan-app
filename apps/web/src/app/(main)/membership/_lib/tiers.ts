...
export function listingLimitText(tierId: TierId, limits: ListingLimits, t: TFn): string {
	...
	return limit === -1 ? `${t('membership.unlimited')} ${suffix}` : `${limit} ${suffix}`;
}

/** Build all four display tiers. Runs client-side (needs the i18n `t`). */
export function buildTiers(prices: TierPrices, limits: ListingLimits, t: TFn): Tier[] {
	return [
		{
			id: 'free',
...
export function displayPrice(tier: Tier, period: Period, prices: TierPrices): number {
	if (period !== 'yearly' || tier.price === 0) return tier.price;
	if (tier.id === 'premium' && prices.premium_yearly_price) return prices.premium_yearly_price;
	if (tier.id === 'business' && prices.business_yearly_price) return prices.business_yearly_price;
