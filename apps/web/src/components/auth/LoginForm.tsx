...
export function LoginForm() {
  ...
  const onSubmit = (values: LoginValues) => submit(values.email, values.password);

  return (
    <AuthCard
      title={t('auth.welcomeBack')}
      description={locale === 'tr' ? 'Hesabınıza giriş yapın' : 'Sign in to your account'}
      footer={
        <>
          {t('auth.noAccount')}{' '}
      ...
      {showVerificationBanner && (
        <div className="mb-6 rounded-xl border border-warning-300 bg-warning-50 p-5">
          ...
          <div className="space-y-2">
            ...
            >
              {locale === 'tr' ? 'Doğrulama sayfasına git' : 'Go to verification page'}
            </Link>
          </div>
        </div>
      )}
