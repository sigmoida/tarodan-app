...
export function useListingCategories(enabled: boolean, excludeBrandScale = false) {
	const query = useQuery({
		queryKey: ['listing-form-categories'],
...
export function useCarModels(brandSlug: string | undefined) {
	const query = useQuery({
		queryKey: ['listing-form-car-models', brandSlug],
...
export function useCommissionPreview(price: string | number, categoryId: string) {
	...
	const query = useQuery({
		queryKey: ['listing-form-commission', String(price), categoryId],
		queryFn: async () => {
...
export function useManufacturerAttributes(manufacturerSlug: string | undefined) {
	const query = useQuery({
		queryKey: ['listing-form-mfr-attrs', manufacturerSlug],
		queryFn: async (): Promise<AttributeGroup[]> => {
			const res = await listingsApi.getAttributeGroups({ manufacturer: manufacturerSlug! });
