...
const triggerClasses = (error?: string, selectSize: 'sm' | 'md' | 'lg' = 'md') =>
  cn(
...
function normalizeItems(options?: SelectOption[], children?: React.ReactNode): SelectOption[] {
  if (options) return options;
  const items: SelectOption[] = [];
  React.Children.forEach(children, (child) => {
    if (!React.isValidElement(child)) return;
    const p = child.props as { value?: React.Key; children?: React.ReactNode; disabled?: boolean };
    if (p.value === undefined && p.children === undefined) return;
    items.push({ value: String(p.value ?? ''), label: p.children, disabled: p.disabled });
  });
  return items;
}
...
export const Select = React.forwardRef<HTMLButtonElement, SelectProps>(
  (
    ...
    const emit = (v: string) => {
      ...
      onChange?.({ target: { value: real, name } } as React.ChangeEvent<HTMLSelectElement>);
    };
    ...
    return (
      <div className="w-full">
        {label && (
          <label htmlFor={selectId} className="mb-1.5 block text-sm font-medium text-body">
            {label}
          </label>
        )}
        {control}
