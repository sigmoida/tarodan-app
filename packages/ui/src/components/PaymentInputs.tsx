...
type MaskedProps = Omit<InputProps, 'value' | 'onChange' | 'type' | 'inputMode'> & {
...
export const CardNumberInput = React.forwardRef<HTMLInputElement, CardNumberInputProps>(
	({ value, onValueChange, maxDigits = 16, ...rest }, ref) => (
		<Input
			...
			onChange={(e) => onValueChange(digitsOnly(e.target.value).slice(0, maxDigits))}
			{...rest}
		/>
	),
);
...
export const IbanInput = React.forwardRef<HTMLInputElement, IbanInputProps>(
	({ value, onValueChange, ...rest }, ref) => (
		<Input
			...
			onChange={(e) =>
				// Keep TR + digits, uppercase, cap at 26 chars (TR + 24 digits).
				onValueChange(
					e.target.value
						.replace(/\s/g, '')
						.toUpperCase()
